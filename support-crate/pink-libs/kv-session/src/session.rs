...
use crate::{
	traits::{
		AccessTracking, KvSession, KvSnapshot, KvTransaction, QueueIndex, QueueIndexCodec,
		QueueSession,
	},
	Result,
};
...
impl<Codec: QueueIndexCodec> QueueInfo<Codec> {
	pub fn new<Snap: KvSnapshot>(prefix: impl Into<Vec<u8>>, snapshot: &Snap) -> Result<Self> {
		...
		Ok(Self { prefix, codec: PhantomData, head, tail, popped: false })
	}
	...
	Codec: QueueIndexCodec,
{
	pub fn new(kvdb: Snap, tracker: Tracker, queue_prefix: &[u8]) -> Result<Self> {
		...
		Ok(Self { kvdb, tracker, updates: Default::default(), queue })
	}
	...
	pub fn commit(self) -> (KvTransaction, Snap) {
		...
		(
			KvTransaction {
				...
				queue_head: if self.queue.popped { Some(self.queue.head) } else { None },
...
mod tests {
	...
	fn session_with_one_lock_works() {
		...
		assert_eq!(
			tx.value_updates,
			vec![(b"42".to_vec(), None), (b"43".to_vec(), Some(b"34".to_vec())),]
		);
	...
	fn session_with_rw_tracker_works() {
		...
		assert_eq!(tx.accessed_keys, vec![b"0".to_vec(), b"42".to_vec(), b"43".to_vec()]);
		assert_eq!(
			tx.value_updates,
			vec![(b"42".to_vec(), None), (b"43".to_vec(), Some(b"34".to_vec())),]
		);
