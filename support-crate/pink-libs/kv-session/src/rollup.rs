...
use crate::{
	traits::{BumpVersion, Key, KvSnapshot, KvTransaction, QueueIndex, Value},
	Result,
};
...
{
	let VersionLayout::Standalone { key_postfix: postfix } = layout;

	fn concat(a: &[u8], b: &[u8]) -> Vec<u8> {
		[a, b].concat()
	}

	let lookup_keys: BTreeSet<_> =
		tx.accessed_keys.iter().chain(tx.version_updates.iter()).cloned().collect();

	let version_keys: Vec<_> = lookup_keys.into_iter().map(|k| concat(&k, &postfix)).collect();
	...
	let mut conditions: Vec<_> = tx
		.accessed_keys
		.into_iter()
		.map(|k| (concat(&k, &postfix), versions.get(&concat(&k, &postfix)).and_then(Clone::clone)))
		.collect();
...
mod tests {
	...
	fn rollup_works() {
		...
		let rollup =
			rollup(&kvdb, tx, VersionLayout::Standalone { key_postfix: "_ver".into() }).unwrap();
		assert_eq!(
			rollup.conditions,
			[
