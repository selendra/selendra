...
impl OneLock {
	pub fn new(lock_key: &[u8], write_lock: bool) -> Self {
		Self { lock_key: lock_key.to_vec(), write_lock }
	}
}
...
impl AccessTracking for OneLock {
	...
	fn collect_into(self) -> (Vec<Key>, Vec<Key>) {
		(vec![self.lock_key.clone()], if self.write_lock { vec![self.lock_key] } else { vec![] })
	}
}
...
impl<const TRACK_WRITE: bool> AccessTracker<TRACK_WRITE> {
	...
	pub fn new() -> Self {
		Self { track: Default::default(), writes: Default::default() }
	}
}
...
impl<const TRACK_WRITE: bool> AccessTracking for AccessTracker<TRACK_WRITE> {
	...
	fn collect_into(self) -> (Vec<Key>, Vec<Key>) {
		(self.track.into_iter().collect(), self.writes.into_iter().collect())
	}
}
