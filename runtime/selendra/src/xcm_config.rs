...
parameter_types! {
	...
	pub const ThisNetwork: NetworkId = NetworkId::Selendra;
...
parameter_types! {
	pub const Sel: MultiAssetFilter = Wild(AllOf { fun: WildFungible, id: Concrete(TokenLocation::get()) });
	pub const SelForIndranet: (MultiAssetFilter, MultiLocation) = (Sel::get(), Parachain(INDRANET_ID).into_location());
