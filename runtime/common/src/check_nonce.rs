...
pub struct CheckNonce<T: frame_system::Config + pallet_evm::Config> {
	#[codec(compact)]
	pub nonce: T::Index,
	#[codec(skip)]
	pub is_eth_tx: bool,
...
impl<T: frame_system::Config + pallet_evm::Config> Default for CheckNonce<T> {
	fn default() -> Self {
		Self { nonce: 0u32.into(), is_eth_tx: false, eth_tx_valid_until: 0u32.into() }
	}
}

impl<T: frame_system::Config + pallet_evm::Config> CheckNonce<T> {
...
impl<T: frame_system::Config + pallet_evm::Config> sp_std::fmt::Debug for CheckNonce<T> {
	#[cfg(feature = "std")]
...
impl<T: frame_system::Config + pallet_evm::Config> SignedExtension for CheckNonce<T>
where
	...
	T::AddressMapping: AddressMapping<T::AccountId>,
{
	...
	fn pre_dispatch(
		...
		if self.is_eth_tx {
			...
			let address = <T as pallet_evm::Config>::AddressMapping::get_evm_address(who)
				.unwrap_or_else(|| {
					<T as pallet_evm::Config>::AddressMapping::get_default_evm_address(who)
				});
			let evm_nonce =
				pallet_evm::Accounts::<T>::get(address).map(|x| x.nonce).unwrap_or_default();
