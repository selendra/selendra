...
use pallets_support::{AddressMapping, TransactionPayment};
use selendra_primitives::{Balance, BlockNumber};
use sp_core::H160;
...
type PalletBalanceOf<T> = <<T as pallet_evm::Config>::Currency as Currency<
	<T as frame_system::Config>::AccountId,
>>::Balance;
type NegativeImbalanceOf<T> = <<T as pallet_evm::Config>::Currency as Currency<
	<T as frame_system::Config>::AccountId,
>>::NegativeImbalance;
...
where
	...
	pallet_transaction_payment::ChargeTransactionPayment<Runtime>: TransactionPayment<
		Runtime::AccountId,
	...
	<Runtime as pallet_scheduler::Config>::RuntimeCall:
		Dispatchable + Debug + From<pallet_evm::Call<Runtime>>,
	<<Runtime as pallet_scheduler::Config>::RuntimeCall as Dispatchable>::RuntimeOrigin:
		IsType<<Runtime as frame_system::Config>::RuntimeOrigin>
			+ OriginTrait<
		...
		match action {
			Action::Schedule => {
				...
				#[cfg(not(feature = "with-ethereum-compatibility"))]
				{
					...
					let fee = <pallet_transaction_payment::ChargeTransactionPayment<Runtime>>::weight_to_fee(weight);
					_fee = <pallet_transaction_payment::ChargeTransactionPayment<Runtime>>::reserve_fee(
						&from_account,
						fee,
						None,
					)
			...
			Action::Cancel => {
				...
				#[cfg(not(feature = "with-ethereum-compatibility"))]
				{
					...
					let from_account = Runtime::AddressMapping::get_account_id(&from);
					let _err_amount = <pallet_transaction_payment::ChargeTransactionPayment<Runtime>>::unreserve_fee(
						&from_account,
						task_info.fee,
						None,
					);
...
where
	Runtime: pallet_evm::Config + pallet_transaction_payment::Config + pallet_scheduler::Config,
{
