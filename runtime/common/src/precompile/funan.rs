...
mod tests {
	...
	use crate::precompile::mock::{
		alice, alice_evm_addr, new_test_ext, CDPEngine, Currencies, DexModule, Funan, Loans, One, Origin, Test, SUSD,
		BOB, DOT,
	};
	...
	fn adjust_loan_works() {
		new_test_ext().execute_with(|| {
			...
			let res = FunanPrecompile::execute(&input, None, &context, false);
	...
	fn close_loan_by_dex_works() {
		new_test_ext().execute_with(|| {
			...
			assert_ok!(DexModule::enable_trading_pair(Origin::signed(One::get()), DOT, SUSD));
			assert_ok!(Currencies::update_balance(Origin::root(), BOB, SUSD, 1_000_000_000_000));
