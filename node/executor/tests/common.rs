...
pub const SPEC_VERSION: u32 = selendra_runtime::VERSION.spec_version;

pub const TRANSACTION_VERSION: u32 = selendra_runtime::VERSION.transaction_version;
...
pub fn new_test_ext(code: &[u8]) -> TestExternalities<BlakeTwo256> {
	let ext = TestExternalities::new_with_code(
		code,
		selendra_node_test::genesis::config(Some(code)).build_storage().unwrap(),
	);
	ext
}
