...
use codec::Decode;
use finality_selendra::{BlockId, Justification, JustificationTranslator, SelendraJustification};
...
use selendra_primitives::{AccountId, BlockNumber, Signature};
...
pub enum Error {
	...
	FailedStorageDecoding(&'static str, &'static str, String, codec::Error),
...
// SelendraNodeApiServer is failed to send translated justification.
const FAILED_JUSTIFICATION_SEND_ERROR: i32 = BASE_ERROR + 2;
// SelendraNodeApiServer failed to translate justification into internal representation.
...
#[rpc(client, server)]
pub trait SelendraNodeApi<Block: BlockT, BE> {
	...
	fn selendra_node_block_author(&self, hash: Block::Hash) -> RpcResult<Option<AccountId>>;
}

/// Selendra Node API implementation
pub struct SelendraNode<Header, JT, Client>
where
...
impl<Header, JT, Client> SelendraNode<Header, JT, Client>
where
	Header: HeaderT<Number = BlockNumber>,
	JT: JustificationTranslator<Header> + Send + Sync + Clone + 'static,
{
	pub fn new(
		...
		SelendraNode { import_justification_tx, justification_translator, client }
	}
}

impl<Block, JT, Client, BE> SelendraNodeApiServer<Block, BE>
	for SelendraNode<Block::Header, JT, Client>
where
	Block: BlockT,
	...
	BE: sc_client_api::Backend<Block> + 'static,
{
	fn selendra_node_emergency_finalize(
		...
		let justification: SelendraJustification = SelendraJustification::EmergencySignature(
			justification.0.try_into().map_err(|_| {
