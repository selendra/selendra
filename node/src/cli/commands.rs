...
pub struct NodeParams {
	...
	/// Directory under which SelendraBFT backup is stored
...
/// returns Selendra key, if absent a new key is generated
fn selendra_key(keystore: &impl SyncCryptoStore) -> SelendraId {
	SyncCryptoStore::ed25519_public_keys(keystore, selendra_primitives::KEY_TYPE)
		.pop()
		.unwrap_or_else(|| {
			SyncCryptoStore::ed25519_generate_new(keystore, selendra_primitives::KEY_TYPE, None)
				.expect("Could not create Selendra key")
		})
		.into()
}
...
fn authority_keys(
	...
	let selendra_key = selendra_key(keystore);
	let node_key_path = base_path.join(node_key_file);
	let peer_id = p2p_key(node_key_path.as_path());

	AuthorityKeys { account_id, aura_key, selendra_key, peer_id }
}
	...
	pub fn run(&self) -> Result<(), Error> {
		...
		let genesis_authorities = self
			...
			.map(|account_id| {
				...
				authority_keys(&keystore, account_base_path.path(), node_key_file, account_id)
			})
			.collect();
...
/// The `bootstrap-node` command is used to generate key pairs and SelendraBFT backup folder for a single authority
	...
	fn account_id(&self) -> AccountId {
		match &self.account_id {
			...
			None => account_id_from_string(
				self.account_seed.clone().expect("Pass account-seed argument").as_str(),
			),
		}
	}
}
...
/// The `purge-chain` command used to remove the whole chain and backup made by SelendraBFT.
/// First runs substrate PurgeChainCmd and after that removes SelendraBFT backup.
#[derive(Debug, Parser)]
...
impl PurgeBackupCmd {
	pub fn run(&self, skip_prompt: bool, base_path: BasePath) -> Result<(), Error> {
		...
		if !skip_prompt {
			print!("Are you sure you want to remove {:?}? [y/N]: ", &backup_path);
