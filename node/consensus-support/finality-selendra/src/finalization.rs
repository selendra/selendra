	...
	C: HeaderBackend<B> + LockImportRun<B, BE> + Finalizer<B, BE>,
{
	pub(crate) fn new(client: Arc<C>) -> Self {
		SelendraFinalizer { client, phantom: PhantomData }
	}
}

impl<B, BE, C> BlockFinalizer<B> for SelendraFinalizer<B, BE, C>
where
	B: Block,
	BE: Backend<B>,
	C: HeaderBackend<B> + LockImportRun<B, BE> + Finalizer<B, BE>,
{
	fn finalize_block(
		...
		debug!(target: "selendra-finality", "Finalizing block with hash {:?} and number {:?}. Previous best: #{:?}.", hash, block_number, status.finalized_number);
		...
		let update_res = self.client.lock_import_and_run(|import_op| {
			...
			self.client.apply_finality(import_op, hash, justification, true)
		});
		let status = self.client.info();
		debug!(target: "selendra-finality", "Attempted to finalize block with hash {:?}. Current best: #{:?}.", hash, status.finalized_number);
		update_res
	}
}
