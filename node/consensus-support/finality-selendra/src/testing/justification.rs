...
fn create_justification_notification_for(block: TBlock) -> JustificationNotification<TBlock> {
	JustificationNotification {
		justification: SelendraJustification::CommitteeMultisignature(SignatureSet::with_size(
			0.into(),
		)),
		hash: block.hash(),
...
fn prepare_env(
	...
	(justification_handler, backend, requester, finalizer, justification_request_scheduler)
}
...
async fn requests_for_session_ending_justification() {
	run_test(
		...
		|_, imp_just_tx, backend, requester, _, justification_request_scheduler| async move {
			...
			expect_requested(&requester, &justification_request_scheduler, last_block.clone())
				.await;
			expect_requested(&requester, &justification_request_scheduler, last_block.clone())
				.await;
