	...
	BB: BlockchainBackend<B> + 'static,
{
	...
	pub fn handle_justification_notification(
		...
		let JustificationNotification { justification, number, hash } = notification;

		if number <= last_finalized || number > stop_h {
			debug!(target: "selendra-justification", "Not finalizing block {:?}. Last finalized {:?}, stop_h {:?}", number, last_finalized, stop_h);
			return
		};

		if !(verifier.verify(&justification, hash)) {
			warn!(target: "selendra-justification", "Error when verifying justification for block {:?} {:?}", number, hash);
			return
		};

		debug!(target: "selendra-justification", "Finalizing block {:?} {:?}", number, hash);
		let finalization_res = self.finalizer.finalize_block(
			hash,
			number,
			Some((SELENDRA_ENGINE_ID, versioned_encode(justification))),
		);
		match finalization_res {
			Ok(()) => {
				self.justification_request_scheduler.on_block_finalized();
				self.request_status.reset();
				debug!(target: "selendra-justification", "Successfully finalized {:?}", number);
				if let Some(metrics) = &self.metrics {
					metrics.report_block(hash, Instant::now(), Checkpoint::Finalized);
				}
			},
			Err(e) => {
				error!(target: "selendra-justification", "Fail in finalization of {:?} {:?} -- {:?}", number, hash, e);
			},
		}
	}
	...
	pub fn status_report(&self) {
		if self.request_status.should_report() {
			info!(target: "selendra-justification", "Justification requester status report: {}", self.request_status);
		}
	}
	...
	pub fn request_justification(&mut self, wanted: NumberFor<B>) {
		match self.justification_request_scheduler.schedule_action() {
			...
			SchedulerActions::ClearQueue => {
				debug!(target: "selendra-justification", "Clearing justification request queue");
	...
	fn do_request(&mut self, hash: &<B as BlockT>::Hash, num: NumberFor<B>) {
		debug!(target: "selendra-justification",
	...
	fn request_children(&mut self, info: &Info<B>) {
		...
		if !children.is_empty() {
			self.request_status.save_children(finalized_hash, children.len());
		}
	...
	fn request_wanted(&mut self, mut top_wanted: NumberFor<B>, info: &Info<B>) {
		...
		match self.blockchain_backend.header(BlockId::Number(top_wanted)) {
			...
			Ok(None) => {
				warn!(target: "selendra-justification", "Cancelling request, because we don't have block {:?}.", top_wanted);
			},
			Err(err) => {
				warn!(target: "selendra-justification", "Cancelling request, because fetching block {:?} failed {:?}.", top_wanted, err);
			},
		}
	}
}
