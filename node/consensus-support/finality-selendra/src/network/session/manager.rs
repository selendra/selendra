...
impl<A: AddressingInformation> ManagerActions<A> {
	fn noop() -> Self {
		ManagerActions { maybe_command: None, maybe_message: None }
	}
}
...
impl<NI: NetworkIdentity, D: Data> Manager<NI, D> {
	...
	fn discover_authorities(
		...
		self.sessions
			.get_mut(session_id)
			.and_then(|Session { handler, discovery, .. }| discovery.discover_authorities(handler))
	}
	...
	) -> (Option<Authentication<NI::AddressingInformation>>, mpsc::UnboundedReceiver<D>) {
		let PreValidatorSession { session_id, verifier, node_id, pen } = pre_session;
		let handler =
	...
	pub async fn update_validator_session(
		...
		(ManagerActions<NI::AddressingInformation>, mpsc::UnboundedReceiver<D>),
		SessionHandlerError,
	> {
		...
		let session = match self.sessions.get_mut(&pre_session.session_id) {
			...
			None => {
				...
				return Ok((
					ManagerActions { maybe_command: None, maybe_message },
					data_from_network,
				))
			},
		};
		...
		Ok((
			ManagerActions { maybe_command, maybe_message: self.discover_authorities(&session_id) },
			data_from_network,
		))
	}
	...
	async fn start_nonvalidator_session(
		...
		let PreNonvalidatorSession { session_id, verifier } = pre_session;
		let handler = SessionHandler::new(None, verifier, session_id, address).await;
		let discovery = Discovery::new(self.discovery_cooldown);
		self.sessions
			.insert(session_id, Session { handler, discovery, data_for_user: None });
	}
	...
	pub async fn update_nonvalidator_session(
		...
		match self.sessions.get_mut(&pre_session.session_id) {
			Some(session) => {
				session.handler.update(None, pre_session.verifier, address).await?;
			},
			None => {
	...
	pub fn on_user_message(
		...
		if let Some(handler) = self.sessions.get(&session_id).map(|session| &session.handler) {
	...
	pub fn on_discovery_message(
		...
		match self.sessions.get_mut(&session_id) {
			Some(Session { handler, discovery, .. }) => {
	...
	pub fn send_session_data(&self, session_id: &SessionId, data: D) -> Result<(), SendError> {
		match self.sessions.get(session_id).and_then(|session| session.data_for_user.as_ref()) {
			Some(data_for_user) =>
				data_for_user.unbounded_send(data).map_err(|_| SendError::UserSend),
	...
	pub fn status_report(&self) {
		...
		if !authenticated.is_empty() {
			let authenticated_status = authenticated
				...
				.map(|(session_id, node_count, peers)| {
					...
					format!("{:?}: {}/{} {{{}}}", session_id, peers.len() + 1, node_count, peer_ids)
				})
				.collect::<Vec<_>>()
				.join(", ");
			status.push_str(&format!("authenticated authorities: {}; ", authenticated_status));
		}
		...
		let mut missing: Vec<_> = self
			...
			.map(|(session_id, session)| {
				(
					session_id.0,
					session.handler.missing_nodes().iter().map(|id| id.0).collect::<Vec<_>>(),
				)
			})
...
mod tests {
	...
	async fn starts_nonvalidator_session() {
		...
		let ManagerActions { maybe_command, maybe_message } = manager
			.update_nonvalidator_session(PreNonvalidatorSession { session_id, verifier })
			.await
			.unwrap();
		assert!(maybe_command.is_none());
		assert!(maybe_message.is_none());
		assert_eq!(manager.send_session_data(&session_id, -43), Err(SendError::NoSession));
	}
	...
	async fn starts_validator_session() {
		...
		let session_id = SessionId(43);
		let (ManagerActions { maybe_command, maybe_message }, _data_from_network) = manager
			.update_validator_session(PreValidatorSession { session_id, verifier, node_id, pen })
			.await
			.unwrap();
		assert!(maybe_command.is_none());
	...
	async fn stops_session() {
		...
		let (node_id, pen) = validator_data[0].clone();
		let session_id = SessionId(43);
		let (ManagerActions { maybe_command, maybe_message }, mut data_from_network) = manager
			.update_validator_session(PreValidatorSession { session_id, verifier, node_id, pen })
			.await
			.unwrap();
		assert!(maybe_command.is_none());
		...
		let ManagerActions { maybe_command, maybe_message } = manager.finish_session(session_id);
		assert!(maybe_command.is_none());
		assert!(maybe_message.is_none());
		assert_eq!(manager.send_session_data(&session_id, -43), Err(SendError::NoSession));
	...
	async fn handles_broadcast() {
		...
		let ManagerActions { maybe_command, maybe_message } = manager.on_discovery_message(message);
		assert_eq!(
			maybe_command,
			Some(ConnectionCommand::AddReserved(iter::once(address).collect()))
		);
		assert!(maybe_message.is_some());
	}
	...
	async fn sends_user_data() {
		...
		assert_eq!(network_data, &DataInSession { data: 2137, session_id });
	}
}
