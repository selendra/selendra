...
fn split_sender<LeftData: Data, RightData: Data, S: Sender<Split<LeftData, RightData>>>(
	sender: S,
) -> (LeftSender<LeftData, RightData, S>, RightSender<LeftData, RightData, S>) {
	(
		LeftSender { sender: sender.clone(), phantom: PhantomData },
		RightSender { sender, phantom: PhantomData },
	)
}
...
) -> (LeftReceiver<LeftData, RightData, R>, RightReceiver<LeftData, RightData, R>) {
...
///
/// The main example for now is creating an `selendra_bft::Network` and a separate one for accumulating
