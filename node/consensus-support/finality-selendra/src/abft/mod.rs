...
pub use current::{
	create_selendra_config as current_create_selendra_config, run_member as run_current_member,
	VERSION as CURRENT_VERSION,
};
pub use legacy::{
	create_selendra_config as legacy_create_selendra_config, run_member as run_legacy_member,
...
/// Inner `SignatureSet` is imported from `selendra_bft_crypto` with fixed version for compatibility reasons:
...
pub struct SignatureSet<Signature>(pub selendra_bft_crypto::SignatureSet<Signature>);
...
impl<S: Clone> SignatureSet<S> {
	...
	pub fn with_size(len: NodeCount) -> Self {
		SignatureSet(legacy_selendra_bft::SignatureSet::with_size(len.into()))
	}
