	...
	R: Receiver<Message> + 'static,
{
	...
	let task = {
		async move {
			debug!(target: "selendra-party", "Running the data store task for {:?}", session_id);
			data_store.run(exit).await;
			debug!(target: "selendra-party", "Data store task stopped for {:?}", session_id);
		}
	};

	let handle = spawn_handle.spawn_essential("selendra/consensus_session_data_store", task);
