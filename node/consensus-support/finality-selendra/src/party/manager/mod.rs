	...
	SM: SessionManager<VersionedNetworkData<B>>,
{
	...
	fn legacy_subtasks<N: Network<VersionedNetworkData<B>> + 'static>(
		...
		let consensus_config =
			legacy_create_selendra_config(n_members, node_id, session_id, self.unit_creation_delay);
		let data_network = data_network.map();

		let (unfiltered_selendra_network, rmc_network) =
			split(data_network, "selendra_network", "rmc_network");
		let (data_store, selendra_network) = DataStore::new(
			session_boundaries.clone(),
			self.client.clone(),
			self.block_requester.clone(),
			Default::default(),
			unfiltered_selendra_network,
		);
		Subtasks::new(
			exit_rx,
			run_legacy_member(
	...
	async fn spawn_subtasks(
			...
			.next_session_finality_version(&BlockId::Number(last_block_of_previous_session))
		{
			...
			_ if self.only_legacy() => {
				info!(target: "selendra-party", "Running session with legacy-only SelendraBFT version.");
			...
			Ok(version) if version == CURRENT_VERSION as u32 => {
				info!(target: "selendra-party", "Running session with SelendraBFT version {}, which is current.", version);
				self.current_subtasks(params)
			},
			Ok(version) if version == LEGACY_VERSION as u32 => {
				info!(target: "selendra-party", "Running session with SelendraBFT version {}, which is legacy.", version);
				self.legacy_subtasks(params)
			},
			Ok(version) => {
	...
	fn only_legacy(&self) -> bool {
		std::env::var(ONLY_LEGACY_ENV).map(|legacy| !legacy.is_empty()).unwrap_or(false)
	}
}
	...
	SM: SessionManager<VersionedNetworkData<B>>,
{
	...
	async fn spawn_authority_task_for_session(
		...
		AuthorityTask::new(
			self.spawn_handle.spawn_essential("selendra/session_authority", async move {
				if subtasks.wait_completion().await.is_err() {
					warn!(target: "selendra-party", "Authority subtasks failed.");
				}
			}),
			node_id,
			exit,
	...
	fn start_nonvalidator_session(
		...
		self.session_manager.start_nonvalidator_session(session, authority_verifier)
	}
