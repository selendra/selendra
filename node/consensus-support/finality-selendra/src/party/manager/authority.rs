...
impl Task {
	...
	pub fn new(handle: Handle, node_id: NodeIndex, exit: oneshot::Sender<()>) -> Self {
		Task { task: PureTask::new(handle, exit), node_id }
	}
	...
	pub async fn stopped(&mut self) -> NodeIndex {
		if self.task.stopped().await.is_err() {
			debug!(target: "selendra-party", "Authority task failed for {:?}", self.node_id);
		}
		self.node_id
	}
}
...
impl Subtasks {
	...
	pub fn new(
		...
		Subtasks { exit, member, aggregator, refresher, data_store }
	}
	...
	async fn stop(self) -> Result<(), ()> {
		...
		debug!(target: "selendra-party", "Started to stop all tasks");
		let mut result = Ok(());
		if self.member.stop().await.is_err() {
			warn!(target: "selendra-party", "Member stopped with en error");
			result = Err(());
		}
		trace!(target: "selendra-party", "Member stopped");
		if self.aggregator.stop().await.is_err() {
			warn!(target: "selendra-party", "Aggregator stopped with en error");
			result = Err(());
		}
		trace!(target: "selendra-party", "Aggregator stopped");
		if self.refresher.stop().await.is_err() {
			warn!(target: "selendra-party", "Refresher stopped with en error");
			result = Err(());
		}
		trace!(target: "selendra-party", "Refresher stopped");
		if self.data_store.stop().await.is_err() {
			warn!(target: "selendra-party", "DataStore stopped with en error");
			result = Err(());
		}
		trace!(target: "selendra-party", "DataStore stopped");
		result
	}
	...
	pub async fn wait_completion(mut self) -> Result<(), ()> {
		let result = tokio::select! {
			_ = &mut self.exit => Ok(()),
			res = self.member.stopped() => { debug!(target: "selendra-party", "Member stopped early"); res },
			res = self.aggregator.stopped() => { debug!(target: "selendra-party", "Aggregator stopped early"); res },
			res = self.refresher.stopped() => { debug!(target: "selendra-party", "Refresher stopped early"); res },
			res = self.data_store.stopped() => { debug!(target: "selendra-party", "DataStore stopped early"); res },
		};
		let stop_result = self.stop().await;
		debug!(target: "selendra-party", "Stopped all processes");
