	...
	fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
		match self {
			BackupLoadError::BackupIncomplete(backups) => {
				write!(f, "Backup is not complete. Got backup for runs numbered: {:?}", backups)
			},
...
pub fn rotate(
	...
	debug!(target: "selendra-party", "Loading SelendraBFT backup for session {:?}", session_id);
	let session_path = if let Some(path) = backup_path {
		path.join(format!("{}", session_id))
	} else {
		debug!(target: "selendra-party", "Passing empty backup for session {:?} as no backup argument was provided", session_id);
		return Ok((Box::new(io::sink()), Box::new(io::empty())))
	};
	debug!(target: "selendra-party", "Loading backup for session {:?} at path {:?}", session_id, session_path);
	...
	debug!(target: "selendra-party", "Loaded backup for session {:?}. Creating new backup file at {:?}", session_id, next_backup_path);
	let backup_saver = Box::new(File::create(next_backup_path)?);

	debug!(target: "selendra-party", "Backup rotation done for session {:?}", session_id);
...
pub fn remove(path: Option<PathBuf>, session_id: u32) {
	...
	match fs::remove_dir_all(path) {
		Ok(()) => {
			debug!(target: "selendra-party", "Removed backup for session {}", session_id);
		},
		Err(err) =>
			if err.kind() != io::ErrorKind::NotFound {
				warn!(target: "selendra-party", "Error cleaning up backup for session {}: {}", session_id, err);
			},
	}
}
