	...
	C: HeaderBackend<B>,
{
	...
	if curr_block.hash == finalized_block.hash {
		...
		warn!(target: "selendra-data-store", "Error computing proposal. Conflicting blocks: {:?}, finalized {:?}", curr_block, finalized_block);
		Err(())
	}
}
...
impl Default for ChainTrackerConfig {
	fn default() -> ChainTrackerConfig {
		ChainTrackerConfig { refresh_interval: DEFAULT_REFRESH_INTERVAL }
	}
}
	...
	SC: SelectChain<B> + 'static,
{
	pub fn new(
		...
		(
			...
			DataProvider { data_to_propose, metrics },
		)
	}
	...
	pub async fn run(mut self, mut exit: oneshot::Receiver<()>) {
		...
		loop {
			...
			tokio::select! {
				...
				_ = &mut exit => {
					debug!(target: "selendra-data-store", "Task for refreshing best chain received exit signal. Terminating.");
					return;
				}
			}
		}
...
/// Provides data to SelendraBFT for ordering.
#[derive(Clone)]
...
impl<B: BlockT> DataProvider<B> {
	pub async fn get_data(&mut self) -> Option<SelendraData<B>> {
...
mod tests {
		...
		S: FnOnce(ClientChainBuilder, DataProvider<Block>) -> F,
	{
		...
		exit.send(()).unwrap();
		chain_tracker_handle.await.expect("Chain tracker did not terminate cleanly.");
	}
	...
	async fn proposes_empty_and_nonempty_when_expected() {
		run_test(|mut chain_builder, mut data_provider| async move {
			...
			assert_eq!(data_provider.get_data().await, None, "Expected empty proposal");

			let blocks =
				chain_builder.initialize_single_branch_and_import(2 * MAX_DATA_BRANCH_LEN).await;

			sleep_enough().await;

			let data = data_provider.get_data().await.unwrap();
			let expected_data = selendra_data_from_blocks(blocks[..MAX_DATA_BRANCH_LEN].to_vec());
	...
	async fn proposal_changes_with_finalization() {
		run_test(|mut chain_builder, mut data_provider| async move {
			let blocks =
				chain_builder.initialize_single_branch_and_import(3 * MAX_DATA_BRANCH_LEN).await;
	...
	async fn returns_empty_proposal_above_session_end() {
		run_test(|mut chain_builder, mut data_provider| async move {
			...
			sleep_enough().await;
			let data = data_provider.get_data().await.unwrap();
			let expected_data = selendra_data_from_blocks(blocks[0..MAX_DATA_BRANCH_LEN].to_vec());
			assert_eq!(data, expected_data);
