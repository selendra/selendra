...
impl<B: BlockT, C: HeaderBackend<B>> OrderedDataInterpreter<B, C> {
	pub fn new(
		...
		let chain_info_provider =
			AuxFinalizationChainInfoProvider::new(client, last_finalized_by_selendra.clone());
		let chain_info_provider =
	...
	pub fn blocks_to_finalize_from_data(&mut self, new_data: SelendraData<B>) -> Vec<BlockHashNum<B>> {
		...
		let proposal = match unvalidated_proposal.validate_bounds(&self.session_boundaries) {
			...
			Err(error) => {
				warn!(target: "selendra-finality", "Incorrect proposal {:?} passed through data availability, session bounds: {:?}, error: {:?}", unvalidated_proposal, self.session_boundaries, error);
				return Vec::new()
			},
		};
		...
		match status {
			...
			Ignore => {
				debug!(target: "selendra-finality", "Ignoring proposal {:?} in interpreter.", proposal);
				Vec::new()
			},
	...
	pub fn data_finalized(&mut self, data: SelendraData<B>) {
		for block in self.blocks_to_finalize_from_data(data) {
			self.set_last_finalized(block.clone());
			self.chain_info_provider().inner().update_aux_finalized(block.clone());
			if let Err(err) = self.send_block_to_finalize(block) {
				error!(target: "selendra-finality", "Error in sending a block from FinalizationHandler, {}", err);
			}
		}
	}
}
