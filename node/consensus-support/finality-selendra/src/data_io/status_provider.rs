	...
	CIP: ChainInfoProvider<B>,
{
	...
	if is_hopeless_fork(chain_info_provider, proposal) {
		debug!(target: "selendra-finality", "Encountered a hopeless fork proposal {:?}.", proposal);
		return Ignore
	}
...
mod tests {
	...
	fn prepare_proposal_test() -> (ClientChainBuilder, TestCachedChainInfo, TestAuxChainInfo) {
		...
		let config = ChainInfoCacheConfig { block_cache_capacity: 2 };
		...
		(chain_builder, cached_chain_info_provider, aux_chain_info_provider)
	}
	...
	async fn incorrect_branch_handled_correctly() {
		...
		let incorrect_branch =
			vec![blocks[0].clone(), blocks[1].clone(), blocks[3].clone(), blocks[5].clone()];
	...
	async fn pending_top_block_handled_correctly() {
		let (mut chain_builder, mut cached_cip, mut aux_cip) = prepare_proposal_test();
		let blocks = chain_builder.initialize_single_branch(MAX_DATA_BRANCH_LEN * 10).await;
