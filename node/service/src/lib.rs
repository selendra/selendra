...
impl<N: BaseArithmetic> BackoffAuthoringBlocksStrategy<N> for LimitNonfinalized {
	fn should_backoff(
		...
		let nonfinalized_blocks: u32 =
			chain_head_number.saturating_sub(finalized_number).unique_saturated_into();
...
fn backup_path(selendra_config: &SelendraCli, base_path: &Path) -> Option<PathBuf> {
	if selendra_config.no_backup() {
		return None
	}
	if let Some(path) = selendra_config.backup_path() {
		Some(path)
	} else {
		let path = base_path.join(DEFAULT_BACKUP_FOLDER);
		eprintln!("No backup path provided, using default path: {:?} for SelendraBFT backups. Please do not remove this folder", path);
		Some(path)
	}
}
...
> {
	...
	let executor = SelendraExecutor::new(
		config.wasm_method,
...
fn setup(
	...
	ServiceError,
> {
	let genesis_hash = client.block_hash(0).ok().flatten().expect("we should have a hash");
	...
	config.network.extra_sets.push(finality_selendra::peers_set_config(
...
pub fn new_authority(
	...
	task_manager.spawn_essential_handle().spawn_blocking(
		"selendra",
		None,
		run_validator_node(selendra_config),
	);
