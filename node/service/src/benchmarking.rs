		...
		match $chain {
			Chain::Selendra => {
				#[cfg(feature = "selendra-native")]
				{
					...
					Ok(selendra_sign_call(call, $nonce, $current_block, $period, $genesis, $signer))
				}

				#[cfg(not(feature = "selendra-native"))]
				{
					Err("`selendra-native` feature not enabled")
				}
			},
...
impl frame_benchmarking_cli::ExtrinsicBuilder for RemarkBuilder {
	...
	fn build(&self, nonce: u32) -> std::result::Result<OpaqueExtrinsic, &'static str> {
		let period = selendra_runtime_common::BlockHashCount::get()
...
pub fn benchmark_inherent_data(
	...
	let para_data = selendra_primitives::InherentData {
		bitfields: Vec::new(),
	...
	inherent_data.put_data(selendra_primitives::PARACHAINS_INHERENT_IDENTIFIER, &para_data)?;
