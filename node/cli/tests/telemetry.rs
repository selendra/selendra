...
async fn telemetry_works() {
	...
	let mut selendra = process::Command::new(cargo_bin("selendra"));

	let mut selendra = selendra
	...
	assert!(selendra.try_wait().unwrap().is_none(), "the process should still be running");

	// Stop the process
	kill(Pid::from_raw(selendra.id().try_into().unwrap()), SIGINT).unwrap();
	assert!(common::wait_for(&mut selendra, 40).map(|x| x.success()).unwrap_or_default());

	let output = selendra.wait_with_output().unwrap();
