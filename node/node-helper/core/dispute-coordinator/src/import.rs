...
use selendra_node_primitives::{CandidateVotes, DisputeStatus, SignedDisputeStatement, Timestamp};
use selendra_node_subsystem_util::rolling_session_window::RollingSessionWindow;
...
impl CandidateVoteState<CandidateVotes> {
	...
	pub fn new<'a>(votes: CandidateVotes, env: &CandidateEnvironment<'a>, now: Timestamp) -> Self {
		...
		let supermajority_threshold =
			selendra_primitives::v2::supermajority_threshold(n_validators);
		...
		let dispute_status = if is_disputed {
			...
			let byzantine_threshold = selendra_primitives::v2::byzantine_threshold(n_validators);
