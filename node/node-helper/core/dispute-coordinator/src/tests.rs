...
fn make_keystore(seeds: impl Iterator<Item = String>) -> LocalKeystore {
	...
	for s in seeds {
		store
			.sr25519_generate_new(selendra_primitives::PARACHAIN_KEY_TYPE_ID, Some(&s))
			.unwrap();
	}
...
impl Default for TestState {
	fn default() -> TestState {
		...
		let db = selendra_node_subsystem_util::database::kvdb_impl::DbAdapter::new(db, &[0]);
		let db = Arc::new(db);
	...
	test_harness(|mut test_state, mut virtual_overseer| {
		Box::pin(async move {
			...
			let supermajority_threshold =
				selendra_primitives::supermajority_threshold(test_state.validators.len());
	...
	test_harness(|mut test_state, mut virtual_overseer| {
		Box::pin(async move {
			...
			let byzantine_threshold =
				selendra_primitives::byzantine_threshold(test_state.validators.len());
