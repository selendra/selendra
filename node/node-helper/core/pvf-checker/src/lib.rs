...
impl<Context> PvfCheckerSubsystem {
	fn start(self, ctx: Context) -> SpawnedSubsystem {
		if self.enabled {
			...
			selendra_overseer::DummySubsystem.start(ctx)
		}
	}
}
...
async fn check_signing_credentials(
	...
	selendra_node_subsystem_util::signing_key_and_index(&validators, keystore).map(
...
async fn sign_and_submit_pvf_check_statement(
	...
	let signature = match selendra_node_subsystem_util::sign(
		keystore,
