...
//! # Selendra Staking Miner.
//!
//! Simple bot capable of monitoring a selendra (and cousins) chain and submitting solutions to the
...
pub(crate) static mut RUNTIME: AnyRuntime = AnyRuntime::Selendra;
...
fn signed_ext_builder_selendra(
	nonce: Index,
	tip: Balance,
	era: sp_runtime::generic::Era,
) -> selendra_runtime_exports::SignedExtra {
	use selendra_runtime_exports::Runtime;
	(
...
construct_runtime_prelude!(selendra);
...
macro_rules! any_runtime {
	($($code:tt)*) => {
		unsafe {
			match $crate::RUNTIME {
				$crate::AnyRuntime::Selendra => {
					#[allow(unused)]
					use $crate::selendra_runtime_exports::*;
					$($code)*
				},
			}
		}
...
async fn main() {
	...
	match chain.to_lowercase().as_str() {
		"selendra" | "development" => {
			sp_core::crypto::set_default_ss58_version(Ss58AddressFormat::custom(204));
			sub_tokens::dynamic::set_name("SEL");
...
mod tests {
	...
	fn any_runtime_works() {
		...
		let selendra_version = any_runtime! { get_version::<Runtime>() };

		assert_eq!(selendra_version.spec_name, "selendra".into());
	}
}
