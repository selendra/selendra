...
You can build from the root of the Selendra repository using:
```
...
You may then inject the binary into a Docker base image from the root of the Selendra repository:
```
...
You may build the multi-stage image the root of the Selendra repository with:
```
...
2. $ selendra --chain selendra-dev --tmp --alice --execution Native -lruntime=debug --offchain-worker=Always --ws-port 9999
