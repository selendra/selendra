...
use pb::{
    indratory_api_server::{IndratoryApi, IndratoryApiServer},
    server::Error as RpcError,
};
use indratory_api::blocks::StorageState;
use indratory_api::{blocks, crypto, endpoints::EndpointType, irpc as pb};
use indranet_crypto::ecdh::{self, EcdhPublicKey};
use indranet_crypto::{
    key_share,
    sr25519::{Persistence, KDF},
};
use indranet_mq::MessageOrigin;
use indranet_pallets::utils::attestation::{validate as validate_attestation_report, IasFields};
use indranet_types::contract::contract_id_preimage;
use indranet_types::{
...
impl<Platform: pal::Platform + Serialize + DeserializeOwned> Indratory<Platform> {
    ...
    pub fn get_info(&self) -> pb::IndratoryInfo {
    ...
    fn init_runtime(
        ...
        // derive ecdh key
        let ecdh_pubkey = indranet_types::EcdhPublicKey(ecdh_key.public());
        ...
        // being patched) and iruntime doesn't read any data from the chain storage until the first
    ...
    fn get_runtime_info(
        ...
        //
        // 1. It's generated by iruntime thus is safe;
    ...
    fn handle_inbound_messages(&mut self, block_number: chain::BlockNumber) -> RpcResult<()> {
        ...
        for message in messages {
            use indranet_types::messaging::SystemEvent;
            macro_rules! log_message {
                ($msg: expr, $t: ident) => {{
                    ...
                    match event {
                        Ok(event) => {
                            debug!(target: "indranet_mq",
                                "mq dispatching message: sender={} dest={:?} payload={:?}",
                                $msg.sender, $msg.destination, event
                            );
                        }
                        Err(_) => {
                            debug!(target: "indranet_mq", "mq dispatching message (decode failed): {:?}", $msg);
                        }
                    }
                }};
            }
    ...
    fn load_cluster_state(&mut self, filename: &str) -> RpcResult<()> {
        ...
        let cluster_state: ClusterState =
            indranet_mq::checkpoint_helper::using_dispatcher(recv_mq, move || {
                indranet_mq::checkpoint_helper::using_send_mq(send_mq, move || {
...
pub struct RpcService<Platform> {
    req_id: u64,
    pub(crate) indratory: Arc<Mutex<Indratory<Platform>>>,
}

impl<Platform: pal::Platform> RpcService<Platform> {
    pub fn new(platform: Platform) -> RpcService<Platform> {
        RpcService {
            indratory: Arc::new(Mutex::new(Indratory::new(platform))),
            req_id: 0,
        }
    }
    ...
    pub fn with_id(&self, req_id: u64) -> RpcService<Platform> {
        RpcService {
            indratory: self.indratory.clone(),
            req_id,
        }
    }
}
    ...
    Platform: pal::Platform + Serialize + DeserializeOwned,
{
    pub fn dispatch_request(
        ...
        use irpc::server::{Error, ProtoError};
        let data = data.to_vec();

        let mut server = IndratoryApiServer::new(self.with_id(req_id));
            ...
            let (code, data) = match result {
                ...
                Err(err) => {
                    ...
                    if json {
                        ...
                        (code, irpc::codec::encode_message_to_vec(&err))
                    }
                }
            };
            (code, data)
...
impl<T> Drop for LogOnDrop<T> {
    fn drop(&mut self) {
        debug!(target: "indratory::lock", "{}", self.msg);
    }
}

impl<Platform: pal::Platform> RpcService<Platform> {
    pub fn lock_indratory(
        &self,
        allow_rcu: bool,
        allow_safemode: bool,
    ) -> RpcResult<LogOnDrop<MutexGuard<'_, Indratory<Platform>>>> {
        debug!(target: "indratory::lock", "Locking indratory...");
        let guard = self.indratory.lock().unwrap();
        debug!(target: "indratory::lock", "Locked indratory");
...
impl<Platform: pal::Platform + Serialize + DeserializeOwned> IndratoryApi for RpcService<Platform> {
    /// Get basic information about Indratory state.
    async fn get_info(&mut self, _request: ()) -> RpcResult<pb::IndratoryInfo> {
        let info = self.lock_indratory(true, true)?.get_info();
    ...
    async fn sync_header(&mut self, request: pb::HeadersToSync) -> RpcResult<pb::SyncedTo> {
        ...
        self.lock_indratory(false, true)?
            .sync_header(headers, authority_set_change)
    }
    ...
    ) -> Result<pb::HeadersSyncedTo, irpc::server::Error> {
        self.lock_indratory(false, true)?.sync_combined_headers(
    ...
    async fn dispatch_blocks(&mut self, request: pb::Blocks) -> RpcResult<pb::SyncedTo> {
        ...
        let mut indratory = {
            let mut indratory = self.lock_indratory(false, true)?;
            if indratory.args.no_rcu || benchmark::syncing() {
                // If RCU way is not suitable here, we do the traditional locked dispatch.
                return indratory.dispatch_blocks(self.req_id, blocks);
            }
            // Otherwise, we clone the indratory and execute/dispatch the events of the blocks with
            // a cloned indratory without locking(the lock would be released in the end of this scope)
            // the singleton indratory. This way, we can avoid blocking the RPC server.
            info!("Cloning Indratory to do RCU dispatch...");
            let cloned = indratory.clone();
            // We set rcu_dispatching = true to avoid a reentrant call to dispatch_blocks which
            // would cause a state inconsistency.
            indratory.rcu_dispatching = true;
            cloned
        };
        // Start to dispatch the blocks with the cloned indratory without locking the singleton indratory.
        info!("Unlocked Indratory, dispatching blocks...");
        let req_id = self.req_id;
        let span = tracing::Span::current();
        let (synced_to, indratory) = tokio::task::spawn_blocking(move || {
            let _guard = span.enter();
            let synced_to = indratory.dispatch_blocks(req_id, blocks);
            (synced_to, indratory)
        })
        .await
        .expect("Dispatch blocks failed");
        info!("Done, writing state back to Indratory...");
        let mut guard = self.lock_indratory(true, true).unwrap();
        let pending_effects = std::mem::take(&mut guard.pending_effects);
        // While putting the cloned indratory back, the rcu_dispatching flag is also overwritten with
        // its original value `false`.
        **guard = indratory;
        if !pending_effects.is_empty() {
            // Apply pending effects with the singleton indratory locked.
    ...
    async fn init_runtime(
        ...
        self.lock_indratory(false, false)?.init_runtime(
    ...
    async fn get_runtime_info(
        ...
        self.lock_indratory(true, false)?
    ...
    async fn contract_query(
        ...
        'apply_effects: {
            ...
            let mut indratory = self.lock_indratory(true, false)?;
            if indratory.rcu_dispatching {
                const MAX_PENDING: usize = 64;
                if indratory.pending_effects.len() >= MAX_PENDING {
                    error!("Too many pending effects, dropping this");
                    return Err(from_display("Too many pending effects"));
                }
                indratory.pending_effects.push(effects);
            } else {
                indratory.apply_side_effects(effects);
            }
        }
        Ok(response)
    }
    ...
    async fn add_endpoint(
        ...
        self.lock_indratory(false, false)?
    ...
    async fn refresh_endpoint_signing_time(&mut self, _: ()) -> RpcResult<pb::GetEndpointResponse> {
        self.lock_indratory(false, false)?.sign_endpoints()
    }

    async fn get_endpoint_info(&mut self, _: ()) -> RpcResult<pb::GetEndpointResponse> {
        self.lock_indratory(true, false)?.get_endpoint_info()
    }

    async fn sign_endpoint_info(
        &mut self,
        request: pb::SignEndpointsRequest,
    ) -> Result<pb::GetEndpointResponse, irpc::server::Error> {
        self.lock_indratory(true, false)?
            .sign_endpoint_info(VersionedWorkerEndpoints::V1(request.decode_endpoints()?))
    }

    async fn derive_indranet_i2p_key(&mut self, _: ()) -> RpcResult<pb::DeriveIndranetI2pKeyResponse> {
        let mut indratory = self.lock_indratory(true, false)?;
        let system = indratory.system()?;
        let derive_key = system
            .identity_key
            .derive_sr25519_pair(&[b"IndranetI2PKey"])
            .expect("should not fail with valid key");
        Ok(pb::DeriveIndranetI2pKeyResponse {
            indranet_i2p_key: derive_key.dump_secret_key().to_vec(),
        })
    }
    ...
    async fn handover_create_challenge(
        ...
        let mut indratory = self.lock_indratory(false, true)?;
        let (block, ts) = indratory.current_block()?;
        let system = indratory.system()?;
    ...
    async fn handover_start(
        ...
        let mut indratory = self.lock_indratory(false, true)?;
        let attestation_provider = indratory.attestation_provider;
        let dev_mode = indratory.dev_mode;
        let in_sgx = attestation_provider == Some(AttestationProvider::Ias);
        let (block_number, now_ms) = indratory.current_block()?;
        let system = indratory.system()?;
        ...
        let attestation = if !dev_mode && in_sgx {
            ...
            // By ensuring our system timestamp is within the valid period, we know that this iruntime is not hold back by
            // malicious workers.
        ...
        // 3. verify sgx local attestation report to ensure the handover iruntimes are on the same machine
        if !dev_mode && in_sgx {
        ...
        // 5. verify iruntime launch date, never handover to old iruntime
        if !dev_mode && in_sgx {
            let my_la_report = {
                ...
                sgx_api_lite::report(&target_info, &[0; 64])
                    .map_err(|_| from_display("Cannot read server iruntime info"))?
            };
            let my_runtime_hash = {
            ...
            let runtime_state = indratory.runtime_state()?;
            let my_runtime_timestamp = runtime_state
                .chain_storage
                .get_iruntime_added_at(&my_runtime_hash)
                .ok_or_else(|| from_display("Server iruntime not allowed on chain"))?;
            ...
            let req_runtime_timestamp = runtime_state
                .chain_storage
                .get_iruntime_added_at(&runtime_hash)
                .ok_or_else(|| from_display("Client iruntime not allowed on chain"))?;

            if my_runtime_timestamp >= req_runtime_timestamp {
                return Err(from_display("No handover for old iruntime"));
            }
        } else {
            info!("Skip iruntime timestamp check in dev mode");
        }
    ...
    async fn handover_accept_challenge(
        ...
        let ecdh_pubkey = indranet_types::EcdhPublicKey(handover_ecdh_key.public());
        indratory.handover_ecdh_key = Some(handover_ecdh_key);

        let challenge = request.decode_challenge().map_err(from_display)?;
        let dev_mode = challenge.dev_mode;
        // generate local attestation report to ensure the handover iruntimes are on the same machine
    ...
    async fn handover_receive(&mut self, request: pb::HandoverWorkerKey) -> RpcResult<()> {
        ...
        indratory.runtime_info = None;
        indratory.handover_ecdh_key = None;
        Ok(())
    }
    ...
    ) -> Result<(), irpc::server::Error> {
        self.lock_indratory(false, false)?.set_netconfig(request);
        Ok(())
    }
    ...
    ) -> Result<pb::HttpResponse, irpc::server::Error> {
        use reqwest::{
    ...
    ) -> Result<pb::GetContractInfoResponse, irpc::server::Error> {
        self.lock_indratory(true, false)?
    ...
    async fn get_cluster_info(
        &mut self,
        _request: (),
    ) -> Result<pb::GetClusterInfoResponse, irpc::server::Error> {
        self.lock_indratory(true, false)?.get_cluster_info()
    }
    ...
    ) -> Result<pb::ContractId, irpc::server::Error> {
        let deployer =
    ...
    async fn get_network_config(
        &mut self,
        _request: (),
    ) -> Result<pb::NetworkConfigResponse, irpc::server::Error> {
        let indratory = self.lock_indratory(true, false)?;
        Ok(pb::NetworkConfigResponse {
            public_rpc_port: indratory.args.public_port.map(Into::into),
            config: indratory.netconfig.clone(),
        })
    }
    ...
    async fn stop(&mut self, request: pb::StopOptions) -> Result<(), irpc::server::Error> {
        self.lock_indratory(true, true)?
            .stop(request.remove_checkpoints)
    }
    async fn load_storage_proof(
        &mut self,
        req: indratory_api::irpc::StorageProof,
    ) -> Result<(), irpc::server::Error> {
        self.lock_indratory(false, true)?
            .load_storage_proof(req.proof)?;
        Ok(())
    }
    async fn take_checkpoint(&mut self, _req: ()) -> Result<pb::SyncedTo, irpc::server::Error> {
        let synced_to = self
            .lock_indratory(false, false)?
            .take_checkpoint()
            .map_err(from_debug)?;
    ...
    ) -> Result<pb::StatisticsResponse, irpc::server::Error> {
        self.lock_indratory(true, false)?
            .statistics(request)
            .map_err(from_debug)
    }
    async fn generate_cluster_state_request(
        &mut self,
        _: (),
    ) -> Result<pb::SaveClusterStateArguments, irpc::server::Error> {
        let mut indratory = self.lock_indratory(true, false)?;
        let system = indratory.system()?;
        if system.contract_cluster.is_some() {
            return Err(from_display("Already in cluster"));
    ...
    ) -> Result<pb::SaveClusterStateResponse, irpc::server::Error> {
