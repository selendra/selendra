...
use indratory_api::storage_sync::{BlockValidator, Error as SyncError, Result};
...
impl BlockValidator for LightValidation<chain::Runtime> {
    fn submit_finalized_headers(
        ...
        authority_set_change: Option<indratory_api::blocks::AuthoritySetChange>,
    ) -> Result<()> {
...
mod storage_ext {
    ...
    use chain::{pallet_computation, pallet_indra, pallet_mq, pallet_registry};
    use log::error;
    use parity_scale_codec::{Decode, Error};
    use indranet_mq::{ContractClusterId, Message, MessageOrigin};
    use indranet_trie_storage::TrieStorage;
    use indranet_types::messaging::TokenomicParameters;
    ...
    impl ChainStorage {
        ...
        pub fn mq_messages(&self) -> Result<Vec<Message>, Error> {
            for key in ["OutboundMessagesV2", "OutboundMessages"] {
                let messages: Vec<Message> = self
                    .get_decoded_result(storage_prefix("IndranetMq", key))
        ...
        pub fn pink_system_code(&self) -> (u16, Vec<u8>) {
            self.execute_with(pallet_indra::PinkSystemCode::<chain::Runtime>::get)
        }

        pub fn pink_runtime_version(&self) -> (u32, u32) {
            // !! DO NOT CHANGE THIS VALUE !!
            const DEFAULT_VERSION: (u32, u32) = (1, 0);
            self.execute_with(pallet_indra::PinkRuntimeVersion::<chain::Runtime>::get)
        ...
        /// Return `None` if given iruntime hash is not allowed on-chain
        pub(crate) fn get_iruntime_added_at(
            &self,
            runtime_hash: &[u8],
        ) -> Option<chain::BlockNumber> {
            self.execute_with(|| {
                pallet_registry::IruntimeAddedAt::<chain::Runtime>::get(runtime_hash)
            })
        }

        pub(crate) fn gatekeepers(&self) -> Vec<indranet_types::WorkerPublicKey> {
            self.execute_with(pallet_registry::Gatekeeper::<chain::Runtime>::get)
        }

        pub(crate) fn is_worker_registered(&self, worker: &indranet_types::WorkerPublicKey) -> bool {
            self.execute_with(|| pallet_registry::Workers::<chain::Runtime>::get(worker))
                .is_some()
        }

        pub(crate) fn minimum_iruntime_version(&self) -> (u32, u32, u32) {
            self.execute_with(pallet_registry::MinimumIruntimeVersion::<chain::Runtime>::get)
        }

        pub(crate) fn iruntime_consensus_version(&self) -> u32 {
            self.execute_with(pallet_registry::IruntimeConsensusVersion::<chain::Runtime>::get)
        }

        pub(crate) fn is_iruntime_in_whitelist(&self, measurement: &[u8]) -> bool {
            let list = self.execute_with(pallet_registry::IruntimeAllowList::<chain::Runtime>::get);
        ...
        pub(crate) fn get_worker_cluster(
            &self,
            worker: &indranet_types::WorkerPublicKey,
        ) -> Option<ContractClusterId> {
            self.execute_with(|| pallet_indra::ClusterByWorkers::<chain::Runtime>::get(worker))
        }
    }
}
