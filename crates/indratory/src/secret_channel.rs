...
use indratory_api::crypto::EncryptedData;
...
mod sender {
    ...
    use indratory_api::crypto::{ecdh, EncryptedData};
    use indranet_mq::traits::{MessageChannel, MessagePrepareChannel};
    use indranet_mq::Path;
    ...
    impl<'a, MsgChan: MessageChannel> indranet_mq::traits::MessageChannel
    ...
    impl<'a, MsgChan: MessagePrepareChannel> indranet_mq::traits::MessagePrepareChannel
        for BoundSecretMessageChannel<'a, MsgChan>
    {
        type Signer = MsgChan::Signer;
        ...
        fn prepare_with_data(
            ...
            data: Vec<u8>,
            to: impl Into<Path>,
        ) -> indranet_mq::SigningMessage<Self::Signer> {
            let payload = self.encrypt_payload(data);
...
mod receiver {
    ...
    use indratory_api::crypto::ecdh;
    use indranet_mq::{MessageOrigin, ReceiveError, TypedReceiver};
            ...
            let msg = match self.peeler.peel(msg) {
                ...
                Err(PeelError::CodecError) => {
                    if origin.always_well_formed() {
                        panic!("Failed to decode critical mq message, please upgrade the iruntime client");
                    } else {
