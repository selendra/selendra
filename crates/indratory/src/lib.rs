...
#[macro_use]
extern crate log;
extern crate indratory_pal as pal;
...
use indratory_api::{
    ...
    irpc::{self as pb, GetEndpointResponse, InitRuntimeResponse, NetworkConfig},
...
use indranet_mq::{BindTopic, ChannelState, MessageDispatcher, MessageSendQueue, MessageOrigin};
use indranet_scheduler::RequestScheduler;
use indranet_serde_more as more;
use std::time::Instant;
use types::Error;

pub use chain::BlockNumber;
pub use contracts::pink;
pub use irpc_service::RpcService;
pub use storage::ChainStorage;
pub use system::gk;
pub use types::BlockInfo;
pub type IruntimeLightValidation = LightValidation<chain::Runtime>;
...
// TODO: Completely remove the reference to Indranet/Khala runtime. Instead we can create a minimal
...
impl<Platform: pal::Platform> Indratory<Platform> {
...
impl<P: pal::Platform> Indratory<P> {
    ...
    fn check_requirements(&self) {
        ...
        let chain_storage = &self
            ...
            .chain_storage;
        let min_version = chain_storage.minimum_iruntime_version();

        let measurement = self.platform.measurement().unwrap_or_else(|| vec![0; 32]);
        let in_whitelist = chain_storage.is_iruntime_in_whitelist(&measurement);

        if (ver.major, ver.minor, ver.patch) < min_version && !in_whitelist {
            error!("This iruntime is outdated. Please update to the latest version.");
            std::process::abort();
        }

        let consensus_version = chain_storage.iruntime_consensus_version();
        if consensus_version > system::MAX_SUPPORTED_CONSENSUS_VERSION {
            error!(
                "{} {}",
                "This iruntime is outdated and doesn't meet the consensus version requirement.",
    ...
    fn update_runtime_info(
        &mut self,
        f: impl FnOnce(&mut indranet_types::WorkerRegistrationInfoV2<chain::AccountId>),
    ) {
...
impl<Platform: pal::Platform + Serialize + DeserializeOwned> Indratory<Platform> {
    ...
    pub fn take_checkpoint_to_writer<W: std::io::Write>(
        ...
        serde_cbor::ser::to_writer(&mut enc_writer, &IndratoryDumper(self))
...
impl<Platform: Serialize + DeserializeOwned> Indratory<Platform> {
    ...
    fn load_state<'de, D: Deserializer<'de>>(deserializer: D) -> Result<Self, D::Error> {
        struct IndratoryVisitor<Platform>(PhantomData<Platform>);

        impl<'de, Platform: Serialize + DeserializeOwned> Visitor<'de> for IndratoryVisitor<Platform> {
            type Value = Indratory<Platform>;

            fn expecting(&self, formatter: &mut core::fmt::Formatter) -> core::fmt::Result {
                formatter.write_str("Indratory")
            }
            ...
            fn visit_seq<A: SeqAccess<'de>>(self, mut seq: A) -> Result<Self::Value, A::Error> {
                ...
                let mut factory: Self::Value = seq
                    .next_element()?
                    .ok_or_else(|| de::Error::custom("Missing Indratory"))?;
                ...
                factory.system = {
                    ...
                    indranet_mq::checkpoint_helper::using_dispatcher(recv_mq, move || {
                        indranet_mq::checkpoint_helper::using_send_mq(send_mq, || {
                            seq.next_element()?
        ...
        deserializer.deserialize_seq(IndratoryVisitor(PhantomData))
    }
}

struct IndratoryDumper<'a, Platform>(&'a Indratory<Platform>);
impl<Platform: Serialize + DeserializeOwned> Serialize for IndratoryDumper<'_, Platform> {
    fn serialize<S: Serializer>(&self, serializer: S) -> Result<S::Ok, S::Error> {
        self.0.dump_state(serializer)
    }
}
struct IndratoryLoader<Platform>(Indratory<Platform>);
impl<'de, Platform: Serialize + DeserializeOwned + pal::Platform> Deserialize<'de>
    for IndratoryLoader<Platform>
{
    fn deserialize<D: Deserializer<'de>>(deserializer: D) -> Result<Self, D::Error> {
        Ok(Self(Indratory::load_state(deserializer)?))
    }
}
...
// TODO.kevin: Move to indratory-api when the std ready.
