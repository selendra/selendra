...
use crate::contracts;
use crate::pal;
use chain::pallet_indra::ContractRegistryEvent;
use chain::pallet_registry::RegistryEvent;
pub use master_key::{gk_master_key_exists, RotatedMasterKey};
use parity_scale_codec::{Decode, Encode};
pub use indratory_api::irpc::{GatekeeperRole, GatekeeperStatus, SystemInfo};
use indranet_crypto::{
    ecdh::EcdhKey,
    key_share,
...
impl<Platform: pal::Platform> System<Platform> {
    ...
    ///
    /// This function could panic a lot, thus should only handle data from other iruntimes.
    fn decrypt_key_from(
        &self,
...
fn apply_instantiating_events(
    ...
    for (deployer, address) in instantiated_events {
        ...
        let message = ContractRegistryEvent::PubkeyAvailable {
            ...
            deployer: indranet_types::messaging::AccountId(deployer.into()),
        };
    ...
    for (origin, event) in pink_events {
        ...
        match event {
            ...
            PinkEvent::UpgradeRuntimeTo { version } => {
                ...
                panic!(
                    "The runtime version {:?} is not supported by this iruntime yet, please upgrade iruntime",
                    version
                );
            }
        }
