...
use chain::pallet_indra::ClusterRegistryEvent;
use chain::pallet_registry::GatekeeperRegistryEvent;
use indranet_crypto::{
    aead, key_share,
    sr25519::{Persistence, Sr25519SecretKey, KDF},
};
use indranet_mq::{traits::MessageChannel, MessageDispatcher, Sr25519Signer};
    ...
    MsgChan: MessageChannel<Signer = Sr25519Signer> + Clone,
{
    ...
    pub fn process_messages(&mut self, block: &BlockInfo<'_>) {
        ...
        loop {
            let ok = indranet_mq::select_ignore_errors! {
    ...
    fn process_gatekeeper_event(&mut self, origin: MessageOrigin, event: GatekeeperEvent) {
        ...
        match event {
            ...
            | GatekeeperEvent::_IndranetLaunched
...
impl<MsgChan: MessageChannel<Signer = Sr25519Signer>> ComputingEconomics<MsgChan> {
    ...
    fn process_gatekeeper_event(
        ...
        match event {
            ...
            // These events are no longer used in Indranet, but kept in the type define for Khala Network.
            GatekeeperEvent::_RepairV
            | GatekeeperEvent::_IndranetLaunched
            | GatekeeperEvent::_UnrespFix => unreachable!(),
        }
    ...
    pub fn update_tokenomic_parameters(
        &mut self,
        params: indranet_types::messaging::TokenomicParameters,
    ) {
...
pub mod tests {
    ...
    use indranet_mq::{BindTopic, Message, MessageDispatcher, MessageOrigin, Path, Sr25519Signer};
    use indranet_types::{messaging as msg, WorkerPublicKey};
    ...
    fn with_block(block_number: chain::BlockNumber, call: impl FnOnce(&BlockInfo)) {
        ...
        let mut recv_mq = indranet_mq::MessageDispatcher::new();
        let mut send_mq = indranet_mq::MessageSendQueue::new();
        let block = BlockInfo {
            block_number,
