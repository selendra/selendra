...
pub use indranet_mq::{ContractClusterId, ContractId};
...
pub mod messaging {
	...
	use indranet_mq::bind_topic;
	use sp_core::crypto::AccountId32;

	bind_topic!(ClusterEvent, b"indranet/cluster/event");
	...
	bind_topic!(ContractOperation<CodeHash, AccountId>, b"indranet/contract/op");
	#[derive(Encode, Decode, Debug)]
	...
	bind_topic!(WorkerClusterReport, b"indranet/cluster/worker/report");
	...
	bind_topic!(ClusterOperation<AccountId>, b"indranet/cluster/key");
	...
	pub enum ClusterOperation<AccountId> {
		...
		///
		/// This leaves a door to clean up the beta clusters in indra v1.
...
pub fn command_topic(id: ContractId) -> Vec<u8> {
	format!("indranet/contract/{}/command", hex::encode(id)).as_bytes().to_vec()
}
