...
pub fn generate<T: Service>(
    ...
    quote! {
        ...
        pub mod #client_mod {
            ...
            where
                Client: irpc::client::RequestClient
            {
...
fn generate_unary<T: Method>(
    ...
    quote! {
        pub async fn #ident(
            &self,
            request: #request,
        ) -> Result<#response, irpc::client::Error> {
            let response = self.client.request(#path, irpc::codec::encode_message_to_vec(&request)).await?;
            Ok(irpc::Message::decode(&response[..])?)
        }
    }
}
