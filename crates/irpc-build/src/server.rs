    ...
    quote! {
        ...
        pub mod #server_mod {
            ...
            impl<T: #server_trait> #server_service<T> {
                ...
                pub async fn dispatch_request(&mut self, path: &str, data: impl AsRef<[u8]>) -> Result<Vec<u8>, irpc::server::Error> {
                    #![allow(clippy::let_unit_value)]
                    match path {
                        #methods
                        _ => Err(irpc::server::Error::NotFound),
                    }
                }

                pub async fn dispatch_json_request(&mut self, path: &str, data: impl AsRef<[u8]>) -> Result<Vec<u8>, irpc::server::Error> {
                    #![allow(clippy::let_unit_value)]
                    match path {
                        #json_methods
                        _ => Err(irpc::server::Error::NotFound),
            ...
            (false, false) => {
                quote! {
                    ...
                    async fn #name(&mut self, request: #req_message)
                        -> Result<#res_message, irpc::server::Error>;
                }
            }
            _ => {
...
fn generate_unary<T: Method>(
    ...
    if json {
        ...
        quote! {
            let input: #request = irpc::Message::decode(data.as_ref())?;
            let response = self.inner.#method_ident(input).await?;
            Ok(irpc::codec::encode_message_to_vec(&response))
        }
    }
}
