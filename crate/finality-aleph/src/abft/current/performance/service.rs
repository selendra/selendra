	...
	FH: current_aleph_bft::FinalizationHandler<AlephData<UH>>,
{
	fn new(finalization_handler: FH, batches_for_scorer: mpsc::UnboundedSender<Batch<UH>>) -> Self {
		FinalizationWrapper { finalization_handler, batches_for_scorer }
	}
}
	...
	RA: RuntimeApi,
{
	...
	where
		FH: current_aleph_bft::FinalizationHandler<AlephData<UH>>,
	{
		let ServiceIO { hashes_for_aggregator, signatures_from_aggregator } = io;
	...
	fn make_score(&mut self, points: RawScore) -> Score {
		let result = Score { session_id: self.session_id.0, nonce: self.nonce, points };
		self.nonce += 1;
		result
	}
}
