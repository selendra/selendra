...
impl Scorer {
	...
	pub fn new(node_count: NodeCount) -> Self {
		Scorer { newest_unit_by: NodeMap::with_size(node_count) }
	}
...
mod tests {
	...
	fn processes_initial_batch() {
		...
		let unit = units_up_to(0).pop().expect("there is a round").pop().expect("there is a unit");
	...
	fn processes_perfect_performance_batch() {
		...
		assert_eq!(scorer.process_batch(vec![first_head]), vec![1, 1, 1, 1, 1, 1, 0]);
		let second_head = round_one_units.pop().expect("there is a unit");
		round_zero_units.push(second_head);
		assert_eq!(scorer.process_batch(round_zero_units), vec![1, 1, 1, 1, 1, 1, 0]);
	}
	...
	fn processes_lacking_creator_batch() {
		...
		let first_head = round_zero_units.pop().expect("there is a unit");
		assert_eq!(scorer.process_batch(vec![first_head]), vec![1, 1, 1, 1, 1, 0, 1]);
		let second_head = round_one_units.pop().expect("there is a unit");
		round_zero_units.push(second_head);
		assert_eq!(scorer.process_batch(round_zero_units), vec![1, 1, 1, 1, 1, 0, 2]);
	}
