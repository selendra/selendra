	...
	B::Header: Header<Number = BlockNumber>,
{
	...
	pub fn new(genesis_hash: B::Hash, net_config: &NetworkConfiguration) -> Self {
		...
		Handler { reserved_nodes, peers: HashMap::new(), limits, genesis_hash }
	}
	...
	pub fn on_peer_disconnect(&mut self, peer_id: PeerId) -> Result<(), DisconnectError> {
		let peer = self.peers.remove(&peer_id).ok_or(DisconnectError::PeerWasNotConnected)?;
	...
	pub fn verify_inbound_connection(
		...
		self.verify_connection(peer_id, handshake, Direction::Inbound).map(|_| ())
	}
