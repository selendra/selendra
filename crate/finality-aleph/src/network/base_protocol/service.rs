	...
	B::Header: Header<Number = BlockNumber>,
{
	...
	fn handle_network_event(&mut self, event: SubstrateEvent) {
		use SubstrateEvent::*;
		match event {
			ValidateInboundSubstream { peer, handshake, result_tx } => {
			...
			NotificationStreamOpened { peer, handshake, direction, negotiated_fallback: _ } => {
				match self.handler.on_peer_connect(peer, handshake, direction) {
					Ok(()) => {
						...
						self.events_for_users.retain(|for_user| {
							for_user.unbounded_send(SyncEvent::PeerConnected(peer)).is_ok()
						});
					},
			...
			NotificationStreamClosed { peer } => {
				...
				for name in &self.protocol_names {
					if let Err(e) =
						self.network.remove_peers_from_reserved_set(name.clone(), addresses.clone())
					{
						warn!(target: LOG_TARGET, "Removing peer from the {} reserved set failed: {}", name, e)
					}
				}
				self.events_for_users.retain(|for_user| {
					for_user.unbounded_send(SyncEvent::PeerDisconnected(peer)).is_ok()
				});
			},
