	...
	C: ClientForAleph<TP::Block, BE>,
{
	...
	let (network, Networks { block_sync_network, authentication_network }, transaction_prototype) =
		base_network(
			network_config,
			protocol_id,
			client.clone(),
	...
	let rpc_interface =
		spawn_rpc_service(network.clone(), syncing_service.clone(), client, spawn_handle);
	Ok(NetworkOutput {
		network,
		block_sync_network,
