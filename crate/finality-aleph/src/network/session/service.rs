...
impl<D: Data> SessionManager<D> for ManagerInterface<D> {
	...
	async fn start_validator_session(
		...
		let data_from_network =
			result_from_service.await.map_err(|_| ManagerError::NetworkReceiveFailed)?;
		let messages_for_network = self.messages_for_service.clone();

		Ok(Network::new(data_from_network, SessionSender { session_id, messages_for_network }))
	}
...
impl Config {
	fn new(
		...
		Config { discovery_cooldown, maintenance_period, initial_delay }
	}
	...
	pub fn with_session_period(
		...
		let initial_delay =
			cmp::min(Duration::from_millis(millisecs_per_block.0 * 10), maintenance_period);
	...
	NI::PeerId: PublicKey,
{
	pub fn new(
		...
		config: Config,
	) -> (Service<D, NI, CN, GN, VCU>, impl SessionManager<D, Error = ManagerError>) {
		let Config { discovery_cooldown, maintenance_period, initial_delay } = config;
		let manager =
			Manager::new(network_identity, validator_address_cache_updater, discovery_cooldown);
		...
		(
			...
			ManagerInterface { commands_for_service, messages_for_service },
		)
	}
	...
	fn send_authentications(
		...
		for auth in to_send {
			self.gossip_network.broadcast(auth).map_err(Error::GossipNetwork)?;
		}
		Ok(())
	}
		...
		match connection_command {
			ConnectionCommand::AddReserved(addresses) => {
				for address in addresses {
					self.validator_network.add_connection(address.peer_id(), address);
				}
			},
	...
	fn handle_manager_actions(
		&mut self,
		ManagerActions { maybe_command, maybe_message }: ManagerActions<NI::AddressingInformation>,
	) -> Result<(), Error<GN::Error>> {
	...
	fn handle_command(
		...
		match command {
			StartValidator(session_id, verifier, node_id, pen, result_for_user) => {
				let pre_session = PreValidatorSession { session_id, verifier, node_id, pen };
				...
				if let Some(result_for_user) = result_for_user {
					if result_for_user.send(data_from_network).is_err() {
						warn!(target: "selendra-network", "Failed to send started session.")
					}
				}
				Ok(actions)
			},
			StartNonvalidator(session_id, verifier) => {
				let pre_session = PreNonvalidatorSession { session_id, verifier };
	...
	pub async fn run(mut self) -> Result<(), Error<GN::Error>> {
		...
		loop {
			trace!(target: "selendra-network", "Manager Loop started a next iteration");
			tokio::select! {
				maybe_command = self.commands_from_user.next() => {
					trace!(target: "selendra-network", "Manager received a command from user");
					let command = maybe_command.ok_or(Error::CommandsChannel)?;
					match self.handle_command(command) {
						Ok(to_send) => self.handle_manager_actions(to_send)?,
						Err(e) => warn!(target: "selendra-network", "Failed to update handler: {:?}", e),
					}
				},
				maybe_message = self.messages_from_user.next() => {
					trace!(target: "selendra-network", "Manager received a message from user");
				...
				maybe_data = self.validator_network.next() => {
					trace!(target: "selendra-network", "Manager received some data from network");
					...
					if let Err(e) = self.manager.send_session_data(&session_id, data) {
						match e {
							SendError::UserSend => trace!(target: "selendra-network", "Failed to send to user in session."),
							SendError::NoSession => trace!(target: "selendra-network", "Received message for unknown session."),
						}
					}
				},
				maybe_authentication = self.gossip_network.next() => {
					let (authentication, _) = maybe_authentication.map_err(Error::GossipNetwork)?;
					trace!(target: "selendra-network", "Manager received an authentication from network");
				...
				_ = maintenance.tick() => {
					debug!(target: "selendra-network", "Manager starts maintenence");
