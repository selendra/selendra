...
impl AddressingInformation for SignedTcpAddressingInformation {
	...
	fn verify(&self) -> bool {
		self.peer_id().verify(&self.addressing_information.encode(), &self.signature)
	}
...
impl SignedTcpAddressingInformation {
	fn new(
		...
		Ok(SignedTcpAddressingInformation { addressing_information, signature })
	}
}
...
impl Dialer<SignedTcpAddressingInformation> for TcpDialer {
	...
	async fn connect(
		...
		let SignedTcpAddressingInformation { addressing_information, .. } = address;
		let TcpAddressingInformation { primary_address, other_addresses, .. } =
