...
use selendra_primitives::TransactionHash;
use selendra_primitives::{AuraId, Block};
		...
		+ 'static,
{
	...
	spawn_handle.spawn("aleph/slo-metrics", {
		...
		async move {
			run_metrics_service(&slo_metrics, &mut transaction_pool.import_notification_stream())
				.await;
		}
	});
