...
impl BestBlockMetrics {
	pub fn new(
		...
		Ok(Self::Prometheus {
			...
			best_block: register(
				Gauge::new("aleph_best_block", "Best (or more precisely, favourite) block number")?,
				&registry,
			)?,
			reorgs: register(
				Histogram::with_opts(
	...
	pub fn report_best_block_imported(&mut self, block_id: BlockId) {
		if let Self::Prometheus {
			best_block, ref mut best_block_id, reorgs, chain_status, ..
		} = self
		{
	...
	pub fn report_block_finalized(&self, block_id: BlockId) {
		if let Self::Prometheus { top_finalized_block, .. } = self {
...
fn retracted_path_length(
	...
	let lca = chain_status.lowest_common_ancestor(from, to).map_err(Box::new)?;
