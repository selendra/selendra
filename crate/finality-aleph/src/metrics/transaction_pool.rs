...
pub mod test {
	...
	impl TestTransactionPoolSetup {
		pub fn new(client: Arc<TestClient>) -> Self {
			...
			TestTransactionPoolSetup { client, pool, proposer_factory }
		}
		...
		pub async fn import_block(&mut self, block: TBlock) -> TBlock {
			...
			self.client.import(BlockOrigin::Own, block.clone()).await.unwrap();
		...
		pub async fn submit(&mut self, at: &THash, xt: Extrinsic) {
			self.pool.submit_one(*at, TransactionSource::External, xt.into()).await.unwrap();
		}
	}
	...
	impl TestSetup {
		...
		fn transactions_histogram(&self) -> &Histogram {
			match &self.metrics {
				TransactionPoolMetrics::Prometheus { time_till_block_inclusion, .. } => {
					time_till_block_inclusion
				},
				_ => panic!("metrics"),
			}
		...
		fn process_notifications(&mut self) -> HashMap<NotificationType, usize> {
			...
			while let Some(transaction) = self.pool_import_notifications.next().now_or_never() {
				self.metrics.report_in_pool(transaction.expect("stream should not end"));
	...
	async fn transactions_are_reported() {
		...
		let xt = setup.pool.extrinsic(AccountKeyring::Alice, AccountKeyring::Bob, 0);
	...
	async fn transactions_are_reported_only_if_ready_when_added_to_the_pool() {
		...
		let xt1 = setup.pool.extrinsic(AccountKeyring::Alice, AccountKeyring::Bob, 0);
		let xt2 = setup.pool.extrinsic(AccountKeyring::Alice, AccountKeyring::Bob, 1);
		let xt3 = setup.pool.extrinsic(AccountKeyring::Alice, AccountKeyring::Bob, 2);
	...
	async fn retracted_transactions_are_reported_only_once() {
		...
		let xt1 = setup.pool.extrinsic(AccountKeyring::Alice, AccountKeyring::Bob, 0);
		let xt2 = setup.pool.extrinsic(AccountKeyring::Charlie, AccountKeyring::Dave, 0);
	...
	async fn transactions_skipped_in_block_authorship_are_not_reported_at_that_time() {
		...
		let xt1 = setup.pool.extrinsic(AccountKeyring::Alice, AccountKeyring::Bob, 0);
		let xt2 = setup.pool.extrinsic(AccountKeyring::Charlie, AccountKeyring::Eve, 0);
		...
		let block_1 = setup.pool.propose_block(genesis, Some(2 * xt1.encoded_size() - 1)).await;
		...
		let block_2 =
			setup.pool.propose_block(block_1.hash(), Some(2 * xt1.encoded_size() - 1)).await;
