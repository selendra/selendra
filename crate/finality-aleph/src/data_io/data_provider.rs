...
where
	...
	C: HeaderBackend<H>,
{
	assert!(header.id().number() >= num, "Cannot reduce {header:?} to number {num:?}");
	let mut curr_header = header;
	while curr_header.id().number() > num {
		curr_header = client
			.header(&curr_header.parent_id().expect("number() > num >= 0, so parent exists qed."))
...
impl<UH: UnverifiedHeader> DataProvider<UH> {
	pub async fn get_data(&mut self) -> Option<AlephData<UH>> {
		...
		if let Some(data) = &data_to_propose {
			...
			self.metrics.report_block(top_block.hash(), Checkpoint::Proposed);
...
mod tests {
	...
	) -> (impl Future<Output = ()>, oneshot::Sender<()>, ClientChainBuilder, DataProvider<THeader>)
	{
