...
impl<UH: UnverifiedHeader> UnvalidatedAlephProposal<UH> {
	...
	pub fn validate_bounds(
		...
		if self.branch_len() > MAX_DATA_BRANCH_LEN {
			return Err(BranchTooLong { branch_size: self.branch_len() });
		}
		...
		Ok(AlephProposal { head: self.head.clone(), tail: self.tail.clone() })
	}
}
...
mod tests {
	...
	fn valid_proposal_is_validated_positively() {
		...
		let genesis = MockHeader::genesis();
		let branch: Vec<_> = genesis.random_branch().take(MAX_DATA_BRANCH_LEN - 1).collect();
