...
impl BlockId {
	...
	pub fn random_child(&self) -> MockHeader {
		...
		MockHeader { id, parent, valid: true, equivocated: false, own: false }
	}

	pub fn random_branch(&self) -> impl Iterator<Item = MockHeader> {
		RandomBranch { parent: self.clone() }
	}
}
...
impl MockHeader {
	pub fn genesis() -> Self {
		MockHeader {
			id: BlockId { number: 0, hash: BlockHash::zero() },
			parent: None,
			valid: true,
			equivocated: false,
			own: false,
	...
	pub fn random_parentless(number: BlockNumber) -> Self {
		...
		MockHeader { id, parent: None, valid: true, equivocated: false, own: false }
	}
...
impl MockBlock {
	pub fn new(header: MockHeader, is_correct: bool) -> Self {
		Self { header, justification: None, is_correct }
	}
