...
impl Backend {
	pub fn setup(
		...
		(Backend::new(notification_sender, session_boundary_info), notification_receiver)
	}
	...
	fn new(
		...
		Self { inner: storage, notification_sender }
	}
	...
	fn prune(&self) {
		let top_finalized_id =
			&self.top_finalized().expect("should be at least genesis").header().id();
...
impl Finalizer<MockJustification> for Backend {
	...
	fn finalize(&self, justification: MockJustification) -> Result<(), Self::Error> {
		...
		let allowed_numbers = match storage.finalized.last() {
			Some(id) => [
				...
				storage.session_boundary_info.last_block_of_session(
					storage.session_boundary_info.session_id_from_block_num(id.number + 1),
				),
			],
			None => [0, storage.session_boundary_info.last_block_of_session(SessionId(0))],
		};
		...
		let first_to_finalize =
			blocks_to_finalize.front().expect("At least one block is being finalized");
		...
		let parent_of_first_block_to_finalize = storage
			...
			.expect("We already checked parent exists");
		let last_finalized =
			storage.finalized.last().expect("At least one block is always finalized");
...
impl BlockImport<MockBlock> for Backend {
	fn import_block(&mut self, block: MockBlock, _own: bool) {
		...
		let top_finalized_number =
			self.top_finalized().expect("should be at least genesis").header().id().number();
		let mut storage = self.inner.lock();
...
impl ChainStatus<MockBlock, MockJustification> for Backend {
	...
	fn top_finalized(&self) -> Result<MockJustification, Self::Error> {
		let storage = self.inner.lock();
		let id = storage.finalized.last().expect("there is a top finalized").clone();
		storage
			.blockchain
			.get(&id)
...
impl Backend {
	fn cached(&self, block_number: BlockNumber) -> Result<(), VerifierError> {
		let top_number = self.top_finalized().expect("should be at least genesis").header.id.number;
		let storage = self.inner.lock();
		let current_session = storage.session_boundary_info.session_id_from_block_num(top_number);
		let justification_session =
			storage.session_boundary_info.session_id_from_block_num(block_number);
...
impl HeaderVerifier<MockHeader> for Backend {
	...
	fn verify_header(
		...
		match (header.valid(), header.equivocated()) {
			(true, false) => Ok(VerifiedHeader { header, maybe_equivocation_proof: None }),
