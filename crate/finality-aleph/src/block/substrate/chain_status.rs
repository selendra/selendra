...
impl Display for Error {
	fn fmt(&self, f: &mut Formatter<'_>) -> Result<(), FmtError> {
		...
		match self {
			MissingHash(hash) => {
				write!(f, "data availability problem: no block for existing hash {hash:?}")
			},
			MissingBody(hash) => {
				write!(f, "data availability problem: no block body for existing hash {hash:?}")
			},
...
impl SubstrateChainStatus {
	...
	fn header(&self, id: &BlockId) -> Result<Option<AlephHeader>, Error> {
		...
		match maybe_header.as_ref().map(|header| header.number() == &id.number) {
	...
	fn justification(&self, header: AlephHeader) -> Result<Option<Justification>, BackendError> {
		...
		match backwards_compatible_decode(encoded_justification) {
			Ok(aleph_justification) => {
				Ok(Some(Justification::aleph_justification(header, aleph_justification)))
			},
...
impl ChainStatus<Block, Justification> for SubstrateChainStatus {
	...
	fn top_finalized(&self) -> Result<Justification, Self::Error> {
		...
		self.justification(header)?.ok_or(Error::MissingJustification(finalized_hash))
	}
...
impl HeaderBackend<AlephHeader> for SubstrateChainStatus {
	...
	fn top_finalized_id(&self) -> BlockId {
		let info = self.info();
		BlockId { hash: info.finalized_hash, number: info.finalized_number }
	}
