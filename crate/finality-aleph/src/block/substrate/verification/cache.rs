...
impl Display for CacheError {
	fn fmt(&self, f: &mut Formatter<'_>) -> Result<(), FmtError> {
		use CacheError::*;
		match self {
			SessionTooOld(session, lower_bound) => {
				write!(f, "session {session:?} is too old. Should be at least {lower_bound:?}")
			},
			...
			BadGenesisHeader => {
				write!(f, "the provided genesis header does not match the cached genesis header")
			},
		}
	}
}
	...
	H: HeaderT,
{
	...
	fn try_prune(&mut self, session_id: SessionId) {
		if session_id.0 >= self.lower_bound.0.saturating_add(self.cache_size.saturated_into()) {
			let new_lower_bound =
				SessionId(session_id.0.saturating_sub(self.cache_size.saturated_into()) + 1);
	...
	pub fn get_verifier(&mut self, number: BlockNumber) -> Result<&SessionVerifier, CacheError> {
		Ok(&self.get_data(self.session_id_from_block_num(number))?.session_verifier)
	}
}
	...
	FS: FinalizationInfo,
{
	...
	fn slot_author(
		...
		Ok((expected_author.clone(), Some(authority_accounts.get(idx).ok_or(())?).cloned()))
	}
	...
	fn verify_header(
		...
		if *header.number() == parent_number {
			...
			return match header == self.genesis_header {
				true => Ok(VerifiedHeader { header, maybe_equivocation_proof: None }),
...
mod tests {
	...
	impl AuthorityProvider for MockAuthorityProvider {
		...
		fn next_authority_data(&self, block_number: BlockNumber) -> Option<SessionAuthorityData> {
			self.session_map
				.get(&SessionId(self.session_info.session_id_from_block_num(block_number).0 + 1))
				.cloned()
		}
		...
		fn next_aura_authorities(
			...
			self.aura_authority_map
				.get(&SessionId(self.session_info.session_id_from_block_num(block_number).0 + 1))
				.cloned()
				.map(|v| v.into_iter().map(|aura_id| (placeholder_id.clone(), aura_id)).collect())
		}
	}
	...
	fn setup_test(max_session_n: u32, finalized_number: Arc<Mutex<u32>>) -> TestVerifierCache {
		...
		let finalization_info =
			MockFinalizationInfo { finalized_number, session_info: session_info.clone() };
	...
	fn session_verifier(
		...
		verifier.get_verifier((session_id + 1) * SESSION_PERIOD - 1).cloned()
	}
