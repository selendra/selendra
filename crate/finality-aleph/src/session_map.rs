	...
	RA: RuntimeApi,
{
	pub fn new(client: Arc<C>, api: RA) -> Self {
		Self { client, api, _phantom: PhantomData }
	}
...
{
	fn aura_authorities(&self, block_number: BlockNumber) -> Option<Vec<AuraId>> {
		AuraApi::authorities(self.client.runtime_api().deref(), self.block_hash(block_number)?).ok()
	}

	fn next_aura_authorities(&self, block_number: BlockNumber) -> Option<Vec<(AccountId, AuraId)>> {
		self.api.next_aura_authorities(self.block_hash(block_number)?).ok()
	}
	...
	FI: FinalizationInfo,
{
	pub fn new(finalization_info: FI, session_info: SessionBoundaryInfo) -> Self {
		Self { session_info, finalization_info }
	}
}
