...
{
	fn clone(&self) -> Self {
		let RuntimeApiImpl { client, transaction_pool_factory, _phantom } = self;
		RuntimeApiImpl {
			client: client.clone(),
	...
	BE: Backend<B> + 'static,
{
	pub fn new<TP: LocalTransactionPool<Block = B> + 'static>(
		...
		Self { client, transaction_pool_factory, _phantom: PhantomData }
	}
	...
	BE: Backend<B> + 'static,
{
	...
	fn submit_abft_score(
		...
		runtime_api.register_extension(
			self.transaction_pool_factory.offchain_transaction_pool(block_hash),
		);
...
mod test {
	...
	fn test_missing_storage() {
		...
		*client_builder.genesis_init_mut().extra_storage() = storage;
		...
		assert_eq!(result1, Err(ApiError::NoStorageMapEntry("Pallet".into(), "Map".into())));
		assert_eq!(result2, Err(ApiError::NoStorageValue("Pallet".into(), "StorageValue".into())));
	}
	...
	fn test_wrong_data_type_decode_error() {
		...
		let storage = Storage {
			top: BTreeMap::from([(map_path1, 1u32.encode()), (storage_value_path, 3u32.encode())]),
			children_default: HashMap::new(),
		};
