...
impl<T: Display> Display for RequestHandlerError<T> {
	fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
		match self {
			RequestHandlerError::RootMismatch => {
				write!(f, "invalid request, top_justification is not an ancestor of target")
			},
	...
	B: Block<UnverifiedHeader = UnverifiedHeaderFor<J>>,
{
	fn new(head: HeadOfChunk<J>, state: State) -> Self {
		Self { pre_chunk: PreChunk::new(&head), state, head }
	}
	...
	fn add_block<CS: ChainStatus<B, J>>(
		...
		let block = chain_status.block(id.clone())?.ok_or(RequestHandlerError::MissingBlock(id))?;
	...
	B: Block<UnverifiedHeader = UnverifiedHeaderFor<J>>,
{
	...
	fn empty() -> Self {
		Self { just: None, blocks: vec![], headers: vec![] }
	}
	...
	fn from_just(justification: J) -> Self {
		Self { just: Some(justification), blocks: vec![], headers: vec![] }
	}
	...
	CS: ChainStatus<B, J>,
{
	pub fn new(chain_status: &'a CS, session_info: &'a SessionBoundaryInfo) -> Self {
		Self { chain_status, session_info, _phantom: PhantomData }
	}

	fn upper_limit(&self, id: BlockId) -> BlockNumber {
		let session = self.session_info.session_id_from_block_num(id.number());
		self.session_info.last_block_of_session(SessionId(session.0 + 1))
	}
