	...
	BI: BlockImport<B>,
{
	pub fn new(chain_status: CS, finalizer: F, block_importer: BI) -> Self {
		Self { chain_status, finalizer, block_importer, _phantom: PhantomData }
	}
}
...
type HandleRequestOutput<B, J, V> =
	(Action<B, J>, Option<<V as HeaderVerifier<<J as Justification>::Header>>::EquivocationProof>);
type HandleInternalRequestOutput<J, V> =
	(bool, Option<<V as HeaderVerifier<<J as Justification>::Header>>::EquivocationProof>);
	...
	B: Block<UnverifiedHeader = UnverifiedHeaderFor<J>>,
{
	fn response(justification: J::Unverified, other_justification: Option<J::Unverified>) -> Self {
		Self::Response(NetworkData::StateBroadcastResponse(justification, other_justification))
	}
	...
	fn fmt(&self, f: &mut Formatter<'_>) -> Result<(), FmtError> {
		...
		match self {
			...
			BlockNotImportable(id) => {
				write!(f, "cannot import a block {} that we do not consider required", id)
			},
	...
	BI: BlockImport<B>,
{
	...
	pub fn new(
		...
		let DatabaseIO { chain_status, finalizer, block_importer, .. } = database_io;
	...
	fn verify_header(
		...
		self.verifier.verify_header(header, just_created).map_err(Error::HeaderVerifier)
	}
	...
	pub fn handle_request(
		...
		Ok(match request_handler.action(request)? {
			Action::RequestBlock(unverified_header) => {
				let VerifiedHeader { header, maybe_equivocation_proof } =
	...
	fn handle_justification(
		...
		let new_highest = self.forest.update_justification(justification, maybe_peer)?;
		self.try_finalize()?;
		self.sync_oracle.update_behind(self.forest.behind_finalization());
		Ok(new_highest)
	}
	...
	fn handle_header(
		...
		let h = match self.verify_header(header, false) {
			Ok(VerifiedHeader { header: h, maybe_equivocation_proof }) => {
	...
	pub fn handle_request_response(
		...
		peer: I,
	) -> (bool, Vec<V::EquivocationProof>, Option<<Self as HandlerTypes>::Error>) {
	...
	pub fn handle_state(
		...
		let remote_session = self.session_info.session_id_from_block_num(remote_top_number);
		let local_session = self.session_info.session_id_from_block_num(local_top_number);
	...
	pub fn interest_provider(&self) -> InterestProvider<I, J> {
		InterestProvider { forest: &self.forest }
	}
	...
	> {
		let VerifiedHeader { maybe_equivocation_proof, .. } =
...
mod tests {
	...
	fn setup() -> (TestHandler, Backend, impl ChainStatusNotifier<MockHeader>, BlockId) {
	...
	fn assert_dangling_branch_required(
		...
		assert!(
			matches!(handler.interest_provider().get(bottom), Interest::Uninterested),
			"should not be interested in the bottom"
		);
		match handler.interest_provider().get(top) {
			Interest::Required { header: _, know_most, branch_knowledge } => {
	...
	fn grow_light_branch(
		...
		let (newly_required, equivocation) =
			handler.handle_internal_request(top.clone()).expect("should work");
		...
		let (new_highest_justified, _, maybe_error) = handler.handle_request_response(
			branch.iter().cloned().rev().map(ResponseItem::Header).collect(),
			peer_id,
		);
	...
	async fn grow_trunk(
		...
		for header in branch.iter() {
			...
			handler.handle_justification_from_user(justification).expect("should work");
			...
			match notifier.next().await {
				Ok(BlockFinalized(finalized_header)) => {
					assert_eq!(header, &finalized_header, "should finalize the current header")
				},
	...
	async fn accepts_response_twice() {
		...
		let response = branch_response(
			branch.clone(),
			BranchResponseContent { headers: false, blocks: true, justifications: true },
		);
	...
	async fn accepts_long_response_after_handling_short_one() {
		...
		let short_response = branch_response(
			branch[..15].to_vec(),
			BranchResponseContent { headers: false, blocks: true, justifications: false },
		);
	...
	async fn handles_multiple_overlapping_responses() {
		...
		let full_response = branch_response(
			branch.clone(),
			BranchResponseContent { headers: true, blocks: true, justifications: true },
		);
	...
	async fn finalizes_with_justification_gaps() {
		...
		for session in 0.. {
			...
			let response_items = branch_response(
				branch.clone(),
				BranchResponseContent { headers: true, blocks: true, justifications: false },
			);
			let (new_info, _, maybe_error) =
			...
			let response_items = branch_response(
				partial.clone(),
				BranchResponseContent { headers: false, blocks: false, justifications: true },
			);
			let (new_info, _, maybe_error) =
				handler.handle_request_response(response_items, peer_id);
			assert!(new_info);
	...
	async fn skips_justification_gap_with_last_of_current_session_only() {
		...
		let peer_id = 0;
		let branch_low =
			grow_light_branch_till(&mut handler, &genesis, &last_block_of_first_session, peer_id);
		...
		let response_items = branch_response(
			branch_low.iter().cloned().chain(branch_high.iter().cloned()).collect(),
			BranchResponseContent { headers: true, blocks: true, justifications: false },
		);
		...
		assert_eq!(
			handler.state().expect("should work").top_justification().header().id(),
			genesis,
	...
	async fn uninterested_in_dangling_branch() {
		...
		assert!(
			!matches!(handler.interest_provider().get(&top), Interest::Uninterested),
	...
	async fn uninterested_in_dangling_branch_when_connected_below_finalized() {
		...
		let response = branch_response(
			vec![fork_child],
			BranchResponseContent { headers: true, blocks: false, justifications: false },
		);
		...
		assert!(
			matches!(handler.interest_provider().get(&fork_top), Interest::Uninterested),
			"should be uninterested"
		);
	}
	...
	async fn uninterested_in_dangling_branch_when_connected_to_composted_header() {
		...
		assert!(
			matches!(handler.interest_provider().get(&further_fork_bottom), Interest::Uninterested),
			"should be uninterested"
		);
		...
		assert!(
			further_fork_child.id().number()
				> handler.state().expect("should work").top_justification().header().id().number()
		);
	...
	async fn syncs_to_a_long_trunk() {
		...
		loop {
			...
			let (header, branch_knowledge) = match syncing_handler.extension_request() {
				ExtensionRequest::HighestJustified { header, branch_knowledge, .. } => {
	...
	fn handles_state_with_large_difference() {
		...
		match handler.handle_state(initial_state, peer).expect("correct justification").0 {
	...
	fn detects_equivocated_state() {
		...
		let justifications: Vec<MockJustification> =
			headers.into_iter().map(MockJustification::for_header).collect();
		let mut states = vec![];
	...
	fn setup_request_tests(
		...
		let blocks = headers.into_iter().map(|h| backend.block(h.id()).unwrap().unwrap()).collect();
	...
	fn handles_request_too_far_into_future() {
		...
		let request =
			Request::new(requested_header.clone(), LowestId(requested_header.id()), initial_state);
	...
	fn handles_chain_extension_request_for_just_blocks() {
		...
		match handler.handle_chain_extension_request(remote_state).expect("correct request") {
	...
	fn handles_new_internal_request() {
		...
		assert!(handler.handle_internal_request(headers[1].clone()).unwrap().0);
		assert!(!handler.handle_internal_request(headers[1].clone()).unwrap().0);
	}
	...
	fn broadcasts_own_block() {
		...
		let result = handler.block_imported(header).expect("correct").expect("known");
	...
	fn detects_equivocated_own_block() {
		...
		header.make_equivocated();
		let block = MockBlock::new(header.clone(), true);
		let proof = handler.handle_own_block(block).expect("correct").expect("should return proof");
	...
	async fn accepts_broadcast_block() {
		...
		handler.handle_own_block(block.clone()).expect("handled");
		let broadcast = handler.block_imported(header).expect("correct").expect("known");
	...
	async fn handles_wide_fork_scenario() {
		...
		let branches: Vec<_> =
			(0..100).map(|i| grow_light_branch(&mut handler, &genesis, 2, i)).collect();
		let interest_provider = handler.interest_provider();

		// Verify initial interests in H0, H0', ..., H99, H99'.
		for (i, branch) in branches.iter().enumerate() {
			assert!(matches!(interest_provider.get(&branch[0].id()), Interest::Uninterested));
		...
		let interest_provider = handler.interest_provider();
		for branch in branches.iter().take(25) {
			assert!(matches!(interest_provider.get(&branch[1].id()), Interest::Uninterested));
		}
		...
		for (i, branch) in branches.iter().enumerate() {
			let response = handler.handle_request(Request::new(
				...
				State::new(MockJustification::for_header(MockHeader::genesis()), branch[1].clone()),
			));
			if i < 25 {
	...
	async fn accepts_chain_extension_branch() {
		...
		for h in branch.clone() {
			...
			match n1.next().await {
				Ok(BlockImported(header)) => {
					h1.block_imported(header).expect("block imported should succeed");
				},
				_ => panic!("should notify about imported block"),
			}
		}
		...
		let items = match action {
			Action::Response(items) => {
				assert_eq!(SimplifiedItem::from_response_items(items.clone()), vec![B(1), B(2)],);
				items
			},
		...
		for h in branch.clone() {
			match n2.next().await {
				Ok(BlockImported(header)) => {
					assert_eq!(header, h);
					h2.block_imported(header).expect("block imported should succeed");
				},
				_ => panic!("should notify about imported block"),
			}
		}
	...
	async fn headers_above_favourite_trigger_chain_extension_request() {
		...
		let state1 =
			State::new(MockJustification::for_header(MockHeader::genesis()), header1.clone());
		let state2 =
			State::new(MockJustification::for_header(MockHeader::genesis()), header2.clone());

		let (action1, eq_proofs1) =
			h1.handle_state(state1, 1).expect("handle state should succeed");
		let (action2, eq_proofs2) =
			h1.handle_state(state2, 2).expect("handle state should succeed");
