...
use selendra_primitives::MAXIMUM_BLOCK_LENGTH as MAX_BLOCK_SIZE;
...
impl<J: Justification> State<J> {
	pub fn new(top_justification: J::Unverified, favourite_block: UnverifiedHeaderFor<J>) -> Self {
		State { top_justification, favourite_block }
	}
...
impl<J: Justification> TryFrom<RequestV2<J>> for Request<J> {
	...
	fn try_from(other: RequestV2<J>) -> Result<Self, Self::Error> {
		let RequestV2 { target, branch_knowledge, state } = other;
		let target = match target {
			MaybeHeader::Header(header) => header,
			MaybeHeader::Id(_) => return Err(()),
		};
		Ok(Request { target, branch_knowledge, state })
	}
}

impl<J: Justification> From<Request<J>> for RequestV2<J> {
	fn from(other: Request<J>) -> Self {
		let Request { target, branch_knowledge, state } = other;
		RequestV2 { target: MaybeHeader::Header(target), branch_knowledge, state }
	}
}
...
impl<J: Justification> Request<J> {
	pub fn new(
		...
		Request { target, branch_knowledge, state }
	}
}
...
impl<UH: UnverifiedHeader, I: PeerId> PreRequest<UH, I> {
	pub fn new(header: UH, branch_knowledge: BranchKnowledge, know_most: HashSet<I>) -> Self {
		PreRequest { header, branch_knowledge, know_most }
	}
	...
	{
		let PreRequest { header, branch_knowledge, know_most } = self;
	...
	B: Block<UnverifiedHeader = UnverifiedHeaderFor<J>>,
{
	...
	pub fn new(inner: N) -> Self {
		VersionWrapper { inner, _phantom: PhantomData }
	}
}
	...
	B: Block<UnverifiedHeader = UnverifiedHeaderFor<J>>,
{
	...
	fn send_to(
		...
		self.inner
			.send_to(VersionedNetworkData::V3(data.clone().into()), peer_id.clone())?;
	...
	fn send_to_random(
		...
		self.inner
			.send_to_random(VersionedNetworkData::V3(data.clone().into()), peer_ids.clone())?;
		self.inner.send_to_random(VersionedNetworkData::V4(data), peer_ids)
	}

	fn broadcast(&mut self, data: NetworkData<B, J>) -> Result<(), Self::Error> {
		self.inner.broadcast(VersionedNetworkData::V3(data.clone().into()))?;
