...
enum InnerVertex<J: Justification> {
	...
	Header { importance: HeaderImportance, header: J::Header },
	/// Vertex with added Header and Justification.
	Justification { imported: bool, justification: J, parent: BlockId },
}
...
impl<I: PeerId, J: Justification> Vertex<I, J> {
	...
	pub fn new() -> Self {
		Vertex {
			inner: InnerVertex::Empty { required: Importance::Auxiliary },
	...
	pub fn importable(&self) -> bool {
		...
		matches!(
			self.inner,
			Empty { required: Required | ExplicitlyRequired }
				| Header {
					importance: HeaderImportance::Unimported(Required | ExplicitlyRequired),
					..
				} | Justification { imported: false, .. }
		)
	}
	...
	pub fn requestable(&self) -> bool {
		...
		matches!(
			self.inner,
			Empty { required: ExplicitlyRequired }
				| Header { importance: HeaderImportance::Unimported(ExplicitlyRequired), .. }
		)
	}
	...
	pub fn imported(&self) -> bool {
		...
		matches!(
			self.inner,
			Header { importance: HeaderImportance::Imported, .. }
	...
	pub fn justified_block(&self) -> bool {
		matches!(self.inner, InnerVertex::Justification { imported: true, .. })
	}
	...
	pub fn ready(self) -> Result<J, Self> {
		match self.inner {
			InnerVertex::Justification { imported: true, justification, .. } => Ok(justification),
			_ => Err(self),
		}
	}
	...
	pub fn know_most(&self) -> HashSet<I> {
		self.know_most.iter().map(|(peer, ())| peer).cloned().collect()
	}
	...
	pub fn set_explicitly_required(&mut self) -> bool {
		...
		use InnerVertex::*;
		match &self.inner {
			Empty { required: Required | Auxiliary } => {
				self.inner = Empty { required: ExplicitlyRequired };
				true
			},
			Header { importance: Unimported(Required | Auxiliary), header } => {
				self.inner =
					Header { importance: Unimported(ExplicitlyRequired), header: header.clone() };
				true
			},
			_ => false,
		}
	...
	pub fn set_required(&mut self) -> bool {
		...
		use InnerVertex::*;
		match &self.inner {
			Empty { required: Auxiliary } => {
				self.inner = Empty { required: Required };
				true
			},
			Header { importance: Unimported(Auxiliary), header } => {
				self.inner = Header { importance: Unimported(Required), header: header.clone() };
				true
			},
			_ => false,
		}
	...
	pub fn insert_body(&mut self, header: J::Header) -> bool {
		use InnerVertex::*;
		match &self.inner {
			Empty { .. } | Header { importance: HeaderImportance::Unimported(_), .. } => {
				self.inner = Header { header, importance: HeaderImportance::Imported };
				true
			},
			Justification { imported: false, parent, justification } => {
	...
	pub fn insert_justification(&mut self, parent: BlockId, justification: J, holder: Option<I>) {
		...
		match self.inner {
			Empty { .. } | Header { importance: HeaderImportance::Unimported(_), .. } => {
				self.inner = Justification { imported: false, parent, justification };
			...
			Header { importance: HeaderImportance::Imported, .. } => {
				self.inner = Justification { imported: true, parent, justification };
				self.know_most.clear();
				if let Some(peer) = holder {
					self.know_most.put(peer, ());
				}
