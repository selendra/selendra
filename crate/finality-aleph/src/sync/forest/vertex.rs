...
enum InnerVertex<J: Justification> {
	...
	Justification { importance: JustificationImportance, justification: J, parent: BlockId },
}
...
impl<I: PeerId, J: Justification> Vertex<I, J> {
	...
	pub fn importable(&self) -> bool {
		...
		use InnerVertex::*;
		match self.inner {
			Empty { required: Required | ExplicitlyRequired } => true,
	...
	pub fn requestable(&self) -> bool {
		...
		use InnerVertex::*;
		match self.inner {
			Empty { required: ExplicitlyRequired } => true,
	...
	pub fn importing(&self) -> bool {
		use InnerVertex::*;
		match self.inner {
			Header { importance: HeaderImportance::Importing(_, importing), .. }
			| Justification { importance: JustificationImportance::Importing(importing), .. } => {
				importing.imported()
			},
			_ => false,
		}
	...
	pub fn justified_block(&self) -> bool {
		matches!(
			self.inner,
			InnerVertex::Justification { importance: JustificationImportance::Imported, .. }
		)
	}
	...
	pub fn set_explicitly_required(&mut self) -> bool {
		...
		match &mut self.inner {
			Empty { required }
			| Header { importance: Unimported(required) | Importing(required, _), .. } => {
	...
	pub fn insert_justification(&mut self, parent: BlockId, justification: J, holder: Option<I>) {
		...
		match self.inner {
			...
			Header { importance, .. } => {
				self.inner = Justification { importance: importance.into(), parent, justification };
				self.know_most.clear();
				if let Some(peer) = holder {
					self.know_most.put(peer, ());
				}
