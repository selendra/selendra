...
pub enum ExtensionRequest<H: Header, I: PeerId> {
	...
	HighestJustified { header: H, know_most: HashSet<I>, branch_knowledge: BranchKnowledge },
}
...
impl Display for Error {
	fn fmt(&self, f: &mut Formatter<'_>) -> Result<(), FmtError> {
		...
		match self {
			...
			IncorrectParentState => {
				write!(f, "parent was in a state incompatible with importing this block")
			},
...
impl<I: PeerId, J: Justification> VertexWithChildren<I, J> {
	fn new() -> Self {
		Self { vertex: Vertex::new(), children: HashSet::new() }
	}
	...
	J: Justification,
{
		...
		CS: ChainStatus<B, J>,
	{
		...
		while let Some(id) = deque.pop_front() {
			let children = chain_status.children(id).map_err(InitializationError::ChainStatus)?;
			for header in children.iter() {
				forest.update_body(header).map_err(InitializationError::Error)?;
			}
		...
		if let Candidate(mut entry) = self.get_mut(id) {
			...
			if let Some(parent_id) = vertex.vertex.parent() {
				match self.get_mut(&parent_id) {
					Unknown(entry) => {
						entry.insert(VertexWithChildren::new()).add_child(id.clone());
						if required {
	...
	fn process_header(&mut self, header: &J::Header) -> Result<Edge, Error> {
		Ok((header.id(), header.parent_id().ok_or(Error::HeaderMissingParentId)?))
	}
	...
	fn pick_favourite(&mut self) {
		use SpecialState::*;
		use VertexHandle::*;
		if matches!(self.get(&self.favourite.id()), Special(HighestFinalized) | Candidate(_)) {
	...
	fn prune_level(&mut self, level: BlockNumber) {
		let to_prune: Vec<_> =
			self.vertices.keys().filter(|k| k.number() <= level).cloned().collect();
	...
	pub fn behind_finalization(&self) -> u32 {
		self.highest_justified.id().number().saturating_sub(self.root.id().number())
	}
	...
	pub fn extension_request(&self) -> ExtensionRequest<J::Header, I> {
		...
		if let Candidate(vertex) = self.get(&self.favourite.id()) {
			let know_most: HashSet<_> =
				vertex.children.iter().flat_map(|child| self.know_most(child)).collect();
		...
		if let Special(SpecialState::HighestFinalized) = self.get(&self.favourite.id()) {
			let know_most: HashSet<_> =
				self.root_children.iter().flat_map(|child| self.know_most(child)).collect();
			if !know_most.is_empty() {
				return FavouriteBlock { know_most };
			}
		}
...
mod tests {
	...
	fn setup() -> (MockHeader, MockForest) {
		...
		let header = backend.top_finalized().expect("should return genesis").header().clone();
	...
	fn initially_empty() {
		...
		assert!(matches!(forest.request_interest(&initial_header.id()), Uninterested));
	...
	fn accepts_first_unimportant_header() {
		...
		assert!(forest.update_header(&child, Some(peer_id), false).expect("header was correct"));
		assert!(forest.try_finalize(&1).is_none());
	...
	fn accepts_unimportant_descendant_of_unimportant_header() {
		...
		assert!(forest.try_finalize(&1).is_none());
		assert!(matches!(forest.request_interest(&grandchild.id()), Uninterested));
		assert!(!forest.importable(&grandchild.id()));
		assert_eq!(forest.extension_request(), FavouriteBlock { know_most: child_know_most });
	}
	...
	fn accepts_first_important_header() {
		...
		let child = initial_header.random_child();
		let peer_id = rand::random();
		assert!(forest.update_header(&child, Some(peer_id), true).expect("header was correct"));
		assert!(forest.try_finalize(&1).is_none());
	...
	fn accepts_first_justification() {
		...
		assert!(forest
			...
			.expect("header was correct"));
		assert!(forest.try_finalize(&1).is_none());
		assert!(matches!(forest.request_interest(&child.header().id()), Uninterested));
	...
	fn accepts_descendand_justification() {
		...
		assert!(forest.try_finalize(&1).is_none());
		assert!(matches!(forest.request_interest(&child.header().id()), Uninterested));
		assert!(matches!(forest.request_interest(&grandchild.header().id()), Uninterested));
		match forest.extension_request() {
			HighestJustified { know_most, .. } => {
	...
	fn unimportant_parent_of_justification_importable() {
		...
		assert!(forest
			...
			.expect("header was correct"));
		assert!(matches!(forest.request_interest(&first_child.id()), Uninterested));
		match forest.extension_request() {
		...
		assert!(forest
			...
			.expect("header was correct"));
		assert!(matches!(forest.request_interest(&second_child.id()), Uninterested));
		match forest.extension_request() {
			FavouriteBlock { know_most, .. } => {
				assert!(know_most.contains(&peer_id))
			},
		...
		forest.update_body(&first_child).expect("header was correct");
		assert!(!forest.importable(&second_child.id()));
	...
	fn ignores_genesis_justification() {
		...
		let peer_id = rand::random();
		assert!(matches!(forest.update_justification(parentless, Some(peer_id)), Ok(false)));
	}
	...
	fn rejects_body_when_parent_unimported() {
		...
		assert!(matches!(forest.update_body(&grandchild), Err(Error::ParentNotImported)));
		assert!(forest.try_finalize(&1).is_none());
		assert!(matches!(forest.request_interest(&child.id()), Uninterested));
		assert!(forest.importable(&child.id()));
		assert!(matches!(forest.request_interest(&grandchild.id()), Uninterested));
	...
	fn finalizes_first_block() {
		...
		assert!(forest.importable(&child.header().id()));
		forest.update_body(child.header()).expect("header was correct");
	...
	fn ancestor_does_not_become_highest_finalized() {
		...
		assert!(
			!forest.update_justification(child, Some(peer_id)).expect("header was correct"),
	...
	fn prunes_forks() {
		...
		forest.update_body(child.header()).expect("header was correct");
		assert_eq!(forest.try_finalize(&1).expect("the block is ready"), child);
		assert!(matches!(forest.request_interest(&fork_child.id()), Uninterested));
		assert!(!forest.importable(&fork_child.id()));
		assert_eq!(forest.update_header(&fork_child, Some(fork_peer_id), true), Ok(false));
		assert_eq!(forest.extension_request(), Noop);
	}
	...
	fn uninterested_in_forks() {
		...
		for header in &fork_branch {
			let peer_id = rand::random();
			assert!(forest.update_header(header, Some(peer_id), true).expect("header was correct"));
		...
		assert_eq!(forest.try_finalize(&1).expect("the block is ready"), child);
		for header in fork_branch {
			assert!(matches!(forest.request_interest(&header.id()), Uninterested));
			assert!(!forest.importable(&header.id()));
		}
		assert_eq!(forest.extension_request(), Noop);
	}
	...
	fn picks_new_favourite() {
		...
		assert_eq!(forest.extension_request(), FavouriteBlock { know_most: fork_child_know_most });
	...
	fn updates_importability_on_parent_connect() {
		...
		let peer_id = rand::random();
		assert!(
			forest.update_header(header, Some(peer_id), false).expect("header was correct"),
			"header is above favourite"
		);
		assert!(matches!(forest.request_interest(&header.id()), Uninterested));
		assert!(!forest.importable(&header.id()));
		...
		assert!(matches!(forest.request_interest(&branch[3].id()), Required { .. }));
	}
	...
	fn finds_ancestors() {
		...
		let header = &branch[0];
		let peer_id = rand::random();
		assert!(forest.update_header(header, Some(peer_id), false).expect("header was correct"));
		assert!(matches!(forest.request_interest(&header.id()), Uninterested));
		assert!(forest.importable(&header.id()));
		// skip branch[1]
		let header = &branch[2];
		...
		assert!(forest.update_header(header, Some(peer_id), true).expect("header was correct"));
		match forest.request_interest(&header.id()) {
			Required { header: required_header, know_most, branch_knowledge } => {
	...
	fn finalizes_huge_branch() {
		...
		for justification in &justifications {
			...
			assert!(forest
				...
				.expect("header was correct"));
			assert!(matches!(forest.request_interest(&justification.header().id()), Uninterested));
			assert!(forest.importable(&justification.header().id()));
			forest.update_body(justification.header()).expect("header was correct");
		}
		for (number, justification) in justifications.into_iter().enumerate() {
			assert_eq!(
				forest.try_finalize(&(number as BlockNumber + 1)).expect("the block is ready"),
				justification
			);
		}
	}
	...
	fn prunes_huge_branch() {
		let (initial_header, mut forest) = setup();
		let fork: Vec<_> = initial_header.random_branch().take(HUGE_BRANCH_LENGTH).collect();
		for header in &fork {
			let peer_id = rand::random();
			assert!(forest.update_header(header, Some(peer_id), true).expect("header was correct"));
			assert!(forest.try_finalize(&1).is_none());
	...
	fn updates_interest_on_huge_branch() {
		let (initial_header, mut forest) = setup();
		let branch: Vec<_> = initial_header.random_branch().take(HUGE_BRANCH_LENGTH).collect();
