...
impl<T> Debug for TaskQueue<T> {
	fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
		f.debug_struct("TaskQueue").field("task count", &self.queue.len()).finish()
	}
}
...
impl<T> TaskQueue<T> {
	...
	pub fn schedule_in(&mut self, task: T, delay: Duration) {
		...
		self.queue.push(ScheduledTask { task, scheduled_time });
	}
	...
	async fn sleep_until_the_next_task_is_ready(&self) {
		if let Some(scheduled_task) = self.queue.peek() {
			let duration = scheduled_task.scheduled_time.saturating_duration_since(Instant::now());
			if !duration.is_zero() {
...
mod tests {
	...
	async fn test_scheduling() {
		...
		assert_eq!(timeout(Duration::from_millis(20), q.pop()).await, Ok(Some(1)));
		assert!(timeout(Duration::from_millis(10), q.pop()).await.is_err());
		assert_eq!(timeout(Duration::from_millis(50), q.pop()).await, Ok(Some(2)));
	}
}
