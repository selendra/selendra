	...
	BI: BlockImport<B>,
{
	...
	fn request_block(&mut self, block_id: BlockId) {
		...
		self.tasks.schedule_in(RequestTask::new(block_id), Duration::ZERO);
	}
	...
	fn request_chain_extension(&mut self, force: bool) {
		...
		match self.handler.extension_request() {
			...
			HighestJustified { header, know_most, branch_knowledge } => {
	...
	fn handle_justification_from_user(&mut self, justification: J::Unverified) {
		...
		self.metrics.report_event(Event::HandleJustificationFromUser);
		match self.handler.handle_justification_from_user(justification) {
			Ok(true) => self.try_request_chain_extension(),
			Ok(false) => {},
			Err(e) => {
				self.metrics.report_event_error(Event::HandleJustificationFromUser);
				match e {
					HandlerError::JustificationVerifier(e) => debug!(
						target: LOG_TARGET,
	...
	fn handle_request_response(&mut self, response_items: ResponseItems<B, J>, peer: N::PeerId) {
		...
		let (new_info, equivocation_proofs, maybe_error) =
			self.handler.handle_request_response(response_items, peer.clone());
		match maybe_error {
			...
			BlockImported(header) => {
				...
				match self.handler.block_imported(header) {
					Ok(Some(broadcast)) => {
						own_block = true;
						if let Err(e) =
							self.network.broadcast(NetworkData::RequestResponse(broadcast))
						{
							warn!(
								target: LOG_TARGET,
				...
				self.slo_metrics.report_block_imported(id, is_new_best, own_block);
			},
		...
		match self.handler.handle_chain_extension_request(state) {
			Ok(Action::Response(response_items)) => {
				if let Err(e) = self.send_big_response(&response_items, peer) {
					...
					self.metrics.report_event_error(Event::HandleExtensionRequest);
				}
			},
