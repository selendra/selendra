...
fn prepare_data_store(
	...
	(
		...
		exit_data_store_tx,
		TestHandler { chain_builder, block_requests_rx, network_tx, network: Box::new(network) },
	)
}
...
async fn branch_not_within_session_boundaries_does_not_go_through() {
	...
	test_handler.finalize_block(&blocks[session_end + MAX_DATA_BRANCH_LEN].header().id().hash());
	...
	run_test(|mut test_handler| async move {
		...
		for i in 1..MAX_DATA_BRANCH_LEN {
			let test_data: TestData = vec![aleph_data_from_headers(
				(0..i).map(|num| test_handler.get_header_at(num as BlockNumber)).collect(),
			)];
			test_handler.send_data(test_data.clone());
		}
