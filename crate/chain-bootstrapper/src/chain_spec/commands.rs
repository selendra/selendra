...
use selendra_primitives::DEFAULT_BACKUP_FOLDER;

use crate::chain_spec::{
	build_chain_spec_json,
	cli::ChainSpecParams,
	keystore::{
		create_abft_backup_dir, create_account_session_keys, create_p2p_key, open_keystore,
	},
	SelendraNodeChainSpec,
};
...
impl BootstrapChainCmd {
	pub fn run(&self) -> Result<(), Error> {
		...
		let account_session_keys =
			self.chain_spec_params.clone().account_ids().into_iter().map(|account_id| {
...
impl ConvertChainspecToRawCmd {
	pub fn run(&self) -> Result<(), Error> {
		let spec = SelendraNodeChainSpec::from_json_file(self.chain.to_owned())
