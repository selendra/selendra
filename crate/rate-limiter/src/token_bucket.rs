...
mod tests {
			...
			)>,
	{
		...
		let rate_limiter =
			RL::from((limit_per_second, Arc::new(time_provider), SharedTracingSleepUntil::new()));
			...
			)>,
	{
		...
		let time_provider: Box<dyn TimeProvider + Send + Sync> =
			Box::new(move || *time_provider.read());
		let rate_limiter =
			RL::from((limit_per_second, time_provider.into(), SharedTracingSleepUntil::new()));
	...
	async fn two_peers_can_share_bandwidth() {
		...
		let total_data_sent = thread::scope(|s| {
			...
			let total_data_sent: u128 =
				first_handle.join().expect("first thread should finish without errors")
