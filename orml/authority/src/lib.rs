...
pub mod module {
	...
	#[pallet::origin]
	pub type Origin<T> =
		DelayedOrigin<<T as frame_system::Config>::BlockNumber, <T as Config>::PalletsOrigin>;
	...
	pub trait Config: frame_system::Config {
		...
		type AsOriginId: Parameter
			+ AsOriginId<<Self as frame_system::Config>::Origin, Self::PalletsOrigin>;
	...
	pub enum Event<T: Config> {
		...
		Delayed { origin: T::PalletsOrigin, index: ScheduleTaskIndex, when: T::BlockNumber },
	...
	pub type SavedCalls<T: Config> =
		StorageMap<_, Identity, T::Hash, (CallOf<T>, Option<T::AccountId>), OptionQuery>;
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn schedule_dispatch(
			...
			let id = NextTaskIndex::<T>::mutate(
				|id| -> sp_std::result::Result<ScheduleTaskIndex, DispatchError> {
					let current_id = *id;
		...
		pub fn cancel_scheduled_dispatch(
			...
			T::Scheduler::cancel_named((&initial_origin, task_id).encode())
				.map_err(|_| Error::<T>::FailedToCancel)?;
		...
		pub fn trigger_call(
			...
			SavedCalls::<T>::try_mutate_exists(hash, |maybe_call| {
				let (call, maybe_caller) =
					maybe_call.take().ok_or(Error::<T>::CallNotAuthorized)?;
