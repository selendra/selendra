...
//! Forests Collator implementation for Substrate.

use forests_client_network::WaitToAnnounce;
...
/// The logging target.
const LOG_TARGET: &str = "forests-collator";

/// The implementation of the Forests `Collator`.
	...
	RA::Api: CollectCollationInfo<Block>,
{
	...
	async fn produce_candidate(
			...
			.into_compact_proof::<HashFor<Block>>(last_head.state_root().clone())
		{
			Ok(proof) => proof,
			Err(e) => {
				tracing::error!(target: "forests-collator", "Failed to compact proof: {:?}", e);
				return None
			},
		};
...
mod tests {
	use super::*;
	use forests_client_consensus_common::ParachainCandidate;
	use forests_test_client::{
