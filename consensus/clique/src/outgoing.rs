	...
	fn fmt(&self, f: &mut Formatter<'_>) -> Result<(), FmtError> {
		...
		match self {
			...
			ProtocolNegotiation(addr, e) =>
				write!(f, "communication with {} failed, protocol negotiation error: {}", addr, e),
			Protocol(addr, e) =>
				write!(f, "communication with {} failed, protocol error: {}", addr, e),
...
async fn manage_outgoing<SK: SecretKey, D: Data, A: Data, ND: Dialer<A>>(
	...
	debug!(target: LOG_TARGET, "Performing outgoing protocol negotiation.");
	...
	protocol
		.manage_outgoing(stream, secret_key, public_key, result_for_parent, data_for_user)
		.await
