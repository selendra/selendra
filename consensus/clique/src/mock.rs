...
impl UnreliableConnectionMaker {
	...
	pub async fn run(&mut self, connections_end_after: Option<usize>) {
		loop {
			info!(target: LOG_TARGET, "UnreliableConnectionMaker: waiting for new request...");
			let (dialer_address, listener_address, c) =
				self.dialers.next().await.expect("should receive");
			info!(target: LOG_TARGET, "UnreliableConnectionMaker: received request");
			let (dialer_stream, listener_stream) =
				Connection::new(4096, connections_end_after, dialer_address, listener_address);
			info!(target: LOG_TARGET, "UnreliableConnectionMaker: sending stream");
