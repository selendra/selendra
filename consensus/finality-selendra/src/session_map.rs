...
use selendra_primitives::{BlockNumber, SelendraSessionApi, SessionAuthorityData};
use sp_runtime::{
	generic::BlockId,
	traits::{Block, Header},
};
...
use crate::{session::SessionBoundaryInfo, ClientForSelendra, SessionId, SessionPeriod};
...
where
	C: ClientForSelendra<B, BE> + Send + Sync + 'static,
...
{
	fn authority_data(&self, block_number: BlockNumber) -> Option<SessionAuthorityData> {
		match self.client.runtime_api().authority_data(&BlockId::Number(block_number)) {
			Ok(data) => Some(data),
			Err(_) => self
				.client
				.runtime_api()
	...
	fn next_authority_data(&self, block_number: BlockNumber) -> Option<SessionAuthorityData> {
			...
			.map(|r| r.ok())
		{
			...
			Err(_) => self
				...
				.map(|r| r.map(|authorities| SessionAuthorityData::new(authorities, None)).ok())
				.ok()
				.flatten(),
		}
	}
...
{
	async fn next(&mut self) -> Option<BlockNumber> {
		self.notification_stream.next().await.map(|block| *block.header.number())
	}
...
impl SharedSessionMap {
		...
		if let Some(senders) = guard.1.remove(&id) {
			for sender in senders {
				if let Err(e) = sender.send(authority_data.clone()) {
					error!(target: "selendra-session-updater", "Error while sending notification: {:?}", e);
				}
			}
		}
	...
	pub fn read_only(&self) -> ReadOnlySessionMap {
		ReadOnlySessionMap { inner: self.0.clone() }
	}
}
...
impl ReadOnlySessionMap {
	...
	pub async fn subscribe_to_insertion(
		...
		if let Some(authority_data) = guard.0.get(&id) {
			...
			sender.send(authority_data.clone()).expect("we control both ends");
		} else {
	...
	FN: FinalityNotifier,
{
	...
	async fn handle_first_block_of_session(&mut self, session_id: SessionId) {
		...
		debug!(target: "selendra-session-updater",
		...
		if let Some(authority_data) = self.authority_provider.next_authority_data(first_block) {
			self.session_map.update(SessionId(session_id.0 + 1), authority_data).await;
		} else {
		...
		if session_id.0 > PRUNING_THRESHOLD && session_id.0 % PRUNING_THRESHOLD == 0 {
			...
			self.session_map.prune_below(SessionId(session_id.0 - PRUNING_THRESHOLD)).await;
		}
	}
	...
	async fn catch_up(&mut self) -> SessionId {
		...
		for session in starting_session.0..current_session.0 {
			...
			if let Some(authority_data) = self.authorities_for_session(id) {
				...
				debug!(target: "selendra-session-updater", "No authorities for session {:?} during catch-up. Most likely already pruned.", id.0)
			}
		}
		...
		match self.authorities_for_session(current_session) {
			Some(current_authority_data) =>
				self.session_map.update(current_session, current_authority_data).await,
			None => panic!(
	...
	pub async fn run(mut self) {
		...
		while let Some(last_finalized) = self.finality_notifier.next().await {
			trace!(target: "selendra-session-updater", "got FinalityNotification about #{:?}", last_finalized);
...
mod tests {
	...
	impl MockProvider {
		fn new() -> Self {
			Self { session_map: HashMap::new(), next_session_map: HashMap::new() }
		}

		fn add_session(&mut self, session_id: BlockNumber) {
			self.session_map.insert(session_id, authority_data_for_session(session_id));
	...
	impl MockNotifier {
		fn new(receiver: TracingUnboundedReceiver<BlockNumber>) -> Self {
			Self { receiver, last_finalized: 0 }
		}
	}
	...
	async fn updates_session_map_on_notifications() {
		...
		assert_eq!(session_map.get(SessionId(0)).await, Some(authority_data(0, 4)));
		assert_eq!(session_map.get(SessionId(1)).await, Some(authority_data(4, 8)));
		assert_eq!(session_map.get(SessionId(2)).await, Some(authority_data(8, 12)));
		assert_eq!(session_map.get(SessionId(3)).await, Some(authority_data(12, 16)));
	}
	...
	async fn catch_up() {
		...
		assert_eq!(session_map.get(SessionId(2)).await, Some(authority_data_for_session(2)));
		assert_eq!(session_map.get(SessionId(3)).await, Some(authority_data_for_session(3)));
	}
	...
	async fn deals_with_database_pruned_authorities() {
		...
		assert_eq!(session_map.get(SessionId(5)).await, Some(authority_data_for_session(5)));
		assert_eq!(session_map.get(SessionId(6)).await, Some(authority_data_for_session(6)));
	}
