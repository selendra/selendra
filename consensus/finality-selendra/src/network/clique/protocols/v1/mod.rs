...
async fn receiving<PK: PublicKey, D: Data, S: AsyncRead + Unpin + Send>(
	...
	use Message::*;
	loop {
		let (old_stream, message) =
			timeout(MAX_MISSED_HEARTBEATS * HEARTBEAT_TIMEOUT, receive_data(stream))
				.await
...
pub async fn outgoing<SK: SecretKey, D: Data, S: Splittable>(
	...
	info!(target: LOG_TARGET, "Outgoing handshake with {} finished successfully.", public_key);
	let (data_for_network, data_from_user) = mpsc::unbounded();
	result_for_parent
		.unbounded_send((public_key.clone(), Some(data_for_network)))
		.map_err(|_| ProtocolError::NoParentConnection)?;

	debug!(target: LOG_TARGET, "Starting worker for communicating with {}.", public_key);
...
pub async fn incoming<SK: SecretKey, D: Data, S: Splittable>(
	...
	info!(target: LOG_TARGET, "Incoming handshake with {} finished successfully.", public_key);
