	...
	SK::PublicKey: PeerId,
{
	...
	fn spawn_new_outgoing(
		...
		self.spawn_handle.spawn("selendra/clique_network_outgoing", None, async move {
			outgoing(secret_key, public_key, dialer, address, result_for_parent, next_to_interface)
				.await;
		});
	}
	...
	fn spawn_new_incoming(
		...
		let secret_key = self.secret_key.clone();
		let next_to_interface = self.next_to_interface.clone();
		self.spawn_handle.spawn("selendra/clique_network_incoming", None, async move {
			incoming(
				secret_key,
				stream,
				result_for_parent,
