...
/// Name of the network protocol used by Selendra Zero to disseminate validator
/// authentications.
const AUTHENTICATION_PROTOCOL_NAME: &str = "/auth/0";

/// Legacy name of the network protocol used by Selendra Zero to disseminate validator
/// authentications. Might be removed after some updates.
const LEGACY_AUTHENTICATION_PROTOCOL_NAME: &str = "/selendra/1";

/// Name of the network protocol used by Selendra Zero to synchronize the block state.
	...
	fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
		match self {
			...
			SenderError::LostConnectionToPeer(peer_id) => {
				write!(f, "Lost connection to peer {:?} while preparing sender", peer_id)
			},
			SenderError::LostConnectionToPeerReady(peer_id) => {
				write!(f, "Lost connection to peer {:?} after sender was ready", peer_id)
			},
		}
	}
}
				...
				Some(event) = self.sync_stream.next() => {
					match event {
						PeerConnected(remote) => {
							...
							trace!(target: "selendra-network", "Connected event from address {:?}", multiaddress);
							if let Err(e) = self.network.add_peers_to_reserved_set(
								...
								error!(target: "selendra-network", "add_reserved failed for authentications: {}", e);
							}
							if let Err(e) = self.network.add_peers_to_reserved_set(
								self.naming.protocol_name(&Protocol::BlockSync),
								iter::once(multiaddress).collect(),
							) {
								error!(target: "selendra-network", "add_reserved failed for block sync: {}", e);
							}
							continue;
						}
						PeerDisconnected(remote) => {
							trace!(target: "selendra-network", "Disconnected event for peer {:?}", remote);
...
impl<B: Block, H: ExHashT> SubstrateNetwork<B, H> {
	...
	pub fn new(
		...
		SubstrateNetwork { network, sync_network, naming }
	}
}
...
impl<B: Block, H: ExHashT> RawNetwork for SubstrateNetwork<B, H> {
	...
	fn event_stream(&self) -> Self::EventStream {
		NetworkEventStream {
			stream: Box::pin(self.network.as_ref().event_stream("selendra-network")),
			sync_stream: Box::pin(
				self.sync_network.as_ref().event_stream("selendra-syncing-network"),
			),
