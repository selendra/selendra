...
impl<PID: PeerId> Connections<PID> {
	...
	pub fn new() -> Self {
		Connections { associated_sessions: HashMap::new(), peers_by_session: HashMap::new() }
	}
	...
	pub fn add_peers(&mut self, session_id: SessionId, peers: impl IntoIterator<Item = PID>) {
		for peer in peers {
			self.associated_sessions.entry(peer.clone()).or_default().insert(session_id);
			self.peers_by_session.entry(session_id).or_default().insert(peer);
		}
	}
