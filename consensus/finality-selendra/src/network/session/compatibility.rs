...
fn encode_with_version(version: Version, payload: &[u8]) -> Vec<u8> {
	...
	let size = payload.len().try_into().unwrap_or(MAX_AUTHENTICATION_SIZE + 1);
...
mod test {
	...
	async fn handler() -> SessionHandler<SignedTcpAddressingInformation> {
		...
		let external_addresses =
			vec![String::from("addr1"), String::from("addr2"), String::from("addr3")];
	...
	fn authentication_v2(
		...
		VersionedAuthentication::V2(handler.authentication().expect("should have authentication"))
	}
	...
	/// encoded at version of Selendra Node after 8.0
