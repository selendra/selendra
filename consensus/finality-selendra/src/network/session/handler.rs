...
async fn construct_session_info<A: AddressingInformation>(
	...
	match authority_index_and_pen {
		Some((node_index, authority_pen)) => {
			let auth_data = AuthData { address, node_id: *node_index, session_id };
...
impl<A: AddressingInformation> Handler<A> {
	...
	pub fn handle_authentication(&mut self, authentication: Authentication<A>) -> Option<A> {
		...
		self.peers_by_node.insert(auth_data.creator(), peer_id.clone());
	...
	pub async fn update(
		...
		*self =
			Handler::new(authority_index_and_pen, authority_verifier, self.session_id(), address)
				.await;
...
pub mod tests {
	...
	pub fn authentication(
		...
		handler.authentication().expect("this is a validator handler")
	}
	...
	async fn identifies_whether_node_is_authority_in_current_session() {
		...
		let no_authority_handler =
			Handler::new(None, crypto_basics.1.clone(), SessionId(43), random_address()).await;
	...
	async fn non_validator_handler_returns_none_for_authentication() {
		...
		assert!(Handler::new(None, crypto_basics.1, SessionId(43), random_address(),)
			.await
			.authentication()
			.is_none());
	}
	...
	async fn fails_to_update_from_validator_to_non_validator() {
		...
		.await;
		assert!(matches!(
			handler0.update(None, crypto_basics.1.clone(), address).await,
	...
	async fn fails_to_update_from_non_validator_to_validator() {
		...
		let address = random_address();
		let mut handler0 =
			Handler::new(None, crypto_basics.1.clone(), SessionId(43), address.clone()).await;
		assert!(matches!(
			handler0
				.update(Some(crypto_basics.0.pop().unwrap()), crypto_basics.1.clone(), address,)
				.await,
			Err(HandlerError::TypeChange)
		));
	}
	...
	async fn accepts_correct_authentication() {
		...
		.await;
		assert!(handler0.handle_authentication(authentication(&handler1)).is_some());
		let missing_nodes = handler0.missing_nodes();
	...
	async fn ignores_invalid_authentication() {
		...
		.await;
		assert!(handler0.handle_authentication(authentication(&handler1)).is_none());
		let missing_nodes = handler0.missing_nodes();
	...
	async fn ignores_own_authentication() {
		...
		.await;
		assert!(handler0.handle_authentication(authentication(&handler0)).is_none());
		let missing_nodes = handler0.missing_nodes();
		let expected_missing: Vec<_> = (1..NUM_NODES).map(NodeIndex).collect();
		assert_eq!(missing_nodes, expected_missing);
	}
