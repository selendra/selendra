...
impl<A: AddressingInformation> Discovery<A> {
	...
	pub fn new(cooldown: Duration) -> Self {
		Discovery { cooldown, last_broadcast: HashMap::new(), _phantom: PhantomData }
	}
...
mod tests {
	...
	async fn build_number(
		...
		let non_validator =
			SessionHandler::new(None, crypto_basics.1.clone(), SessionId(43), random_address())
				.await;
	...
	async fn broadcasts_when_clueless() {
		for num_nodes in 2..NUM_NODES {
			...
			assert_eq!(
				...
				handler.authentication().expect("the handler has an authentication"),
			);
		}
	}
