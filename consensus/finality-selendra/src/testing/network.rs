...
async fn prepare_one_session_test_data() -> TestData {
	...
	for (index, p) in authority_pens {
		...
		authorities.push(Authority { pen: p, peer_id: address.peer_id(), address, auth_peer_id });
	}
...
impl TestData {
	fn connect_identity_to_network(&mut self, peer_id: MockPublicKey, protocol: Protocol) {
		self.network.emit_event(MockEvent::StreamOpened(peer_id, protocol));
	}
		...
		for (index, authority) in self.authorities.clone().into_iter().enumerate().skip(1) {
				...
				Vec::<VersionedAuthentication<_>>::from(handler.authentication().unwrap())
			{
				self.network.emit_event(MockEvent::Messages(
					...
					vec![(Protocol::Authentication, versioned_authentication.encode().into())],
				));
			}
		}
	}
	...
	async fn next_sent_auth(
		&mut self,
	) -> Option<(VersionedAuthentication<MockAddressingInformation>, MockPublicKey, Protocol)> {
		loop {
...
async fn test_sends_discovery_message() {
	...
	assert_eq!(timeout(DEFAULT_TIMEOUT, data_network.next()).await, Ok(None));
}
...
async fn test_forwards_authentication_broadcast() {
	...
	for authority in test_data.authorities.iter().skip(1) {
		expected_authentication
			.insert(authority.auth_peer_id(), authentication(&sending_peer_handler));
	}
...
async fn test_connects_to_others() {
	...
	test_data
		.validator_network
		.next
		.send(DataInSession { data: data.clone(), session_id: SessionId(session_id) });

	assert_eq!(timeout(DEFAULT_TIMEOUT, data_network.next()).await, Ok(Some(data)));
...
async fn test_connects_to_others_early_validator() {
	...
	test_data
		...
		.next
		.send(DataInSession { data: data.clone(), session_id: SessionId(session_id) });
	assert_eq!(timeout(DEFAULT_TIMEOUT, data_network.next()).await, Ok(Some(data.clone())));
...
async fn test_stops_session() {
	...
	test_data.session_manager.stop_session(SessionId(session_id)).unwrap();
...
async fn test_receives_data_in_correct_session() {
	...
	test_data
		.validator_network
		.next
		.send(DataInSession { data: data_1_1.clone(), session_id: SessionId(session_id_1) });
	test_data
		.validator_network
		.next
		.send(DataInSession { data: data_2_1.clone(), session_id: SessionId(session_id_2) });

	test_data
		.validator_network
		.next
		.send(DataInSession { data: data_2_2.clone(), session_id: SessionId(session_id_2) });
	test_data
		.validator_network
		.next
		.send(DataInSession { data: data_1_2.clone(), session_id: SessionId(session_id_1) });

	assert_eq!(timeout(DEFAULT_TIMEOUT, data_network_1.next()).await, Ok(Some(data_1_1)));
	assert_eq!(timeout(DEFAULT_TIMEOUT, data_network_1.next()).await, Ok(Some(data_1_2)));
	assert_eq!(timeout(DEFAULT_TIMEOUT, data_network_2.next()).await, Ok(Some(data_2_1)));
	assert_eq!(timeout(DEFAULT_TIMEOUT, data_network_2.next()).await, Ok(Some(data_2_2)));

	test_data.cleanup().await;

	assert_eq!(timeout(DEFAULT_TIMEOUT, data_network_1.next()).await, Ok(None));
	assert_eq!(timeout(DEFAULT_TIMEOUT, data_network_2.next()).await, Ok(None));
}
...
async fn test_broadcasts_data_to_correct_session() {
	...
	data_network_1.send(data_1.clone(), Recipient::Everyone).expect("Should send");
	data_network_2.send(data_2.clone(), Recipient::Everyone).expect("Should send");
