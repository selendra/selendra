...
use crate::{
	...
	testing::{
		...
		mocks::{selendra_data_from_blocks, selendra_data_from_headers},
	},
...
impl<B: BlockT> TestBlockRequester<B> {
	fn new() -> (Self, UnboundedReceiver<BlockHashNum<B>>, UnboundedReceiver<BlockHashNum<B>>) {
		let (blocks_tx, blocks_rx) = mpsc::unbounded();
		let (justifications_tx, justifications_rx) = mpsc::unbounded();
		(
			TestBlockRequester { blocks: blocks_tx, justifications: justifications_tx },
			blocks_rx,
			justifications_rx,
		)
	}
...
impl<B: BlockT> RequestBlocks<B> for TestBlockRequester<B> {
	fn request_justification(&self, hash: &B::Hash, number: NumberFor<B>) {
		self.justifications.unbounded_send((*hash, number).into()).unwrap();
	}
...
impl SelendraNetworkMessage<Block> for TestData {
	fn included_data(&self) -> Vec<SelendraData<Block>> {
		self.clone()
	}
}
...
impl TestHandler {
	...
	async fn initialize_single_branch_and_import(&mut self, len: usize) -> Vec<Block> {
		self.chain_builder.initialize_single_branch_and_import(len).await
	}
...
fn prepare_data_store(
	...
	let test_network = TestComponentNetwork { sender: sender_tx, receiver: network_rx };
...
async fn correct_messages_go_through() {
	run_test(|mut test_handler| async move {
		let blocks =
			test_handler.initialize_single_branch_and_import(MAX_DATA_BRANCH_LEN * 10).await;

		for i in 1..=MAX_DATA_BRANCH_LEN {
			let blocks_branch = blocks[0..i].to_vec();
			let test_data: TestData = vec![selendra_data_from_blocks(blocks_branch)];
			test_handler.send_data(test_data.clone());

			let message =
				test_handler.assert_message_out("Did not receive message from Data Store").await;
...
async fn branch_not_within_session_boundaries_does_not_go_through() {
	...
	let data_store_handle = tokio::spawn(task_handle);
	let blocks = test_handler.initialize_single_branch_and_import(MAX_DATA_BRANCH_LEN * 10).await;
		...
		for l in 0..MAX_DATA_BRANCH_LEN {
			for r in 0..MAX_DATA_BRANCH_LEN {
					...
					!(session_start <= left_end && right_end <= session_end)
				{
					...
					test_handler.send_data(vec![selendra_data_from_blocks(blocks_branch)]);
				}
			}
		}
	}
		...
		for l in 0..MAX_DATA_BRANCH_LEN {
			for r in 0..MAX_DATA_BRANCH_LEN {
					...
					right_end <= session_end
				{
					...
					let blocks_branch = blocks[(left_end - 1)..right_end].to_vec();
					test_handler.send_data(vec![selendra_data_from_blocks(blocks_branch)]);
					test_handler.assert_message_out("Data Store held available proposal").await;
				}
			}
		}
	}
...
async fn correct_messages_go_through_with_late_import() {
	run_test(|mut test_handler| async move {
		let blocks = test_handler.initialize_single_branch(MAX_DATA_BRANCH_LEN * 10).await;
...
async fn message_with_multiple_data_gets_through_when_it_should() {
	run_test(|mut test_handler| async move {
		...
		for i in 1..=max_height {
			...
			test_data.push(selendra_data_from_blocks(blocks_branch));
		}
		test_handler.send_data(test_data.clone());
...
async fn sends_justification_request_when_not_finalized() {
	run_test(|mut test_handler| async move {
		...
		let test_data = vec![selendra_data_from_blocks(blocks_branch)];
...
async fn message_with_genesis_block_does_not_get_through() {
	run_test(|mut test_handler| async move {
		let _ = test_handler.initialize_single_branch_and_import(MAX_DATA_BRANCH_LEN * 10).await;

		for i in 1..MAX_DATA_BRANCH_LEN {
			let test_data: TestData = vec![selendra_data_from_headers(
				(0..i).into_iter().map(|num| test_handler.get_header_at(num as u64)).collect(),
			)];
			test_handler.send_data(test_data.clone());
		}
...
async fn imported_hopeless_forks_go_through() {
	run_test(|mut test_handler| async move {
		let blocks = test_handler.initialize_single_branch(10 * MAX_DATA_BRANCH_LEN).await;
...
async fn hopeless_fork_at_the_boundary_goes_through() {
	run_test(|mut test_handler| async move {
		...
		let honest_data = vec![selendra_data_from_blocks(honest_hopeless_fork)];
		let honest_data2 = vec![selendra_data_from_blocks(honest_hopeless_fork2)];
		let malicious_data = vec![selendra_data_from_blocks(malicious_hopeless_fork)];
		let malicious_data2 = vec![selendra_data_from_blocks(malicious_hopeless_fork2)];
