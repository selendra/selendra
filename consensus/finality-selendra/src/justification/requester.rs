	...
	CS: ChainStatus<J>,
{
	pub fn new(block_requester: RB, chain_status: CS, boundary_info: SessionBoundaryInfo) -> Self {
		Requester { block_requester, chain_status, boundary_info, _phantom: PhantomData }
	}
	...
	fn children_ids(
		...
		Ok(self.chain_status.children(id)?.into_iter().map(|header| header.id()).collect())
	}
	...
	fn perform_request(&self) -> Result<(), CS::Error> {
		...
		let session_id = self.boundary_info.session_id_from_block_num(top_finalized.number());
