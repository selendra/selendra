	...
	BB: BlockchainBackend<B> + 'static,
{
	...
	pub async fn run(
		...
		loop {
			...
			let SessionInfo { verifier, last_block_height: stop_h, current_session } = self
				.session_info_provider
				.for_block_num(last_finalized_number + 1u32.into())
				.await;
			if verifier.is_none() {
				debug!(target: "selendra-justification", "Verifier for session {:?} not yet available. Waiting {}ms and will try again ...", current_session, self.verifier_timeout.as_millis());
...
fn wrap_channel_with_logging<B: BlockT>(
	...
	channel
		.inspect(move |_| {
			debug!(target: "selendra-justification", "Got justification ({})", label);
		})
		.chain(futures::stream::iter(std::iter::from_fn(move || {
			error!(target: "selendra-justification", "Justification ({}) stream ended.", label);
			None
		})))
}
