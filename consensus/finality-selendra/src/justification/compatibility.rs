...
use crate::{
	...
	justification::{SelendraJustification, LOG_TARGET},
	Version,
};
...
struct SelendraJustificationV1 {
	pub signature: SignatureSet<SignatureV1>,
}

impl From<SelendraJustificationV1> for SelendraJustification {
	fn from(justification: SelendraJustificationV1) -> SelendraJustification {
		...
		SelendraJustification::CommitteeMultisignature(just_drop_id)
	}
}
...
#[derive(Clone, Encode, Decode, Debug, PartialEq, Eq)]
struct SelendraJustificationV2 {
	pub signature: SignatureSet<Signature>,
}

impl From<SelendraJustificationV2> for SelendraJustification {
	fn from(justification: SelendraJustificationV2) -> SelendraJustification {
		SelendraJustification::CommitteeMultisignature(justification.signature)
	}
}

#[derive(Clone, Debug, PartialEq, Eq)]
enum VersionedSelendraJustification {
	// Most likely from the future.
	Other(Version, Vec<u8>),
	V1(SelendraJustificationV1),
	V2(SelendraJustificationV2),
	V3(SelendraJustification),
}
...
impl Encode for VersionedSelendraJustification {
	fn size_hint(&self) -> usize {
		use VersionedSelendraJustification::*;
...
impl Decode for VersionedSelendraJustification {
	fn decode<I: CodecInput>(input: &mut I) -> Result<Self, CodecError> {
		...
		match version {
			Version(1) => Ok(V1(SelendraJustificationV1::decode(input)?)),
			Version(2) => Ok(V2(SelendraJustificationV2::decode(input)?)),
			Version(3) => Ok(V3(SelendraJustification::decode(input)?)),
			_ => {
...
impl Display for Error {
	fn fmt(&self, f: &mut Formatter<'_>) -> Result<(), FmtError> {
		...
		match self {
			...
			UnknownVersion(version) => {
				write!(f, "justification encoded with unknown version {}", version.0)
			},
		}
	}
}
...
fn decode_pre_compatibility_justification(
...
) -> Result<SelendraJustification, Error> {
	use Error::*;
	...
	match SelendraJustificationV2::decode_all(&mut justification_cloned.as_slice()) {
		Ok(justification) => Ok(justification.into()),
		Err(_) => match SelendraJustificationV1::decode_all(&mut justification_raw.as_slice()) {
			Ok(justification) => Ok(justification.into()),
			Err(_) => Err(BadFormat),
		},
	}
...
pub fn backwards_compatible_decode(
	...
	use Error::*;
	let justification_cloned = justification_raw.clone();
	match VersionedSelendraJustification::decode_all(&mut justification_cloned.as_slice()) {
		Ok(justification) => {
			use VersionedSelendraJustification::*;
			match justification {
				...
				Other(version, _) => {
					...
					// which is in VersionedSelendraJustification byte_count_size, can be small enough
...
pub fn versioned_encode(justification: SelendraJustification) -> Vec<u8> {
	VersionedSelendraJustification::V3(justification).encode()
}
...
mod test {
	...
	fn correctly_decodes_v1() {
		...
		for i in 0..7 {
			...
			let signature_v1 = SignatureV1 {
				_id: id,
				sgn: AuthorityPair::generate().0.sign(vec![0u8, 0u8, 0u8, 0u8].as_slice()),
			};
			signature_set = signature_set.add_signature(&signature_v1, id);
		}

		let just_v1 = SelendraJustificationV1 { signature: signature_set };
		let encoded_just: Vec<u8> = just_v1.encode();
		let decoded = backwards_compatible_decode(encoded_just);
		let just_v1: SelendraJustification = just_v1.into();
	...
	fn correctly_decodes_v2() {
		...
		for i in 0..7 {
			let authority_signature: AuthoritySignature =
				AuthorityPair::generate().0.sign(vec![0u8, 0u8, 0u8, 0u8].as_slice());
			signature_set = signature_set.add_signature(&authority_signature.into(), i.into());
		}

		let just_v2 = SelendraJustificationV2 { signature: signature_set };
		let encoded_just: Vec<u8> = just_v2.encode();
		let decoded = backwards_compatible_decode(encoded_just);
		let just_v2: SelendraJustification = just_v2.into();
	...
	fn correctly_decodes_v3_committee() {
		...
		let just_v3 = SelendraJustification::CommitteeMultisignature(signature_set);
	...
	fn correctly_decodes_other() {
		let other = VersionedSelendraJustification::Other(Version(43), vec![21, 37]);
		let encoded = other.encode();
		let decoded = VersionedSelendraJustification::decode(&mut encoded.as_slice());
	...
	fn assert_backwards_compatible_decodes_pre_compatibility_justification(
		...
		match backwards_compatible_decode(raw_justification_legacy_pre_compatibility) {
			Ok(SelendraJustification::CommitteeMultisignature(signature)) => {
				assert_eq!(signature.size(), NodeCount(expected_node_count))
			},
			Ok(SelendraJustification::EmergencySignature(_)) => {
