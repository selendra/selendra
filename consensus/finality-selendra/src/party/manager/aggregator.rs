...
{
	trace!(target: "selendra-party", "Received unit {:?} in aggregator.", block);
	...
	JT: JustificationTranslator<B::Header> + Send + Sync + Clone,
{
	...
	let justification = match justification_translator.translate(
		SelendraJustification::CommitteeMultisignature(multisignature),
		BlockId::new(hash, number),
	) {
		Ok(justification) => justification,
		Err(e) => {
			error!(target: "selendra-party", "Issue with translating justification from Aggregator to Sync Justification: {}.", e);
			return Err(())
		},
	};
	if let Err(e) = justifications_for_chain.submit(justification) {
		error!(target: "selendra-party", "Issue with sending justification from Aggregator to JustificationHandler {}.", e);
		return Err(())
	}
	Ok(())
}
	...
	<B as Block>::Hash: AsRef<[u8]>,
{
	...
	let blocks_from_interpreter = blocks_from_interpreter.take_while(|block| {
		...
		async move {
			if block_num == session_boundaries.last_block() {
				debug!(target: "selendra-party", "Aggregator is processing last block in session.");
			}
	...
	loop {
		trace!(target: "selendra-party", "Aggregator Loop started a next iteration");
		tokio::select! {
			maybe_block = blocks_from_interpreter.next() => {
				if let Some(block) = maybe_block {
					...
					debug!(target: "selendra-party", "Blocks ended in aggregator.");
					no_more_blocks = true;
				}
			}
			multisigned_hash = aggregator.next_multisigned_hash() => {
				if let Some((hash, multisignature)) = multisigned_hash {
					...
					debug!(target: "selendra-party", "The stream of multisigned hashes has ended. Terminating.");
					break;
				}
			}
			...
			_ = &mut exit_rx => {
				debug!(target: "selendra-party", "Aggregator received exit signal. Terminating.");
				break;
			}
		}
		if hash_of_last_block.is_none() && no_more_blocks {
			debug!(target: "selendra-party", "Aggregator processed all provided blocks. Terminating.");
			break
		}
	}
	debug!(target: "selendra-party", "Aggregator finished its work.");
	Ok(())
}
	...
	CN: Network<CurrentRmcNetworkData<B>> + 'static,
{
	...
	let task = {
		async move {
			...
			debug!(target: "selendra-party", "Running the aggregator task for {:?}", session_id);
			let result =
				run_aggregator(aggregator_io, io, client, &session_boundaries, metrics, exit).await;
			debug!(target: "selendra-party", "Aggregator task stopped for {:?}", session_id);
			result
		}
	};

	let handle =
		spawn_handle.spawn_essential_with_result("selendra/consensus_session_aggregator", task);
