...
#[derive(PartialEq, Eq, Clone, Debug)]
pub struct PendingMessageInfo<B: BlockT, M: SelendraNetworkMessage<B>> {
	message: M,
	// Data items that we still wait for
	pending_proposals: HashSet<SelendraProposal<B>>,
}

impl<B: BlockT, M: SelendraNetworkMessage<B>> PendingMessageInfo<B, M> {
	fn new(message: M) -> Self {
		PendingMessageInfo { message, pending_proposals: HashSet::new() }
	}
}
...
// DataStore is the data availability proxy for the SelendraBFT protocol, meaning that whenever we receive
// a message `m` we must check whether the data `m.included_data()` is available to pass it to SelendraBFT.
// Data is represented by the `SelendraData<B>` type -- we refer to the docs of this type to learn what
// it represents and how honest nodes form `SelendraData<B>` instances.
// An `SelendraData<B>` is considered available if it is either `Empty` or it is `HeadProposal(p)` where
...
/// This component is used for filtering available data for Selendra Network.
	...
	R: Receiver<Message>,
{
	...
	pending_proposals: HashMap<SelendraProposal<B>, PendingProposalInfo<B>>,
	event_triggers: HashMap<ChainEvent<B>, HashSet<SelendraProposal<B>>>,
	...
	messages_for_selendra: UnboundedSender<Message>,
}
	...
	R: Receiver<Message>,
{
	...
	pub fn new<N: ComponentNetwork<Message, R = R>>(
		...
		let (messages_for_selendra, messages_from_data_store) = mpsc::unbounded();
		...
		loop {
			...
			tokio::select! {
				Some(message) = self.messages_from_network.next() => {
					trace!(target: "selendra-data-store", "Received message at Data Store {:?}", message);
					self.on_message_received(message);
				}
				Some(block) = &mut import_stream.next() => {
					trace!(target: "selendra-data-store", "Block import notification at Data Store for block {:?}", block);
					self.on_block_imported((block.header.hash(), *block.header.number()).into());
				},
				Some(block) = &mut finality_stream.next() => {
					trace!(target: "selendra-data-store", "Finalized block import notification at Data Store for block {:?}", block);
				...
				_ = &mut exit => {
					debug!(target: "selendra-data-store", "Data Store task received exit signal. Terminating.");
					break;
				}
			}
		}
	...
	fn run_maintenance(&mut self) {
		...
		match proposals_with_timestamps.len() {
			0 => {
				trace!(target: "selendra-data-store", "No pending proposals in data store during maintenance.");
			},
			1..=5 => {
				info!(target: "selendra-data-store", "Data Store maintenance. Awaiting {:?} proposals: {:?}",proposals_with_timestamps.len(), proposals_with_timestamps);
			},
			_ => {
				info!(target: "selendra-data-store", "Data Store maintenance. Awaiting {:?} proposals: (showing 5 initial only) {:?}",proposals_with_timestamps.len(), &proposals_with_timestamps[..5]);
			},
		}
		...
		for (proposal, first_occurrence) in proposals_with_timestamps {
			...
			let parent_hash = match self.chain_info_provider.get_parent_hash(&bottom_block) {
				Ok(ph) => ph,
				_ => {
					warn!(target: "selendra-data-store", "Expected the block below the proposal {:?} to be imported", proposal);
					continue
				},
			};
			...
			if let Ok(finalized_block) = self.chain_info_provider.get_finalized_at(parent_num) {
				if parent_hash != finalized_block.hash {
					...
					warn!(target: "selendra-data-store", "The proposal {:?} is pending even though blocks \
	...
	fn register_next_finality_trigger(&mut self, proposal: &SelendraProposal<B>) {
	...
	fn on_proposal_available(&mut self, proposal: &SelendraProposal<B>) {
	...
	fn bump_proposal(&mut self, proposal: &SelendraProposal<B>) -> bool {
	...
	fn check_proposal_availability(
		...
		match status {
			ProposalStatus::Finalize(_) | ProposalStatus::Ignore => {
				...
				self.available_proposals_cache.put(proposal.clone(), status.clone());
			},
			_ => {},
		}
		status
	...
	fn add_message_proposal_dependency(
		...
		let proposal_info =
			self.pending_proposals.get_mut(proposal).expect("exists as checked above");
	...
	fn on_message_dependencies_resolved(&self, message: Message) {
		trace!(target: "selendra-data-store", "Sending message from DataStore {:?}", message);
		if let Err(e) = self.messages_for_selendra.unbounded_send(message) {
			error!(target: "selendra-data-store", "Unable to send a ready message from DataStore {}", e);
		}
	}
	...
	fn remove_proposal_from_pending_message(
		...
		let mut message_info = match self.pending_messages.remove(&id) {
			...
			None => {
				warn!(target: "selendra-data-store", "Message {:?} not found when resolving a proposal dependency {:?}.", id, proposal);
				return
			},
		};
	...
	fn remove_message_id_from_pending_proposal(
		...
		if let Occupied(mut proposal_entry) = self.pending_proposals.entry(proposal.clone()) {
			...
			warn!(target: "selendra-data-store", "Proposal {:?} with id {:?} referenced in message does not exist", proposal, id);
		}
	}
	...
	fn prune_single_message(&mut self) -> bool {
		...
		if let Some(id) = maybe_id {
			if let Some(message_info) = self.pending_messages.remove(&id) {
				...
				warn!("Trying to prune a message whose id is not in pending messages {:?}", id);
				false
			}
		} else {
			warn!(target: "selendra-data-store", "Tried to prune a message but there are none pending.");
			false
		}
	}
	...
	fn prune_pending_messages(&mut self) {
			...
			self.pending_proposals.len() > self.config.max_proposals_pending
		{
			if !self.prune_single_message() {
				warn!(target: "selendra-data-store", "Message pruning in DataStore failed. Moving on.");
				break
			}
		}
	}
	...
	fn on_message_received(&mut self, message: Message) {
		...
		for data in message.included_data() {
			...
			match unvalidated_proposal.validate_bounds(&self.session_boundaries) {
				...
				Err(error) => {
					warn!(target: "selendra-data-store", "Message {:?} dropped as it contains \
