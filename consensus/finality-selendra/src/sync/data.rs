...
use log::warn;
use selendra_primitives::MAX_BLOCK_SIZE;
...
impl<J: Justification> Request<J> {
	pub fn new(
		...
		Self { target_id, branch_knowledge, state }
	}
}
	...
	GossipNetwork<NetworkData<B, J>> for VersionWrapper<B, J, N>
{
	...
	fn send_to(
		...
		self.inner
			.send_to(VersionedNetworkData::V1(data.clone().into()), peer_id.clone())?;
	...
	fn send_to_random(
		...
		self.inner
			.send_to_random(VersionedNetworkData::V1(data.clone().into()), peer_ids.clone())?;
		self.inner.send_to_random(VersionedNetworkData::V2(data), peer_ids)
	}

	fn broadcast(&mut self, data: NetworkData<B, J>) -> Result<(), Self::Error> {
		self.inner.broadcast(VersionedNetworkData::V1(data.clone().into()))?;
