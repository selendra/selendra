...
enum InnerVertex<J: Justification> {
	...
	Header { importance: HeaderImportance, parent: BlockIdFor<J> },
	/// Vertex with added Header and Justification.
	Justification { imported: bool, justification: J, parent: BlockIdFor<J> },
}
...
impl<I: PeerId, J: Justification> Vertex<I, J> {
	...
	pub fn new() -> Self {
		Vertex { inner: InnerVertex::Empty { required: false }, know_most: HashSet::new() }
	}
	...
	pub fn required(&self) -> bool {
		...
		matches!(
			self.inner,
			Empty { required: true } |
				Header { importance: HeaderImportance::Required, .. } |
	...
	pub fn set_required(&mut self) -> bool {
		...
		match &self.inner {
			...
			Header { importance: HeaderImportance::Auxiliary, parent } => {
				self.inner =
	...
	pub fn insert_body(&mut self, parent: BlockIdFor<J>) -> bool {
		...
		match &self.inner {
			Empty { .. } | Header { .. } => {
				self.inner = Header { parent, importance: HeaderImportance::Imported };
				false
			},
	...
	pub fn insert_justification(
		...
		match self.inner {
			...
			Empty { required: true } | Header { importance: HeaderImportance::Required, .. } => {
				self.inner = Justification { imported: false, parent, justification };
				self.know_most = holder.into_iter().collect();
				JustificationAddResult::Noop
			},
			Empty { required: false } | Header { importance: HeaderImportance::Auxiliary, .. } => {
				self.inner = Justification { imported: false, parent, justification };
				self.know_most = holder.into_iter().collect();
