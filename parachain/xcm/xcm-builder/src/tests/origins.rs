...
fn universal_origin_should_work() {
	...
	// Parachain 1 may represent Testnet to us
	add_universal_alias(Parachain(1), Testnet);
	// Parachain 2 may represent Selendra to us
	add_universal_alias(Parachain(2), Selendra);

	let message = Xcm(vec![
		UniversalOrigin(GlobalConsensus(Testnet)),
	...
	add_asset((Ancestor(2), GlobalConsensus(Testnet)), (Parent, 100));
	let message = Xcm(vec![
		UniversalOrigin(GlobalConsensus(Testnet)),
		TransferAsset { assets: (Parent, 100u128).into(), beneficiary: Here.into() },
	]);
	...
	assert_eq!(asset_list((Ancestor(2), GlobalConsensus(Testnet))), vec![]);
}
...
fn export_message_should_work() {
	...
	// Local parachain #1 issues a transfer asset on Selendra Relay-chain, transfering 100 Planck to
	// Selendra parachain #2.
	...
	assert_eq!(
		exported_xcm(),
		vec![(Selendra, 403611790, uni_src, Here, expected_message, expected_hash)]
	);
}
