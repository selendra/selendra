...
mod tests {
	...
	fn ensure_is_remote_works() {
		// A Testnet parachain is remote from the Selendra Relay.
		let x = ensure_is_remote(Selendra, (Parent, Testnet, Parachain(1000)));
		assert_eq!(x, Ok((Testnet, Parachain(1000).into())));

		// Selendra Relay is remote from a Testnet parachain.
		let x = ensure_is_remote((Testnet, Parachain(1000)), (Parent, Parent, Selendra));
		assert_eq!(x, Ok((Selendra, Here)));

		// Our own parachain is local.
		let x = ensure_is_remote(Selendra, Parachain(1000));
		assert_eq!(x, Err(Parachain(1000).into()));

		// Selendra's parachain is not remote if we are Selendra.
		let x = ensure_is_remote(Selendra, (Parent, Selendra, Parachain(1000)));
		assert_eq!(x, Err((Parent, Selendra, Parachain(1000)).into()));

		// If we don't have a consensus ancestor, then we cannot determine remoteness.
		let x = ensure_is_remote((), (Parent, Selendra, Parachain(1000)));
		assert_eq!(x, Err((Parent, Selendra, Parachain(1000)).into()));
	}
}
