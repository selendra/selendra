...
/// Selendra ecosystem are given preference over explicit naming in this enumeration.
#[derive(
	Copy,
	Clone,
	Eq,
...
pub enum NetworkId {
	...
	/// The Selendra mainnet Relay-chain.
	Selendra,
...
impl From<OldNetworkId> for Option<NetworkId> {
	fn from(old: OldNetworkId) -> Option<NetworkId> {
		...
		match old {
			Any => None,
			Named(_) => None,
			Selendra => Some(NetworkId::Selendra),
...
impl TryFrom<OldNetworkId> for NetworkId {
	...
	fn try_from(old: OldNetworkId) -> Result<Self, Self::Error> {
		...
		match old {
			Any | Named(_) => Err(()),
			Selendra => Ok(NetworkId::Selendra),
...
pub enum BodyId {
	...
	Judicial,
	/// The unambiguous defense body (for Selendra, an opinion on the topic given via a public referendum
	/// on the `staking_admin` track).
	Defense,
	/// The unambiguous administration body (for Selendra, an opinion on the topic given via a public referendum
	/// on the `general_admin` track).
	Administration,
	/// The unambiguous treasury body (for Selendra, an opinion on the topic given via a public referendum
