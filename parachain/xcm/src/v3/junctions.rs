...
mod tests {
	...
	fn relative_to_works() {
		use Junctions::*;
		use NetworkId::*;
		assert_eq!(X1(Selendra.into()).relative_to(&X1(Testnet.into())), (Parent, Selendra).into());
		let base = X3(Testnet.into(), Parachain(1), PalletInstance(1));

		// Ancestors.
		assert_eq!(Here.relative_to(&base), (Parent, Parent, Parent).into());
		assert_eq!(X1(Testnet.into()).relative_to(&base), (Parent, Parent).into());
		assert_eq!(X2(Testnet.into(), Parachain(1)).relative_to(&base), (Parent,).into());
		assert_eq!(
			X3(Testnet.into(), Parachain(1), PalletInstance(1)).relative_to(&base),
			Here.into()
		);

		// Ancestors with one child.
		assert_eq!(
			X1(Selendra.into()).relative_to(&base),
			(Parent, Parent, Parent, Selendra).into()
		);
		assert_eq!(
			X2(Testnet.into(), Parachain(2)).relative_to(&base),
			(Parent, Parent, Parachain(2)).into()
		);
		assert_eq!(
			X3(Testnet.into(), Parachain(1), PalletInstance(2)).relative_to(&base),
			(Parent, PalletInstance(2)).into()
		);
		assert_eq!(
			X4(Testnet.into(), Parachain(1), PalletInstance(1), [1u8; 32].into())
				.relative_to(&base),
			([1u8; 32],).into()
		);
		...
		assert_eq!(
			X2(Selendra.into(), Parachain(1)).relative_to(&base),
			(Parent, Parent, Parent, Selendra, Parachain(1)).into()
		);
		assert_eq!(
			X3(Testnet.into(), Parachain(2), PalletInstance(1)).relative_to(&base),
			(Parent, Parent, Parachain(2), PalletInstance(1)).into()
		);
		assert_eq!(
			X4(Testnet.into(), Parachain(1), PalletInstance(2), [1u8; 32].into())
				.relative_to(&base),
			(Parent, PalletInstance(2), [1u8; 32]).into()
		);
		assert_eq!(
			X5(Testnet.into(), Parachain(1), PalletInstance(1), [1u8; 32].into(), 1u128.into())
				.relative_to(&base),
			([1u8; 32], 1u128).into()
		);
	}
	...
	fn global_consensus_works() {
		use Junctions::*;
		use NetworkId::*;
		assert_eq!(X1(Selendra.into()).global_consensus(), Ok(Selendra));
		assert_eq!(X2(Testnet.into(), 1u64.into()).global_consensus(), Ok(Testnet));
		assert_eq!(Here.global_consensus(), Err(()));
		assert_eq!(X1(1u64.into()).global_consensus(), Err(()));
		assert_eq!(X2(1u64.into(), Testnet.into()).global_consensus(), Err(()));
	}

	#[test]
	fn test_conversion() {
		use super::{Junction::*, Junctions::*, NetworkId::*};
		let x: Junctions = GlobalConsensus(Selendra).into();
		assert_eq!(x, X1(GlobalConsensus(Selendra)));
		let x: Junctions = Selendra.into();
		assert_eq!(x, X1(GlobalConsensus(Selendra)));
		let x: Junctions = (Selendra, Testnet).into();
		assert_eq!(x, X2(GlobalConsensus(Selendra), GlobalConsensus(Testnet)));
	}
}
