...
async fn collating_using_adder_collator() {
	use selendra_primitives::Id as ParaId;
	...
	let alice_config = selendra_test_service::node_config(
		|| {},
	...
	// start alice
	let alice = selendra_test_service::run_validator_node(alice_config, Some(PUPPET_EXE.into()));

	let bob_config = selendra_test_service::node_config(
		|| {},
		tokio::runtime::Handle::current(),
		Bob,
	...
	// start bob
	let bob = selendra_test_service::run_validator_node(bob_config, Some(PUPPET_EXE.into()));
	...
	let mut charlie = selendra_test_service::run_collator_node(
		tokio::runtime::Handle::current(),
		Charlie,
		|| {},
