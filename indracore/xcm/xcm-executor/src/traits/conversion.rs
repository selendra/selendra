...
/// // A convertor that will bump the indra id and pass it to the next one.
/// struct BumpIndraId;
/// impl ConvertOrigin<u32> for BumpIndraId {
/// 	fn convert_origin(origin: impl Into<MultiLocation>, _: OriginKind) -> Result<u32, MultiLocation> {
/// 		match origin.into() {
/// 			MultiLocation { parents: 0, interior: Junctions::X1(Junction::Indracore(id)) } => {
/// 				Err(Junctions::X1(Junction::Indracore(id + 1)).into())
/// 			}
/// 			_ => unreachable!()
/// 		}
/// 	}
...
///
/// struct AcceptIndra7;
/// impl ConvertOrigin<u32> for AcceptIndra7 {
/// 	fn convert_origin(origin: impl Into<MultiLocation>, _: OriginKind) -> Result<u32, MultiLocation> {
/// 		match origin.into() {
/// 			MultiLocation { parents: 0, interior: Junctions::X1(Junction::Indracore(id)) } if id == 7 => {
/// 				Ok(7)
/// 			}
/// 			o => Err(o)
/// 		}
...
/// }
/// # fn main() {
/// let origin: MultiLocation = Junctions::X1(Junction::Indracore(6)).into();
/// assert!(
/// 	<(BumpIndraId, AcceptIndra7) as ConvertOrigin<u32>>::convert_origin(origin, OriginKind::Native)
/// 		.is_ok()
/// );
/// # }
/// ```
