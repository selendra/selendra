...
use selendra_indracore::primitives::IsSystem;
...
pub struct ChildSystemIndracoreAsSuperuser<IndraId, Origin>(PhantomData<(IndraId, Origin)>);
impl<IndraId: IsSystem + From<u32>, Origin: OriginTrait> ConvertOrigin<Origin>
	for ChildSystemIndracoreAsSuperuser<IndraId, Origin>
{
	fn convert_origin(
		origin: impl Into<MultiLocation>,
		kind: OriginKind,
		...
		let origin = origin.into();
		log::trace!(target: "xcm::origin_conversion", "ChildSystemIndracoreAsSuperuser origin: {:?}, kind: {:?}", origin, kind);
		match (kind, origin) {
			(
				OriginKind::Superuser,
				MultiLocation { parents: 0, interior: X1(Junction::Indracore(id)) },
			) if IndraId::from(id).is_system() => Ok(Origin::root()),
			(_, origin) => Err(origin),
		}
	}
}

pub struct SiblingSystemIndracoreAsSuperuser<IndraId, Origin>(PhantomData<(IndraId, Origin)>);
impl<IndraId: IsSystem + From<u32>, Origin: OriginTrait> ConvertOrigin<Origin>
	for SiblingSystemIndracoreAsSuperuser<IndraId, Origin>
{
	fn convert_origin(
		origin: impl Into<MultiLocation>,
		kind: OriginKind,
		...
		log::trace!(
			target: "xcm::origin_conversion",
			"SiblingSystemIndracoreAsSuperuser origin: {:?}, kind: {:?}",
			origin, kind,
		);
		match (kind, origin) {
			(
				OriginKind::Superuser,
				MultiLocation { parents: 1, interior: X1(Junction::Indracore(id)) },
			) if IndraId::from(id).is_system() => Ok(Origin::root()),
			(_, origin) => Err(origin),
		}
	}
...
pub struct ChildIndracoreAsNative<IndracoreOrigin, Origin>(PhantomData<(IndracoreOrigin, Origin)>);
impl<IndracoreOrigin: From<u32>, Origin: From<IndracoreOrigin>> ConvertOrigin<Origin>
	for ChildIndracoreAsNative<IndracoreOrigin, Origin>
{
	fn convert_origin(
		origin: impl Into<MultiLocation>,
		kind: OriginKind,
		...
		let origin = origin.into();
		log::trace!(target: "xcm::origin_conversion", "ChildIndracoreAsNative origin: {:?}, kind: {:?}", origin, kind);
		match (kind, origin) {
			(
				OriginKind::Native,
				MultiLocation { parents: 0, interior: X1(Junction::Indracore(id)) },
			) => Ok(Origin::from(IndracoreOrigin::from(id))),
			(_, origin) => Err(origin),
		}
	}
}

pub struct SiblingIndracoreAsNative<IndracoreOrigin, Origin>(
	PhantomData<(IndracoreOrigin, Origin)>,
);
impl<IndracoreOrigin: From<u32>, Origin: From<IndracoreOrigin>> ConvertOrigin<Origin>
	for SiblingIndracoreAsNative<IndracoreOrigin, Origin>
{
	fn convert_origin(
		origin: impl Into<MultiLocation>,
		kind: OriginKind,
		...
		log::trace!(
			target: "xcm::origin_conversion",
			"SiblingIndracoreAsNative origin: {:?}, kind: {:?}",
			origin, kind,
		);
		match (kind, origin) {
			(
