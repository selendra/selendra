...
pub struct ChildIndracoreConvertsVia<IndraId, AccountId>(PhantomData<(IndraId, AccountId)>);
impl<IndraId: From<u32> + Into<u32> + AccountIdConversion<AccountId>, AccountId: Clone>
	Convert<MultiLocation, AccountId> for ChildIndracoreConvertsVia<IndraId, AccountId>
{
	fn convert_ref(location: impl Borrow<MultiLocation>) -> Result<AccountId, ()> {
		match location.borrow() {
			MultiLocation { parents: 0, interior: X1(Indracore(id)) } =>
				Ok(IndraId::from(*id).into_account_truncating()),
			_ => Err(()),
		}
	}

	fn reverse_ref(who: impl Borrow<AccountId>) -> Result<MultiLocation, ()> {
		if let Some(id) = IndraId::try_from_account(who.borrow()) {
			Ok(Indracore(id.into()).into())
		} else {
			Err(())
		}
	}
...
pub struct SiblingIndracoreConvertsVia<IndraId, AccountId>(PhantomData<(IndraId, AccountId)>);
impl<IndraId: From<u32> + Into<u32> + AccountIdConversion<AccountId>, AccountId: Clone>
	Convert<MultiLocation, AccountId> for SiblingIndracoreConvertsVia<IndraId, AccountId>
{
	fn convert_ref(location: impl Borrow<MultiLocation>) -> Result<AccountId, ()> {
		match location.borrow() {
			MultiLocation { parents: 1, interior: X1(Indracore(id)) } =>
				Ok(IndraId::from(*id).into_account_truncating()),
			_ => Err(()),
		}
	}
	...
	fn reverse_ref(who: impl Borrow<AccountId>) -> Result<MultiLocation, ()> {
		if let Some(id) = IndraId::try_from_account(who.borrow()) {
			Ok(MultiLocation::new(1, X1(Indracore(id.into()))))
		} else {
			Err(())
		}
	}
...
/// }
///
/// let input = MultiLocation::new(2, X2(Indracore(2), AccountId32 { network: Any, id: Default::default() }));
/// let inverted = LocationInverter::<Ancestry>::invert_location(&input);
/// assert_eq!(inverted, Ok(MultiLocation::new(
///     2,
///     X2(Indracore(1), AccountKey20 { network: Any, key: Default::default() }),
/// )));
/// # }
/// ```
...
mod tests {
	...
	// Network Topology
	//                                     v Source
	// Relay -> Indra 1 -> SmartContract -> Account
	//       -> Indra 2 -> Account
	//                    ^ Target
	//
	// Inputs and outputs written as file paths:
	//
	// input location (source to target): ../../../indra_2/account32_default
	// ancestry (root to source): indra_1/account20_default/account20_default
	// =>
	// output (target to source): ../../indra_1/account20_default/account20_default
	#[test]
	fn inverter_works_in_tree() {
		parameter_types! {
			pub Ancestry: MultiLocation = X3(Indracore(1), account20(), account20()).into();
		}

		let input = MultiLocation::new(3, X2(Indracore(2), account32()));
		let inverted = LocationInverter::<Ancestry>::invert_location(&input).unwrap();
		assert_eq!(inverted, MultiLocation::new(2, X3(Indracore(1), account20(), account20())));
	}
	...
	// Network Topology
	//                                        v Source
	// Relay -> Indra 1 -> CollectivePallet -> Plurality
	//          ^ Target
	#[test]
	...
	fn inverter_errors_when_location_is_too_large() {
		...
		let input = MultiLocation { parents: 99, interior: X1(Indracore(88)) };
		let inverted = LocationInverter::<Ancestry>::invert_location(&input);
