...
pub enum TestCall {
	...
	OnlyIndracore(Weight, Option<Weight>, Option<u32>),
...
impl Dispatchable for TestCall {
	...
	fn dispatch(self, origin: Self::Origin) -> DispatchResultWithPostInfo {
		...
		post_info.actual_weight = match self {
			...
			TestCall::OnlyIndracore(_, maybe_actual, _) |
			TestCall::Any(_, maybe_actual) => maybe_actual,
		};
		if match (&origin, &self) {
			(TestOrigin::Indracore(i), TestCall::OnlyIndracore(_, _, Some(j))) => i == j,
			(TestOrigin::Signed(i), TestCall::OnlySigned(_, _, Some(j))) => i == j,
			(TestOrigin::Root, TestCall::OnlyRoot(..)) |
			(TestOrigin::Indracore(_), TestCall::OnlyIndracore(_, _, None)) |
...
impl GetDispatchInfo for TestCall {
	fn get_dispatch_info(&self) -> DispatchInfo {
		let weight = *match self {
			...
			TestCall::OnlyIndracore(estimate, ..) |
...
pub fn to_account(l: MultiLocation) -> Result<u64, MultiLocation> {
	Ok(match l {
		// Siblings at 2000+id
		MultiLocation { parents: 1, interior: X1(Indracore(id)) } => 2000 + id as u64,
		// Accounts are their number
		MultiLocation { parents: 0, interior: X1(AccountIndex64 { index, .. }) } => index,
		// Children at 1000+id
		MultiLocation { parents: 0, interior: X1(Indracore(id)) } => 1000 + id as u64,
		// Self at 3000
...
impl ConvertOrigin<TestOrigin> for TestOriginConverter {
	fn convert_origin(
		...
		match (kind, origin.into()) {
			...
			(Native, MultiLocation { parents: 0, interior: X1(Indracore(id)) }) =>
				Ok(TestOrigin::Indracore(id)),
...
parameter_types! {
	pub TestAncestry: MultiLocation = X1(Indracore(42)).into();
