...
fn basic_setup_works() {
	...
	assert_eq!(to_account(X1(Indracore(1)).into()), Ok(1001));
	assert_eq!(to_account(X1(Indracore(50)).into()), Ok(1050));
	assert_eq!(to_account(MultiLocation::new(1, X1(Indracore(1)))), Ok(2001));
	assert_eq!(to_account(MultiLocation::new(1, X1(Indracore(50)))), Ok(2050));
	assert_eq!(
...
fn transfer_should_work() {
	...
	AllowUnpaidFrom::set(vec![X1(Indracore(1)).into()]);
	// Child indracore #1 owns 1000 tokens held by us in reserve.
	add_asset(1001, (Here, 1000));
	// They want to transfer 100 of them to their sibling indracore #2
...
fn basic_asset_trap_should_work() {
	// we'll let them have message execution for free.
	AllowUnpaidFrom::set(vec![X1(Indracore(1)).into(), X1(Indracore(2)).into()]);

	// Child indracore #1 owns 1000 tokens held by us in reserve.
	add_asset(1001, (Here, 1000));
	// They want to transfer 100 of them to their sibling indracore #2 but have a problem
	let r = XcmExecutor::<TestConfig>::execute_xcm(
		Indracore(1).into(),
		Xcm(vec![
...
fn reserve_transfer_should_work() {
	...
	// They want to transfer 100 of our native asset from sovereign account of indracore #1 into #2
	// and let them know to hand it to account #3.
	let r = XcmExecutor::<TestConfig>::execute_xcm(
		Indracore(1),
		Xcm(vec![TransferReserveAsset {
			assets: (Here, 100).into(),
			dest: Indracore(2).into(),
...
fn simple_version_subscriptions_should_work() {
	...
	let origin = Indracore(1000).into();
...
fn version_subscription_instruction_should_work() {
	...
	assert_eq!(SubscriptionRequests::get(), vec![(Indracore(1000).into(), Some((42, 5000)))]);
}
...
fn version_unsubscription_instruction_should_work() {
	...
	assert_eq!(SubscriptionRequests::get(), vec![(Indracore(1000).into(), None)]);
	assert_eq!(sent_xcm(), vec![]);
}
...
fn weight_trader_tuple_should_work() {
	pub const INDRA_1: MultiLocation = X1(Indracore(1)).into();
	pub const INDRA_2: MultiLocation = X1(Indracore(2)).into();

	parameter_types! {
		pub static HereWeightPrice: (AssetId, u128) = (Here.into().into(), WEIGHT_PER_SECOND.into());
		pub static INDRA1WeightPrice: (AssetId, u128) = (INDRA_1.into(), WEIGHT_PER_SECOND.into());
	}
	...
	type Traders = (
		...
		// trader two
		FixedRateOfFungible<INDRA1WeightPrice, ()>,
	);
	...
	assert_eq!(
		traders.buy_weight(5, fungible_multi_asset(INDRA_1, 10).into()),
		Ok(fungible_multi_asset(INDRA_1, 5).into()),
	);
	// trader two refunds
	assert_eq!(traders.refund_weight(2), Some(fungible_multi_asset(INDRA_1, 2)));

	let mut traders = Traders::new();
	// all traders fails
	assert_err!(
		traders.buy_weight(5, fungible_multi_asset(INDRA_2, 10).into()),
