...
/// Allows a message only if it is from a system-level child indracore.
pub struct IsChildSystemIndracore<IndraId>(PhantomData<IndraId>);
impl<IndraId: IsSystem + From<u32>> Contains<MultiLocation> for IsChildSystemIndracore<IndraId> {
	fn contains(l: &MultiLocation) -> bool {
		matches!(
			l.interior(),
			Junctions::X1(Junction::Indracore(id))
				if IndraId::from(*id).is_system() && l.parent_count() == 0,
		)
	}
}
