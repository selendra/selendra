...
use mock::{
	selendra_like_with_balances, AccountId, Balance, Balances, BaseXcmWeight, System, XcmConfig,
	CENTS,
};
...
/// Scenario:
/// A indracore transfers funds on the relaychain to another indracore's account.
///
/// Asserts that the indracore accounts are updated as expected.
#[test]
fn withdraw_and_deposit_works() {
	let indra_acc: AccountId = IndraId::from(INDRA_ID).into_account_truncating();
	let balances = vec![(ALICE, INITIAL_BALANCE), (indra_acc.clone(), INITIAL_BALANCE)];
	selendra_like_with_balances(balances).execute_with(|| {
		let other_indra_id = 3000;
		let amount = REGISTER_AMOUNT;
		let weight = 3 * BaseXcmWeight::get();
		let r = XcmExecutor::<XcmConfig>::execute_xcm(
			Indracore(INDRA_ID).into(),
			Xcm(vec![
				...
				DepositAsset {
					assets: All.into(),
					max_assets: 1,
					beneficiary: Indracore(other_indra_id).into(),
				},
			]),
			weight,
		);
		assert_eq!(r, Outcome::Complete(weight));
		let other_indra_acc: AccountId = IndraId::from(other_indra_id).into_account_truncating();
		assert_eq!(Balances::free_balance(indra_acc), INITIAL_BALANCE - amount);
		assert_eq!(Balances::free_balance(other_indra_acc), amount);
	});
}
...
/// Scenario:
/// A indracore wants to be notified that a transfer worked correctly.
...
fn query_holding_works() {
	...
	selendra_like_with_balances(balances).execute_with(|| {
		...
		assert_eq!(Balances::free_balance(indra_acc.clone()), INITIAL_BALANCE - amount);
		...
		let other_indra_acc: AccountId = IndraId::from(other_indra_id).into_account_truncating();
		assert_eq!(Balances::free_balance(other_indra_acc), amount);
		assert_eq!(Balances::free_balance(indra_acc), INITIAL_BALANCE - 2 * amount);
		assert_eq!(
			mock::sent_xcm(),
			vec![(
				Indracore(INDRA_ID).into(),
...
/// Scenario:
/// A indracore wants to move ESEL from Selendra to Selendra-Evm.
/// The indracore sends an XCM to withdraw funds combined with a teleport to the destination.
///
/// This way of moving funds from a relay to a indracore will only work for trusted chains.
/// Reserve based transfer should be used to move SEL to a community indracore.
///
	...
	selendra_like_with_balances(balances).execute_with(|| {
		...
		let teleport_effects = vec![
			...
			DepositAsset {
				assets: All.into(),
				max_assets: 1,
				beneficiary: (1, Indracore(INDRA_ID)).into(),
			},
		];
		let weight = 3 * BaseXcmWeight::get();
		...
		let r = XcmExecutor::<XcmConfig>::execute_xcm(
			...
			Xcm(vec![
				...
				InitiateTeleport {
					assets: All.into(),
					dest: Indracore(other_indra_id).into(),
		...
		let r = XcmExecutor::<XcmConfig>::execute_xcm(
			...
			Xcm(vec![
				...
				InitiateTeleport {
					assets: All.into(),
					dest: Indracore(statemine_id).into(),
					xcm: Xcm(teleport_effects.clone()),
				},
			]),
			weight,
...
/// Scenario:
/// A indracore wants to move SEL from Selendra to the indracore.
