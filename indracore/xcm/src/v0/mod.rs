...
/// Basically just the XCM (more general) version of `IndracoreDispatchOrigin`.
#[derive(Copy, Clone, Eq, PartialEq, Encode, Decode, Debug, TypeInfo)]
pub enum OriginKind {
	/// Origin should just be the native dispatch origin representation for the sender in the
	/// local runtime framework. For Cumulus/Frame chains this is the `Indracore` or `Relay` origin
...
pub enum Xcm<Call> {
	...
	/// change. This message is meant to be sent by the relay-chain to a indra.
	///
	/// Safety: The message should originate directly from the relay-chain.
	///
	/// Kind: *System Notification*
