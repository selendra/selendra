...
/// - A (normal, layer-1) block chain, e.g. the Bitcoin mainnet or a indracore.
...
impl MultiLocation {
	...
	/// # use xcm::v1::{Junctions::*, Junction::*, MultiLocation};
	/// # fn main() {
	/// let mut m = MultiLocation::new(1, X1(Indracore(21)));
	/// assert_eq!(m.append_with(X1(PalletInstance(3))), Ok(()));
	/// assert_eq!(m, MultiLocation::new(1, X2(Indracore(21), PalletInstance(3))));
	/// # }
	/// ```
	...
	/// assert_eq!(m.prepend_with(MultiLocation::new(1, X2(Indracore(21), OnlyChild))), Ok(()));
	...
	pub fn reanchor(&mut self, target: &MultiLocation, ancestry: &MultiLocation) -> Result<(), ()> {
		// TODO: https://github.com/paritytech/selendra/issues/4489 Optimize this.
...
impl Junctions {
	...
	/// # fn main() {
	/// let mut m = X3(Indracore(2), PalletInstance(3), OnlyChild);
	/// assert_eq!(m.match_and_split(&X2(Indracore(2), PalletInstance(3))), Some(&OnlyChild));
	/// assert_eq!(m.match_and_split(&X1(Indracore(2))), None);
	/// # }
	/// ```
...
mod tests {
	...
	#[test]
	fn inverted_works() {
		let ancestry: MultiLocation = (Indracore(1000), PalletInstance(42)).into();
		...
		let ancestry: MultiLocation = (Indracore(1000), PalletInstance(42), GeneralIndex(1)).into();
	...
	fn simplify_basic_works() {
		let mut location: MultiLocation =
			(Parent, Parent, Indracore(1000), PalletInstance(42), GeneralIndex(69)).into();
		let context = X2(Indracore(1000), PalletInstance(42));
		...
		let mut location: MultiLocation =
			(Parent, Parent, Indracore(1000), PalletInstance(42), GeneralIndex(69)).into();
		let context = X3(OnlyChild, Indracore(1000), PalletInstance(42));
		let expected = GeneralIndex(69).into();
		location.simplify(&context);
		assert_eq!(location, expected);
	}
	...
	fn simplify_incompatible_location_fails() {
		let mut location: MultiLocation =
			(Parent, Parent, Indracore(1000), PalletInstance(42), GeneralIndex(69)).into();
		let context = X3(Indracore(1000), PalletInstance(42), GeneralIndex(42));
		let expected =
			(Parent, Parent, Indracore(1000), PalletInstance(42), GeneralIndex(69)).into();
		location.simplify(&context);
		assert_eq!(location, expected);
	...
	#[test]
	fn reanchor_works() {
		let mut id: MultiLocation = (Parent, Indracore(1000), GeneralIndex(42)).into();
		let ancestry = Indracore(2000).into();
		let target = (Parent, Indracore(1000)).into();
	...
	fn encode_and_decode_works() {
		let m = MultiLocation {
			parents: 1,
			interior: X2(Indracore(42), AccountIndex64 { network: Any, index: 23 }),
		};
	...
	fn match_and_split_works() {
		...
		assert_eq!(
			m.match_and_split(&MultiLocation { parents: 1, interior: X1(Indracore(42)) }),
	...
	fn append_with_works() {
		...
		let mut m = MultiLocation { parents: 1, interior: X1(Indracore(42)) };
		assert_eq!(m.append_with(X2(PalletInstance(3), acc.clone())), Ok(()));
		assert_eq!(
			m,
			MultiLocation {
				parents: 1,
				interior: X3(Indracore(42), PalletInstance(3), acc.clone())
			}
		);
		...
		let m = MultiLocation {
			parents: 254,
			interior: X5(Indracore(42), OnlyChild, OnlyChild, OnlyChild, OnlyChild),
		};
	...
	fn prepend_with_works() {
		...
		let mut m = MultiLocation { parents: 254, interior: X1(Indracore(42)) };
		...
		assert_eq!(m, MultiLocation { parents: 255, interior: X1(Indracore(42)) });
	}

	#[test]
	fn double_ended_ref_iteration_works() {
		let m = X3(Indracore(1000), Indracore(3), PalletInstance(5));
		let mut iter = m.iter();

		let first = iter.next().unwrap();
		assert_eq!(first, &Indracore(1000));
		let third = iter.next_back().unwrap();
		assert_eq!(third, &PalletInstance(5));
		let second = iter.next_back().unwrap();
		assert_eq!(iter.next(), None);
		assert_eq!(iter.next_back(), None);
		assert_eq!(second, &Indracore(3));
	...
	fn conversion_from_other_types_works() {
		...
		takes_multilocation(X1(Indracore(42)));
		takes_multilocation((255, PalletInstance(8)));
		takes_multilocation((Ancestor(5), Indracore(1), PalletInstance(3)));
		takes_multilocation((Ancestor(2), Here));
		takes_multilocation(AncestorThen(
			3,
			X2(Indracore(43), AccountIndex64 { network: Any, index: 155 }),
		));
		takes_multilocation((Parent, AccountId32 { network: Any, id: [0; 32] }));
		takes_multilocation((Parent, Here));
		takes_multilocation(ParentThen(X1(Indracore(75))));
		takes_multilocation([Indracore(100), PalletInstance(3)]);
		...
		assert_eq!(
			v0::MultiLocation::X2(v0::Junction::Indracore(88), v0::Junction::Parent).try_into(),
