...
pub mod pallet {
	...
	impl<T: Config> Pallet<T> {
		...
		/// - `dest`: Destination context for the assets. Will typically be `X2(Parent, Indracore(..))` to send
		///   from indracore to indracore, or `X1(Indracore(..))` to send from relay to indracore.
								...
								.using_encoded(|mut bytes| <T as Config>::Call::decode(&mut bytes))
							{
								...
								match call.dispatch(dispatch_origin) {
									...
									Err(error_and_info) => {
										...
										Self::deposit_event(e);
										// Not much to do with the result as it is. It's up to the indracore to ensure that the
										// message makes sense.
