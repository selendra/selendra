...
fn report_outcome_notify_works() {
	let balances = vec![
		...
		(IndraId::from(INDRA_ID).into_account_truncating(), INITIAL_BALANCE),
	];
	...
	new_test_ext_with_balances(balances).execute_with(|| {
		XcmPallet::report_outcome_notify(&mut message, Indracore(INDRA_ID).into(), notify, 100)
			.unwrap();
		assert_eq!(
			message,
			Xcm(vec![
		...
		let status = QueryStatus::Pending {
			responder: MultiLocation::from(Indracore(INDRA_ID)).into(),
...
fn report_outcome_works() {
	...
	new_test_ext_with_balances(balances).execute_with(|| {
		XcmPallet::report_outcome(&mut message, Indracore(INDRA_ID).into(), 100).unwrap();
		assert_eq!(
			message,
			Xcm(vec![
				SetAppendix(Xcm(vec![ReportError {
...
fn reserve_transfer_assets_works() {
	...
	new_test_ext_with_balances(balances).execute_with(|| {
		...
		assert_ok!(XcmPallet::reserve_transfer_assets(
			Origin::signed(ALICE),
			Box::new(Indracore(INDRA_ID).into().into()),
			Box::new(dest.clone().into()),
			Box::new((Here, SEND_AMOUNT).into()),
			0,
		));
		...
		// Destination account (indracore account) has amount
		let indra_acc: AccountId = IndraId::from(INDRA_ID).into_account_truncating();
		assert_eq!(Balances::free_balance(indra_acc), INITIAL_BALANCE + SEND_AMOUNT);
		assert_eq!(
			sent_xcm(),
			vec![(
				Indracore(INDRA_ID).into(),
...
fn fake_latest_versioned_multilocation_works() {
	use codec::Encode;
	let remote = Indracore(1000).into();
...
fn subscriptions_increment_id() {
	new_test_ext_with_balances(vec![]).execute_with(|| {
		...
		let remote2 = Indracore(1001).into();
		assert_ok!(XcmPallet::force_subscribe_version_notify(
			Origin::root(),
...
/// Indracore 1000 is asking us for a version subscription.
#[test]
...
fn subscription_side_upgrades_work_with_notify() {
	new_test_ext_with_balances(vec![]).execute_with(|| {
		...
		assert_eq!(
			sent,
			vec![
				(Indracore(1000).into(), Xcm(vec![instr0])),
				(Indracore(1001).into(), Xcm(vec![instr1])),
				(Indracore(1002).into(), Xcm(vec![instr2])),
			]
		);
		...
		assert_eq!(
			contents,
			vec![
				(2, Indracore(1000).into().versioned(), (69, 0, 2)),
				(2, Indracore(1001).into().versioned(), (70, 0, 2)),
				(2, Indracore(1002).into().versioned(), (71, 0, 2)),
			]
		);
	});
}
...
fn auto_subscription_works() {
	new_test_ext_with_balances(vec![]).execute_with(|| {
		let remote0 = Indracore(1000).into();
		let remote1 = Indracore(1001).into();
