...
benchmarks! {
	...
	verify {
		...
		for (indra_id, backing_validators) in vote.backing_validators_per_candidate.iter().enumerate() {
			let descriptor = backing_validators.0.descriptor();
			assert_eq!(IndraId::from(indra_id), descriptor.indra_id);
	...
	verify {
		...
		let header = BenchBuilder::<T>::header(scenario._block_number.clone());
		// Traverse candidates and assert descriptors are as expected
		for (indra_id, backing_validators)
