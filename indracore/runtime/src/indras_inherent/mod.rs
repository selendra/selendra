...
//! one inherent per block that can include new indra candidates and bitfields.
//!
...
use primitives::v2::{
	...
	InherentData as IndracoresInherentData, MultiDisputeStatementSet, ScrapedOnChainVotes,
	...
	INDRACORES_INHERENT_IDENTIFIER,
};
...
pub mod pallet {
	...
	/// Whether the indras inherent was included within this block.
	///
	...
	impl<T: Config> ProvideInherent for Pallet<T> {
		...
		const INHERENT_IDENTIFIER: InherentIdentifier = INDRACORES_INHERENT_IDENTIFIER;
		...
		fn create_inherent(data: &InherentData) -> Option<Self::Call> {
			...
			let inherent_data = match Self::enter_inner(inherent_data.clone(), FullCheck::Skip) {
				...
				Err(err) => {
					log::error!(
						target: LOG_TARGET,
						"dropping indras inherent data because they produced \
							an invalid indras inherent: {:?}",
						err.error,
					);
		...
		T: Config,
	{
		...
		// Schedule indras again, given freed cores, and reasons for freeing.
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn enter(
			origin: OriginFor<T>,
			data: IndracoresInherentData<T::Header>,
	...
	pub(crate) fn enter_inner(
		...
		let (checked_disputes, total_consumed_weight) = {
			...
			if candidates_weight
				...
				max_block_weight
			{
				log::warn!("Overweight indra inherent data reached the runtime {:?}", parent_hash);
		...
		let disputed_bitfield = {
			...
			if T::DisputesHandler::is_frozen() {
				// Relay chain freeze, at this point we will not include any indracore blocks.
				METRICS.on_relay_chain_freeze();

				// The relay chain we are currently on is invalid. Proceed no further on indracores.
...
impl<T: Config> Pallet<T> {
	/// Create the `IndracoresInherentData` that gets passed to [`Self::enter`] in [`Self::create_inherent`].
	/// This code is pulled out of [`Self::create_inherent`] so it can be unit tested.
	fn create_inherent_inner(data: &InherentData) -> Option<IndracoresInherentData<T::Header>> {
		let IndracoresInherentData::<T::Header> {
			bitfields,
			backed_candidates,
			mut disputes,
			parent_header,
			...
			Ok(None) => return None,
			Err(_) => {
				log::warn!(target: LOG_TARGET, "IndracoresInherentData failed to decode");
				return None
			},
		};
		...
		if parent_hash != parent_header.hash() {
			log::warn!(
				target: LOG_TARGET,
				"IndracoresInherentData references a different parent header hash than frame"
			);
			return None
		}
		...
		Some(IndracoresInherentData::<T::Header> {
			bitfields,
			backed_candidates,
			disputes,
			parent_header,
...
/// for backed candidates, since with a increasing number of indracores their chances of
...
>(
	...
	let scheduled_indras_to_core_idx = scheduled
		.into_iter()
		.map(|core_assignment| (core_assignment.indra_id, core_assignment.core))
		.collect::<BTreeMap<IndraId, CoreIndex>>();

	// Assure the backed candidate's `IndraId`'s core is free.
	...
	backed_candidates.retain(|backed_candidate| {
		...
		desc.relay_parent == relay_parent &&
			scheduled_indras_to_core_idx.get(&desc.indra_id).is_some()
	});
	...
	backed_candidates.sort_by(|x, y| {
		...
		scheduled_indras_to_core_idx[&x.descriptor().indra_id]
			.cmp(&scheduled_indras_to_core_idx[&y.descriptor().indra_id])
	});
