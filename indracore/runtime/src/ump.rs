...
use primitives::v2::{Id as IndraId, UpwardMessage};
...
/// All upward messages coming from indracores will be funneled into an implementation of this trait.
///
...
///
/// It is possible that by the time the message is sank the origin indracore was offboarded. It is
	...
	fn process_upward_message(
		...
		match maybe_msg_and_weight {
			...
			Ok((Ok(xcm_message), weight_used)) => {
				let xcm_junction = Junction::Indracore(origin.into());
...
pub mod pallet {
	...
	pub enum Event {
		...
		/// \[ indra, count, size \]
		UpwardMessagesReceived(IndraId, u32, u32),
		...
		///
		/// \[ indra, id, overweight_index, required \]
		OverweightEnqueued(IndraId, MessageId, OverweightIndex, Weight),
	...
	/// The messages waiting to be handled by the relay-chain originating from a certain indracore.
	///
	...
	pub type RelayDispatchQueues<T: Config> =
		StorageMap<_, Twox64Concat, IndraId, Vec<UpwardMessage>, ValueQuery>;
	...
	pub type RelayDispatchQueueSize<T: Config> =
		StorageMap<_, Twox64Concat, IndraId, (u32, u32), ValueQuery>;

	/// The ordered list of `IndraId`s that have a `RelayDispatchQueue` entry.
	///
	/// Invariant:
	/// - The set of items from this vector should be exactly the set of the keys in
	///   `RelayDispatchQueues` and `RelayDispatchQueueSize`.
	#[pallet::storage]
	pub type NeedsDispatch<T: Config> = StorageValue<_, Vec<IndraId>, ValueQuery>;

	/// This is the indra that gets will get dispatched first during the next upward dispatchable queue
	/// execution round.
	///
	/// Invariant:
	/// - If `Some(indra)`, then `indra` must be present in `NeedsDispatch`.
	#[pallet::storage]
	pub type NextDispatchRoundStartWith<T: Config> = StorageValue<_, IndraId>;
	...
	pub type Overweight<T: Config> =
		StorageMap<_, Twox64Concat, OverweightIndex, (IndraId, Vec<u8>), OptionQuery>;
...
impl<T: Config> Pallet<T> {
	...
	fn perform_outgoing_indra_cleanup(outgoing: &[IndraId]) -> Weight {
		let mut weight: Weight = 0;
		for outgoing_indra in outgoing {
			weight = weight.saturating_add(Self::clean_ump_after_outgoing(outgoing_indra));
		}
		weight
	}

	/// Remove all relevant storage items for an outgoing indracore.
	pub(crate) fn clean_ump_after_outgoing(outgoing_indra: &IndraId) -> Weight {
		<Self as Store>::RelayDispatchQueueSize::remove(outgoing_indra);
		<Self as Store>::RelayDispatchQueues::remove(outgoing_indra);

		// Remove the outgoing indra from the `NeedsDispatch` list and from
		...
		<Self as Store>::NeedsDispatch::mutate(|v| {
			if let Ok(i) = v.binary_search(outgoing_indra) {
				v.remove(i);
			}
		});
		<Self as Store>::NextDispatchRoundStartWith::mutate(|v| {
			*v = v.filter(|p| p == outgoing_indra)
		});
	...
	pub(crate) fn check_upward_messages(
		...
		let (mut indra_queue_count, mut indra_queue_size) =
			<Self as Store>::RelayDispatchQueueSize::get(&indra);
	...
	/// Enqueues `upward_messages` from a `indra`'s accepted candidate block.
	pub(crate) fn receive_upward_messages(
		...
		if !upward_messages.is_empty() {
			...
			<Self as Store>::RelayDispatchQueues::mutate(&indra, |v| {
			...
			Self::deposit_event(Event::UpwardMessagesReceived(indra, extra_count, extra_size));
		}
	...
	pub(crate) fn process_pending_upward_messages() -> Weight {
		...
		while let Some(dispatchee) = cursor.peek() {
			...
			if queue_cache.is_empty::<T>(dispatchee) {
				// the queue is empty now - this indra doesn't need attention anymore.
				cursor.remove();
			} else {
				cursor.advance();
			}
	...
	/// later.
	fn stash_overweight(sender: IndraId, upward_message: Vec<u8>) -> OverweightIndex {
...
struct QueueCache(BTreeMap<IndraId, QueueCacheEntry>);
...
impl QueueCache {
	...
	fn ensure_cached<T: Config>(&mut self, indra: IndraId) -> &mut QueueCacheEntry {
		self.0.entry(indra).or_insert_with(|| {
			let queue = RelayDispatchQueues::<T>::get(&indra);
			let (_, total_size) = RelayDispatchQueueSize::<T>::get(&indra);
			QueueCacheEntry { queue, total_size, consumed_count: 0, consumed_size: 0 }
		})
	}

	/// Returns the message at the front of `indra`'s queue, or `None` if the queue is empty.
	///
	/// Does not mutate the queue.
	fn peek_front<T: Config>(&mut self, indra: IndraId) -> Option<&UpwardMessage> {
		let entry = self.ensure_cached::<T>(indra);
		entry.queue.get(entry.consumed_count)
	}

	/// Attempts to remove one message from the front of `indra`'s queue. If the queue is empty, then
	/// does nothing.
	fn consume_front<T: Config>(&mut self, indra: IndraId) -> Option<UpwardMessage> {
		let cache_entry = self.ensure_cached::<T>(indra);
	...
	/// Returns if the queue for the given indra is empty.
	///
	/// That is, if this returns `true` then the next call to [`peek_front`] will return `None`.
	///
	/// Does not mutate the queue.
	fn is_empty<T: Config>(&mut self, indra: IndraId) -> bool {
		let cache_entry = self.ensure_cached::<T>(indra);
...
///
/// This cursor will start with the indra indicated by `NextDispatchRoundStartWith` storage entry.
...
struct NeedsDispatchCursor {
	needs_dispatch: Vec<IndraId>,
	index: usize,
}

impl NeedsDispatchCursor {
	fn new<T: Config>() -> Self {
		let needs_dispatch: Vec<IndraId> = <Pallet<T> as Store>::NeedsDispatch::get();
		let start_with = <Pallet<T> as Store>::NextDispatchRoundStartWith::get();

		let initial_index = match start_with {
			Some(indra) => match needs_dispatch.binary_search(&indra) {
