...
//! Configuration manager for the Selendra runtime indracores logic.
//!
...
/// All configuration of the runtime with respect to indracores and indrabases.
...
pub struct HostConfiguration<BlockNumber> {
	...
	// corresponding migration of the indracores.
	/**
	 * The parameters that are required for the indracores.
	 */
	...
	///
	/// This number is used to prevent indracores from spamming the relay chain with validation code
	/// upgrades. The only thing it controls is the number of blocks the `UpgradeRestrictionSignal`
	/// is set for the indracore in question.
	///
	...
	///
	/// The upgrade for a indracore takes place when the first candidate which has relay-parent >=
	...
	/**
	 * The parameters that are not essential, but still may be of interest for indracores.
	 */
	...
	/// the PoV size. Of course, there is a lot of other different things that a indracore may
	...
	/// The maximum number of outbound HRMP channels a indracore is allowed to open.
	pub hrmp_max_indracore_outbound_channels: u32,
	/// The maximum number of outbound HRMP channels a indrabase is allowed to open.
	pub hrmp_max_indrabase_outbound_channels: u32,
	...
	/// The maximum number of inbound HRMP channels a indracore is allowed to accept.
	pub hrmp_max_indracore_inbound_channels: u32,
	/// The maximum number of inbound HRMP channels a indrabase is allowed to accept.
	pub hrmp_max_indrabase_inbound_channels: u32,
	...
	/**
	 * Parameters that will unlikely be needed by indracores.
	 */

	/// How long to keep code on-chain, in blocks. This should be sufficiently long that disputes
	/// have concluded.
	pub code_retention_period: BlockNumber,
	/// The amount of execution cores to dedicate to indrabase execution.
	pub indrabase_cores: u32,
	/// The number of retries that a indrabase author has to submit their block.
	pub indrabase_retries: u32,
	/// How often indracore groups should be rotated across indracores.
	///
	/// Must be non-zero.
	pub group_rotation_frequency: BlockNumber,
	/// The availability period, in blocks, for indracores. This is the amount of blocks
	/// after inclusion that validators have to make the block available and signal its availability to
	/// the chain.
	///
	/// Must be at least 1.
	pub chain_availability_period: BlockNumber,
	/// The availability period, in blocks, for indrabases. Same as the `chain_availability_period`,
	/// but a differing timeout due to differing requirements.
	///
	/// Must be at least 1.
	pub thread_availability_period: BlockNumber,
	/// The amount of blocks ahead to schedule indracores and indrabases.
	pub scheduling_lookahead: u32,
	/// The maximum number of validators to have per core.
	///
	/// `None` means no maximum.
	pub max_validators_per_core: Option<u32>,
	/// The maximum number of validators to use for indracore consensus, period.
	///
	/// `None` means no maximum.
	...
	/// upgrade procedure is time-based and indracores that do not look at the go-ahead signal
	/// should still work.
	...
	/// the relay-parent of the indrablock that signalled the validation code upgrade. However,
	/// in the case a pre-checking voting was concluded in a longer duration the upgrade will be
	/// scheduled to the next block.
	///
	/// That can disrupt indracore inclusion. Specifically, it will make the blocks that were
...
impl<BlockNumber: Default + From<u32>> Default for HostConfiguration<BlockNumber> {
	fn default() -> Self {
		Self {
			...
			indrabase_cores: Default::default(),
			indrabase_retries: Default::default(),
			...
			hrmp_max_indracore_inbound_channels: Default::default(),
			hrmp_max_indrabase_inbound_channels: Default::default(),
			hrmp_channel_max_message_size: Default::default(),
			hrmp_max_indracore_outbound_channels: Default::default(),
			hrmp_max_indrabase_outbound_channels: Default::default(),
	...
	BlockNumber: Zero + PartialOrd + sp_std::fmt::Debug + Clone + From<u32>,
{
	...
	pub fn check_consistency(&self) -> Result<(), InconsistentError<BlockNumber>> {
		...
		if self.hrmp_max_indracore_outbound_channels > crate::hrmp::HRMP_MAX_OUTBOUND_CHANNELS_BOUND
		{
			return Err(MaxHrmpOutboundChannelsExceeded)
		}

		if self.hrmp_max_indracore_inbound_channels > crate::hrmp::HRMP_MAX_INBOUND_CHANNELS_BOUND {
...
pub mod pallet {
	...
	impl<T: Config> Pallet<T> {
		...
		/// Set the max head data size for indras.
		#[pallet::weight((
			T::WeightInfo::set_config_with_u32(),
			DispatchClass::Operational,
		))]
		...
		/// Set the number of indrabase execution cores.
		#[pallet::weight((
			T::WeightInfo::set_config_with_u32(),
			DispatchClass::Operational,
		))]
		pub fn set_indrabase_cores(origin: OriginFor<T>, new: u32) -> DispatchResult {
			ensure_root(origin)?;
			Self::schedule_config_update(|config| {
				config.indrabase_cores = new;
			})
		}

		/// Set the number of retries for a particular indrabase.
		#[pallet::weight((
			T::WeightInfo::set_config_with_u32(),
			DispatchClass::Operational,
		))]
		pub fn set_indrabase_retries(origin: OriginFor<T>, new: u32) -> DispatchResult {
			ensure_root(origin)?;
			Self::schedule_config_update(|config| {
				config.indrabase_retries = new;
			})
		}

		/// Set the indracore validator-group rotation frequency
		#[pallet::weight((
			T::WeightInfo::set_config_with_block_number(),
			DispatchClass::Operational,
		))]
		...
		/// Set the availability period for indracores.
		#[pallet::weight((
			T::WeightInfo::set_config_with_block_number(),
			DispatchClass::Operational,
		))]
		...
		/// Set the availability period for indrabases.
		#[pallet::weight((
			T::WeightInfo::set_config_with_block_number(),
			DispatchClass::Operational,
		))]
		...
		/// Set the maximum number of validators to use in indracore consensus.
		#[pallet::weight((
			T::WeightInfo::set_config_with_option_u32(),
			DispatchClass::Operational,
		))]
		...
		/// Sets the maximum number of inbound HRMP channels a indracore is allowed to accept.
		#[pallet::weight((
			T::WeightInfo::set_config_with_u32(),
			DispatchClass::Operational,
		))]
		pub fn set_hrmp_max_indracore_inbound_channels(
			origin: OriginFor<T>,
			new: u32,
		) -> DispatchResult {
			ensure_root(origin)?;
			Self::schedule_config_update(|config| {
				config.hrmp_max_indracore_inbound_channels = new;
			})
		}

		/// Sets the maximum number of inbound HRMP channels a indrabase is allowed to accept.
		#[pallet::weight((
			T::WeightInfo::set_config_with_u32(),
			DispatchClass::Operational,
		))]
		pub fn set_hrmp_max_indrabase_inbound_channels(
			origin: OriginFor<T>,
			new: u32,
		) -> DispatchResult {
			ensure_root(origin)?;
			Self::schedule_config_update(|config| {
				config.hrmp_max_indrabase_inbound_channels = new;
			})
		}
		...
		/// Sets the maximum number of outbound HRMP channels a indracore is allowed to open.
		#[pallet::weight((
			T::WeightInfo::set_config_with_u32(),
			DispatchClass::Operational,
		))]
		pub fn set_hrmp_max_indracore_outbound_channels(
			origin: OriginFor<T>,
			new: u32,
		) -> DispatchResult {
			ensure_root(origin)?;
			Self::schedule_config_update(|config| {
				config.hrmp_max_indracore_outbound_channels = new;
			})
		}

		/// Sets the maximum number of outbound HRMP channels a indrabase is allowed to open.
		#[pallet::weight((
			T::WeightInfo::set_config_with_u32(),
			DispatchClass::Operational,
		))]
		pub fn set_hrmp_max_indrabase_outbound_channels(
			origin: OriginFor<T>,
			new: u32,
		) -> DispatchResult {
			ensure_root(origin)?;
			Self::schedule_config_update(|config| {
				config.hrmp_max_indrabase_outbound_channels = new;
			})
		}
		...
		/// Sets the minimum delay between announcing the upgrade block for a indracore until the
...
impl<T: Config> Pallet<T> {
	...
	/// only when enabling indracores runtime pallets for the first time on a chain which has
