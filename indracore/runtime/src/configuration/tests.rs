...
fn on_new_session(session_index: SessionIndex) -> (HostConfiguration<u32>, HostConfiguration<u32>) {
	IndrasShared::set_session_index(session_index);
...
fn setting_pending_config_members() {
	new_test_ext(Default::default()).execute_with(|| {
		let new_config = HostConfiguration {
			...
			hrmp_max_indracore_inbound_channels: 37,
			hrmp_max_indrabase_inbound_channels: 19,
			hrmp_channel_max_message_size: 8192,
			hrmp_max_indracore_outbound_channels: 10,
			hrmp_max_indrabase_outbound_channels: 20,
		...
		.unwrap();
		Configuration::set_hrmp_max_indracore_inbound_channels(
			Origin::root(),
			new_config.hrmp_max_indracore_inbound_channels,
		)
		.unwrap();
		Configuration::set_hrmp_max_indrabase_inbound_channels(
			Origin::root(),
			new_config.hrmp_max_indrabase_inbound_channels,
		)
		.unwrap();
		...
		.unwrap();
		Configuration::set_hrmp_max_indracore_outbound_channels(
			Origin::root(),
			new_config.hrmp_max_indracore_outbound_channels,
		)
		.unwrap();
		Configuration::set_hrmp_max_indrabase_outbound_channels(
			Origin::root(),
			new_config.hrmp_max_indrabase_outbound_channels,
		)
		.unwrap();
