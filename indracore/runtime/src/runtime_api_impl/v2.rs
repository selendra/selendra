...
use primitives::v2::{
	...
	CoreState, GroupIndex, GroupRotationInfo, Hash, Id as IndraId, InboundDownwardMessage,
...
pub fn availability_cores<T: initializer::Config>() -> Vec<CoreState<T::Hash, T::BlockNumber>> {
	...
	let group_responsible_for =
		|backed_in_number, core_index| match <scheduler::Pallet<T>>::group_assigned_to_core(
			...
			None => {
				log::warn!(
					target: "runtime::selendra-api::v2",
		...
		.map(|(i, core)| match core {
			Some(occupied) => CoreState::Occupied(match occupied {
				CoreOccupied::Indracore => {
					let indra_id = indracores[i];
					let pending_availability =
						<inclusion::Pallet<T>>::pending_availability(indra_id)
				...
				CoreOccupied::Indrabase(p) => {
					let indra_id = p.claim.0;
					let pending_availability =
						<inclusion::Pallet<T>>::pending_availability(indra_id)
							.expect("Occupied core always has pending availability; qed");
	...
	F: FnOnce() -> Option<T>,
{
	match assumption {
		OccupiedCoreAssumption::Included => {
			<inclusion::Pallet<Config>>::force_enact(indra_id);
			build()
		},
		OccupiedCoreAssumption::TimedOut => build(),
		OccupiedCoreAssumption::Free => {
			if <inclusion::Pallet<Config>>::pending_availability(indra_id).is_some() {
				None
			} else {
				build()
			}
...
pub fn assumed_validation_data<T: initializer::Config>(
	...
	let (relay_parent_number, relay_parent_storage_root) = current_relay_parent::<T>();
	// This closure obtains the `persisted_validation_data` for the given `indra_id` and matches
	...
	let persisted_validation_data = make_validation_data().or_else(|| {
		...
		<inclusion::Pallet<T>>::pending_availability(indra_id).and_then(|_| {
			<inclusion::Pallet<T>>::force_enact(indra_id);
...
pub fn check_validation_outputs<T: initializer::Config>(
	...
	<inclusion::Pallet<T>>::check_validation_outputs_for_runtime_api(indra_id, outputs)
}
...
pub fn relevant_authority_ids<T: initializer::Config + pallet_authority_discovery::Config>(
	...
	// selected to participate in indracore consensus. We'd like all authorities for the current
...
pub fn candidate_pending_availability<T: initializer::Config>(
	...
	<inclusion::Pallet<T>>::candidate_pending_availability(indra_id)
}
