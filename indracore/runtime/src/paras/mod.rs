...
//! head data and the current validation code (AKA Indracore Validation Function (PVF)).
//!
//! A indra is not considered live until it is registered and activated in this pallet.
//!
//! The set of indracores and indrabases cannot change except at session boundaries. This is
...
//!
//! When a indra signals the validation code upgrade it will be processed by this module. This can
//! be in turn split into more fine grained items:
//!
//! - Part of the acceptance criteria checks if the indra can indeed signal an upgrade,
//!
...
//!   fulfilled.
//!
//! The conditions that must be met before the indra can use the new validation code are:
//!
...
//!
//! # Indra Lifecycle Management
//!
//! A indra can be in one of the two stable states: it is either a indracore or a indrabase.
//!
...
//!
//! Once the indra is in one of the two stable states, it can switch to the other stable state or to
...
use primitives::v2::{
	ConsensusLog, HeadData, Id as IndraId, PvfCheckStatement, SessionIndex, UpgradeGoAhead,
...
pub use crate::Origin as IndracoreOrigin;
...
pub struct ReplacementTimes<N> {
	...
	/// This is when the code change occurs from the indra's perspective - after the
	/// first indrablock included with a relay-parent with number >= this value.
	expected_at: N,
	/// The relay-chain block number at which the indrablock activating the code upgrade was
	/// actually included. This means considered included and available, so this is the time at which
	/// that indrablock enters the acceptance period in this fork of the relay-chain.
	activated_at: N,
}

/// Metadata used to track previous indracore validation code that we keep in
/// the state.
#[derive(Default, Encode, Decode, TypeInfo)]
#[cfg_attr(test, derive(Debug, Clone, PartialEq))]
pub struct ParaPastCodeMeta<N> {
	...
	/// into the `PastCodeHash` map along with the `IndraId` to fetch the code itself.
...
/// The possible states of a indra, to take into account delayed lifecycle changes.
///
/// If the indra is in a "transition state", it is expected that the indracore is
/// queued in the `ActionsQueue` to transition it into a stable state. Its lifecycle
/// state will be used to determine the state transition to apply to the indra.
#[derive(PartialEq, Eq, Clone, Encode, Decode, RuntimeDebug, TypeInfo)]
pub enum IndraLifecycle {
	/// Indra is new and is onboarding as a Indrabase or Indracore.
	Onboarding,
	/// Indra is a Indrabase.
	Indrabase,
	/// Indra is a Indracore.
	Indracore,
	/// Indra is a Indrabase which is upgrading to a Indracore.
	UpgradingIndrabase,
	/// Indra is a Indracore which is downgrading to a Indrabase.
	DowngradingIndracore,
	/// Indrabase is queued to be offboarded.
	OffboardingIndrabase,
	/// Indracore is queued to be offboarded.
	OffboardingIndracore,
}

impl IndraLifecycle {
	/// Returns true if indracore is currently onboarding. To learn if the
	/// indracore is onboarding as a indracore or indrabase, look at the
	/// `UpcomingGenesis` storage item.
	pub fn is_onboarding(&self) -> bool {
		matches!(self, IndraLifecycle::Onboarding)
	}

	/// Returns true if indra is in a stable state, i.e. it is currently
	/// a indracore or indrabase, and not in any transition state.
	pub fn is_stable(&self) -> bool {
		matches!(self, IndraLifecycle::Indrabase | IndraLifecycle::Indracore)
	}

	/// Returns true if indra is currently treated as a indracore.
	/// This also includes transitioning states, so you may want to combine
	/// this check with `is_stable` if you specifically want `IndraLifecycle::Indracore`.
	pub fn is_indracore(&self) -> bool {
		matches!(
			self,
			IndraLifecycle::Indracore |
				IndraLifecycle::DowngradingIndracore |
				IndraLifecycle::OffboardingIndracore
		)
	}

	/// Returns true if indra is currently treated as a indrabase.
	/// This also includes transitioning states, so you may want to combine
	/// this check with `is_stable` if you specifically want `IndraLifecycle::Indrabase`.
	pub fn is_indrabase(&self) -> bool {
		matches!(
			self,
			IndraLifecycle::Indrabase |
				IndraLifecycle::UpgradingIndrabase |
				IndraLifecycle::OffboardingIndrabase
		)
	}

	/// Returns true if indra is currently offboarding.
	pub fn is_offboarding(&self) -> bool {
		matches!(self, IndraLifecycle::OffboardingIndrabase | IndraLifecycle::OffboardingIndracore)
	}

	/// Returns true if indra is in any transitionary state.
...
impl<N: Ord + Copy + PartialEq> ParaPastCodeMeta<N> {
	...
	//
	// Since we don't want to prune anything inside the acceptance period, and the indrablock only
...
/// Arguments for initializing a indra.
#[derive(PartialEq, Eq, Clone, Encode, Decode, RuntimeDebug, TypeInfo)]
#[cfg_attr(feature = "std", derive(Serialize, Deserialize))]
pub struct IndraGenesisArgs {
	/// The initial head data to use.
	pub genesis_head: HeadData,
	/// The initial validation code to use.
	pub validation_code: ValidationCode,
	/// True if indracore, false if indrabase.
	pub indracore: bool,
}
...
enum PvfCheckCause<BlockNumber> {
	/// PVF vote was initiated by the initial onboarding process of the given indra.
	Onboarding(IndraId),
	/// PVF vote was initiated by signalling of an upgrade by the given indra.
	Upgrade {
		/// The ID of the indracore that initiated or is waiting for the conclusion of pre-checking.
		id: IndraId,
		/// The relay-chain block number that was used as the relay-parent for the indrablock that
...
impl<BlockNumber> PvfCheckCause<BlockNumber> {
	/// Returns the ID of the indra that initiated or subscribed to the pre-checking vote.
	fn indra_id(&self) -> IndraId {
		match *self {
...
pub mod pallet {
	...
	pub enum Event {
		...
		/// The given indra either initiated or subscribed to a PVF check for the given validation
		/// code. `code_hash` `indra_id`
		PvfCheckStarted(ValidationCodeHash, IndraId),
		/// The given validation code was accepted by the PVF pre-checking vote.
		/// `code_hash` `indra_id`
		PvfCheckAccepted(ValidationCodeHash, IndraId),
		/// The given validation code was rejected by the PVF pre-checking vote.
		/// `code_hash` `indra_id`
		PvfCheckRejected(ValidationCodeHash, IndraId),
	}

	#[pallet::error]
	pub enum Error<T> {
		/// Indra is not registered in our system.
		NotRegistered,
		/// Indra cannot be onboarded because it is already tracked by our system.
		CannotOnboard,
		/// Indra cannot be offboarded at this time.
		CannotOffboard,
		/// Indra cannot be upgraded to a indracore.
		CannotUpgrade,
		/// Indra cannot be downgraded to a indrabase.
		CannotDowngrade,
	...
	///
	/// Consider using the [`IndracoresCache`] type of modifying.
	#[pallet::storage]
	#[pallet::getter(fn indracores)]
	pub(crate) type Indracores<T: Config> = StorageValue<_, Vec<IndraId>, ValueQuery>;
	...
	#[pallet::storage]
	#[pallet::getter(fn indra_head)]
	pub(super) type Heads<T: Config> = StorageMap<_, Twox64Concat, IndraId, HeadData>;

	/// The validation code hash of every live indra.
	///
	/// Corresponding code can be retrieved with [`CodeByHash`].
	#[pallet::storage]
	#[pallet::getter(fn current_code_hash)]
	pub(super) type CurrentCodeHash<T: Config> =
		StorageMap<_, Twox64Concat, IndraId, ValidationCodeHash>;

	/// Actual past code hash, indicated by the indra id as well as the block number at which it
	/// became outdated.
	///
	/// Corresponding code can be retrieved with [`CodeByHash`].
	#[pallet::storage]
	pub(super) type PastCodeHash<T: Config> =
		StorageMap<_, Twox64Concat, (IndraId, T::BlockNumber), ValidationCodeHash>;

	/// Past code of indracores. The indracores themselves may not be registered anymore,
	...
	/// from the time at which the indracore perceives a code upgrade as having occurred.
	/// Multiple entries for a single indra are permitted. Ordered ascending by block number.
	#[pallet::storage]
	pub(super) type PastCodePruning<T: Config> =
		StorageValue<_, Vec<(IndraId, T::BlockNumber)>, ValueQuery>;

	/// The block number at which the planned code change is expected for a indra.
	/// The change will be applied after the first indrablock for this ID included which executes
	/// in the context of a relay chain block with a number >= `expected_at`.
	#[pallet::storage]
	#[pallet::getter(fn future_code_upgrade_at)]
	pub(super) type FutureCodeUpgrades<T: Config> =
		StorageMap<_, Twox64Concat, IndraId, T::BlockNumber>;

	/// The actual future code hash of a indra.
	///
	/// Corresponding code can be retrieved with [`CodeByHash`].
	#[pallet::storage]
	...
	/// This is used by the relay-chain to communicate to a indracore a go-ahead with in the upgrade procedure.
	///
	/// This value is absent when there are no upgrades scheduled or during the time the relay chain
	/// performs the checks. It is set at the first relay-chain block when the corresponding indracore
	/// can switch its upgrade function. As soon as the indracore's block is included, the value
	/// gets reset to `None`.
	///
	/// NOTE that this field is used by indracores via merkle storage proofs, therefore changing
	/// the format will require migration of indracores.
	#[pallet::storage]
	pub(super) type UpgradeGoAheadSignal<T: Config> =
		StorageMap<_, Twox64Concat, IndraId, UpgradeGoAhead>;

	/// This is used by the relay-chain to communicate that there are restrictions for performing
	/// an upgrade for this indracore.
	///
	/// This may be a because the indracore waits for the upgrade cooldown to expire. Another
	...
	pub(super) type UpgradeRestrictionSignal<T: Config> =
		StorageMap<_, Twox64Concat, IndraId, UpgradeRestriction>;

	/// The list of indracores that are awaiting for their upgrade restriction to cooldown.
	///
	...
	/// The list of upcoming code upgrades. Each item is a pair of which indra performs a code
	...
	pub(super) type ActionsQueue<T: Config> =
		StorageMap<_, Twox64Concat, SessionIndex, Vec<IndraId>, ValueQuery>;

	/// Upcoming indras instantiation arguments.
	///
	...
	impl<T: Config> GenesisBuild<T> for GenesisConfig {
		fn build(&self) {
			...
			for (id, genesis_args) in &self.paras {
				...
				Pallet::<T>::initialize_indra_now(&mut indracores, *id, genesis_args);
			}
			// indracores are flushed on drop
		}
	}

	#[pallet::call]
	impl<T: Config> Pallet<T> {
		/// Set the storage for the indracore validation code immediately.
		...
		pub fn force_set_current_code(
			...
			ensure_root(origin)?;
			let maybe_prior_code_hash = <Self as Store>::CurrentCodeHash::get(&indra);
			let new_code_hash = new_code.hash();
			Self::increase_code_ref(&new_code_hash, &new_code);
			<Self as Store>::CurrentCodeHash::insert(&indra, new_code_hash);

			let now = frame_system::Pallet::<T>::block_number();
			if let Some(prior_code_hash) = maybe_prior_code_hash {
				Self::note_past_code(indra, now, now, prior_code_hash);
			} else {
				log::error!(
					target: LOG_TARGET,
					"Pallet indras storage is inconsistent, prior code not found {:?}",
					&indra
				);
			}
			Self::deposit_event(Event::CurrentCodeUpdated(indra));
			Ok(())
		}

		/// Set the storage for the current indracore head data immediately.
		...
		pub fn force_set_current_head(
			...
			ensure_root(origin)?;
			<Self as Store>::Heads::insert(&indra, new_head);
			Self::deposit_event(Event::CurrentHeadUpdated(indra));
			Ok(())
		}
		...
		pub fn force_schedule_code_upgrade(
			...
			Self::schedule_code_upgrade(indra, new_code, relay_parent_number, &config);
			Self::deposit_event(Event::CodeUpgradeScheduled(indra));
			Ok(())
		}

		/// Note a new block head for indra within the context of the current block.
		...
		pub fn force_note_new_head(
			...
			ensure_root(origin)?;
			let now = frame_system::Pallet::<T>::block_number();
			Self::note_new_head(indra, new_head, now);
			Self::deposit_event(Event::NewHeadNoted(indra));
			Ok(())
		}

		/// Put a indracore directly into the next session's action queue.
		/// We can't queue it any sooner than this without going into the
		/// initializer...
		#[pallet::weight(<T as Config>::WeightInfo::force_queue_action())]
		pub fn force_queue_action(origin: OriginFor<T>, indra: IndraId) -> DispatchResult {
			ensure_root(origin)?;
			let next_session = shared::Pallet::<T>::session_index().saturating_add(One::one());
			ActionsQueue::<T>::mutate(next_session, |v| {
				if let Err(i) = v.binary_search(&indra) {
					v.insert(i, indra);
				}
			});
			Self::deposit_event(Event::ActionQueued(indra, next_session));
			Ok(())
		}
		...
		/// used by some indracore or removed from the storage to avoid storage leaks. For the latter
		/// prefer to use the `poke_unused_validation_code` dispatchable to raw storage manipulation.
		///
		/// This function is mainly meant to be used for upgrading indracores that do not follow
		...
		/// that was suddenly got used by some indracore while this dispatchable was pending
		/// dispatching.
...
impl<T: Config> Pallet<T> {
	/// Called by the initializer to initialize the indras pallet.
	pub(crate) fn initializer_initialize(now: T::BlockNumber) -> Weight {
		let weight = Self::prune_old_code(now);
		weight + Self::process_scheduled_upgrade_changes(now)
	}

	/// Called by the initializer to finalize the indras pallet.
	...
	///
	/// Returns the list of outgoing indras from the actions queue.
	pub(crate) fn initializer_on_new_session(
		notification: &SessionChangeNotification<T::BlockNumber>,
	) -> Vec<IndraId> {
		let outgoing_indras = Self::apply_actions_queue(notification.session_index);
		Self::groom_ongoing_pvf_votes(&notification.new_config, notification.validators.len());
		outgoing_indras
	}

	/// The validation code of live indra.
	pub(crate) fn current_code(indra_id: &IndraId) -> Option<ValidationCode> {
		Self::current_code_hash(indra_id).and_then(|code_hash| {
			let code = CodeByHash::<T>::get(&code_hash);
			if code.is_none() {
				log::error!(
					"Pallet indras storage is inconsistent, code not found for hash {}",
					code_hash,
				);
				debug_assert!(false, "inconsistent indras storages");
			}
			code
		})
	}
	...
	//
	// Returns the list of outgoing indras from the actions queue.
	fn apply_actions_queue(session: SessionIndex) -> Vec<IndraId> {
		let actions = ActionsQueue::<T>::take(session);
		let mut indracores = IndracoresCache::new();
		let now = <frame_system::Pallet<T>>::block_number();
		let mut outgoing = Vec::new();

		for indra in actions {
			let lifecycle = IndraLifecycles::<T>::get(&indra);
			match lifecycle {
				...
				Some(IndraLifecycle::UpgradingIndrabase) => {
					indracores.add(indra);
					IndraLifecycles::<T>::insert(&indra, IndraLifecycle::Indracore);
				},
				// Downgrade a indracore to a indrabase
				Some(IndraLifecycle::DowngradingIndracore) => {
					indracores.remove(indra);
					IndraLifecycles::<T>::insert(&indra, IndraLifecycle::Indrabase);
				},
				// Offboard a indrabase or indracore from the system
				Some(IndraLifecycle::OffboardingIndracore) |
				Some(IndraLifecycle::OffboardingIndrabase) => {
					indracores.remove(indra);

					<Self as Store>::Heads::remove(&indra);
					<Self as Store>::FutureCodeUpgrades::remove(&indra);
					<Self as Store>::UpgradeGoAheadSignal::remove(&indra);
					<Self as Store>::UpgradeRestrictionSignal::remove(&indra);
					IndraLifecycles::<T>::remove(&indra);
					let removed_future_code_hash = <Self as Store>::FutureCodeHash::take(&indra);
					if let Some(removed_future_code_hash) = removed_future_code_hash {
						Self::decrease_code_ref(&removed_future_code_hash);
					}

					let removed_code_hash = <Self as Store>::CurrentCodeHash::take(&indra);
					if let Some(removed_code_hash) = removed_code_hash {
						Self::note_past_code(indra, now, now, removed_code_hash);
					}
		...
		if !outgoing.is_empty() {
			// Filter offboarded indracores from the upcoming upgrades and upgrade cooldowns list.
			//
			...
			<Self as Store>::UpcomingUpgrades::mutate(|upcoming_upgrades| {
				*upcoming_upgrades = mem::take(upcoming_upgrades)
					.into_iter()
					.filter(|&(ref indra, _)| !outgoing.contains(indra))
					.collect();
			});
		...
		// Persist indracores into the storage explicitly.
		drop(indracores);

		return outgoing
	}

	// note replacement of the code of indra with given `id`, which occured in the
	// context of the given relay-chain block number. provide the replaced code.
	//
	// `at` for indra-triggered replacement is the block number of the relay-chain
	// block in whose context the indrablock was executed
		...
		let pruning_tasks_done = <Self as Store>::PastCodePruning::mutate(
			|pruning_tasks: &mut Vec<(_, T::BlockNumber)>| {
				...
				for (indra_id, _) in pruning_tasks_to_do {
					...
					if full_deactivate {
						<Self as Store>::PastCodeMeta::remove(&indra_id);
					}
				}
	...
	fn process_scheduled_upgrade_changes(now: T::BlockNumber) -> Weight {
		...
		let upgrades_signaled = <Self as Store>::UpcomingUpgrades::mutate(
			|upcoming_upgrades: &mut Vec<(IndraId, T::BlockNumber)>| {
				let num = upcoming_upgrades.iter().take_while(|&(_, at)| at <= &now).count();
				for (indra, _) in upcoming_upgrades.drain(..num) {
					<Self as Store>::UpgradeGoAheadSignal::insert(&indra, UpgradeGoAhead::GoAhead);
				}
				num
			},
		);
	...
	fn process_scheduled_upgrade_cooldowns(now: T::BlockNumber) {
		<Self as Store>::UpgradeCooldowns::mutate(
			|upgrade_cooldowns: &mut Vec<(IndraId, T::BlockNumber)>| {
				for &(indra, _) in upgrade_cooldowns.iter().take_while(|&(_, at)| at <= &now) {
					<Self as Store>::UpgradeRestrictionSignal::remove(&indra);
				}
			},
		);
	...
	fn enact_pvf_accepted(
		...
		for cause in causes {
			...
			Self::deposit_event(Event::PvfCheckAccepted(*code_hash, cause.indra_id()));
	...
	fn proceed_with_onboarding(id: IndraId, sessions_observed: SessionIndex) -> Weight {
	...
	fn proceed_with_upgrade(
		...
		// applied.
		//
		// The first indrablock that has a relay-parent higher or at the same height of `expected_at`
		// will trigger the code upgrade. The indrablock that comes after that will be validated
		...
		// the last vote for pre-checking comes the indracore will have some time until the upgrade
		// finally takes place.
		...
		let log = ConsensusLog::IndraScheduleUpgradeCode(id, *code_hash, expected_at);
	...
	fn enact_pvf_rejected(
		...
		for cause in causes {
			...
			weight += T::DbWeight::get().reads_writes(3, 2);
			Self::deposit_event(Event::PvfCheckRejected(*code_hash, cause.indra_id()));

			match cause {
				PvfCheckCause::Onboarding(id) => {
					// Here we need to undo everything that was done during `schedule_indra_initialize`.
	...
	/// Verify that `schedule_indra_initialize` can be called successfully.
	///
	/// Returns false if indra is already registered in the system.
	pub fn can_schedule_indra_initialize(id: &IndraId) -> bool {
		IndraLifecycles::<T>::get(id).is_none()
	}

	/// Schedule a indra to be initialized. If the validation code is not already stored in the
	/// code storage, then a PVF pre-checking process will be initiated.
	///
	/// Only after the PVF pre-checking succeeds can the indra be onboarded. Note, that calling this
	/// does not guarantee that the indracore will eventually be onboarded. This can happen in case
	/// the PVF does not pass PVF pre-checking.
	///
	/// The Indra ID should be not activated in this pallet. The validation code supplied in
	/// `genesis_data` should not be empty. If those conditions are not met, then the indra cannot
	/// be onboarded.
	pub(crate) fn schedule_indra_initialize(
		id: IndraId,
		mut genesis_data: IndraGenesisArgs,
	) -> DispatchResult {
		// Make sure indracore isn't already in our system and that the onboarding parameters are
		// valid.
		ensure!(Self::can_schedule_indra_initialize(&id), Error::<T>::CannotOnboard);
		ensure!(!genesis_data.validation_code.0.is_empty(), Error::<T>::CannotOnboard);
		IndraLifecycles::<T>::insert(&id, IndraLifecycle::Onboarding);
		...
		// reason for the validation code to be copied into the `IndraGenesisArgs`. We also do not
		// want to risk it by copying the validation code needlessly to not risk adding more
		// memory pressure.
		//
		// That said, we also want to preserve `IndraGenesisArgs` as it is, for now. There are two
		// reasons:
		//
		...
		//   as long we do not allow registering indracores with empty code. At the moment of writing
	...
	///
	/// - indra is not a stable indracore or indrabase (i.e. [`IndraLifecycle::is_stable`] is `false`)
	/// - indra has a pending upgrade.
	///
	/// No-op if indra is not registered at all.
	pub(crate) fn schedule_indra_cleanup(id: IndraId) -> DispatchResult {
		...
		//
		// This implicitly assumes that the given indra exists, i.e. it's lifecycle != None.
		if FutureCodeHash::<T>::contains_key(&id) {
			return Err(Error::<T>::CannotOffboard.into())
		}

		let lifecycle = IndraLifecycles::<T>::get(&id);
		match lifecycle {
			// If indra is not registered, nothing to do!
			None => return Ok(()),
			Some(IndraLifecycle::Indrabase) => {
				IndraLifecycles::<T>::insert(&id, IndraLifecycle::OffboardingIndrabase);
			},
			Some(IndraLifecycle::Indracore) => {
				IndraLifecycles::<T>::insert(&id, IndraLifecycle::OffboardingIndracore);
			},
	...
	///
	/// Will return error if `IndraLifecycle` is not `Indrabase`.
	pub(crate) fn schedule_indrabase_upgrade(id: IndraId) -> DispatchResult {
		let scheduled_session = Self::scheduled_session();
		let lifecycle = IndraLifecycles::<T>::get(&id).ok_or(Error::<T>::NotRegistered)?;

		ensure!(lifecycle == IndraLifecycle::Indrabase, Error::<T>::CannotUpgrade);

		IndraLifecycles::<T>::insert(&id, IndraLifecycle::UpgradingIndrabase);
		ActionsQueue::<T>::mutate(scheduled_session, |v| {
			if let Err(i) = v.binary_search(&id) {
				v.insert(i, id);
			}
	...
	///
	/// Noop if `IndraLifecycle` is not `Indracore`.
	pub(crate) fn schedule_indracore_downgrade(id: IndraId) -> DispatchResult {
		let scheduled_session = Self::scheduled_session();
		let lifecycle = IndraLifecycles::<T>::get(&id).ok_or(Error::<T>::NotRegistered)?;

		ensure!(lifecycle == IndraLifecycle::Indracore, Error::<T>::CannotDowngrade);

		IndraLifecycles::<T>::insert(&id, IndraLifecycle::DowngradingIndracore);
		ActionsQueue::<T>::mutate(scheduled_session, |v| {
			if let Err(i) = v.binary_search(&id) {
				v.insert(i, id);
			}
	...
	/// Schedule a future code upgrade of the given indracore.
	///
	...
	/// If there is already a scheduled code upgrade for the indra, this is a no-op.
	pub(crate) fn schedule_code_upgrade(
		...
		if FutureCodeHash::<T>::contains_key(&id) {
			// This branch should never be reached. Signalling an upgrade is disallowed for a indra
		...
		// indra signals an update to the same code? This does not make a lot of sense, so abort the
		// process right away.
		//
		...
		if CurrentCodeHash::<T>::get(&id) == Some(code_hash) {
			...
			log::warn!(
				target: LOG_TARGET,
				"indra tried to upgrade to the same code. Abort the upgrade",
			);
			return weight
		}
	...
	fn kick_off_pvf_check(
		...
		weight += T::DbWeight::get().reads_writes(3, 2);
		Self::deposit_event(Event::PvfCheckStarted(code_hash, cause.indra_id()));
		...
		// We increase the code RC here in any case. Intuitively the indracore that requested this
	...
	/// Note that a indra has progressed to a new head, where the new head was executed in the context
	...
	pub(crate) fn note_new_head(
		...
		if let Some(expected_at) = <Self as Store>::FutureCodeUpgrades::get(&id) {
			if expected_at <= execution_context {
				...
				let log = ConsensusLog::IndraUpgradeCode(id, new_code_hash);
				<frame_system::Pallet<T>>::deposit_log(log.into());
				...
				let weight = if let Some(prior_code_hash) = maybe_prior_code_hash {
					...
					log::error!(target: LOG_TARGET, "Missing prior code hash for indra {:?}", &id);
					0 as Weight
				};
	...
	/// Returns the current lifecycle state of the indra.
	pub fn lifecycle(id: IndraId) -> Option<IndraLifecycle> {
		IndraLifecycles::<T>::get(&id)
	}

	/// Returns whether the given ID refers to a valid indra.
	///
	/// indras that are onboarding or offboarding are not included.
	pub fn is_valid_indra(id: IndraId) -> bool {
		if let Some(state) = IndraLifecycles::<T>::get(&id) {
	...
	/// Whether a indra ID corresponds to any live indracore.
	///
	/// Includes indracores which will downgrade to a indrabase in the future.
	pub fn is_indracore(id: IndraId) -> bool {
		if let Some(state) = IndraLifecycles::<T>::get(&id) {
			state.is_indracore()
		} else {
			false
	...
	/// Whether a indra ID corresponds to any live indrabase.
	///
	/// Includes indrabases which will upgrade to indracores in the future.
	pub fn is_indrabase(id: IndraId) -> bool {
		if let Some(state) = IndraLifecycles::<T>::get(&id) {
			state.is_indrabase()
		} else {
			false
	...
	/// If a candidate from the specified indracore were submitted at the current block, this
	/// function returns if that candidate passes the acceptance criteria.
	pub(crate) fn can_upgrade_validation_code(id: IndraId) -> bool {
	...
	pub fn heads_insert(indra_id: &IndraId, head_data: HeadData) {
		Heads::<T>::insert(indra_id, head_data);
	}

	/// A low-level function to eagerly initialize a given indra.
	pub(crate) fn initialize_indra_now(
		indracores: &mut IndracoresCache<T>,
		id: IndraId,
		genesis_data: &IndraGenesisArgs,
	) {
		if genesis_data.indracore {
			indracores.add(id);
			IndraLifecycles::<T>::insert(&id, IndraLifecycle::Indracore);
		} else {
			IndraLifecycles::<T>::insert(&id, IndraLifecycle::Indrabase);
		}

		// HACK: see the notice in `schedule_indra_initialize`.
		//
...
/// An overlay over the `Indracores` storage entry that provides a convenient interface for adding
/// or removing indracores in bulk.
pub(crate) struct IndracoresCache<T: Config> {
	// `None` here means the indracores list has not been accessed yet, nevermind modified.
	indracores: Option<Vec<IndraId>>,
	_config: PhantomData<T>,
}

impl<T: Config> IndracoresCache<T> {
	pub fn new() -> Self {
		Self { indracores: None, _config: PhantomData }
	}

	fn ensure_initialized(&mut self) -> &mut Vec<IndraId> {
		self.indracores.get_or_insert_with(|| Indracores::<T>::get())
	}

	/// Adds the given indra id to the list.
	pub fn add(&mut self, id: IndraId) {
		let indracores = self.ensure_initialized();
		if let Err(i) = indracores.binary_search(&id) {
			indracores.insert(i, id);
		}
	}

	/// Removes the given indra id from the list of indracores. Does nothing if the id is not in the
	/// list.
	pub fn remove(&mut self, id: IndraId) {
		let indracores = self.ensure_initialized();
		if let Ok(i) = indracores.binary_search(&id) {
			indracores.remove(i);
		}
	}
}

impl<T: Config> Drop for IndracoresCache<T> {
	fn drop(&mut self) {
		if let Some(indracores) = self.indracores.take() {
			Indracores::<T>::put(&indracores);
		}
	}
}
