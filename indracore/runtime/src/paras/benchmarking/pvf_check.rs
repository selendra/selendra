	...
	T: Config + shared::Config,
{
	...
	IndrasShared::<T>::set_active_validators_ascending(validators.clone());
	IndrasShared::<T>::set_session_index(SESSION_INDEX);
}
	...
	T: Config + shared::Config,
{
	for i in 0..CAUSES_NUM {
		...
		if vote_cause == VoteCause::Upgrade {
			...
			let r = Pallet::<T>::schedule_indra_initialize(
				id,
				IndraGenesisArgs {
					indracore: true,
					genesis_head: HeadData(vec![1, 2, 3, 4]),
...
where
	T: Config + shared::Config,
{
	let validators = IndrasShared::<T>::active_validator_keys();
