...
use primitives::v2::{HeadData, Id as IndraId, ValidationCode, MAX_CODE_SIZE, MAX_HEAD_DATA_SIZE};
...
// round number.
// Due to the limited number of indracores, the number of pruning, upcoming upgrades and cooldowns
...
benchmarks! {
	force_set_current_code {
		...
		let indra_id = IndraId::from(c as u32);
		<Pallet<T> as Store>::CurrentCodeHash::insert(&indra_id, new_code.hash());
		generate_disordered_pruning::<T>();
	}: _(RawOrigin::Root, indra_id, new_code)
	verify {
		assert_last_event::<T>(Event::CurrentCodeUpdated(indra_id).into());
	}
	force_set_current_head {
		let s in 1 .. MAX_HEAD_DATA_SIZE;
		let new_head = HeadData(vec![0; s as usize]);
		let indra_id = IndraId::from(1000);
	}: _(RawOrigin::Root, indra_id, new_head)
	verify {
		assert_last_event::<T>(Event::CurrentHeadUpdated(indra_id).into());
	}
	...
	}: _(RawOrigin::Root, indra_id, new_code, block)
	verify {
		assert_last_event::<T>(Event::CodeUpgradeScheduled(indra_id).into());
	}
	force_note_new_head {
		let s in 1 .. MAX_HEAD_DATA_SIZE;
		let indra_id = IndraId::from(1000);
		...
		<Pallet<T> as Store>::CurrentCodeHash::insert(&indra_id, old_code_hash);
		// schedule an expired code upgrade for this `indra_id` so that force_note_new_head would use
		...
		generate_disordered_pruning::<T>();
		Pallet::<T>::schedule_code_upgrade(indra_id, ValidationCode(vec![0]), expired, &config);
	}: _(RawOrigin::Root, indra_id, new_head)
	verify {
		assert_last_event::<T>(Event::NewHeadNoted(indra_id).into());
	}
	force_queue_action {
		let indra_id = IndraId::from(1000);
		generate_disordered_actions_queue::<T>();
	}: _(RawOrigin::Root, indra_id)
	verify {
		let next_session = crate::shared::Pallet::<T>::session_index().saturating_add(One::one());
		assert_last_event::<T>(Event::ActionQueued(indra_id, next_session).into());
	}
