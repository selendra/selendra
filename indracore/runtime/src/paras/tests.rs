...
use primitives::v2::{BlockNumber, ValidatorId, INDRACORE_KEY_TYPE_ID};
...
fn run_to_block(to: BlockNumber, new_session: Option<Vec<BlockNumber>>) {
	...
	while System::block_number() < to {
		...
		if new_session.as_ref().map_or(false, |v| v.contains(&(b + 1))) {
			...
			session_change_notification.session_index = IndrasShared::session_index() + 1;
...
impl EventValidator {
	...
	fn started(&mut self, code: &ValidationCode, id: IndraId) -> &mut Self {
	...
	fn rejected(&mut self, code: &ValidationCode, id: IndraId) -> &mut Self {
		self.events.push(frame_system::EventRecord {
			phase: frame_system::Phase::Initialization,
	...
	fn accepted(&mut self, code: &ValidationCode, id: IndraId) -> &mut Self {
		self.events.push(frame_system::EventRecord {
			phase: frame_system::Phase::Initialization,
...
fn upgrade_restriction_elapsed_doesnt_mean_can_upgrade() {
	// Situation: indracore scheduled upgrade but it doesn't produce any candidate after
	// `expected_at`. When `validation_upgrade_cooldown` elapsed the indracore produces a
...
#[test]
fn full_indracore_cleanup_storage() {
	...
	new_test_ext(genesis_config).execute_with(|| {
		...
		// cleaning up the indracore should place the current indracore code
...
fn pvf_check_coalescing_onboarding_and_upgrade() {
	...
	let a = IndraId::from(111);
	let b = IndraId::from(222);
	...
	new_test_ext(genesis_config).execute_with(|| {
		...
		// Relay parent of the indrablock that schedules the upgrade.
...
fn add_trusted_validation_code_insta_approval() {
	...
	new_test_ext(genesis_config).execute_with(|| {
		...
		// Verify that the required events were emitted.
		EventValidator::new()
			.started(&validation_code, indra_id)
			.accepted(&validation_code, indra_id)
			.check();
	});
}
