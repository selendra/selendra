...
use crate::{
	...
	mock::{
		new_test_ext, Configuration, IndrasShared, MockGenesisConfig, Origin, SessionInfo, System,
		Test,
	},
...
fn session_info_active_subsets() {
	...
	new_test_ext(genesis_config()).execute_with(|| {
		IndrasShared::set_active_validators_with_indices(active_set.clone(), validators.clone());

		assert_eq!(IndrasShared::active_validator_indices(), active_set);
