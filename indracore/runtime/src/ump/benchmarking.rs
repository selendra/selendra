...
fn queue_upward_msg<T: Config>(
	...
	let msgs = vec![msg];
	Ump::<T>::check_upward_messages(host_conf, indra, &msgs).unwrap();
	let _ = Ump::<T>::receive_upward_messages(indra, msgs);
	assert_last_event_type::<T>(Event::UpwardMessagesReceived(indra, 1, len).into());
}
...
frame_benchmarking::benchmarks! {
	...
	process_upward_message {
		...
		let indra = IndraId::from(1978);
		let data = create_message_min_size::<T>(s);
	}: {
		assert!(T::UmpSink::process_upward_message(indra, &data[..], Weight::MAX).is_ok());
	}
	...
	clean_ump_after_outgoing {
		...
		// fill the queue, each message has it's own indra-id.
		for id in 0..count {
			queue_upward_msg::<T>(&host_conf, IndraId::from(id), msg.clone());
		}
	}: {
		Ump::<T>::clean_ump_after_outgoing(&IndraId::from(0));
	}
	...
	service_overweight {
		...
		// emitted during the genesis block they will be implicitly wiped.
		frame_system::Pallet::<T>::set_block_number(1u32.into());
		queue_upward_msg::<T>(&host_conf, indra, msg.clone());
		Ump::<T>::process_pending_upward_messages();
		assert_last_event_type::<T>(
			Event::OverweightEnqueued(indra, upward_message_id(&msg), 0, 0).into()
			);
