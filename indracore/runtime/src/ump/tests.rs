...
fn queue_upward_msg(indra: IndraId, msg: UpwardMessage) {
	let msgs = vec![msg];
	assert!(Ump::check_upward_messages(&Configuration::config(), indra, &msgs).is_ok());
	let _ = Ump::receive_upward_messages(indra, msgs);
}

fn assert_storage_consistency_exhaustive() {
	// check that empty queues don't clutter the storage.
	for (_indra, queue) in <Ump as Store>::RelayDispatchQueues::iter() {
		assert!(!queue.is_empty());
	}

	// actually count the counts and sizes in queues and compare them to the bookkept version.
	for (indra, queue) in <Ump as Store>::RelayDispatchQueues::iter() {
		let (expected_count, expected_size) = <Ump as Store>::RelayDispatchQueueSize::get(indra);
	...
	// since we wipe the empty queues the sets of indras in queue contents, queue sizes and
	// need dispatch set should all be equal.
	let queue_contents_set = <Ump as Store>::RelayDispatchQueues::iter()
		.map(|(k, _)| k)
		.collect::<HashSet<IndraId>>();
	let queue_sizes_set = <Ump as Store>::RelayDispatchQueueSize::iter()
		.map(|(k, _)| k)
		.collect::<HashSet<IndraId>>();
	let needs_dispatch_set =
		<Ump as Store>::NeedsDispatch::get().into_iter().collect::<HashSet<IndraId>>();
	assert_eq!(queue_contents_set, queue_sizes_set);
	assert_eq!(queue_contents_set, needs_dispatch_set);

	// `NextDispatchRoundStartWith` should point into a indra that is tracked.
	if let Some(indra) = <Ump as Store>::NextDispatchRoundStartWith::get() {
		assert!(queue_contents_set.contains(&indra));
	}
...
fn dispatch_resume_after_exceeding_dispatch_stage_weight() {
	let a = IndraId::from(128);
	let c = IndraId::from(228);
	let q = IndraId::from(911);
...
fn dispatch_correctly_handle_remove_of_latest() {
	let a = IndraId::from(1991);
	let b = IndraId::from(1999);
	...
	.execute_with(|| {
		...
		// indra id (i.e. last in the `needs_dispatch` order).
		//
...
fn overweight_queue_works() {
	...
	.execute_with(|| {
		...
		queue_upward_msg(indra_a, a_msg_1.clone());
		Ump::process_pending_upward_messages();
		assert_eq!(take_processed(), vec![(indra_a, a_msg_1)]);

		// This is overweight and this message cannot fit into the total weight budget.
		queue_upward_msg(indra_a, a_msg_2.clone());
		queue_upward_msg(indra_a, a_msg_3.clone());
		Ump::process_pending_upward_messages();
		assert_last_event(
			Event::OverweightEnqueued(indra_a, upward_message_id(&a_msg_3[..]), 0, 500).into(),
		);
