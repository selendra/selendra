...
fn queue_downward_message(
	...
	Dmp::queue_downward_message(&Configuration::config(), indra_id, msg)
}

#[test]
fn clean_dmp_works() {
	let a = IndraId::from(1312);
	...
	new_test_ext(default_genesis_config()).execute_with(|| {
		...
		let outgoing_indras = vec![a, b];
		Dmp::initializer_on_new_session(&notification, &outgoing_indras);
