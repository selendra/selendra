...
fn sets_and_shuffles_validators() {
	...
	new_test_ext(MockGenesisConfig::default()).execute_with(|| {
		let validators = IndrasShared::initializer_on_new_session(1, [1; 32], &config, pubkeys);
		...
		assert_eq!(IndrasShared::active_validator_keys(), validators);

		assert_eq!(
			IndrasShared::active_validator_indices(),
			vec![
				ValidatorIndex(4),
				ValidatorIndex(1),
				ValidatorIndex(2),
