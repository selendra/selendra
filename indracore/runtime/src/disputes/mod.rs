...
//! Runtime component for handling disputes of indracore candidates.
...
pub trait PunishValidators {
	/// Punish a series of validators who were for an invalid indrablock. This is expected to be a major
	/// punishment.
	...
	/// Punish a series of validators who were against a valid indrablock. This is expected to be a minor
	/// punishment.
...
///
/// Allows decoupling indracores handling from disputes so that it can
...
pub trait DisputesHandler<BlockNumber: Ord> {
	...
	/// any new indracore blocks for backing or inclusion.
	fn is_frozen() -> bool;
...
pub mod pallet {
	...
	/// the chain will not accept any new indracore blocks for backing or inclusion,
	...
	pub enum Event<T: Config> {
		...
		/// `\[indra id, candidate hash, dispute result\]`
		DisputeConcluded(CandidateHash, DisputeResult),
		/// A dispute has timed out due to insufficient participation.
		/// `\[indra id, candidate hash\]`
