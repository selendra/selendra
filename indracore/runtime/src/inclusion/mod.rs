...
//! The inclusion pallet is responsible for inclusion and availability of scheduled indracores
//! and indrabases.
//!
...
use primitives::v2::{
	...
	HeadData, Id as IndraId, SigningContext, UncheckedSignedAvailabilityBitfields, ValidatorId,
...
pub mod pallet {
	...
	/// Candidates pending availability by `IndraId`.
	#[pallet::storage]
	pub(crate) type PendingAvailability<T: Config> =
		StorageMap<_, Twox64Concat, IndraId, CandidatePendingAvailability<T::Hash, T::BlockNumber>>;

	/// The commitments of candidates pending availability, by `IndraId`.
	#[pallet::storage]
	pub(crate) type PendingAvailabilityCommitments<T: Config> =
		StorageMap<_, Twox64Concat, IndraId, CandidateCommitments>;
...
impl<T: Config> Pallet<T> {
	...
	where
		F: Fn(CoreIndex) -> Option<IndraId>,
	{
		let mut assigned_indras_record = (0..expected_bits)
			.map(|bit_index| core_lookup(CoreIndex::from(bit_index as u32)))
			.map(|opt_indra_id| {
				opt_indra_id.map(|indra_id| (indra_id, PendingAvailability::<T>::get(&indra_id)))
			})
			.collect::<Vec<_>>();
		...
		for (checked_bitfield, validator_index) in
			...
			for (bit_idx, _) in checked_bitfield.0.iter().enumerate().filter(|(_, is_av)| **is_av) {
				let pending_availability = if let Some((_, pending_availability)) =
					assigned_indras_record[bit_idx].as_mut()
				{
		...
		for (indra_id, pending_availability) in assigned_indras_record
			.into_iter()
			.filter_map(|x| x)
			.filter_map(|(id, p)| p.map(|p| (id, p)))
		{
			if pending_availability.availability_votes.count_ones() >= threshold {
				<PendingAvailability<T>>::remove(&indra_id);
				let commitments = match PendingAvailabilityCommitments::<T>::take(&indra_id) {
				...
				<PendingAvailability<T>>::insert(&indra_id, &pending_availability);
			}
		}
	...
	pub(crate) fn process_bitfields(
		...
		core_lookup: impl Fn(CoreIndex) -> Option<IndraId>,
		...
		GV: Fn(GroupIndex) -> Option<Vec<ValidatorIndex>>,
	{
		...
		let core_indices_and_backers = {
				...
				candidates.iter().enumerate()
			{
				...
				let indra_id = backed_candidate.descriptor().indra_id;
				let mut backers = bitvec::bitvec![u8, BitOrderLsb0; 0; validators.len()];

				for (i, assignment) in scheduled[skip..].iter().enumerate() {
					check_assignment_in_order(assignment)?;

					if indra_id == assignment.indra_id {
			...
			candidates.into_iter().zip(core_indices_and_backers)
		{
			...
			<PendingAvailabilityCommitments<T>>::insert(&indra_id, commitments);
		}
	...
	pub(crate) fn check_validation_outputs_for_runtime_api(
		...
		if let Err(err) = check_ctx.check_validation_outputs(
			...
			log::debug!(
				target: LOG_TARGET,
				"Validation outputs checking for indracore `{}` failed: {:?}",
				u32::from(indra_id),
				err,
			);
			false
	...
	/// Cleans up all indras pending availability that the predicate returns true for.
	///
	...
	pub(crate) fn collect_pending(
		...
		for (indra_id, pending_record) in <PendingAvailability<T>>::iter() {
			if pred(pending_record.core, pending_record.backed_in_number) {
				cleaned_up_ids.push(indra_id);
				cleaned_up_cores.push(pending_record.core);
			}
		}

		for indra_id in cleaned_up_ids {
			let pending = <PendingAvailability<T>>::take(&indra_id);
			let commitments = <PendingAvailabilityCommitments<T>>::take(&indra_id);
	...
	/// Cleans up all indras pending availability that are in the given list of disputed candidates.
	///
	/// Returns a vector of cleaned-up core IDs.
	pub(crate) fn collect_disputed(disputed: &BTreeSet<CandidateHash>) -> Vec<CoreIndex> {
		...
		for indra_id in cleaned_up_ids {
			let _ = <PendingAvailability<T>>::take(&indra_id);
			let _ = <PendingAvailabilityCommitments<T>>::take(&indra_id);
		}
	...
	/// by bitfields.
	///
	/// Is a no-op if there is no candidate pending availability for this indra-id.
	...
	pub(crate) fn force_enact(indra: IndraId) {
		let pending = <PendingAvailability<T>>::take(&indra);
		let commitments = <PendingAvailabilityCommitments<T>>::take(&indra);
	...
	/// Returns the `CommittedCandidateReceipt` pending availability for the indra provided, if any.
	pub(crate) fn candidate_pending_availability(
		indra: IndraId,
	) -> Option<CommittedCandidateReceipt<T::Hash>> {
		<PendingAvailability<T>>::get(&indra)
			.map(|p| p.descriptor)
			.and_then(|d| <PendingAvailabilityCommitments<T>>::get(&indra).map(move |c| (d, c)))
...
impl<T: Config> CandidateCheckContext<T> {
	...
	///  * indra head in the descriptor and commitments match
	pub(crate) fn verify_backed_candidate(
		...
		{
			// this should never fail because the indra is registered
		...
		if let Err(err) = self.check_validation_outputs(
			...
			log::debug!(
				target: LOG_TARGET,
				"Validation outputs checking during inclusion of a candidate {} for indracore `{}` failed: {:?}",
				candidate_idx,
				u32::from(indra_id),
				err,
			);
	...
	fn check_validation_outputs(
		...
		horizontal_messages: &[primitives::v2::OutboundHrmpMessage<IndraId>],
