...
fn default_availability_votes() -> BitVec<u8, BitOrderLsb0> {
	bitvec::bitvec![u8, BitOrderLsb0; 0; IndrasShared::active_validator_keys().len()]
}
...
pub(crate) struct TestCandidateBuilder {
	pub(crate) indra_id: IndraId,
	pub(crate) head_data: HeadData,
	pub(crate) indra_head_hash: Option<Hash>,
	...
	pub(crate) fn build(self) -> CommittedCandidateReceipt {
		CommittedCandidateReceipt {
			descriptor: CandidateDescriptor {
				...
				indra_head: self.indra_head_hash.unwrap_or_else(|| self.head_data.hash()),
...
pub(crate) fn make_vdata_hash(indra_id: IndraId) -> Option<Hash> {
...
fn supermajority_bitfields_trigger_availability() {
	...
	new_test_ext(genesis_config(paras)).execute_with(|| {
		...
		let core_lookup = |core| match core {
			...
			core if core == CoreIndex::from(1) => Some(chain_b),
			core if core == CoreIndex::from(2) => Some(thread_a),
			_ => panic!("Core out of bounds for 2 indracores and 1 indrabase core."),
		};
...
fn candidate_checks() {
	...
	new_test_ext(genesis_config(paras)).execute_with(|| {
		...
		let thread_a_assignment = CoreAssignment {
			...
			indra_id: thread_a,
			kind: AssignmentKind::Indrabase(thread_collator.clone(), 0),
		...
		// Indra head hash in descriptor doesn't match head data
		{
			let mut candidate = TestCandidateBuilder {
				indra_id: chain_a,
				relay_parent: System::parent_hash(),
				...
				persisted_validation_data_hash: make_vdata_hash(chain_a).unwrap(),
				hrmp_watermark: RELAY_PARENT_NUM,
				indra_head_hash: Some(Hash::random()),
				..Default::default()
			}
			.build();
	...
	new_test_ext(genesis_config(paras)).execute_with(|| {
		...
		let assure_candidate_sorting = |mut candidate_receipts_with_backers: Vec<(
			...
			candidate_receipts_with_backers.sort_by(|(cr1, _), (cr2, _)| {
				cr1.descriptor().indra_id.cmp(&cr2.descriptor().indra_id)
			});
	...
	new_test_ext(genesis_config(paras)).execute_with(|| {
		...
		let group_validators = |group_index: GroupIndex| {
			match group_index {
				...
				_ => panic!("Group index out of bounds for 1 indracore"),
			}
			.map(|vs| vs.into_iter().map(ValidatorIndex).collect::<Vec<_>>())
		};
