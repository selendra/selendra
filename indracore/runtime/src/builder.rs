...
use primitives::v2::{
	...
	HeadData, Id as IndraId, InherentData as IndracoresInherentData, InvalidDisputeStatementKind,
...
impl<T: paras_inherent::Config> BenchBuilder<T> {
	...
	/// Set a map from core/indra id seed to number of validity votes.
	...
	/// Maximum number of validators participating in indracores consensus (a.k.a. active validators).
	...
	/// Specify a mapping of core index/ indra id to the number of dispute statements for the
	...
	/// Create indra id, core index, and grab the associated group index from the scheduler pallet.
	fn create_indexes(&self, seed: u32) -> (IndraId, CoreIndex, GroupIndex) {
		let indra_id = IndraId::from(seed);
		let core_idx = CoreIndex(seed);
		let group_idx =
			scheduler::Pallet::<T>::group_assigned_to_core(core_idx, self.block_number).unwrap();

		(indra_id, core_idx, group_idx)
	}
	...
	fn add_availability(
		...
		inclusion::PendingAvailability::<T>::insert(indra_id, candidate_availability);
		inclusion::PendingAvailabilityCommitments::<T>::insert(&indra_id, commitments);
	}
	...
	/// Register `cores` count of indracores.
	///
	...
	fn setup_indra_ids(cores: u32) {
		// make sure indracores exist prior to session change.
		for i in 0..cores {
	...
	fn create_availability_bitfields(
		...
		for (seed, _) in concluding_cores.iter() {
			...
			let (indra_id, core_idx, group_idx) = self.create_indexes(seed.clone());
	...
	/// scheduled _within_ indras inherent, which requires marking the available bitfields as fully
	/// available.
	/// - `cores_with_backed_candidates` Mapping of `indra_id`/`core_idx`/`group_idx` seed to number of
	/// validity votes.
	fn create_backed_candidates(
		...
		cores_with_backed_candidates
			...
			.map(|(seed, num_votes)| {
				...
				let (indra_id, _core_idx, group_idx) = self.create_indexes(seed.clone());
	...
	fn create_disputes_with_no_spam(
		...
		(start..last)
			.map(|seed| {
				...
				let (indra_id, core_idx, group_idx) = self.create_indexes(seed);
	...
	pub(crate) fn build(self) -> Bench<T> {
		...
		Self::setup_indra_ids(used_cores);
