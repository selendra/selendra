...
/// Enumerates the phase in the setup process of a channel between two indracores.
enum IndracoreSetupStep {
...
fn establish_indra_connection<T: Config>(
	from: u32,
	to: u32,
	until: IndracoreSetupStep,
) -> [(IndraId, crate::Origin); 2]
where
	<T as frame_system::Config>::Origin: From<crate::Origin>,
{
	...
	let sender: IndraId = from.into();
	let sender_origin: crate::Origin = from.into();

	let recipient: IndraId = to.into();
...
/// Prefix value for account generation. These numbers are used as seeds to create distinct (indra)
/// accounts.
///
...
frame_benchmarking::benchmarks! {
	...
	hrmp_init_open_channel {
		let sender_id: IndraId = 1u32.into();
		let sender_origin: crate::Origin = 1u32.into();

		let recipient_id: IndraId = 2u32.into();

		// make sure indra is registered, and has enough balance.
		let deposit: BalanceOf<T> = Configuration::<T>::config().hrmp_sender_deposit.unique_saturated_into();
		register_indracore_with_balance::<T>(sender_id, deposit);
		register_indracore_with_balance::<T>(recipient_id, deposit);
	...
	hrmp_accept_open_channel {
		let [(sender, _), (recipient, recipient_origin)] =
			establish_indra_connection::<T>(1, 2, IndracoreSetupStep::Requested);
	...
	hrmp_close_channel {
		let [(sender, sender_origin), (recipient, _)] =
			establish_indra_connection::<T>(1, 2, IndracoreSetupStep::Established);
		let channel_id = HrmpChannelId { sender, recipient };
	...
	// NOTE: a single indracore should have the maximum number of allowed ingress and egress
	// channels.
	force_clean_hrmp {
		// ingress channels to a single leaving indracore that need to be closed.
		let i in 0 .. (HRMP_MAX_INBOUND_CHANNELS_BOUND - 1);
		// egress channels to a single leaving indracore that need to be closed.
		let e in 0 .. (HRMP_MAX_OUTBOUND_CHANNELS_BOUND - 1);

		// first, update the configs to support this many open channels...
		assert_ok!(
			Configuration::<T>::set_hrmp_max_indracore_outbound_channels(frame_system::RawOrigin::Root.into(), e + 1)
		);
		assert_ok!(
			Configuration::<T>::set_hrmp_max_indracore_inbound_channels(frame_system::RawOrigin::Root.into(), i + 1)
		);
		// .. and enact it.
		...
		let indra: IndraId = 1u32.into();
		let indra_origin: crate::Origin = 1u32.into();
		register_indracore_with_balance::<T>(indra, deposit);
		T::Currency::make_free_balance_be(&indra.into_account_truncating(), deposit * 256u32.into());

		for ingress_indra_id in 0..i {
			// establish ingress channels to `indra`.
			let ingress_indra_id = ingress_indra_id + PREFIX_0;
			let _ = establish_indra_connection::<T>(ingress_indra_id, indra.into(), IndracoreSetupStep::Established);
		}

		// nothing should be left unprocessed.
		assert_eq!(HrmpOpenChannelRequestsList::<T>::decode_len().unwrap_or_default(), 0);

		for egress_indra_id in 0..e {
			// establish egress channels to `indra`.
			let egress_indra_id = egress_indra_id + PREFIX_1;
			let _ = establish_indra_connection::<T>(indra.into(), egress_indra_id, IndracoreSetupStep::Established);
		}
	...
	}: _(frame_system::Origin::<T>::Root, indra, i, e) verify {
	...
	force_process_hrmp_open {
		...
		for id in 0 .. c {
			let _ = establish_indra_connection::<T>(PREFIX_0 + id, PREFIX_1 + id, IndracoreSetupStep::Accepted);
		}
	...
	force_process_hrmp_close {
		...
		for id in 0 .. c {
			let _ = establish_indra_connection::<T>(PREFIX_0 + id, PREFIX_1 + id, IndracoreSetupStep::CloseRequested);
		}
	...
	hrmp_cancel_open_request {
		...
		for id in 0 .. c {
			let _ = establish_indra_connection::<T>(PREFIX_0 + id, PREFIX_1 + id, IndracoreSetupStep::Requested);
		}
	...
	// worse case will be `n` indracore channel requests, where in all of them either the sender or
	...
	clean_open_channel_requests {
		...
		assert_eq!(HrmpOpenChannelRequestsList::<T>::decode_len().unwrap_or_default() as u32, c);
		let outgoing = (0..c).map(|id| (id + PREFIX_1).into()).collect::<Vec<IndraId>>();
		let config = Configuration::<T>::config();
	}: {
