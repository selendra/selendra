	...
	while System::block_number() < to {
		...
		if new_session.as_ref().map_or(false, |v| v.contains(&(b + 1))) {
			let notification = crate::initializer::SessionChangeNotification {
				...
				session_index: IndrasShared::session_index() + 1,
				..Default::default()
			};

			// NOTE: this is in initialization order.
			IndrasShared::initializer_on_new_session(
...
pub(super) struct GenesisConfigBuilder {
	...
	hrmp_max_indrabase_outbound_channels: u32,
	hrmp_max_indracore_outbound_channels: u32,
	hrmp_max_indrabase_inbound_channels: u32,
	hrmp_max_indracore_inbound_channels: u32,
...
impl Default for GenesisConfigBuilder {
	fn default() -> Self {
		Self {
			...
			hrmp_max_indrabase_outbound_channels: 1,
			hrmp_max_indracore_outbound_channels: 2,
			hrmp_max_indrabase_inbound_channels: 1,
			hrmp_max_indracore_inbound_channels: 2,
...
impl GenesisConfigBuilder {
	pub(super) fn build(self) -> crate::mock::MockGenesisConfig {
		...
		config.hrmp_max_indrabase_outbound_channels = self.hrmp_max_indrabase_outbound_channels;
		config.hrmp_max_indracore_outbound_channels = self.hrmp_max_indracore_outbound_channels;
		config.hrmp_max_indrabase_inbound_channels = self.hrmp_max_indrabase_inbound_channels;
		config.hrmp_max_indracore_inbound_channels = self.hrmp_max_indracore_inbound_channels;
...
fn register_indracore(id: IndraId) {
	register_indracore_with_balance(id, 1000);
}
...
fn open_channel_works() {
	let indra_a = 1.into();
	let indra_a_origin: crate::Origin = 1.into();
	let indra_b = 3.into();
	let indra_b_origin: crate::Origin = 3.into();

	new_test_ext(GenesisConfigBuilder::default().build()).execute_with(|| {
		// We need both A & B to be registered and alive indracores.
		register_indracore(indra_a);
		register_indracore(indra_b);

		run_to_block(5, Some(vec![4, 5]));
		Hrmp::hrmp_init_open_channel(indra_a_origin.into(), indra_b, 2, 8).unwrap();
		Hrmp::assert_storage_consistency_exhaustive();
		assert!(System::events().iter().any(|record| record.event ==
			MockEvent::Hrmp(Event::OpenChannelRequested(indra_a, indra_b, 2, 8))));

		Hrmp::hrmp_accept_open_channel(indra_b_origin.into(), indra_a).unwrap();
		Hrmp::assert_storage_consistency_exhaustive();
		assert!(System::events()
			.iter()
			.any(|record| record.event ==
				MockEvent::Hrmp(Event::OpenChannelAccepted(indra_a, indra_b))));
...
fn close_channel_works() {
	let indra_a = 5.into();
	let indra_b = 2.into();
	let indra_b_origin: crate::Origin = 2.into();
	...
	new_test_ext(GenesisConfigBuilder::default().build()).execute_with(|| {
		...
		// session change.
		let channel_id = HrmpChannelId { sender: indra_a, recipient: indra_b };
		Hrmp::hrmp_close_channel(indra_b_origin.into(), channel_id.clone()).unwrap();
		assert!(channel_exists(indra_a, indra_b));
		Hrmp::assert_storage_consistency_exhaustive();
		...
		Hrmp::assert_storage_consistency_exhaustive();
		assert!(System::events().iter().any(|record| record.event ==
			MockEvent::Hrmp(Event::ChannelClosed(indra_b, channel_id.clone()))));
	});
}
...
fn send_recv_messages() {
	...
	new_test_ext(genesis.build()).execute_with(|| {
		...
		run_to_block(5, Some(vec![4, 5]));
		Hrmp::init_open_channel(indra_a, indra_b, 2, 20).unwrap();
		Hrmp::accept_open_channel(indra_b, indra_a).unwrap();
		...
		run_to_block(7, None);
		assert!(Hrmp::check_hrmp_watermark(indra_b, 7, 6).is_ok());
		let _ = Hrmp::prune_hrmp(indra_b, 6);
		Hrmp::assert_storage_consistency_exhaustive();
	});
}
...
fn hrmp_mqc_head_fixture() {
	let indra_a = 2000.into();
	let indra_b = 2024.into();
	...
	new_test_ext(genesis.build()).execute_with(|| {
		...
		let _ = Hrmp::queue_outbound_hrmp(
			indra_a,
			vec![OutboundHrmpMessage { recipient: indra_b, data: vec![1, 2, 3] }],
		);

		run_to_block(4, None);
		let _ = Hrmp::queue_outbound_hrmp(
			indra_a,
			vec![OutboundHrmpMessage { recipient: indra_b, data: vec![4, 5, 6] }],
		);

		assert_eq!(
			Hrmp::hrmp_mqc_heads(indra_b),
			vec![(
				indra_a,
				hex_literal::hex![
...
fn check_sent_messages() {
	...
	new_test_ext(GenesisConfigBuilder::default().build()).execute_with(|| {
		...
		Hrmp::init_open_channel(indra_a, indra_b, 2, 8).unwrap();
		Hrmp::accept_open_channel(indra_b, indra_a).unwrap();
		Hrmp::init_open_channel(indra_c, indra_b, 2, 8).unwrap();
		Hrmp::accept_open_channel(indra_b, indra_c).unwrap();
		...
		let config = Configuration::config();
		assert!(Hrmp::check_outbound_hrmp(&config, indra_a, &msgs).is_ok());
		let _ = Hrmp::queue_outbound_hrmp(indra_a, msgs.clone());

		// Verify that the sent messages are there and that also the empty channels are present.
		let mqc_heads = Hrmp::hrmp_mqc_heads(indra_b);
		let contents = Hrmp::inbound_hrmp_channels_contents(indra_b);
		assert_eq!(
			contents,
			vec![
				(indra_a, vec![InboundHrmpMessage { sent_at: 6, data: b"knock".to_vec() }]),
				(indra_c, vec![])
			]
			.into_iter()
...
fn verify_externally_accessible() {
	...
	new_test_ext(GenesisConfigBuilder::default().build()).execute_with(|| {
		// Register two indracores, wait until a session change, then initiate channel open
		...
		let raw_ingress_index =
			sp_io::storage::get(&well_known_keys::hrmp_ingress_channel_index(indra_b))
				.expect("the ingress index must be present for indra_b");
		let ingress_index = <Vec<IndraId>>::decode(&mut &raw_ingress_index[..])
			.expect("ingress indexx should be decodable as a list of indra ids");
		assert_eq!(ingress_index, vec![indra_a]);

		// Now, verify that we can access and decode the egress index.
		let raw_egress_index =
			sp_io::storage::get(&well_known_keys::hrmp_egress_channel_index(indra_a))
				.expect("the egress index must be present for indra_a");
		let egress_index = <Vec<IndraId>>::decode(&mut &raw_egress_index[..])
			.expect("egress index should be decodable as a list of indra ids");
		assert_eq!(egress_index, vec![indra_b]);
	});
}
...
fn charging_deposits() {
	...
	new_test_ext(GenesisConfigBuilder::default().build()).execute_with(|| {
		register_indracore_with_balance(indra_a, 0);
		register_indracore(indra_b);
		run_to_block(5, Some(vec![4, 5]));

		assert_noop!(
			Hrmp::init_open_channel(indra_a, indra_b, 2, 8),
	...
	new_test_ext(GenesisConfigBuilder::default().build()).execute_with(|| {
		register_indracore(indra_a);
		register_indracore_with_balance(indra_b, 0);
		run_to_block(5, Some(vec![4, 5]));

		Hrmp::init_open_channel(indra_a, indra_b, 2, 8).unwrap();

		assert_noop!(
			Hrmp::accept_open_channel(indra_b, indra_a),
			pallet_balances::Error::<Test, _>::InsufficientBalance
		);
	});
}
...
fn refund_deposit_on_normal_closure() {
	...
	new_test_ext(genesis.build()).execute_with(|| {
		// Register two indracores funded with different amounts of funds and arrange a channel.
		register_indracore_with_balance(indra_a, 100);
		register_indracore_with_balance(indra_b, 110);
		run_to_block(5, Some(vec![4, 5]));
		Hrmp::init_open_channel(indra_a, indra_b, 2, 8).unwrap();
		Hrmp::accept_open_channel(indra_b, indra_a).unwrap();
		assert_eq!(
...
fn refund_deposit_on_offboarding() {
	...
	genesis.hrmp_recipient_deposit = 15;
	new_test_ext(genesis.build()).execute_with(|| {
		// Register two indracores and open a channel between them.
		register_indracore_with_balance(indra_a, 100);
		register_indracore_with_balance(indra_b, 110);
		run_to_block(5, Some(vec![4, 5]));
		Hrmp::init_open_channel(indra_a, indra_b, 2, 8).unwrap();
...
fn no_dangling_open_requests() {
	...
	new_test_ext(genesis.build()).execute_with(|| {
		...
		// The outcome we expect is `indra_b` should receive the refund.
		assert_eq!(
			<Test as Config>::Currency::free_balance(&indra_b.into_account_truncating()),
			110
		);
