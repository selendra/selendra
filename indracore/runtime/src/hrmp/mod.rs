...
use primitives::v2::{
	Balance, Hash, HrmpChannelId, Id as IndraId, InboundHrmpMessage, OutboundHrmpMessage,
	SessionIndex,
};
...
pub struct HrmpChannel {
	// NOTE: This structure is used by indracores via merkle proofs. Therefore, this struct requires
	// special treatment.
	//
	// A indracore requested this struct can only depend on the subset of this struct. Specifically,
	// only a first few fields can be depended upon (See `AbridgedHrmpChannel`). These fields cannot
	// be changed without corresponding migration of indracores.
	/// The maximum number of messages that can be pending in the channel at once.
	pub max_capacity: u32,
	/// The maximum total size of the messages that can be pending in the channel at once.
	pub max_total_size: u32,
...
pub mod pallet {
	...
	pub enum Event<T: Config> {
		...
		OpenChannelRequested(IndraId, IndraId, u32, u32),
		/// An HRMP channel request sent by the receiver was canceled by either party.
		/// `[by_indracore, channel_id]`
		OpenChannelCanceled(IndraId, HrmpChannelId),
		/// Open HRMP channel accepted. `[sender, recipient]`
		OpenChannelAccepted(IndraId, IndraId),
		/// HRMP channel closed. `[by_indracore, channel_id]`
		ChannelClosed(IndraId, HrmpChannelId),
	}
	...
	pub enum Error<T> {
		...
		/// The recipient is not a valid indra.
	...
	// `HRMP_MAX_INBOUND_CHANNELS_BOUND` are per indracore/indrabase, while this storage tracks the
	// global state.
	#[pallet::storage]
	pub type HrmpOpenChannelRequestsList<T: Config> =
		StorageValue<_, Vec<HrmpChannelId>, ValueQuery>;

	/// This mapping tracks how many open channel requests are initiated by a given sender indra.
	/// Invariant: `HrmpOpenChannelRequests` should contain the same number of items that has
	/// `(X, _)` as the number of `HrmpOpenChannelRequestCount` for `X`.
	#[pallet::storage]
	pub type HrmpOpenChannelRequestCount<T: Config> =
		StorageMap<_, Twox64Concat, IndraId, u32, ValueQuery>;

	/// This mapping tracks how many open channel requests were accepted by a given recipient indra.
	...
	pub type HrmpIngressChannelsIndex<T: Config> =
		StorageMap<_, Twox64Concat, IndraId, Vec<IndraId>, ValueQuery>;
	...
	/// Maintains a mapping that can be used to answer the question: What indras sent a message at
	/// the given block number for a given receiver. Invariants:
	/// - The inner `Vec<IndraId>` is never empty.
	/// - The inner `Vec<IndraId>` cannot store two same `IndraId`.
	/// - The outer vector is sorted ascending by block number and cannot store two items with the
	///   same block number.
	#[pallet::storage]
	pub type HrmpChannelDigests<T: Config> =
		StorageMap<_, Twox64Concat, IndraId, Vec<(T::BlockNumber, Vec<IndraId>)>, ValueQuery>;
	...
	impl<T: Config> Pallet<T> {
		/// Initiate opening a channel from a indracore to a given recipient with given channel
		/// parameters.
		///
		...
		pub fn hrmp_init_open_channel(
			...
			let origin = ensure_indracore(<T as Config>::Origin::from(origin))?;
		...
		pub fn hrmp_accept_open_channel(origin: OriginFor<T>, sender: IndraId) -> DispatchResult {
			let origin = ensure_indracore(<T as Config>::Origin::from(origin))?;
		...
		/// a indra may have. Normally this happens once per session, but this allows
		/// you to trigger the cleanup immediately for a specific indracore.
		///
		/// Origin must be Root.
		///
		/// Number of inbound and outbound channels for `indra` must be provided as witness data of weighing.
		...
		pub fn force_clean_hrmp(
			...
			ensure_root(origin)?;
			Self::clean_hrmp_after_outgoing(&indra);
			Ok(())
		}
...
#[cfg(feature = "std")]
fn initialize_storage<T: Config>(preopen_hrmp_channels: &[(IndraId, IndraId, u32, u32)]) {
	let host_config = configuration::Pallet::<T>::config();
...
impl<T: Config> Pallet<T> {
	...
	pub(crate) fn initializer_on_new_session(
		...
		let w1 = Self::perform_outgoing_indra_cleanup(&notification.prev_config, outgoing_indras);
	...
	// Go over the HRMP open channel requests and remove all in which offboarding indras participate.
	//
	...
	pub(crate) fn clean_open_channel_requests(
		...
		// In the most common case there will be only single outgoing indra.
		...
		for req_id in go {
			...
			// Return the deposit of the sender, but only if it is not the indra being offboarded.
			...
			// deposit.
			//
			// We still want to refund the deposit only if the indra is not being offboarded.
	...
	/// Remove all storage entries associated with the given indra.
	fn clean_hrmp_after_outgoing(outgoing_indra: &IndraId) {
		<Self as Store>::HrmpOpenChannelRequestCount::remove(outgoing_indra);
		<Self as Store>::HrmpAcceptedChannelRequestCount::remove(outgoing_indra);

		let ingress = <Self as Store>::HrmpIngressChannelsIndex::take(outgoing_indra)
			.into_iter()
			.map(|sender| HrmpChannelId { sender, recipient: outgoing_indra.clone() });
		let egress = <Self as Store>::HrmpEgressChannelsIndex::take(outgoing_indra)
			.into_iter()
			.map(|recipient| HrmpChannelId { sender: outgoing_indra.clone(), recipient });
	...
	pub(crate) fn check_hrmp_watermark(
		...
		//
		// (a) For ensuring that messages are eventually, a rule requires each indrablock new
		//     watermark should be greater than the last one.
		//
		// (b) However, a indracore cannot read into "the future", therefore the watermark should
		//     not be greater than the relay-chain context block which the indrablock refers to.
		...
		// (b) A relay-chain block in which this indra received at least one message.
		if new_hrmp_watermark == relay_chain_parent_number {
			Ok(())
		} else {
	...
	pub(crate) fn check_outbound_hrmp(
		config: &HostConfiguration<T::BlockNumber>,
		sender: IndraId,
		out_hrmp_msgs: &[OutboundHrmpMessage<IndraId>],
		...
		let mut last_recipient = None::<IndraId>;
	...
	pub(crate) fn prune_hrmp(recipient: IndraId, new_hrmp_watermark: T::BlockNumber) -> Weight {
		let mut weight = 0;

		// sift through the incoming messages digest to collect the indras that sent at least one
		// message to this indracore between the old and new watermarks.
	...
	pub(crate) fn queue_outbound_hrmp(
		sender: IndraId,
		out_hrmp_msgs: Vec<OutboundHrmpMessage<IndraId>>,
	) -> Weight {
		let mut weight = 0;
	...
	/// intended for calling directly from other pallets rather than dispatched.
	pub fn accept_open_channel(origin: IndraId, sender: IndraId) -> DispatchResult {
	...
	fn cancel_open_request(origin: IndraId, channel_id: HrmpChannelId) -> DispatchResult {
	...
	fn close_channel(origin: IndraId, channel_id: HrmpChannelId) -> Result<(), Error<T>> {
		// check if the origin is allowed to close the channel.
	...
	/// Returns the list of MQC heads for the inbound channels of the given recipient indra paired
	/// with the sender indra ids. This vector is sorted ascending by the indra id and doesn't contain
	/// multiple entries with the same sender.
	#[cfg(test)]
	fn hrmp_mqc_heads(recipient: IndraId) -> Vec<(IndraId, Hash)> {
	...
	pub(crate) fn inbound_hrmp_channels_contents(
		recipient: IndraId,
	) -> BTreeMap<IndraId, Vec<InboundHrmpMessage<T::BlockNumber>>> {
		let sender_set = <Self as Store>::HrmpIngressChannelsIndex::get(&recipient);
...
impl<T: Config> Pallet<T> {
	...
	fn decrease_open_channel_request_count(sender: IndraId) {
	...
	fn decrease_accepted_channel_request_count(recipient: IndraId) {
	...
	fn assert_storage_consistency_exhaustive() {
		...
		// A HRMP watermark can be None for an onboarded indracore. However, an offboarded indracore
