...
mod enter {
	...
	fn include_backed_candidates() {
		new_test_ext(MockGenesisConfig::default()).execute_with(|| {
			...
			// candidates.
			let expected_indra_inherent_data = scenario.data.clone();

			// Check the indra inherent data is as expected:
			// * 1 bitfield per validator (2 validators)
			assert_eq!(expected_indra_inherent_data.bitfields.len(), 2);
			// * 1 backed candidate per core (2 cores)
			assert_eq!(expected_indra_inherent_data.backed_candidates.len(), 2);
			// * 0 disputes.
			assert_eq!(expected_indra_inherent_data.disputes.len(), 0);
			let mut inherent_data = InherentData::new();
			inherent_data
				.put_data(INDRACORES_INHERENT_IDENTIFIER, &expected_indra_inherent_data)
				.unwrap();
	...
	fn filter_multi_dispute_data() {
		new_test_ext(MockGenesisConfig::default()).execute_with(|| {
			...
			assert_eq!(expected_indra_inherent_data.bitfields.len(), 15);
			// * 0 backed candidate per core
			assert_eq!(expected_indra_inherent_data.backed_candidates.len(), 0);
			// * 3 disputes.
			assert_eq!(expected_indra_inherent_data.disputes.len(), 3);
			let mut inherent_data = InherentData::new();
			inherent_data
				.put_data(INDRACORES_INHERENT_IDENTIFIER, &expected_indra_inherent_data)
				.unwrap();
			...
			assert!(multi_dispute_inherent_data != expected_indra_inherent_data);
			...
			assert_eq!(
				...
				&expected_indra_inherent_data.disputes[..2],
			);
	...
	fn limit_dispute_data() {
		new_test_ext(MockGenesisConfig::default()).execute_with(|| {
			...
			assert_eq!(expected_indra_inherent_data.bitfields.len(), 18);
			// * 0 backed candidate per core
			assert_eq!(expected_indra_inherent_data.backed_candidates.len(), 0);
			// * 3 disputes.
			assert_eq!(expected_indra_inherent_data.disputes.len(), 3);
			...
			assert!(limit_inherent_data != expected_indra_inherent_data);
	...
	fn limit_dispute_data_ignore_backed_candidates() {
		new_test_ext(MockGenesisConfig::default()).execute_with(|| {
			...
			assert_eq!(expected_indra_inherent_data.bitfields.len(), 20);
			// * 2 backed candidates
			assert_eq!(expected_indra_inherent_data.backed_candidates.len(), 2);
			// * 3 disputes.
			assert_eq!(expected_indra_inherent_data.disputes.len(), 3);
			...
			assert_eq!(
				...
				expected_indra_inherent_data.bitfields.len()
			);
	...
	fn limit_bitfields_some() {
		new_test_ext(MockGenesisConfig::default()).execute_with(|| {
			...
			assert_eq!(expected_indra_inherent_data.bitfields.len(), 25);
			// * 2 backed candidates,
			assert_eq!(expected_indra_inherent_data.backed_candidates.len(), 2);
			// * 3 disputes.
			assert_eq!(expected_indra_inherent_data.disputes.len(), 3);
			...
			let limit_inherent_data =
				Pallet::<Test>::create_inherent_inner(&inherent_data.clone()).unwrap();
			assert_ne!(limit_inherent_data, expected_indra_inherent_data);
			assert!(
				inherent_data_weight(&limit_inherent_data) <=
					inherent_data_weight(&expected_indra_inherent_data)
			);
	...
	fn inherent_data_weight(inherent_data: &IndracoresInherentData) -> Weight {
	...
	fn limit_candidates_over_weight_1() {
		new_test_ext(MockGenesisConfig::default()).execute_with(|| {
			...
			let expected_indra_inherent_data = scenario.data.clone();
			assert!(max_block_weight() < inherent_data_weight(&expected_indra_inherent_data));
...
mod sanitizers {
	...
	use primitives::v2::{
		AvailabilityBitfield, GroupIndex, Hash, Id as IndraId, SignedAvailabilityBitfield,
		ValidatorIndex,
	};
	...
	use primitives::v2::INDRACORE_KEY_TYPE_ID;
	...
	fn candidates() {
		...
		let scheduled = (0_usize..2)
			...
			.map(|idx| {
				let ca = CoreAssignment {
					kind: scheduler::AssignmentKind::Indracore,
					group_idx: GroupIndex::from(idx as u32),
					indra_id: IndraId::from(1_u32 + idx as u32),
		...
		let group_validators = |group_index: GroupIndex| {
			match group_index {
				...
				_ => panic!("Group index out of bounds for 2 indracores and 1 indrabase core"),
			}
		...
		// nothing is scheduled, so no indraids match, thus all backed candidates are skipped
		{
			let scheduled = &[][..];
