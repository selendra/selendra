...
//! indracores modules. It's also responsible for finalization and session change notifications.
//!
...
pub mod pallet {
		...
		+ hrmp::Config
	{
		...
		/// An origin which is allowed to force updates to indracores.
		type ForceOrigin: EnsureOrigin<<Self as frame_system::Config>::Origin>;
		/// Weight information for extrinsics in this pallet.
		type WeightInfo: WeightInfo;
	}

	/// Whether the indracores modules have been initialized within this block.
	///
	...
	impl<T: Config> Pallet<T> {
		/// Issue a signal to the consensus engine to forcibly act as though all indracore
...
impl<T: Config> Pallet<T> {
	fn apply_new_session(
		...
		dmp::Pallet::<T>::initializer_on_new_session(&notification, &outgoing_indras);
		ump::Pallet::<T>::initializer_on_new_session(&notification, &outgoing_indras);
		hrmp::Pallet::<T>::initializer_on_new_session(&notification, &outgoing_indras);
	}
