	...
	SI: SessionInfo<B>,
{
	...
	async fn run_session(&mut self, session_id: SessionId) {
		...
		if self.chain_state.best_block_number() >= last_block {
			...
			for attempt in 0..10 {
				...
				if last_finalized_number >= last_block {
					debug!(target: "selendra-party", "Skipping session {:?} early because block {:?} is already finalized", session_id, last_finalized_number);
					return
				}
			}
		}
		...
		let authority_data =
			match self.session_authorities.subscribe_to_insertion(session_id).await.await {
				Err(e) =>
					panic!("Error while receiving the notification about current session {:?}", e),
				Ok(authority_data) => authority_data,
			};
		let authorities = authority_data.authorities();

		trace!(target: "selendra-party", "Authority data for session {:?}: {:?}", session_id, authorities);
			...
			self.session_manager.node_idx(authorities).await
		{
			match backup::rotate(self.backup_saving_path.clone(), session_id.0) {
				Ok(backup) => {
					debug!(target: "selendra-party", "Running session {:?} as authority id {:?}", session_id, node_id);
					Some(
			...
			debug!(target: "selendra-party", "Running session {:?} as non-authority", session_id);
			if let Err(e) = self.session_manager.start_nonvalidator_session(session_id, authorities)
			{
				warn!(target: "selendra-party", "Failed to start nonvalidator session{:?}: {}", session_id, e);
			}
			None
		};
		...
		let mut start_next_session_network =
			Some(self.session_authorities.subscribe_to_insertion(next_session_id).await);
		loop {
			tokio::select! {
				_ = &mut check_session_status => {
					let last_finalized_number = self.chain_state.finalized_number();
					if last_finalized_number >= last_block {
						debug!(target: "selendra-party", "Terminating session {:?}", session_id);
						break;
					}
				...
				Some(next_session_authority_data) = async {
					match &mut start_next_session_network {
						Some(notification) => {
							match notification.await {
								Err(e) => {
									warn!(target: "selendra-party", "Error with subscription {:?}", e);
					...
					match self.session_manager.node_idx(next_session_authorities).await {
						 Some(next_session_node_id) => if let Err(e) = self
							...
							{
								warn!(target: "selendra-party", "Failed to early start validator session{:?}: {}", next_session_id, e);
							}
						None => {
							if let Err(e) = self
								.session_manager
							...
							{
								warn!(target: "selendra-party", "Failed to early start nonvalidator session{:?}: {}", next_session_id, e);
							}
						}
					}
				...
				Some(_) = async {
					...
					warn!(target: "selendra-party", "Authority task ended prematurely, giving up for this session.");
					maybe_authority_task = None;
				},
			}
		}
		if let Some(task) = maybe_authority_task {
			debug!(target: "selendra-party", "Stopping the authority task.");
			if task.stop().await.is_err() {
				warn!(target: "selendra-party", "Authority task did not stop silently");
			}
		}
		if let Err(e) = self.session_manager.stop_session(session_id) {
			warn!(target: "selendra-party", "Session Manager failed to stop in session {:?}: {}", session_id, e)
		}
	}
	...
	pub async fn run(mut self) {
		...
		for curr_id in starting_session.0.. {
			info!(target: "selendra-party", "Running session {:?}.", curr_id);
	...
	async fn catch_up(&mut self) -> SessionId {
		...
		self.session_info.session_id_from_block_num(finalized_number)
	}
}
...
mod tests {
	...
	impl PartyTest {
		...
		fn assert_state(&self, expected_state: PartyState, block: u32) {
			let PartyState { validator_started, early_started, stopped, non_validator_started } =
				expected_state;
			assert_eq!(
				*self.controller.node_session_manager.validator_session_started.lock().unwrap(),
			...
			assert_eq!(
				*self.controller.node_session_manager.session_early_started.lock().unwrap(),
			...
			assert_eq!(
				*self.controller.node_session_manager.session_stopped.lock().unwrap(),
		...
		async fn handle_events(&mut self, events: BlockEvents, block: u32) {
			let BlockEvents { session_authorities, id, state_to_assert } = events;

			if let Some(expected_state) = state_to_assert {
				// sleep to make sure party catch all events
				sleep(Duration::from_millis(SESSION_STATUS_CHECK_PERIOD.as_millis() as u64 + 100))
					.await;
		...
		async fn set_best_and_finalized_block(
			...
			self.controller.chain_state_mock.set_finalized_block(finalized_block);
	...
	async fn party_starts_session_for_node_in_authorities() {
		...
		let authorities: Vec<_> = (0..10).map(|id| UintAuthorityId(id).to_public_key()).collect();
