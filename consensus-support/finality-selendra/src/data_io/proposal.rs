...
/// the branch length before we transform it into a safer `SelendraProposal` type that guarantees we will not
/// fail on  any integer over- or underflows.
/// We expect that honest nodes create UnvalidatedSelendraProposal {branch: [h_0, h_1, ..., h_n], number: num} objects
...
///     3) height(b_n) = num,
///     4) The parent of b_0 has been finalized (prior to creating this SelendraData).
/// Such an UnvalidatedSelendraProposal  object should be thought of as a proposal for block b_n to be finalized.
...
impl<B: BlockT> Hash for UnvalidatedSelendraProposal<B> {
...
impl<B: BlockT> PartialEq for UnvalidatedSelendraProposal<B> {
...
impl<B: BlockT> UnvalidatedSelendraProposal<B> {
	...
	pub(crate) fn validate_bounds(
		...
		if self.branch.len() > MAX_DATA_BRANCH_LEN {
			return Err(BranchTooLong { branch_size: self.branch.len() })
		}
		...
		Ok(SelendraProposal { branch: self.branch.clone(), number: self.number })
	}
}

/// A version of UnvalidatedSelendraProposal that has been initially validated and fits
...
// Need to be implemented manually, as deriving does not work (`BlockT` is not `Hash`).
impl<B: BlockT> Hash for SelendraProposal<B> {
	fn hash<H: Hasher>(&self, state: &mut H) {
		self.branch.hash(state);
		self.number.hash(state);
	}
...
// Clippy does not allow deriving PartialEq when implementing Hash manually
impl<B: BlockT> PartialEq for SelendraProposal<B> {
	fn eq(&self, other: &Self) -> bool {
		self.number.eq(&other.number) && self.branch.eq(&other.branch)
	}
}

impl<B: BlockT> Eq for SelendraProposal<B> {}

impl<B: BlockT> Index<usize> for SelendraProposal<B> {
	type Output = B::Hash;
