...
use crate::{
	justification::{SelendraJustification, SessionInfo, SessionInfoProvider, Verifier},
...
impl Verifier<TBlock> for VerifierWrapper {
	fn verify(&self, _justification: &SelendraJustification, _hash: THash) -> bool {
...
impl SessionInfoProviderImpl {
	pub(crate) fn new(session_period: SessionPeriod, acceptance_policy: AcceptancePolicy) -> Self {
		Self { session_period, acceptance_policy: Arc::new(Mutex::new(acceptance_policy)) }
	}
}
	...
	async fn for_block_num(&self, number: TNumber) -> SessionInfo<TBlock, VerifierWrapper> {
		...
		SessionInfo {
			...
			verifier: match &*self.acceptance_policy.lock().unwrap() {
				...
				_ => Some(VerifierWrapper { acceptance_policy: self.acceptance_policy.clone() }),
			},
		}
	}
}
