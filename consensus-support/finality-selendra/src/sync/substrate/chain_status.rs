	...
	B::Header: SubstrateHeader<Number = BlockNumber>,
{
	...
	fn justification(&self, hash: B::Hash) -> Result<Option<SelendraJustification>, ClientError> {
		let justification = match self
			.client
			.justifications(hash)?
			.and_then(|j| j.into_justification(SELENDRA_ENGINE_ID))
		{
	...
	B::Header: SubstrateHeader<Number = BlockNumber>,
{
	...
	fn status_of(
		...
		if let Some(raw_justification) = self.justification(id.hash)? {
			Ok(BlockStatus::Justified(Justification { header, raw_justification }))
		} else {
	...
	fn top_finalized(&self) -> Result<Justification<B::Header>, Self::Error> {
		...
		let header = self.header(finalized_hash)?.ok_or(Error::MissingHash(finalized_hash))?;
		let raw_justification = self
			.justification(finalized_hash)?
			.ok_or(Error::MissingJustification(finalized_hash))?;

		Ok(Justification { header, raw_justification })
	}
}
