...
use primitives::{Amount, currency::TokenSymbol, TradingPair};
use sp_runtime::{
	testing::{Header, H256},
...
impl orml_tokens::ConvertBalance<Balance, Balance> for ConvertBalanceHoma {
	...
	fn convert_balance(balance: Balance, asset_id: CurrencyId) -> Balance {
		match asset_id {
			KMD => ExchangeRate::saturating_from_rational(1, 10)
	...
	fn convert_balance_back(balance: Balance, asset_id: CurrencyId) -> Balance {
		match asset_id {
			KMD => ExchangeRate::saturating_from_rational(10, 1)
				.checked_mul_int(balance)
				.unwrap_or(Bounded::max_value()),
			_ => balance,
		}
...
match_types! {
	pub type IsLiquidToken: impl Contains<CurrencyId> = {
		CurrencyId::Token(TokenSymbol::KMD)
	};
}
...
parameter_types! {
	...
	pub const GetLiquidCurrencyId: CurrencyId = KMD;
}
	...
	fn default() -> Self {
		Self {
			balances: vec![
				...
				(BOB, SUSD, 1_000_000_000_000_000_000),
				(BOB, DOT, 1_000_000_000_000_000_000),
				(BOB, KMD, 1_000_000_000_000_000_000),
			],
		}
	}
}
