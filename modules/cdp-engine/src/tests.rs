...
fn adjust_position_work() {
	ExtBuilder::default().build().execute_with(|| {
		...
		setup_default_collateral(SUSD);

		assert_noop!(
			CDPEngineModule::adjust_position(&ALICE, SEL, 100, 500),
			Error::<Runtime>::InvalidCollateralType,
		);
		assert_eq!(Currencies::free_balance(BTC, &ALICE), 1000);
		assert_eq!(Currencies::free_balance(SUSD, &ALICE), 0);
		...
		assert_eq!(Currencies::free_balance(SUSD, &ALICE), 50);
		...
		assert_eq!(Currencies::free_balance(BTC, &ALICE), 900);
		assert_eq!(Currencies::free_balance(SUSD, &ALICE), 30);
...
fn adjust_position_by_debit_value_work() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_noop!(
			CDPEngineModule::adjust_position_by_debit_value(&ALICE, SEL, 100, 5000),
			Error::<Runtime>::InvalidCollateralType,
		);
		...
		assert_eq!(Currencies::free_balance(SUSD, &ALICE), 100);
		...
		assert_eq!(Currencies::free_balance(BTC, &ALICE), 800);
		assert_eq!(Currencies::free_balance(SUSD, &ALICE), 70);
		assert_eq!(LoansModule::positions(BTC, ALICE).collateral, 200);
...
fn expand_position_collateral_work() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_eq!(DEXModule::get_liquidity_pool(DOT, SUSD), (1000, 10000));
		assert_noop!(
		...
		assert_eq!(Currencies::free_balance(SUSD, &LoansModule::account_id()), 0);
		assert_eq!(DEXModule::get_liquidity_pool(DOT, SUSD), (976, 10250));
		...
		assert_eq!(Currencies::free_balance(SUSD, &LoansModule::account_id()), 0);
		assert_eq!(DEXModule::get_liquidity_pool(DOT, SUSD), (958, 10450));
...
#[test]
fn expand_position_collateral_for_lp_susd_dot_work() {
	ExtBuilder::default().build().execute_with(|| {
		assert_ok!(DEXModule::add_liquidity(
			Origin::signed(CAROL),
			SUSD,
		...
		assert_eq!(Currencies::total_issuance(LP_SUSD_DOT), 20000);
		assert_ok!(Currencies::transfer(Origin::signed(CAROL), ALICE, LP_SUSD_DOT, 1000));

		MockPriceSource::set_price(LP_SUSD_DOT, Some(Price::saturating_from_rational(1, 1)));
		assert_ok!(CDPEngineModule::set_collateral_params(
			Origin::signed(1),
			LP_SUSD_DOT,
			Change::NewValue(Some(Rate::saturating_from_rational(1, 100000))),
		...
		assert_ok!(CDPEngineModule::adjust_position(&ALICE, LP_SUSD_DOT, 1000, 2000));
		assert_eq!(
			LoansModule::positions(LP_SUSD_DOT, ALICE),
			Position {
				collateral: 1000,
				debit: 2000
			}
		);
		assert_eq!(Currencies::free_balance(LP_SUSD_DOT, &ALICE), 0);
		assert_eq!(Currencies::free_balance(DOT, &ALICE), 1000);
		assert_eq!(Currencies::free_balance(SUSD, &ALICE), 200);
		assert_eq!(CDPTreasuryModule::debit_pool(), 0);
		assert_eq!(CDPTreasuryModule::surplus_pool(), 0);
		assert_eq!(Currencies::free_balance(LP_SUSD_DOT, &LoansModule::account_id()), 1000);
		assert_eq!(Currencies::free_balance(DOT, &LoansModule::account_id()), 0);
		assert_eq!(Currencies::free_balance(SUSD, &LoansModule::account_id()), 0);
		assert_eq!(DEXModule::get_liquidity_pool(DOT, SUSD), (1000, 10000));

		assert_noop!(
			CDPEngineModule::expand_position_collateral(&ALICE, LP_SUSD_DOT, 200, 200),
		...
		assert_eq!(Currencies::free_balance(LP_SUSD_DOT, &ALICE), 0);
		assert_eq!(Currencies::free_balance(DOT, &ALICE), 1000);
		assert_eq!(Currencies::free_balance(SUSD, &ALICE), 206);
		assert_eq!(CDPTreasuryModule::debit_pool(), 0);
		assert_eq!(CDPTreasuryModule::surplus_pool(), 0);
		assert_eq!(Currencies::free_balance(LP_SUSD_DOT, &LoansModule::account_id()), 1283);
		assert_eq!(Currencies::free_balance(DOT, &LoansModule::account_id()), 0);
		assert_eq!(Currencies::free_balance(SUSD, &LoansModule::account_id()), 0);
		assert_eq!(DEXModule::get_liquidity_pool(DOT, SUSD), (1000, 10294));
	});
}
...
fn shrink_position_debit_work() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_eq!(Currencies::free_balance(DOT, &ALICE), 900);
		assert_eq!(Currencies::free_balance(SUSD, &ALICE), 500);
		assert_eq!(CDPTreasuryModule::debit_pool(), 0);
		assert_eq!(CDPTreasuryModule::surplus_pool(), 0);
		assert_eq!(Currencies::free_balance(DOT, &LoansModule::account_id()), 100);
		assert_eq!(Currencies::free_balance(SUSD, &LoansModule::account_id()), 0);
		...
		assert_eq!(DEXModule::get_liquidity_pool(DOT, SUSD), (1000, 8000));
		assert_noop!(
		...
		assert_eq!(Currencies::free_balance(SUSD, &LoansModule::account_id()), 0);
		assert_eq!(DEXModule::get_liquidity_pool(DOT, SUSD), (1010, 7921));
		...
		assert_eq!(Currencies::free_balance(DOT, &ALICE), 900);
		assert_eq!(Currencies::free_balance(SUSD, &ALICE), 592);
		assert_eq!(CDPTreasuryModule::debit_pool(), 0);
		assert_eq!(CDPTreasuryModule::surplus_pool(), 0);
		assert_eq!(Currencies::free_balance(DOT, &LoansModule::account_id()), 20);
		assert_eq!(Currencies::free_balance(SUSD, &LoansModule::account_id()), 0);
		assert_eq!(DEXModule::get_liquidity_pool(DOT, SUSD), (1080, 7408));
	});
}

#[test]
fn shrink_position_debit_for_lp_susd_dot_work() {
	ExtBuilder::default().build().execute_with(|| {
		MockPriceSource::set_price(LP_SUSD_DOT, Some(Price::saturating_from_rational(1, 1)));
		assert_ok!(DEXModule::add_liquidity(
			Origin::signed(CAROL),
		...
		setup_default_collateral(DOT);
		setup_default_collateral(SUSD);
		assert_ok!(CDPEngineModule::adjust_position(&ALICE, LP_SUSD_DOT, 1000, 5000));
		assert_eq!(
			LoansModule::positions(LP_SUSD_DOT, ALICE),
			Position {
				collateral: 1000,
		...
		assert_noop!(
			CDPEngineModule::shrink_position_debit(&ALICE, LP_SUSD_DOT, 200, 200),
			Error::<Runtime>::NotEnoughDebitDecrement
		);

		assert_ok!(CDPEngineModule::shrink_position_debit(&ALICE, LP_SUSD_DOT, 100, 80));
		assert_eq!(
			LoansModule::positions(LP_SUSD_DOT, ALICE),
			Position {
				collateral: 900,
		...
		assert_eq!(CDPTreasuryModule::debit_pool(), 0);
		assert_eq!(CDPTreasuryModule::surplus_pool(), 0);
		assert_eq!(Currencies::free_balance(LP_SUSD_DOT, &LoansModule::account_id()), 900);
		assert_eq!(Currencies::free_balance(DOT, &LoansModule::account_id()), 0);
		assert_eq!(Currencies::free_balance(SUSD, &LoansModule::account_id()), 0);
		assert_eq!(DEXModule::get_liquidity_pool(DOT, SUSD), (1000, 9901));

		assert_ok!(CDPEngineModule::shrink_position_debit(&ALICE, LP_SUSD_DOT, 600, 500));
		assert_eq!(
			LoansModule::positions(LP_SUSD_DOT, ALICE),
			Position {
				collateral: 300,
		...
		assert_eq!(Currencies::free_balance(LP_SUSD_DOT, &ALICE), 0);
		assert_eq!(Currencies::free_balance(DOT, &ALICE), 1000);
		assert_eq!(Currencies::free_balance(SUSD, &ALICE), 685);
		assert_eq!(CDPTreasuryModule::debit_pool(), 0);
		assert_eq!(CDPTreasuryModule::surplus_pool(), 0);
		assert_eq!(Currencies::free_balance(LP_SUSD_DOT, &LoansModule::account_id()), 300);
		assert_eq!(Currencies::free_balance(DOT, &LoansModule::account_id()), 0);
		assert_eq!(Currencies::free_balance(SUSD, &LoansModule::account_id()), 0);
		assert_eq!(DEXModule::get_liquidity_pool(DOT, SUSD), (1000, 9315));
	});
}
...
fn liquidate_unsafe_cdp_by_swap() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_eq!(DEXModule::get_liquidity_pool(BTC, SUSD), (100, 121));
		...
		assert_eq!(DEXModule::get_liquidity_pool(BTC, SUSD), (199, 61));
...
#[test]
fn liquidate_unsafe_cdp_of_lp_susd_dot_and_swap_dot() {
	ExtBuilder::default().build().execute_with(|| {
		System::set_block_number(1);
		assert_ok!(CDPEngineModule::set_collateral_params(
			Origin::signed(1),
		...
		assert_eq!(DEXModule::get_liquidity_pool(SUSD, DOT), (10000, 500));
		assert_eq!(Currencies::total_issuance(LP_SUSD_DOT), 20000);
		assert_ok!(Currencies::transfer(Origin::signed(CAROL), ALICE, LP_SUSD_DOT, 1000));
		assert_eq!(Currencies::free_balance(LP_SUSD_DOT, &ALICE), 1000);
		assert_eq!(Currencies::free_balance(DOT, &ALICE), 1000);
		assert_eq!(Currencies::free_balance(SUSD, &ALICE), 0);

		MockPriceSource::set_price(DOT, Price::checked_from_rational(20, 1));
		MockPriceSource::set_price(LP_SUSD_DOT, Price::checked_from_rational(1, 1));

		assert_ok!(CDPEngineModule::adjust_position(&ALICE, LP_SUSD_DOT, 1000, 5000));
		assert_eq!(Currencies::free_balance(LP_SUSD_DOT, &ALICE), 0);
		assert_eq!(Currencies::free_balance(DOT, &ALICE), 1000);
		assert_eq!(Currencies::free_balance(SUSD, &ALICE), 500);
		assert_eq!(LoansModule::positions(LP_SUSD_DOT, ALICE).debit, 5000);
		assert_eq!(LoansModule::positions(LP_SUSD_DOT, ALICE).collateral, 1000);
		assert_eq!(Currencies::free_balance(LP_SUSD_DOT, &LoansModule::account_id()), 1000);
		assert_eq!(CDPTreasuryModule::debit_pool(), 0);
		...
		assert_ok!(CDPEngineModule::liquidate_unsafe_cdp(ALICE, LP_SUSD_DOT));
		System::assert_last_event(Event::CDPEngineModule(crate::Event::LiquidateUnsafeCDP {
			collateral_type: LP_SUSD_DOT,
			owner: ALICE,
		...
		assert_eq!(
			MockPriceSource::get_relative_price(SUSD, DOT),
			Price::checked_from_rational(1, 20)
		);
		assert_eq!(DEXModule::get_liquidity_pool(SUSD, DOT), (9400, 481));
		assert_eq!(Currencies::total_issuance(LP_SUSD_DOT), 19000);
		assert_eq!(Currencies::free_balance(LP_SUSD_DOT, &ALICE), 0);
		assert_eq!(Currencies::free_balance(DOT, &ALICE), 1019);
		assert_eq!(Currencies::free_balance(SUSD, &ALICE), 500);
		assert_eq!(LoansModule::positions(LP_SUSD_DOT, ALICE).debit, 0);
		assert_eq!(LoansModule::positions(LP_SUSD_DOT, ALICE).collateral, 0);
		assert_eq!(Currencies::free_balance(LP_SUSD_DOT, &LoansModule::account_id()), 0);
		assert_eq!(CDPTreasuryModule::debit_pool(), 500);
		assert_eq!(Currencies::free_balance(SUSD, &CDPTreasuryModule::account_id()), 600);
		assert_eq!(Currencies::free_balance(DOT, &CDPTreasuryModule::account_id()), 0);
		assert_eq!(
			Currencies::free_balance(LP_SUSD_DOT, &CDPTreasuryModule::account_id()),
			0
...
#[test]
fn liquidate_unsafe_cdp_of_lp_susd_dot_and_susd_take_whole_target() {
	ExtBuilder::default().build().execute_with(|| {
		System::set_block_number(1);
		assert_ok!(CDPEngineModule::set_collateral_params(
			Origin::signed(1),
		...
		assert_eq!(Currencies::free_balance(DOT, &ALICE), 1000);
		assert_eq!(Currencies::free_balance(SUSD, &ALICE), 200);
		assert_eq!(LoansModule::positions(LP_SUSD_DOT, ALICE).debit, 2000);
		assert_eq!(LoansModule::positions(LP_SUSD_DOT, ALICE).collateral, 1000);
		assert_eq!(Currencies::free_balance(LP_SUSD_DOT, &LoansModule::account_id()), 1000);
		assert_eq!(CDPTreasuryModule::debit_pool(), 0);
		assert_eq!(Currencies::free_balance(SUSD, &CDPTreasuryModule::account_id()), 0);
		...
		assert_eq!(DEXModule::get_liquidity_pool(SUSD, DOT), (9500, 475));
		assert_eq!(Currencies::total_issuance(LP_SUSD_DOT), 19000);
		assert_eq!(Currencies::free_balance(LP_SUSD_DOT, &ALICE), 0);
		assert_eq!(Currencies::free_balance(DOT, &ALICE), 1025);
		assert_eq!(Currencies::free_balance(SUSD, &ALICE), 460);
		assert_eq!(LoansModule::positions(LP_SUSD_DOT, ALICE).debit, 0);
		assert_eq!(LoansModule::positions(LP_SUSD_DOT, ALICE).collateral, 0);
		assert_eq!(Currencies::free_balance(LP_SUSD_DOT, &LoansModule::account_id()), 0);
		assert_eq!(CDPTreasuryModule::debit_pool(), 200);
		assert_eq!(Currencies::free_balance(SUSD, &CDPTreasuryModule::account_id()), 240);
		assert_eq!(Currencies::free_balance(DOT, &CDPTreasuryModule::account_id()), 0);
		assert_eq!(
			Currencies::free_balance(LP_SUSD_DOT, &CDPTreasuryModule::account_id()),
			0
...
fn close_cdp_has_debit_by_dex_work() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_eq!(DEXModule::get_liquidity_pool(BTC, SUSD), (100, 1000));
		...
		assert_eq!(DEXModule::get_liquidity_pool(BTC, SUSD), (106, 950));
...
fn close_cdp_has_debit_by_swap_on_alternative_path() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_eq!(DEXModule::get_liquidity_pool(BTC, SEL), (100, 1000));
		assert_eq!(DEXModule::get_liquidity_pool(SEL, SUSD), (1000, 1000));
		assert_ok!(CDPEngineModule::adjust_position(&ALICE, BTC, 100, 500));
		assert_eq!(Currencies::free_balance(BTC, &ALICE), 900);
		assert_eq!(Currencies::free_balance(SUSD, &ALICE), 50);
		assert_eq!(LoansModule::positions(BTC, ALICE).debit, 500);
		...
		assert_eq!(DEXModule::get_liquidity_pool(BTC, SEL), (106, 947));
		assert_eq!(DEXModule::get_liquidity_pool(SEL, SUSD), (1053, 950));
		assert_eq!(Currencies::free_balance(BTC, &ALICE), 994);
		assert_eq!(LoansModule::positions(BTC, ALICE).debit, 0);
		assert_eq!(LoansModule::positions(BTC, ALICE).collateral, 0);
		assert_eq!(CDPTreasuryModule::get_surplus_pool(), 50);
...
fn offchain_worker_works_cdp() {
	...
	ext.execute_with(|| {
		...
		setup_default_collateral(BTC);
		setup_default_collateral(LP_SUSD_DOT);
		setup_default_collateral(DOT);
