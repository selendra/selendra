...
use primitives::{currency::{DexShare,TokenSymbol}, Moment, TradingPair};
use sp_core::H256;
use sp_runtime::{
...
use support::{EmergencyShutdown, SpecificJointsSwap};
...
thread_local! {
	...
	static LP_SUSD_DOT_PRICE: RefCell<Option<Price>> = RefCell::new(Some(Price::one()));
...
impl MockPriceSource {
	pub fn set_price(currency_id: CurrencyId, price: Option<Price>) {
		match currency_id {
			...
			LP_SUSD_DOT => LP_SUSD_DOT_PRICE.with(|v| *v.borrow_mut() = price),
...
impl PriceProvider<CurrencyId> for MockPriceSource {
	fn get_price(currency_id: CurrencyId) -> Option<Price> {
		match currency_id {
			...
			SUSD => Some(Price::one()),
			LP_SUSD_DOT => LP_SUSD_DOT_PRICE.with(|v| *v.borrow()),
...
parameter_types! {
	...
	pub EnabledTradingPairs: Vec<TradingPair> = vec![
		TradingPair::from_currency_ids(SUSD, BTC).unwrap(),
		...
		TradingPair::from_currency_ids(SEL, SUSD).unwrap(),
	];
}
