...
fn deposit_dex_share_works() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_ok!(TokensModule::deposit(BTC_SUSD_LP, &ALICE::get(), 10000));
		assert_eq!(TokensModule::free_balance(BTC_SUSD_LP, &ALICE::get()), 10000);
		assert_eq!(TokensModule::free_balance(BTC_SUSD_LP, &IncentivesModule::account_id()), 0);
		assert_eq!(RewardsModule::pool_infos(PoolId::Dex(BTC_SUSD_LP)), PoolInfo::default(),);

		assert_eq!(
			RewardsModule::shares_and_withdrawn_rewards(PoolId::Dex(BTC_SUSD_LP), ALICE::get()),
			Default::default(),
		);
		...
		System::assert_last_event(Event::IncentivesModule(crate::Event::DepositDexShare {
			who: ALICE::get(),
			dex_share_type: BTC_SUSD_LP,
			deposit: 10000,
		}));
		assert_eq!(TokensModule::free_balance(BTC_SUSD_LP, &ALICE::get()), 0);
		assert_eq!(TokensModule::free_balance(BTC_SUSD_LP, &IncentivesModule::account_id()), 10000);
		assert_eq!(
...
fn withdraw_dex_share_works() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_noop!(
			IncentivesModule::withdraw_dex_share(Origin::signed(BOB::get()), BTC_SUSD_LP, 10000),
		...
		assert_eq!(TokensModule::free_balance(BTC_SUSD_LP, &ALICE::get()), 8000);
		assert_eq!(TokensModule::free_balance(BTC_SUSD_LP, &IncentivesModule::account_id()), 2000);
		assert_eq!(
			RewardsModule::pool_infos(PoolId::Dex(BTC_SUSD_LP)),
...
fn update_incentive_rewards_works() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_eq!(IncentivesModule::incentive_reward_amounts(PoolId::Dex(DOT_SUSD_LP), SEL), 0);
		assert_eq!(IncentivesModule::incentive_reward_amounts(PoolId::Dex(DOT_SUSD_LP), DOT), 0);
		...
		assert_eq!(IncentivesModule::incentive_reward_amounts(PoolId::Dex(DOT_SUSD_LP), SEL), 1000);
		assert_eq!(
			IncentiveRewardAmounts::<Runtime>::contains_key(PoolId::Dex(DOT_SUSD_LP), DOT),
			true
		);
		assert_eq!(IncentivesModule::incentive_reward_amounts(PoolId::Dex(DOT_SUSD_LP), DOT), 100);
		assert_eq!(IncentivesModule::incentive_reward_amounts(PoolId::Loans(DOT), SEL), 500);

		assert_ok!(IncentivesModule::update_incentive_rewards(
			Origin::signed(ROOT::get()),
			vec![
				(PoolId::Dex(DOT_SUSD_LP), vec![(SEL, 200), (DOT, 0)]),
				(PoolId::Loans(DOT), vec![(SEL, 500)]),
			],
		));
		System::assert_has_event(Event::IncentivesModule(
		...
		assert_eq!(IncentivesModule::incentive_reward_amounts(PoolId::Dex(DOT_SUSD_LP), SEL), 200);
		assert_eq!(
			IncentiveRewardAmounts::<Runtime>::contains_key(PoolId::Dex(DOT_SUSD_LP), DOT),
			false
		);
...
fn update_dex_saving_rewards_works() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_noop!(
			IncentivesModule::update_dex_saving_rewards(
				Origin::signed(ROOT::get()),
				vec![(PoolId::Dex(DOT_SUSD_LP), Rate::saturating_from_rational(101, 100))]
			),
			Error::<Runtime>::InvalidRate
		);

		assert_eq!(
			IncentivesModule::dex_saving_reward_rates(PoolId::Dex(DOT_SUSD_LP)),
			Rate::zero()
		);
		assert_eq!(
			IncentivesModule::dex_saving_reward_rates(PoolId::Dex(BTC_SUSD_LP)),
			Rate::zero()
		);
		...
		assert_ok!(IncentivesModule::update_dex_saving_rewards(
			Origin::signed(ROOT::get()),
			vec![
				(PoolId::Dex(DOT_SUSD_LP), Rate::saturating_from_rational(1, 100)),
				(PoolId::Dex(BTC_SUSD_LP), Rate::saturating_from_rational(2, 100))
			]
		));
		...
		assert_eq!(DexSavingRewardRates::<Runtime>::contains_key(PoolId::Dex(BTC_SUSD_LP)), true);
		assert_eq!(
			IncentivesModule::dex_saving_reward_rates(PoolId::Dex(BTC_SUSD_LP)),
		...
		assert_ok!(IncentivesModule::update_dex_saving_rewards(
			Origin::signed(ROOT::get()),
			vec![
				(PoolId::Dex(DOT_SUSD_LP), Rate::saturating_from_rational(5, 100)),
				(PoolId::Dex(BTC_SUSD_LP), Rate::zero())
			]
		));
		System::assert_has_event(Event::IncentivesModule(crate::Event::SavingRewardRateUpdated {
			pool: PoolId::Dex(DOT_SUSD_LP),
		...
		assert_eq!(DexSavingRewardRates::<Runtime>::contains_key(PoolId::Dex(BTC_SUSD_LP)), false);
		assert_eq!(
			IncentivesModule::dex_saving_reward_rates(PoolId::Dex(BTC_SUSD_LP)),
			Rate::zero()
		);
...
fn update_claim_reward_deduction_rates_works() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_eq!(
			IncentivesModule::claim_reward_deduction_rates(PoolId::Dex(DOT_SUSD_LP)),
			Rate::zero()
		);
		assert_eq!(
			IncentivesModule::claim_reward_deduction_rates(PoolId::Dex(BTC_SUSD_LP)),
			Rate::zero()
		);
		...
		assert_eq!(
			ClaimRewardDeductionRates::<Runtime>::contains_key(PoolId::Dex(BTC_SUSD_LP)),
			true
		);
		assert_eq!(
			IncentivesModule::claim_reward_deduction_rates(PoolId::Dex(BTC_SUSD_LP)),
...
fn transfer_failed_when_claim_rewards() {
	ExtBuilder::default().build().execute_with(|| {
		assert_ok!(TokensModule::deposit(SUSD, &VAULT::get(), 100));
		RewardsModule::add_share(&ALICE::get(), &PoolId::Loans(BTC), 100);
		RewardsModule::add_share(&BOB::get(), &PoolId::Loans(BTC), 100);
		assert_ok!(RewardsModule::accumulate_reward(&PoolId::Loans(BTC), SUSD, 18));

		assert_eq!(TokensModule::free_balance(SUSD, &VAULT::get()), 100);
		assert_eq!(TokensModule::free_balance(SUSD, &ALICE::get()), 0);
		assert_eq!(
			RewardsModule::pool_infos(PoolId::Loans(BTC)),
			PoolInfo { total_shares: 200, rewards: vec![(SUSD, (18, 0))].into_iter().collect() }
		);
		assert_eq!(
			RewardsModule::shares_and_withdrawn_rewards(PoolId::Loans(BTC), ALICE::get()),
			(100, Default::default())
		...
		assert_eq!(
			RewardsModule::pool_infos(PoolId::Loans(BTC)),
			PoolInfo { total_shares: 200, rewards: vec![(SUSD, (27, 9))].into_iter().collect() }
		);
		assert_eq!(
			RewardsModule::shares_and_withdrawn_rewards(PoolId::Loans(BTC), ALICE::get()),
			(100, vec![(SUSD, 9)].into_iter().collect())
		);

		assert_eq!(TokensModule::free_balance(SUSD, &BOB::get()), 0);
		assert_eq!(
			RewardsModule::shares_and_withdrawn_rewards(PoolId::Loans(BTC), BOB::get()),
			(100, Default::default())
		);
		...
		assert_eq!(TokensModule::free_balance(SUSD, &VAULT::get()), 87);
		assert_eq!(TokensModule::free_balance(SUSD, &BOB::get()), 13);
		assert_eq!(
			RewardsModule::pool_infos(PoolId::Loans(BTC)),
			PoolInfo { total_shares: 200, rewards: vec![(SUSD, (27, 22))].into_iter().collect() }
		);
		assert_eq!(
			RewardsModule::shares_and_withdrawn_rewards(PoolId::Loans(BTC), BOB::get()),
			(100, vec![(SUSD, 13)].into_iter().collect())
		);
	});
}
...
fn claim_rewards_works() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_ok!(TokensModule::deposit(SUSD, &VAULT::get(), 10000));
		assert_ok!(IncentivesModule::update_claim_reward_deduction_rates(
			Origin::signed(ROOT::get()),
			vec![
				(PoolId::Dex(BTC_SUSD_LP), Rate::saturating_from_rational(50, 100)),
		...
		RewardsModule::add_share(&ALICE::get(), &PoolId::Loans(BTC), 100);
		RewardsModule::add_share(&ALICE::get(), &PoolId::Dex(BTC_SUSD_LP), 100);

		// bob add shares before accumulate rewards
		RewardsModule::add_share(&BOB::get(), &PoolId::Dex(BTC_SUSD_LP), 100);

		// accumulate rewards for different pools
		assert_ok!(RewardsModule::accumulate_reward(&PoolId::Loans(BTC), SEL, 2000));
		assert_ok!(RewardsModule::accumulate_reward(&PoolId::Dex(BTC_SUSD_LP), SEL, 1000));
		assert_ok!(RewardsModule::accumulate_reward(&PoolId::Dex(BTC_SUSD_LP), SUSD, 2000));
		...
		// alice remove share for PoolId::Dex(BTC_SUSD_LP) before claim rewards,
		// rewards will be settled and as pending rewards, will not be deducted.
		assert_eq!(
			RewardsModule::pool_infos(PoolId::Dex(BTC_SUSD_LP)),
			PoolInfo {
				total_shares: 200,
				rewards: vec![(SEL, (1000, 0)), (SUSD, (2000, 0))].into_iter().collect(),
			}
		);
		assert_eq!(
			RewardsModule::shares_and_withdrawn_rewards(PoolId::Dex(BTC_SUSD_LP), ALICE::get()),
		...
		assert_eq!(TokensModule::free_balance(SEL, &VAULT::get()), 9710);
		assert_eq!(TokensModule::free_balance(SUSD, &VAULT::get()), 10000);
		assert_eq!(TokensModule::free_balance(SEL, &ALICE::get()), 200);
		assert_eq!(TokensModule::free_balance(SUSD, &ALICE::get()), 0);
		assert_eq!(
			IncentivesModule::pending_multi_rewards(PoolId::Dex(BTC_SUSD_LP), ALICE::get()),
			BTreeMap::default()
		);
		RewardsModule::remove_share(&ALICE::get(), &PoolId::Dex(BTC_SUSD_LP), 50);
		assert_eq!(
			RewardsModule::pool_infos(PoolId::Dex(BTC_SUSD_LP)),
			PoolInfo {
				total_shares: 150,
				rewards: vec![(SEL, (750, 250)), (SUSD, (1500, 500))].into_iter().collect(),
			}
		);
		assert_eq!(
			RewardsModule::shares_and_withdrawn_rewards(PoolId::Dex(BTC_SUSD_LP), ALICE::get()),
			(50, vec![(SEL, 250), (SUSD, 500)].into_iter().collect())
		);
		assert_eq!(TokensModule::free_balance(SEL, &VAULT::get()), 9710);
		assert_eq!(TokensModule::free_balance(SUSD, &VAULT::get()), 10000);
		assert_eq!(TokensModule::free_balance(SEL, &ALICE::get()), 200);
		...
		assert_eq!(
			IncentivesModule::pending_multi_rewards(PoolId::Dex(BTC_SUSD_LP), ALICE::get()),
			vec![(SEL, 500), (SUSD, 1000)].into_iter().collect()
		);

		// alice claim rewards for PoolId::Dex(BTC_SUSD_LP)
		assert_ok!(IncentivesModule::claim_rewards(
			Origin::signed(ALICE::get()),
			PoolId::Dex(BTC_SUSD_LP)
		));
		System::assert_has_event(Event::IncentivesModule(crate::Event::ClaimRewards {
			who: ALICE::get(),
			pool: PoolId::Dex(BTC_SUSD_LP),
		...
		assert_eq!(
			...
			PoolInfo {
				total_shares: 150,
				rewards: vec![(SEL, (1000, 250)), (SUSD, (2000, 500))].into_iter().collect(),
			}
		);
		assert_eq!(
			RewardsModule::shares_and_withdrawn_rewards(PoolId::Dex(BTC_SUSD_LP), ALICE::get()),
		...
		assert_eq!(TokensModule::free_balance(SUSD, &VAULT::get()), 9500);
		assert_eq!(TokensModule::free_balance(SEL, &ALICE::get()), 450);
		assert_eq!(TokensModule::free_balance(SUSD, &ALICE::get()), 500);
		assert_eq!(
			IncentivesModule::pending_multi_rewards(PoolId::Dex(BTC_SUSD_LP), ALICE::get()),
			BTreeMap::default()
		);
...
fn on_initialize_should_work() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_ok!(TokensModule::deposit(SUSD, &RewardsSource::get(), 10000));

		assert_ok!(IncentivesModule::update_incentive_rewards(
			Origin::signed(ROOT::get()),
			vec![
				(PoolId::Loans(BTC), vec![(SEL, 1000), (SUSD, 500)]),
				(PoolId::Dex(BTC_SUSD_LP), vec![(SEL, 100)]),
				(PoolId::Dex(DOT_SUSD_LP), vec![(SEL, 200)]),
			],
		));
		assert_ok!(IncentivesModule::update_dex_saving_rewards(
			Origin::signed(ROOT::get()),
			vec![
				(PoolId::Dex(BTC_SUSD_LP), Rate::saturating_from_rational(1, 100)),
				(PoolId::Dex(DOT_SUSD_LP), Rate::saturating_from_rational(1, 100)),
			],
		));

		RewardsModule::add_share(&ALICE::get(), &PoolId::Loans(BTC), 1);
		RewardsModule::add_share(&ALICE::get(), &PoolId::Dex(BTC_SUSD_LP), 1);
		RewardsModule::add_share(&ALICE::get(), &PoolId::Dex(DOT_SUSD_LP), 1);

		assert_eq!(TokensModule::free_balance(SEL, &RewardsSource::get()), 10000);
		assert_eq!(TokensModule::free_balance(SUSD, &RewardsSource::get()), 10000);
		assert_eq!(TokensModule::free_balance(SEL, &VAULT::get()), 0);
		assert_eq!(TokensModule::free_balance(SUSD, &VAULT::get()), 0);
		assert_eq!(
			RewardsModule::pool_infos(PoolId::Loans(BTC)),
		...
		// 100 SEL is incentive reward, 5 SUSD is dex saving reward
		assert_eq!(
			RewardsModule::pool_infos(PoolId::Dex(BTC_SUSD_LP)),
			PoolInfo {
				total_shares: 1,
		...
		assert_eq!(TokensModule::free_balance(SUSD, &RewardsSource::get()), 9500 - 500);
		assert_eq!(TokensModule::free_balance(SEL, &VAULT::get()), 1300 + (1000 + 100 + 200));
		...
		assert_eq!(
			...
			PoolInfo {
				total_shares: 1,
				rewards: vec![(SEL, (2000, 0)), (SUSD, (1000, 0))].into_iter().collect(),
			}
		);
		// 100 SEL is incentive reward, 5 SUSD is dex saving reward
		assert_eq!(
		...
		assert_eq!(TokensModule::free_balance(SEL, &RewardsSource::get()), 7400 - (100 + 200));
