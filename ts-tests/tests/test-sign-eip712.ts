...
import { describeWithSelendra, getEvmNonce, transfer } from "./util";
import { Signer } from "@selendra-network/bodhi";
...
describeWithSelendra("Selendra RPC (Sign eip712)", (context) => {
	let alice: Signer;
	let signer: Wallet;
	let subAddr: string;
	...
	it("create should sign and verify", async function () {
		...
		const sig = context.provider.api.createType("ExtrinsicSignature", { SelendraEip712: signature }).toHex()

		tx.addSignature(subAddr, { SelendraEip712: signature } as any, {
