...
//! Runtime API module declares the `trait IndracoreHost` which is part
//! of the Runtime API exposed from the Runtime to the Host.
//!
//! The functions in trait IndracoreHost` can be part of the stable API
...
use selendra_core_primitives as pcp;
use selendra_indracore::primitives as ppp;
use sp_staking;
use sp_std::{collections::btree_map::BTreeMap, prelude::*};

sp_api::decl_runtime_apis! {
	/// The API for querying the state of indracores on-chain.
	#[api_version(2)]
	pub trait IndracoreHost<H: Encode + Decode = pcp::v2::Hash, N: Encode + Decode = pcp::v2::BlockNumber> {
		...
		/// Cores are either free or occupied. Free cores can have indras assigned to them.
		fn availability_cores() -> Vec<v2::CoreState<H, N>>;

		/// Yields the persisted validation data for the given `IndraId` along with an assumption that
		/// should be used if the indra currently occupies a core.
		///
		/// Returns `None` if either the indra is not registered or the assumption is `Freed`
		/// and the indra already occupies a core.
		fn persisted_validation_data(indra_id: ppp::Id, assumption: v2::OccupiedCoreAssumption)
			-> Option<v2::PersistedValidationData<H, N>>;

		/// Returns the persisted validation data for the given `IndraId` along with the corresponding
		/// validation code hash. Instead of accepting assumption about the indra, matches the validation
		...
		fn check_validation_outputs(indra_id: ppp::Id, outputs: v2::CandidateCommitments) -> bool;
		...
		/// Fetch the validation code used by a indra, making the given `OccupiedCoreAssumption`.
		///
		/// Returns `None` if either the indra is not registered or the assumption is `Freed`
		/// and the indra already occupies a core.
		fn validation_code(indra_id: ppp::Id, assumption: v2::OccupiedCoreAssumption)
			-> Option<ppp::ValidationCode>;

		/// Get the receipt of a candidate pending availability. This returns `Some` for any indras
		/// assigned to occupied cores in `availability_cores` and `None` otherwise.
		fn candidate_pending_availability(indra_id: ppp::Id) -> Option<v2::CommittedCandidateReceipt<H>>;
		...
		///
		/// NOTE: This function is only available since indracore host version 2.
		...
		/// Fetch the hash of the validation code used by a indra, making the given `OccupiedCoreAssumption`.
		///
		/// NOTE: This function is only available since indracore host version 2.
		fn validation_code_hash(indra_id: ppp::Id, assumption: v2::OccupiedCoreAssumption)
