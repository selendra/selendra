...
//! Runtime API module declares the `trait XchainHost` which is part
//! of the Runtime API exposed from the Runtime to the Host.
//!
//! The functions in trait XchainHost` can be part of the stable API
...
use selendra_xchain::primitives as ppp;
use sp_staking;
use sp_std::{collections::btree_map::BTreeMap, prelude::*};

sp_api::decl_runtime_apis! {
	/// The API for querying the state of xchains on-chain.
	#[api_version(2)]
	pub trait XchainHost<H: Encode + Decode = pcp::v2::Hash, N: Encode + Decode = pcp::v2::BlockNumber> {
		...
		fn persisted_validation_data(xchain_id: ppp::Id, assumption: v2::OccupiedCoreAssumption)
		...
		fn check_validation_outputs(xchain_id: ppp::Id, outputs: v2::CandidateCommitments) -> bool;
		...
		/// Returns `None` if either the para is not registered or the assumption is `Freed`
		/// and the para already occupies a core.
		fn validation_code(xchain_id: ppp::Id, assumption: v2::OccupiedCoreAssumption)
			-> Option<ppp::ValidationCode>;

		/// Get the receipt of a candidate pending availability. This returns `Some` for any paras
		/// assigned to occupied cores in `availability_cores` and `None` otherwise.
		fn candidate_pending_availability(xchain_id: ppp::Id) -> Option<v2::CommittedCandidateReceipt<H>>;
		...
		///
		/// NOTE: This function is only available since xchain host version 2.
		...
		///
		/// NOTE: This function is only available since xchain host version 2.
		fn validation_code_hash(xchain_id: ppp::Id, assumption: v2::OccupiedCoreAssumption)
