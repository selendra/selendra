...
pub use selendra_rpc_core_debug::{DebugServer, TraceParams};
...
use selendra_client_evm_tracing::{formatters::ResponseFormatter, types::single};
use selendra_rpc_core_types::{RequestBlockId, RequestBlockTag};
use selendra_rpc_primitives_debug::{DebugRuntimeApi, TracerInput};
    ...
    C::Api: ApiExt<B>,
{
    ...
    fn handle_block_request(
        ...
        let f = || -> RpcResult<_> {
            ...
            Ok(selendra_rpc_primitives_debug::Response::Block)
        };

        return match trace_type {
            single::TraceType::CallList => {
                let mut proxy = selendra_client_evm_tracing::listeners::CallList::default();
                proxy.using(f)?;
                proxy.finish_transaction();
                let response = match tracer_input {
                    TracerInput::CallTracer => {
                        selendra_client_evm_tracing::formatters::CallTracer::format(proxy)
        ...
        if let Some(block) = reference_block {
            ...
            if let Some(transaction) = transactions.get(index) {
                let f = || -> RpcResult<_> {
                    ...
                    Ok(selendra_rpc_primitives_debug::Response::Single)
                };
                ...
                return match trace_type {
                    single::TraceType::Raw {
                        ...
                        let mut proxy = selendra_client_evm_tracing::listeners::Raw::new(
                            disable_storage,
                            disable_memory,
                            disable_stack,
                            raw_max_memory_usage,
                        ...
                        proxy.using(f)?;
                        Ok(Response::Single(
                            selendra_client_evm_tracing::formatters::Raw::format(proxy).ok_or(
                                internal_err(
                    ...
                    single::TraceType::CallList => {
                        ...
                        let response = match tracer_input {
                            TracerInput::Blockscout => {
                                selendra_client_evm_tracing::formatters::Blockscout::format(proxy)
                                    .ok_or("Trace result is empty.")
                                    .map_err(|e| internal_err(format!("{:?}", e)))
                            }
                            TracerInput::CallTracer => {
                                let mut res =
                                    selendra_client_evm_tracing::formatters::CallTracer::format(
                                        proxy,
                                    )
                                    .ok_or("Trace result is empty.")
                                    .map_err(|e| internal_err(format!("{:?}", e)))?;
