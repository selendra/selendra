...
use webcontract_types::messaging::Message;
use sp_core::H256;
use sp_runtime::{
	testing::Header,
...
frame_support::construct_runtime!(
		...
		UncheckedExtrinsic = UncheckedExtrinsic,
	{
		...
		WebContractComputation: computation::{Pallet, Event<T>, Storage, Config},
		WebContractStakePoolv2: stake_pool_v2::{Pallet, Event<T>},
		WebContractVault: vault::{Pallet, Event<T>},
		WebContractWrappedBalances: wrapped_balances::{Pallet, Event<T>},
		WebContractBasePool: base_pool::{Pallet, Event<T>},
		WebContractStakePool: stake_pool::{Event<T>},
		Preimage: pallet_preimage::{Event<T>},
	}
);

impl crate::WebContractConfig for Test {
...
impl mq::CallMatcher<Test> for MqCallMatcher {
	fn match_call(call: &RuntimeCall) -> Option<&mq::Call<Test>> {
		match call {
			RuntimeCall::WebContractMq(mq_call) => Some(mq_call),
			_ => None,
		}
	}
}
...
impl pallet_rmrk_core::Config for Test {
	...
	type TransferHooks = WebContractWrappedBalances;
...
impl computation::Config for Test {
	...
	type OnReward = WebContractStakePoolv2;
	type OnUnbound = WebContractStakePoolv2;
	type OnStopped = WebContractStakePoolv2;
...
pub fn new_test_ext() -> sp_io::TestExternalities {
	...
	pallet_balances::GenesisConfig::<Test> {
		balances: vec![
			...
			(WebContractComputation::account_id(), 690_000_000 * DOLLARS),
		],
	}
...
pub fn set_block_1() {
	...
	WebContractRegistry::internal_set_gk_launched_at(0, 0);
}
...
pub fn take_messages() -> Vec<Message> {
	let messages = WebContractMq::messages();
	println!("messages(): {messages:?}");
	mq::OutboundMessages::<Test>::kill();
	messages
}

use webcontract_types::{EcdhPublicKey, WorkerPublicKey};
...
pub fn setup_workers(n: u8) {
	...
	for i in 1..=n {
		...
		WebContractRegistry::internal_set_benchmark(&worker, Some(1));
	}
}
...
pub fn elapse_cool_down() {
	let now = Timestamp::get();
	Timestamp::set_timestamp(now + WebContractComputation::cool_down_period() * 1000);
}
