...
pub mod pallet {
	...
	pub use webcontract_types::AttestationReport;
	// Re-export
	// TODO: Legacy
	...
	bind_topic!(RegistryEvent, b"^webcontract/registry/event");
	...
	bind_topic!(GatekeeperRegistryEvent, b"^webcontract/registry/gk_event");
	#[derive(Encode, Decode, TypeInfo, Clone, Debug)]
	...
	pub struct OldWorkerInfo<AccountId> {
		...
		/// The [confidence level](https://wiki.webcontract.network/en-us/mine/solo/1-2-confidential-level-evaluation/#confidence-level-of-a-miner)
		/// of the worker
	...
	mod test {
		...
		fn test_pruntime_allowlist_works() {
			new_test_ext().execute_with(|| {
				...
				assert_ok!(WebContractRegistry::add_pruntime(Origin::root(), sample.clone()));
				assert_noop!(
					WebContractRegistry::add_pruntime(Origin::root(), sample.clone()),
				...
				assert_ok!(WebContractRegistry::remove_pruntime(
					Origin::root(),
					sample.clone()
				));
				assert_noop!(
					WebContractRegistry::remove_pruntime(Origin::root(), sample.clone()),
		...
		fn test_relaychain_genesis_block_hash_allowlist_works() {
			new_test_ext().execute_with(|| {
				...
				assert_noop!(
					WebContractRegistry::remove_relaychain_genesis_block_hash(Origin::root(), sample),
