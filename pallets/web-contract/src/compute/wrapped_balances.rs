...
pub mod pallet {
	use crate::{
		...
		registry, vault, BalanceOf, NegativeImbalanceOf, WebContractConfig,
	};
	use frame_support::{
		pallet_prelude::*,
		traits::{
	...
	where
		...
		T: pallet_democracy::Config<Currency = <T as crate::WebContractConfig>::Currency>,
		...
		T: Config + vault::Config,
	{
		...
		pub fn wrap(origin: OriginFor<T>, amount: BalanceOf<T>) -> DispatchResult {
			...
			<T as WebContractConfig>::Currency::transfer(
				&user,
		...
		T: Config + vault::Config,
	{
		...
		pub fn remove_dust(who: &T::AccountId, dust: BalanceOf<T>) {
			...
			if dust != Zero::zero() {
				...
				let (imbalance, _remaining) = <T as WebContractConfig>::Currency::slash(
