...
pub mod pallet {
	use crate::{
		...
		registry, BalanceOf, NegativeImbalanceOf, WebContractConfig,
	};
	use frame_support::{
	...
	#[pallet::config]
	pub trait Config:
		frame_system::Config + WebContractConfig + mq::Config + registry::Config
	{
	...
	mod test {
		...
		// Pallets
		use crate::mock::{System, WebContractComputation, WebContractRegistry};
		...
		fn test_heartbeat_challenge() {
			new_test_ext().execute_with(|| {
				use web_contract_types::messaging::{SystemEvent, Topic};
				...
				assert_eq!(message.destination, Topic::new("web_contract/system/event"));
		...
		fn tokenomic_update_is_postponed() {
			new_test_ext().execute_with(|| {
				...
				assert_ok!(WebContractComputation::update_tokenomic(Origin::root(), tokenomic));
				let ev = take_events();
				assert_eq!(ev.len(), 0);
		...
		fn test_benchmark_report() {
			...
			new_test_ext().execute_with(|| {
				...
				// 110% boost
				elapse_seconds(100);
				assert_ok!(WebContractComputation::on_working_message_received(DecodedMessage::<
					WorkingReportEvent,
				> {
					sender: MessageOrigin::Worker(worker_pubkey(1)),
					destination: Topic::new(*b"web-contract/mining/report"),
		...
		fn drop_late_arrived_update() {
			new_test_ext().execute_with(|| {
				use web_contract_types::messaging::Topic;
				...
				> {
					...
					destination: Topic::new(*b"^web-contract/mining/update"),
