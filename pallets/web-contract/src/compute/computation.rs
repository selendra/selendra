...
pub mod pallet {
	...
	use crate::registry;
	use crate::{BalanceOf, NegativeImbalanceOf, WebContractConfig};
	...
	const COMPUTING_PALLETID: PalletId = PalletId(*b"webCo/pp");
	...
	#[pallet::config]
	pub trait Config: frame_system::Config + WebContractConfig + mq::Config + registry::Config {
		...
		BalanceOf<T>: FixedPointConvert,
	{
		...
		pub fn withdraw_subsidy_pool(target: &T::AccountId, value: BalanceOf<T>) -> DispatchResult {
			...
			<T as WebContractConfig>::Currency::transfer(&wallet, target, value, KeepAlive)
		}
		...
		pub fn withdraw_imbalance_from_subsidy_pool(
			...
			let wallet = Self::account_id();
			<T as WebContractConfig>::Currency::withdraw(
				&wallet,
				value,
	...
	impl Default for GenesisConfig {
		/// Default tokenomic parameters for WebContract
		fn default() -> Self {
			use fixed_macro::types::U64F64 as fp;
			let block_sec = 12;
			let hour_sec = 3600;
	...
	mod test {
		...
		// Pallets
		use crate::mock::{WebContractComputation, WebContractRegistry, System};
		...
		fn test_computing_wallet_setup() {
			new_test_ext().execute_with(|| {
				assert!(System::account_exists(&WebContractComputation::account_id()));
			});
		}
		...
		fn test_heartbeat_challenge() {
			new_test_ext().execute_with(|| {
				use webcontract_types::messaging::{SystemEvent, Topic};
				...
				assert_eq!(message.destination, Topic::new("webcontract/system/event"));
		...
		fn test_bind_unbind() {
			new_test_ext().execute_with(|| {
				...
				let _ = take_events();
				assert_ok!(WebContractComputation::bind(1, worker_pubkey(1)));
				assert_ok!(WebContractComputation::unbind_session(&1, false));
				assert_eq!(
					take_events().as_slice(),
					[
						TestEvent::WebContractComputation(Event::SessionBound {
							session: 1,
							worker: worker_pubkey(1)
						}),
						TestEvent::WebContractComputation(Event::SessionUnbound {
							session: 1,
							worker: worker_pubkey(1)
						})
					]
				...
				// Checks edge cases
				assert_noop!(
					WebContractComputation::bind(1, worker_pubkey(100)),
					Error::<Test>::WorkerNotRegistered,
				);
				assert_noop!(
					WebContractComputation::unbind_session(&1, false),
					Error::<Test>::SessionNotBound,
				);
				// No double binding
				assert_ok!(WebContractComputation::bind(2, worker_pubkey(2)));
				assert_noop!(
					WebContractComputation::bind(2, worker_pubkey(1)),
					Error::<Test>::DuplicateBoundSession
				);
				assert_noop!(
					WebContractComputation::bind(1, worker_pubkey(2)),
		...
		fn test_stakepool_callback_panic() {
			new_test_ext().execute_with(|| {
				...
				setup_workers(1);
				assert_ok!(WebContractComputation::bind(1, worker_pubkey(1)));
				assert_ok!(WebContractComputation::start_computing(1, 1000 * DOLLARS));
				// Force unbind without StakePool registration will cause a panic
				let _ = WebContractComputation::unbind(Origin::signed(1), 1);
			});
		}
		...
		fn tokenomic_update_is_postponed() {
			new_test_ext().execute_with(|| {
				...
				assert_ok!(WebContractComputation::update_tokenomic(
					Origin::root(),
					tokenomic
				));
				let ev = take_events();
				assert_eq!(ev.len(), 0);
				WebContractComputation::on_finalize(1);
				let ev = take_events();
				assert_eq!(ev.len(), 1);
		...
		fn test_benchmark_report() {
			use webcontract_types::messaging::{
			...
			new_test_ext().execute_with(|| {
				...
				setup_workers(1);
				// 100 iters per sec
				WebContractRegistry::internal_set_benchmark(&worker_pubkey(1), Some(600));
				assert_ok!(WebContractComputation::bind(1, worker_pubkey(1)));
				assert_ok!(WebContractComputation::start_computing(1, 3000 * DOLLARS));
				...
				// 110% boost
				elapse_seconds(100);
				assert_ok!(WebContractComputation::on_working_message_received(
					DecodedMessage::<WorkingReportEvent> {
						sender: MessageOrigin::Worker(worker_pubkey(1)),
						destination: Topic::new(*b"webcontract/mining/report"),
				...
				let worker = WebContractComputation::sessions(1).unwrap();
				assert_eq!(
					worker.benchmark,
					Benchmark {
						p_init: 600,
		...
		fn drop_late_arrived_update() {
			new_test_ext().execute_with(|| {
				use webcontract_types::messaging::Topic;
				...
				assert_ok!(WebContractComputation::start_computing(1, 3000 * DOLLARS));
				take_events();
				assert_ok!(WebContractComputation::on_gk_message_received(DecodedMessage::<
					WorkingInfoUpdateEvent<BlockNumber>,
				> {
					sender: MessageOrigin::Gatekeeper,
					destination: Topic::new(*b"^webcontract/mining/update"),
				...
				assert_eq!(
					ev[0],
					TestEvent::WebContractComputation(Event::SessionSettlementDropped {
						session: 1,
						v: 1,
						payout: 0,
					})
