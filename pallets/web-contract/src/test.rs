...
use webcontract_types::{messaging::SettleInfo, WorkerPublicKey};
...
fn test_vote() {
	new_test_ext().execute_with(|| {
		...
		let vote = WebContractWrappedBalances::accumulate_account_vote(0);
...
fn test_unlock() {
	new_test_ext().execute_with(|| {
		...
		assert_noop!(
			WebContractWrappedBalances::unlock(RuntimeOrigin::signed(3), 0, 1),
			wrapped_balances::Error::<Test>::ReferendumOngoing,
		);
		pallet_democracy::pallet::Pallet::<Test>::internal_cancel_referendum(0);
		assert_ok!(WebContractWrappedBalances::unlock(RuntimeOrigin::signed(3), 0, 1));
		let account1_status = wrapped_balances::pallet::StakerAccounts::<Test>::get(1).unwrap();
		...
		assert_ok!(WebContractWrappedBalances::unlock(RuntimeOrigin::signed(3), 0, 2));
		let account2_status = wrapped_balances::pallet::StakerAccounts::<Test>::get(2).unwrap();
		...
		assert_ok!(WebContractWrappedBalances::unlock(RuntimeOrigin::signed(3), 1, 2));
		let account1_status = wrapped_balances::pallet::StakerAccounts::<Test>::get(1).unwrap();
		assert_eq!(account1_status.locked, 0);
		let account2_status = wrapped_balances::pallet::StakerAccounts::<Test>::get(2).unwrap();
		assert_eq!(account2_status.locked, 0);
...
fn test_mint_nft() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(WebContractBasePool::mint_nft(
		...
		{
			assert_ok!(WebContractBasePool::get_nft_attr_guard(pool_info.basepool.cid, 0));
			assert_noop!(
				WebContractBasePool::get_nft_attr_guard(pool_info.basepool.cid, 0),
				base_pool::Error::<Test>::AttrLocked
			);
		}
		let nft_attr = WebContractBasePool::get_nft_attr_guard(pool_info.basepool.cid, 0)
			.unwrap()
			.attr
			.clone();
		assert_eq!(nft_attr.shares, 1000 * DOLLARS);
		let nft_attr = WebContractBasePool::get_nft_attr_guard(pool_info.basepool.cid, 1)
			.unwrap()
			.attr
			.clone();
	...
	new_test_ext().execute_with(|| {
		...
		));
		{
			let mut nft_attr_guard =
				WebContractBasePool::get_nft_attr_guard(pool_info.basepool.cid, 0).unwrap();
...
fn test_remove_stake_from_nft() {
	new_test_ext().execute_with(|| {
		...
		let mut nft_attr = WebContractBasePool::get_nft_attr_guard(pool.basepool.cid, nftid_arr[0])
			.unwrap()
			.attr
			.clone();
		assert_eq!(pool.basepool.share_price().unwrap(), 1);
		match WebContractBasePool::remove_stake_from_nft(
			&mut pool.basepool,
			40 * DOLLARS,
			&mut nft_attr,
			&1,
...
fn test_create_vault() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(WebContractVault::create(RuntimeOrigin::signed(2)));
		assert_eq!(
			base_pool::Pools::<Test>::get(0),
...
fn test_contribute() {
	new_test_ext().execute_with(|| {
		...
		setup_stake_pool_with_workers(1, &[1, 2]); // pid = 0
		assert_noop!(
			WebContractStakePoolv2::contribute(RuntimeOrigin::signed(1), 0, 50 * DOLLARS, None,),
		...
		assert_eq!(nftid_arr.len(), 1);
		{
			let nft_attr = WebContractBasePool::get_nft_attr_guard(pool.basepool.cid, nftid_arr[0])
				.unwrap()
				.attr
				.clone();
		...
		assert_noop!(
			WebContractStakePoolv2::contribute(RuntimeOrigin::signed(2), 0, 10 * DOLLARS, Some(1),),
...
fn test_set_pool_description() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(WebContractBasePool::set_pool_description(
			RuntimeOrigin::signed(1),
			0,
			str_hello.clone(),
		));
		let list = WebContractBasePool::pool_descriptions(0).unwrap();
		assert_eq!(list, str_hello);
		let str_bye: base_pool::DescStr = ("bye").as_bytes().to_vec().try_into().unwrap();
		assert_noop!(
			WebContractBasePool::set_pool_description(RuntimeOrigin::signed(2), 0, str_bye,),
...
fn test_staker_whitelist() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(WebContractBasePool::remove_staker_from_whitelist(
			RuntimeOrigin::signed(1),
			0,
			2
		));
		...
		assert_noop!(
			WebContractStakePoolv2::contribute(RuntimeOrigin::signed(2), 0, 20 * DOLLARS, None),
			base_pool::Error::<Test>::NotInContributeWhitelist
		);
		assert_ok!(WebContractBasePool::remove_staker_from_whitelist(
			RuntimeOrigin::signed(1),
		...
		assert!(WebContractBasePool::pool_whitelist(0).is_none());
		assert_ok!(WebContractStakePoolv2::contribute(
			RuntimeOrigin::signed(3),
			0,
			20 * DOLLARS,
...
fn test_pool_cap() {
	new_test_ext().execute_with(|| {
		...
		// Pool existence
		assert_noop!(
			WebContractStakePoolv2::set_cap(RuntimeOrigin::signed(2), 100, 1),
			base_pool::Error::<Test>::PoolDoesNotExist,
		);
		// Owner only
		assert_noop!(
			WebContractStakePoolv2::set_cap(RuntimeOrigin::signed(2), 0, 1),
			stake_pool_v2::Error::<Test>::UnauthorizedPoolOwner,
		);
		// Cap to 1000 PHA
		...
		assert_noop!(
			WebContractStakePoolv2::set_cap(RuntimeOrigin::signed(1), 0, 9 * DOLLARS),
		...
		// Exceed the cap
		assert_noop!(
			WebContractStakePoolv2::contribute(RuntimeOrigin::signed(2), 0, 90 * DOLLARS, None),
		...
		assert_ok!(WebContractStakePoolv2::start_computing(
			RuntimeOrigin::signed(1),
			0,
			worker_pubkey(1),
			100 * DOLLARS
		...
		assert_noop!(
			WebContractStakePoolv2::contribute(RuntimeOrigin::signed(2), 0, 101 * DOLLARS, None),
			stake_pool_v2::Error::<Test>::StakeExceedsCapacity
		);
		assert_ok!(WebContractStakePoolv2::contribute(
			RuntimeOrigin::signed(2),
...
fn test_add_worker() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(WebContractRegistry::force_register_worker(
			RuntimeOrigin::root(),
			worker1,
			ecdh_pubkey(1),
			Some(1)
		...
		// Bad inputs
		assert_noop!(
			WebContractStakePoolv2::add_worker(RuntimeOrigin::signed(1), 1, worker2),
			stake_pool_v2::Error::<Test>::WorkerNotRegistered
		);
		assert_noop!(
			WebContractStakePoolv2::add_worker(RuntimeOrigin::signed(2), 0, worker1),
			stake_pool_v2::Error::<Test>::UnauthorizedOperator
		);
		assert_noop!(
			WebContractStakePoolv2::add_worker(RuntimeOrigin::signed(1), 0, worker1),
		...
		assert_eq!(
			WebContractComputation::ensure_worker_bound(&worker_pubkey(1)).unwrap(),
			subaccount,
		);
		assert_eq!(
			WebContractComputation::ensure_session_bound(&subaccount).unwrap(),
			worker_pubkey(1),
		);
		// Check assignments
		assert_eq!(
		...
		// Other bad cases
		assert_noop!(
			WebContractStakePoolv2::add_worker(RuntimeOrigin::signed(1), 100, worker1),
			base_pool::Error::<Test>::PoolDoesNotExist
		);
		// Bind one worker to antoher pool (pid = 1)
		assert_ok!(WebContractStakePoolv2::create(RuntimeOrigin::signed(1)));
		assert_noop!(
			WebContractStakePoolv2::add_worker(RuntimeOrigin::signed(1), 1, worker1),
...
fn test_start_computing() {
	new_test_ext().execute_with(|| {
		...
		assert_eq!(WebContractComputation::online_workers(), 1);
		let pool = ensure_stake_pool::<Test>(0).unwrap();
...
fn test_force_unbind() {
	new_test_ext().execute_with(|| {
		...
		let worker = WebContractComputation::sessions(&sub_account).unwrap();
...
fn test_stop_computing() {
	new_test_ext().execute_with(|| {
		...
		let lock = get_balance(pool.lock_account);
		assert_eq!((balance, lock), (0, 100 * DOLLARS));
		assert_ok!(WebContractStakePoolv2::stop_computing(
			RuntimeOrigin::signed(1),
			0,
			worker_pubkey(1),
		));
...
fn test_reclaim() {
	new_test_ext().execute_with(|| {
		...
		elapse_cool_down();
		assert_ok!(WebContractStakePoolv2::reclaim_pool_worker(
			RuntimeOrigin::signed(1),
			0,
			worker_pubkey(1),
		));
...
fn restart_computing_should_work() {
	new_test_ext().execute_with(|| {
		...
		// Bad cases
		assert_noop!(
			WebContractStakePoolv2::restart_computing(
				RuntimeOrigin::signed(1),
				0,
				worker_pubkey(1),
				50 * DOLLARS
		...
		assert_ok!(WebContractStakePoolv2::restart_computing(
			RuntimeOrigin::signed(1),
			0,
			worker_pubkey(1),
			151 * DOLLARS
...
fn test_for_cdworkers() {
	new_test_ext().execute_with(|| {
		...
		assert_noop!(
			WebContractStakePoolv2::remove_worker(RuntimeOrigin::signed(1), 0, worker_pubkey(1)),
		...
		assert_ok!(WebContractStakePoolv2::remove_worker(
			RuntimeOrigin::signed(1),
			0,
			worker_pubkey(1),
		));
...
fn test_on_reward_for_vault() {
	...
	new_test_ext().execute_with(|| {
		...
		setup_vault(3); // pid = 0
		setup_stake_pool_with_workers(1, &[1]);
		assert_ok!(WebContractStakePoolv2::set_payout_pref(
			RuntimeOrigin::signed(1),
			1,
		...
		WebContractStakePoolv2::on_reward(&[SettleInfo {
...
fn test_vault_owner_shares() {
	...
	new_test_ext().execute_with(|| {
		...
		setup_workers(1);
		setup_vault(3); // pid = 0
		assert_ok!(WebContractVault::set_payout_pref(
			RuntimeOrigin::signed(3),
			0,
			Some(Permill::from_percent(50))
		));
		...
		assert_noop!(
			WebContractVault::claim_owner_shares(RuntimeOrigin::signed(3), 0, 4, 50 * DOLLARS),
			vault::Error::<Test>::NoEnoughShareToClaim
		);
		assert_ok!(WebContractVault::claim_owner_shares(
			RuntimeOrigin::signed(3),
			0,
			4,
			10 * DOLLARS
...
fn test_check_and_maybe_force_withdraw() {
	new_test_ext().execute_with(|| {
		...
		assert_eq!(get_balance(2), 400 * DOLLARS);
		assert_ok!(WebContractStakePoolv2::check_and_maybe_force_withdraw(
			RuntimeOrigin::signed(3),
			0
		));
		let pool = ensure_stake_pool::<Test>(0).unwrap();
...
fn setup_stake_pool_with_workers(owner: u64, workers: &[u8]) -> u64 {
	let pid = WebContractBasePool::pool_count();
	assert_ok!(WebContractStakePoolv2::create(RuntimeOrigin::signed(owner)));
	for id in workers {
		assert_ok!(WebContractStakePoolv2::add_worker(
...
fn setup_vault(owner: u64) -> u64 {
	let pid = WebContractBasePool::pool_count();
	assert_ok!(WebContractVault::create(RuntimeOrigin::signed(owner)));
	pid
}
