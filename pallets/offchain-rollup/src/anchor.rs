...
//! kv-store for Web contract to read and write, and allows the Web contract to send arbitrary
//! messages to the blockchain to trigger custom actions.
//!
//! Off-chain Rollup enables ACID operations on the blockchain for Web contracts. The kv-stroe
...
//! Transaction submitted by the rollup client running in Web contract, validates it, and aplly
//! the changes.
//!
//! On the other hand, the pallet provides two features to the other pallets:
//!
//! 1. Push messages to the Web contract via the `push_message(name, content)`
//! 2. Receive messages from Web contract by handling `Config::OnResponse` callback trait
//!
//! ## Register a contract
//!
//! The anchor pallet allows arbitrary Web contract to connect to it. Before using, the Web
...
//!
//! The _submitter account_ should be an account solely controlled by the Web contract. After
...
//!
//! The anchor pallet provides a message queue to help pass messages to the Web contracts:
//!
...
//! ## Receive a message
//!
//! The anchor pallet allows Web contracts to send message back to the blockchain. To subscribe
