...
fn test_vote() {
	new_test_ext().execute_with(|| {
		...
		let vote = IndraWrappedBalances::accumulate_account_vote(0);
...
fn test_unlock() {
	new_test_ext().execute_with(|| {
		...
		assert_noop!(
			IndraWrappedBalances::unlock(RuntimeOrigin::signed(3), 0, 1),
			wrapped_balances::Error::<Test>::ReferendumOngoing,
		);
		pallet_democracy::pallet::Pallet::<Test>::internal_cancel_referendum(0);
		assert_ok!(IndraWrappedBalances::unlock(RuntimeOrigin::signed(3), 0, 1));
		let account1_status = wrapped_balances::pallet::StakerAccounts::<Test>::get(1).unwrap();
		...
		assert_ok!(IndraWrappedBalances::unlock(RuntimeOrigin::signed(3), 0, 2));
		let account2_status = wrapped_balances::pallet::StakerAccounts::<Test>::get(2).unwrap();
		...
		assert_ok!(IndraWrappedBalances::unlock(RuntimeOrigin::signed(3), 1, 2));
		let account1_status = wrapped_balances::pallet::StakerAccounts::<Test>::get(1).unwrap();
		assert_eq!(account1_status.locked, 0);
		let account2_status = wrapped_balances::pallet::StakerAccounts::<Test>::get(2).unwrap();
		assert_eq!(account2_status.locked, 0);
...
fn test_mint_nft() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(IndraBasePool::mint_nft(
		...
		{
			assert_ok!(IndraBasePool::get_nft_attr_guard(pool_info.basepool.cid, 0));
			assert_noop!(
				IndraBasePool::get_nft_attr_guard(pool_info.basepool.cid, 0),
				base_pool::Error::<Test>::AttrLocked
			);
		}
		let nft_attr = IndraBasePool::get_nft_attr_guard(pool_info.basepool.cid, 0)
			.unwrap()
			.attr
			.clone();
		assert_eq!(nft_attr.shares, 1000 * DOLLARS);
		let nft_attr = IndraBasePool::get_nft_attr_guard(pool_info.basepool.cid, 1)
			.unwrap()
			.attr
			.clone();
...
fn test_merge_nft() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(IndraBasePool::merge_nft_for_staker(
			pool_info.basepool.cid,
			1,
			pool_info.basepool.pid,
		));
		...
		{
			let nft_attr = IndraBasePool::get_nft_attr_guard(pool_info.basepool.cid, nftid_arr[0])
				.unwrap()
				.attr
				.clone();
	...
	new_test_ext().execute_with(|| {
		...
		));
		{
			let mut nft_attr_guard =
				IndraBasePool::get_nft_attr_guard(pool_info.basepool.cid, 0).unwrap();
...
fn test_remove_stake_from_nft() {
	new_test_ext().execute_with(|| {
		...
		let mut nft_attr = IndraBasePool::get_nft_attr_guard(pool.basepool.cid, nftid_arr[0])
			.unwrap()
			.attr
			.clone();
		assert_eq!(pool.basepool.share_price().unwrap(), 1);
		match IndraBasePool::remove_stake_from_nft(
			&mut pool.basepool,
			40 * DOLLARS,
			&mut nft_attr,
			&1,
...
fn test_create_vault() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(IndraVault::create(RuntimeOrigin::signed(2)));
		assert_eq!(
			base_pool::Pools::<Test>::get(0),
...
fn test_contribute() {
	new_test_ext().execute_with(|| {
		...
		setup_stake_pool_with_workers(1, &[1, 2]); // pid = 0
		assert_noop!(
			IndraStakePoolv2::contribute(RuntimeOrigin::signed(1), 0, 50 * DOLLARS, None,),
		...
		assert_eq!(nftid_arr.len(), 1);
		{
			let nft_attr = IndraBasePool::get_nft_attr_guard(pool.basepool.cid, nftid_arr[0])
				.unwrap()
				.attr
				.clone();
		...
		assert_noop!(
			IndraStakePoolv2::contribute(RuntimeOrigin::signed(2), 0, 10 * DOLLARS, Some(1),),
...
fn test_set_pool_description() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(IndraBasePool::set_pool_description(
			RuntimeOrigin::signed(1),
			0,
			str_hello.clone(),
		));
		let list = IndraBasePool::pool_descriptions(0).unwrap();
		assert_eq!(list, str_hello);
		let str_bye: base_pool::DescStr = ("bye").as_bytes().to_vec().try_into().unwrap();
		assert_noop!(
			IndraBasePool::set_pool_description(RuntimeOrigin::signed(2), 0, str_bye,),
...
fn test_pool_cap() {
	new_test_ext().execute_with(|| {
		...
		// Pool existence
		assert_noop!(
			IndraStakePoolv2::set_cap(RuntimeOrigin::signed(2), 100, 1),
...
fn test_add_worker() {
	new_test_ext().execute_with(|| {
		...
		// Bad inputs
		assert_noop!(
			IndraStakePoolv2::add_worker(RuntimeOrigin::signed(1), 1, worker2),
			stake_pool_v2::Error::<Test>::WorkerNotRegistered
		);
		assert_noop!(
			IndraStakePoolv2::add_worker(RuntimeOrigin::signed(2), 0, worker1),
			stake_pool_v2::Error::<Test>::UnauthorizedOperator
		);
		assert_noop!(
			IndraStakePoolv2::add_worker(RuntimeOrigin::signed(1), 0, worker1),
		...
		// Other bad cases
		assert_noop!(
			IndraStakePoolv2::add_worker(RuntimeOrigin::signed(1), 100, worker1),
			base_pool::Error::<Test>::PoolDoesNotExist
		);
		// Bind one worker to antoher pool (pid = 1)
		assert_ok!(IndraStakePoolv2::create(RuntimeOrigin::signed(1)));
		assert_noop!(
			IndraStakePoolv2::add_worker(RuntimeOrigin::signed(1), 1, worker1),
...
fn test_start_computing() {
	new_test_ext().execute_with(|| {
		...
		assert_noop!(
			IndraStakePoolv2::start_computing(
				RuntimeOrigin::signed(1),
				0,
				worker_pubkey(1),
				30000 * DOLLARS
		...
		assert_eq!(IndraComputation::online_workers(), 1);
		let pool = ensure_stake_pool::<Test>(0).unwrap();
...
fn test_force_unbind() {
	new_test_ext().execute_with(|| {
		...
		let worker = IndraComputation::sessions(&sub_account).unwrap();
...
fn restart_computing_should_work() {
	new_test_ext().execute_with(|| {
		...
		// Bad cases
		assert_noop!(
			IndraStakePoolv2::restart_computing(
				RuntimeOrigin::signed(1),
				0,
				worker_pubkey(1),
				50 * DOLLARS
		...
		assert_ok!(IndraStakePoolv2::restart_computing(
			RuntimeOrigin::signed(1),
			0,
			worker_pubkey(1),
			151 * DOLLARS
...
fn test_for_cdworkers() {
	new_test_ext().execute_with(|| {
		...
		assert_noop!(
			IndraStakePoolv2::remove_worker(RuntimeOrigin::signed(1), 0, worker_pubkey(1)),
...
fn test_on_reward_for_vault() {
	...
	new_test_ext().execute_with(|| {
		...
		setup_vault(3); // pid = 0
		setup_stake_pool_with_workers(1, &[1]);
		assert_ok!(IndraStakePoolv2::set_payout_pref(
			RuntimeOrigin::signed(1),
			1,
...
fn test_vault_owner_shares() {
	...
	new_test_ext().execute_with(|| {
		...
		setup_workers(1);
		setup_vault(3); // pid = 0
		assert_ok!(IndraVault::set_payout_pref(
			RuntimeOrigin::signed(3),
			0,
			Some(Permill::from_percent(50))
		));
		...
		assert_eq!(nftid_arr.len(), 1);
		{
			let nft_attr = IndraBasePool::get_nft_attr_guard(vault_info.basepool.cid, nftid_arr[0])
				.unwrap()
				.attr
				.clone();
...
fn test_withdraw() {
	new_test_ext().execute_with(|| {
		...
		assert_eq!(nftid_arr.len(), 1);
		{
			let user_nft_attr = IndraBasePool::get_nft_attr_guard(pool.basepool.cid, nftid_arr[0])
				.unwrap()
				.attr
				.clone();
		...
		assert_eq!(nftid_arr.len(), 1);
		{
			let user_nft_attr = IndraBasePool::get_nft_attr_guard(vault.basepool.cid, nftid_arr[0])
				.unwrap()
				.attr
				.clone();
			assert_eq!(user_nft_attr.shares, 100 * DOLLARS);
...
fn setup_stake_pool_with_workers(owner: u64, workers: &[u8]) -> u64 {
	let pid = IndraBasePool::pool_count();
	assert_ok!(IndraStakePoolv2::create(RuntimeOrigin::signed(owner)));
	for id in workers {
...
fn setup_vault(owner: u64) -> u64 {
	let pid = IndraBasePool::pool_count();
	assert_ok!(IndraVault::create(RuntimeOrigin::signed(owner)));
	pid
}
