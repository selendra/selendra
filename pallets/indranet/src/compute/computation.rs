...
pub mod pallet {
	...
	const COMPUTING_PALLETID: PalletId = PalletId(*b"indra/pp");
		...
		BalanceOf<T>: FixedPointConvert,
	{
		...
		pub fn withdraw_subsidy_pool(target: &T::AccountId, value: BalanceOf<T>) -> DispatchResult {
			...
			<T as IndranetConfig>::Currency::transfer(&wallet, target, value, KeepAlive)
		}
		...
		pub fn withdraw_imbalance_from_subsidy_pool(
			...
			let wallet = Self::account_id();
			<T as IndranetConfig>::Currency::withdraw(
				&wallet,
				value,
	...
	impl Default for GenesisConfig {
		/// Default tokenomic parameters for Indranet
		fn default() -> Self {
			use fixed_macro::types::U64F64 as fp;
			let block_sec = 12;
			let hour_sec = 3600;
	...
	mod test {
		...
		// Pallets
		use crate::mock::{IndraComputation, IndraRegistry, System};
		...
		fn test_computing_wallet_setup() {
			new_test_ext().execute_with(|| {
				assert!(System::account_exists(&IndraComputation::account_id()));
			});
		}
		...
		fn test_heartbeat_challenge() {
			new_test_ext().execute_with(|| {
				use indranet_types::messaging::{SystemEvent, Topic};
				...
				assert_eq!(message.destination, Topic::new("indranet/system/event"));
		...
		fn test_bind_unbind() {
			new_test_ext().execute_with(|| {
				...
				assert_ok!(IndraComputation::unbind_session(&1, false));
				assert_eq!(
					take_events().as_slice(),
					[
						TestEvent::IndraComputation(Event::SessionBound {
							session: 1,
							worker: worker_pubkey(1)
						}),
						TestEvent::IndraComputation(Event::SessionUnbound {
							session: 1,
							worker: worker_pubkey(1)
						})
					]
				...
				// Checks edge cases
				assert_noop!(
					IndraComputation::bind(1, worker_pubkey(100)),
					Error::<Test>::WorkerNotRegistered,
				);
				assert_noop!(
					IndraComputation::unbind_session(&1, false),
					Error::<Test>::SessionNotBound,
				);
				// No double binding
				assert_ok!(IndraComputation::bind(2, worker_pubkey(2)));
				assert_noop!(
					IndraComputation::bind(2, worker_pubkey(1)),
					Error::<Test>::DuplicateBoundSession
				);
				assert_noop!(
					IndraComputation::bind(1, worker_pubkey(2)),
		...
		fn test_stakepool_callback_panic() {
			new_test_ext().execute_with(|| {
				...
				setup_workers(1);
				assert_ok!(IndraComputation::bind(1, worker_pubkey(1)));
				assert_ok!(IndraComputation::start_computing(1, 1000 * DOLLARS));
				// Force unbind without StakePool registration will cause a panic
				let _ = IndraComputation::unbind(Origin::signed(1), 1);
			});
		}
		...
		fn test_benchmark_report() {
			...
			new_test_ext().execute_with(|| {
				...
				setup_workers(1);
				// 100 iters per sec
				IndraRegistry::internal_set_benchmark(&worker_pubkey(1), Some(600));
				assert_ok!(IndraComputation::bind(1, worker_pubkey(1)));
		...
		fn drop_late_arrived_update() {
			new_test_ext().execute_with(|| {
				use indranet_types::messaging::Topic;
				...
				assert_ok!(IndraComputation::start_computing(1, 3000 * DOLLARS));
				take_events();
				assert_ok!(IndraComputation::on_gk_message_received(DecodedMessage::<
					WorkingInfoUpdateEvent<BlockNumber>,
				> {
					sender: MessageOrigin::Gatekeeper,
					destination: Topic::new(*b"^indranet/mining/update"),
				...
				assert_eq!(
					ev[0],
					TestEvent::IndraComputation(Event::SessionSettlementDropped {
						session: 1,
						v: 1,
						payout: 0,
					})
