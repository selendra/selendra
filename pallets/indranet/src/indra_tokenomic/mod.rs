//! The Indra Contract tokenomic module
...
pub mod pallet {
	...
	const PALLET_ID: PalletId = PalletId(*b"indra/tk");
	...
	where
		T: crate::mq::Config,
		T: crate::indra::Config,
	{
		/// Adjust stake to given contract.
		///
		/// Indra contracts accept depoits from accounts. The deposit info would be sent the cluster's
		...
		pub fn adjust_stake(
			...
			if let Some(the_system_contract) =
				crate::indra::Pallet::<T>::get_system_contract(&contract)
			{
