...
use indranet_types::messaging::Message;
use sp_core::H256;
use sp_runtime::{
	testing::Header,
...
frame_support::construct_runtime!(
		...
		UncheckedExtrinsic = UncheckedExtrinsic,
	{
		...
		// Pallets to test
		IndraMq: mq::{Pallet, Call},
		IndraRegistry: registry::{Pallet, Event<T>, Storage, Config<T>},
		IndraComputation: computation::{Pallet, Event<T>, Storage, Config},
		IndraStakePoolv2: stake_pool_v2::{Pallet, Event<T>},
		IndraVault: vault::{Pallet, Event<T>},
		IndraWrappedBalances: wrapped_balances::{Pallet, Event<T>},
		IndraBasePool: base_pool::{Pallet, Event<T>},
		IndraStakePool: stake_pool::{Event<T>},
		Preimage: pallet_preimage::{Event<T>},
	}
);

impl crate::IndranetConfig for Test {
...
impl mq::CallMatcher<Test> for MqCallMatcher {
	fn match_call(call: &RuntimeCall) -> Option<&mq::Call<Test>> {
		match call {
			RuntimeCall::IndraMq(mq_call) => Some(mq_call),
			_ => None,
		}
	}
}
...
impl pallet_rmrk_core::Config for Test {
	...
	type TransferHooks = IndraWrappedBalances;
...
impl computation::Config for Test {
	...
	type OnReward = IndraStakePoolv2;
	type OnUnbound = IndraStakePoolv2;
	type OnStopped = IndraStakePoolv2;
...
pub fn new_test_ext() -> sp_io::TestExternalities {
	...
	pallet_balances::GenesisConfig::<Test> {
		balances: vec![
			...
			(IndraComputation::account_id(), 690_000_000 * DOLLARS),
		],
	}
...
pub fn set_block_1() {
	...
	IndraRegistry::internal_set_gk_launched_at(0, 0);
}
...
pub fn take_messages() -> Vec<Message> {
	let messages = IndraMq::messages();
	println!("messages(): {messages:?}");
	mq::OutboundMessages::<Test>::kill();
	messages
}

use indranet_types::{EcdhPublicKey, WorkerPublicKey};
...
pub fn setup_workers(n: u8) {
	...
	for i in 1..=n {
		...
		IndraRegistry::internal_set_benchmark(&worker, Some(1));
	}
}
...
pub fn elapse_cool_down() {
	let now = Timestamp::get();
	Timestamp::set_timestamp(now + IndraComputation::cool_down_period() * 1000);
}
