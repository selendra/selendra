...
impl<T: Config> Pallet<T> {
	...
	fn blocks_to_produce_per_session() -> u32 {
		T::SessionPeriod::get().saturating_div(
			T::ValidatorProvider::current_era_committee_size().unwrap_or_default().size(),
		)
	}
	...
	{
		let EraValidators { reserved, non_reserved } =
			T::ValidatorProvider::current_era_validators()?;
		let CommitteeSeats { reserved_seats, non_reserved_seats, non_reserved_finality_seats } =
	...
	pub(crate) fn clear_underperformance_session_counter(session: SessionIndex) {
		...
		if session % clean_session_counter_delay == 0 {
			info!(target: LOG_TARGET, "Clearing UnderperformedValidatorSessionCount");
	...
	pub fn emit_fresh_bans_event() {
		...
		if !fresh_bans.is_empty() {
			info!(target: LOG_TARGET, "Fresh bans in era {}: {:?}", active_era, fresh_bans);
