	...
	ExtBuilder::build().execute_with(|| {
		...
		for new_base_fee_per_gas in [
			...
			System::assert_last_event(mock::RuntimeEvent::DynamicEvmBaseFee(
				Event::NewBaseFeePerGas { fee: new_base_fee_per_gas },
			));
...
fn min_gas_price_works() {
	ExtBuilder::build().execute_with(|| {
		...
		assert_eq!(DynamicEvmBaseFee::min_gas_price(), (new_base_fee_per_gas, expected_weight));
	});
}
...
fn unit_adjustment_factor_no_change() {
	ExtBuilder::build().execute_with(|| {
		...
		assert_eq!(BaseFeePerGas::<TestRuntime>::get(), init_bfpg, "bfpg should remain the same");
	});
}
