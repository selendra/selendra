...
use selendra_primitives::{evm::task::TaskResult, BlockNumber, Index};
use sp_runtime::{
...
pub mod pallet {
	...
	pub trait Config: frame_system::Config {
		...
		/// Number of blocks skipped to disable `on_idle` dispatching scheduled tasks,
		/// this shuts down idle-scheduler when block production is slower than finalize
		#[pallet::constant]
	...
	impl<T: Config> Hooks<T::BlockNumber> for Pallet<T> {
		fn on_initialize(_n: T::BlockNumber) -> Weight {
			...
			let previous_relay_block: BlockNumber = T::BlockNumberProvider::current_block_number();

			PreviousBlockNumber::<T>::put(previous_relay_block);
		...
		fn on_idle(_n: T::BlockNumber, remaining_weight: Weight) -> Weight {
			...
			let previous_relay_block_number = PreviousBlockNumber::<T>::take();
