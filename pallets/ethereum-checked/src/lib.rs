...
//! ## Overview
//!
//! A `pallet-ethereum` like pallet that executes transactions from checked sources,
//! like cross-VM calls, system-level calls, etc. Only `Call` transactions are supported
//! (no `Create`).
//!
...
//! ### Dispatch-able calls
//!
//! - `transact`: transact an Ethereum transaction from a signed origin.
//!
//! ### Implementation
//!
...
pub mod pallet {
    ...
    pub trait Config: frame_system::Config + pallet_evm::Config {
        ...
        /// Xvm transaction weight limit, for block gas limit calculation.
    ...
    pub enum Event<T: Config> {
        ...
        Executed {
            from: H160,
            to: H160,
            transaction_hash: ethereum_types::H256,
    ...
    impl<T: Config> Pallet<T> {
        /// Transact an Ethereum transaction from a signed origin.
        #[pallet::call_index(0)]
        #[pallet::weight({
        ...
        pub fn transact(origin: OriginFor<T>, tx: CheckedEthereumTx) -> DispatchResultWithPostInfo {
            ...
            let from = T::AddressMapper::to_h160_or_default(&source).into_address();
            Self::do_transact(
                from,
                tx.clone(),
                CheckedEthereumTxKind::Call,
                false,
            )
            .map(|(post_info, _call_info)| {
                Self::deposit_event(Event::Executed {
                    from,
                    to: tx.target,
                    transaction_hash: ethereum_types::H256::zero(), // Will be set properly by Ethereum pallet
...
impl<T: Config> Pallet<T> {
    ...
    fn block_gas_limit(tx_kind: &CheckedEthereumTxKind) -> u64 {
        let weight_limit = match tx_kind {
            CheckedEthereumTxKind::Call => T::CheckedTxWeightLimit::get(),
