...
use pallets_support::{mock::MockAddressMapping, AddressMapping};
use sp_core::{
...
fn fail_call_return_ok_and_inc_nonce() {
	new_test_ext().execute_with(|| {
		...
		// success call
		assert_ok!(EVM::call(origin.clone(), contract_b(), Vec::new(), 0, 1000000, 0, vec![]));
		// nonce inc by 1
		assert_eq!(EVM::account_basic(&alice).nonce, U256::from(2));
...
fn should_transfer_from_contract() {
	...
	new_test_ext().execute_with(|| {
		...
		assert_eq!(balance(alice()), alice_balance);
		assert_eq!(eth_balance(alice()), U256::from(balance(alice())));
		...
		assert_eq!(eth_balance(charlie()), U256::from(balance(charlie())));
		...
		.unwrap();
		assert_eq!(result.exit_reason, ExitReason::Succeed(ExitSucceed::Returned));
		assert_eq!(U256::from(result.value.as_slice()), U256::from(balance(charlie())));
	})
}
...
fn create_predeploy_contract_works() {
	...
	new_test_ext().execute_with(|| {
		...
		System::assert_last_event(RuntimeEvent::EVM(crate::Event::ContractPublished {
			contract: addr,
		}));
...
fn should_enable_contract_development() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(EVM::enable_contract_development(RuntimeOrigin::signed(alice_account_id)));
...
fn should_selfdestruct_without_schedule_task() {
	...
	new_test_ext().execute_with(|| {
		...
		assert_ok!(EVM::selfdestruct(RuntimeOrigin::signed(alice_account_id), contract_address));
...
fn should_selfdestruct_with_schedule_task() {
	...
	new_test_ext().execute_with(|| {
		...
		let code_hash =
			H256::from_str("7c96b02b6e32519ac1f47de5dd18efa07efd70b7eb57fc7a3d599eafa8329cd1")
				.unwrap();
...
fn auto_publish_works() {
	let json: serde_json::Value =
		serde_json::from_str(include_str!("../contract/CreateContractFactory.json")).unwrap();
	...
	new_test_ext().execute_with(|| {
			...
			logs: vec![
				crate::Log {
					...
					topics: vec![
						...
						.unwrap(),
						H256::from_str(
							"0x00000000000000000000000030f612c54706d40f65acaf10b8f6989103c2af58",
						)
						.unwrap(),
					],
					data: vec![],
		...
		assert_eq!(
			...
			Some(ContractInfo {
				code_hash: H256::from_str(
					"0xd007bd109daec7dec73d897c079b67b3d2fd6ad4892a916c5e03e21bb60ff384"
				)
				.unwrap(),
				maintainer: alice(),
				published: false
		...
		assert_eq!(
			...
			Some(ContractInfo {
				code_hash: H256::from_str(
					"0xe12fa7753d9cd8de1f8b597fef33ab91c2749fe4a1022b648f949ab2566f391f"
				)
				.unwrap(),
				maintainer: factory,
				published: false
			...
			logs: vec![
				crate::Log {
					...
					topics: vec![
						...
						.unwrap(),
						H256::from_str(
							"0x000000000000000000000000769a55efaf4dbdd6f44efce668455522b61abb82",
						)
						.unwrap(),
					],
					data: vec![],
				...
				crate::Log {
					...
					topics: vec![
						...
						.unwrap(),
						H256::from_str(
							"0x00000000000000000000000039b26a36a8a175ce7d498b5ef187d1ab2f381bbd",
						)
						.unwrap(),
					],
					data: vec![],
		...
		System::assert_last_event(RuntimeEvent::EVM(crate::Event::Executed {
			...
			logs: vec![crate::Log {
				...
				topics: vec![
					...
					.unwrap(),
					H256::from_str(
						"0x000000000000000000000000d8a09b53762a01c2beb363d5355f4eecf7b48360",
					)
					.unwrap(),
				],
				data: vec![],
...
fn aggregated_storage_logs_works() {
	...
	new_test_ext().execute_with(|| {
		...
		let cost_per_byte = StorageDepositPerByte::get();
		...
		.unwrap();
		// get storage address
		let contract_acc = <Runtime as Config>::AddressMapping::get_account_id(&H160::from(
