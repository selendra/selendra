...
use pallet_evm_utility::{
	ethereum::Log,
	evm::{
		backend::Backend, Capture, Config, Context, CreateScheme, ExitError, ExitFatal, ExitReason,
		ExitRevert, ExitSucceed, Opcode, Runtime, Stack, Transfer,
	},
	evm_gasometer::{self as gasometer, Gasometer, StorageTarget},
	evm_runtime::Handler,
};
pub use primitives::evm::{
	EvmAddress, ReserveIdentifier, Vicinity, H160_POSITION_CURRENCY_ID_TYPE,
	H160_POSITION_TOKEN_NFT, MIRRORED_NFT_ADDRESS_START, SYSTEM_CONTRACT_ADDRESS_PREFIX,
};
...
mod tracing {
	pub struct Tracer;
	impl pallet_evm_utility::evm::tracing::EventListener for Tracer {
		fn event(&mut self, event: pallet_evm_utility::evm::tracing::Event) {
			frame_support::log::debug!(
				target: "evm", "evm tracing: {:?}", event
			);
		}
	}
	impl pallet_evm_utility::evm_runtime::tracing::EventListener for Tracer {
		fn event(&mut self, event: pallet_evm_utility::evm_runtime::tracing::Event) {
			frame_support::log::debug!(
				target: "evm", "evm_runtime tracing: {:?}", event
			);
		}
	}
	impl pallet_evm_utility::evm_gasometer::tracing::EventListener for Tracer {
		fn event(&mut self, event: pallet_evm_utility::evm_gasometer::tracing::Event) {
			frame_support::log::debug!(
...
macro_rules! emit_exit {
	($reason:expr) => {{
		let reason = $reason;
		event!(Exit { reason: &reason, return_value: &Vec::new() });
		reason
	}};
	($reason:expr, $return_value:expr) => {{
		let reason = $reason;
		let return_value = $return_value;
		event!(Exit { reason: &reason, return_value: &return_value });
		(reason, return_value)
	}};
}
...
impl<'config> StackSubstateMetadata<'config> {
	pub fn new(gas_limit: u64, storage_limit: u32, config: &'config Config) -> Self {
		let accessed =
			if config.increase_state_access_gas { Some(Accessed::default()) } else { None };
		Self {
	...
	pub fn swallow_commit(&mut self, other: Self) -> Result<(), ExitError> {
		...
		{
			self_accessed.accessed_addresses.append(&mut other_accessed.accessed_addresses);
	...
	StackExecutor<'config, 'precompiles, S, P>
{
	...
	pub fn new_with_precompiles(
		...
		Self { config, state, precompile_set }
	}
	...
	pub fn transact_create_at_address(
		...
		event!(TransactCreate { caller, value, init_code: &init_code, gas_limit, address });
	...
	pub fn transact_call(
		...
		event!(TransactCall { caller, address, value, data: &data, gas_limit });
	...
	fn call_inner(
		...
		*self.state.metadata_mut().target_mut() = Some(code_address);
		...
		let reason = self.execute(&mut runtime);
		#[cfg(feature = "tracing")]
		//let reason = pallet_evm_utility::evm::tracing::using(&mut Tracer, || self.execute(&mut runtime));
		let reason = pallet_evm_utility::evm_runtime::tracing::using(&mut Tracer, || {
			self.execute(&mut runtime)
		});
		//let reason = pallet_evm_utility::evm_gasometer::tracing::using(&mut Tracer, || self.execute(&mut
		// runtime));
	...
	for StackExecutor<'config, 'precompiles, S, P>
{
	...
	fn mark_delete(&mut self, address: H160, target: H160) -> Result<(), ExitError> {
		...
		event!(Suicide { target, address, balance });

		self.state.transfer(Transfer { source: address, target, value: balance })?;
