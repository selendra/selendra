...
pub use webcontract_mq::{ContractClusterId, ContractId};
...
pub mod messaging {
    ...
    use webcontract_mq::bind_topic;
    use sp_core::crypto::AccountId32;

    bind_topic!(ClusterEvent, b"webcontract/cluster/event");
    ...
    bind_topic!(ContractOperation<CodeHash, AccountId>, b"webcontract/contract/op");
    #[derive(Encode, Decode, Debug)]
    ...
    bind_topic!(WorkerClusterReport, b"webcontract/cluster/worker/report");
    ...
    bind_topic!(ClusterOperation<AccountId>, b"webcontract/cluster/key");
...
pub fn command_topic(id: ContractId) -> Vec<u8> {
    format!("webcontract/contract/{}/command", hex::encode(id))
        .as_bytes()
        .to_vec()
}
