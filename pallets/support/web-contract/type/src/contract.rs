...
pub use web_contract_mq::{ContractClusterId, ContractId};
...
pub mod messaging {
	...
	use crate::{messaging::EncryptedKey, ClusterPublicKey, WorkerIdentity, WorkerPublicKey};
	use sp_core::crypto::AccountId32;
	use web_contract_mq::bind_topic;

	bind_topic!(ClusterEvent, b"web_contract/cluster/event");
	...
	bind_topic!(ContractOperation<CodeHash, AccountId>, b"web_contract/contract/op");
	#[derive(Encode, Decode, Debug)]
	...
	bind_topic!(WorkerClusterReport, b"web_contract/cluster/worker/report");
	#[derive(Encode, Decode, Debug, TypeInfo)]
	pub enum WorkerClusterReport {
		ClusterDeployed { id: ContractClusterId, pubkey: ClusterPublicKey },
		ClusterDeploymentFailed { id: ContractClusterId },
	}
	...
	bind_topic!(ClusterOperation<AccountId>, b"web_contract/cluster/key");
	...
	pub enum ClusterOperation<AccountId> {
		...
		///
		/// This leaves a door to clean up the beta clusters in WebContract v1.
...
pub fn command_topic(id: ContractId) -> Vec<u8> {
	format!("web_contract/contract/{}/command", hex::encode(id)).as_bytes().to_vec()
}
