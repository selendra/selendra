...
use crate::{
	simple_mpsc::{channel, ReceiveError, Receiver as RawReceiver, Sender, Seq},
	types::{Message, Path},
	BindTopic, MessageOrigin,
};
...
impl MessageDispatcher {
	pub fn new() -> Self {
		MessageDispatcher { subscribers: Default::default(), local_index: 0 }
	}
...
impl<T> Clone for TypedReceiver<T> {
	fn clone(&self) -> Self {
		Self { queue: self.queue.clone(), _t: self._t }
	}
}
