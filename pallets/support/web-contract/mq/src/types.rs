...
pub use sp_core::{H256 as ContractId, H256 as AccountId, H256 as ContractClusterId};

use crate::MessageSigner;
use serde::{Deserialize, Serialize};
use serde_more as more;

/// The origin of a web_contract message
...
impl MessageOrigin {
	...
	pub fn is_offchain(&self) -> bool {
		matches!(self, Self::Cluster(_) | Self::Contract(_) | Self::Worker(_) | Self::Gatekeeper)
	}
...
impl Message {
	pub fn new(
		...
		Message { sender: sender.into(), destination: Topic::new(destination), payload }
	}
...
impl SignedMessage {
	pub fn data_be_signed(&self) -> Vec<u8> {
		MessageToBeSigned { message: &self.message, sequence: self.sequence }.raw_data()
	}
}
...
impl<Signer: MessageSigner> SigningMessage<Signer> {
	pub fn sign(self, sequence: u64) -> SignedMessage {
		let data = MessageToBeSigned { message: &self.message, sequence };
		let signature = self.signer.sign(&data.raw_data());
		SignedMessage { message: self.message, sequence, signature }
	}
}
