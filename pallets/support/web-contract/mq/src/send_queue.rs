...
impl<'de> Deserialize<'de> for MessageSendQueue {
		...
		D: serde::Deserializer<'de>,
	{
		...
		Ok(MessageSendQueue { inner: Arc::new(Mutex::new(inner)) })
	}
}

impl MessageSendQueue {
	pub fn new() -> Self {
		MessageSendQueue { inner: Default::default() }
	}
	...
	pub fn enqueue_message(
		...
		if !entry.dummy {
			...
			if log::log_enabled!(target: "web_contract_mq", log::Level::Debug) {
				log::debug!(target: "web_contract_mq",
				...
				log::info!(target: "web_contract_mq",
	...
	pub fn all_messages(&self) -> Vec<SignedMessage> {
		let inner = self.inner.lock();
		inner.iter().flat_map(|(_k, v)| v.messages.iter().cloned()).collect()
	}

	pub fn all_messages_grouped(&self) -> BTreeMap<MessageOrigin, Vec<SignedMessage>> {
		let inner = self.inner.lock();
		inner.iter().map(|(k, v)| (k.clone(), v.messages.clone())).collect()
	}
	...
	pub fn count_messages(&self) -> usize {
		self.inner.lock().iter().map(|(_k, v)| v.messages.len()).sum()
	}
...
mod msg_channel {
	...
	impl<Si> MessageChannel<Si> {
		pub fn new(queue: MessageSendQueue, sender: SenderId, signer: Si) -> Self {
			MessageChannel { queue, sender, signer }
		}
	}
	...
	impl<Si: MessageSigner + Clone> MessageChannel<Si> {
		fn prepare_with_data(
			...
			let message = Message { sender, destination: to.into().into(), payload };
