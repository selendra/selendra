...
use selendra_primitives::{
	BanConfig as BanConfigStruct, BanInfo, BanReason, CommitteeSeats, EraValidators,
};
	...
	T: Config,
{
	...
	fn get_committee_and_non_committee() -> (Vec<T::AccountId>, Vec<T::AccountId>) {
		...
		let EraValidators { reserved, non_reserved } = CurrentEraValidators::<T>::get();
	...
	fn rotate_committee(current_session: SessionIndex) -> Option<Vec<T::AccountId>> {
		...
		let EraValidators { reserved, non_reserved } = CurrentEraValidators::<T>::get();
		let CommitteeSeats { reserved_seats, non_reserved_seats } = CommitteeSize::<T>::get();
	...
	fn populate_next_era_validators_on_next_era_start(session: SessionIndex) {
		...
		Self::if_era_starts_do(active_era + 1, session, || {
			...
			let retain_elected = |vals: Vec<T::AccountId>| -> Vec<T::AccountId> {
				vals.into_iter().filter(|v| elected_committee.contains(v)).collect()
			};
...
mod tests {
	...
	fn adjusted_session_points_all_blocks_created_are_calculated_correctly() {
		...
		assert_eq!(6250000, calculate_adjusted_session_points(96, 900, 900, 600_000_000));

		assert_eq!(6145833, calculate_adjusted_session_points(96, 900, 900, 590_000_000));
	}
	...
	fn adjusted_session_points_above_90_perc_are_calculated_correctly() {
		...
		assert_eq!(6250000, calculate_adjusted_session_points(96, 900, 811, 600_000_000));

		assert_eq!(6145833, calculate_adjusted_session_points(96, 900, 899, 590_000_000));
	}

	#[test]
	fn adjusted_session_points_more_than_all_blocks_created_are_calculated_correctly() {
		assert_eq!(5000, calculate_adjusted_session_points(5, 30, 2 * 30, 25_000));

		assert_eq!(6250000, calculate_adjusted_session_points(96, 900, 3 * 900, 600_000_000));

		assert_eq!(6145833, calculate_adjusted_session_points(96, 900, 901, 590_000_000));
	}
