...
impl<T: Config, P: PalletInfoAccess> OnRuntimeUpgrade for Migration<T, P> {
	...
	fn pre_upgrade() -> Result<Vec<u8>, &'static str> {
		...
		Ok(MigrationChecksState { committee_size, next_era_committee_size }.encode())
	}
	...
	fn post_upgrade(state: Vec<u8>) -> Result<(), &'static str> {
		...
		} = <MigrationChecksState>::decode(&mut &*state).map_err(|_| "Failed to decode old state")?;
		...
		let currently_reserved = current_era_validators.reserved.len();
			...
			new_committee_size.non_reserved_seats ==
				old_committee_size.unwrap_or_default().saturating_sub(currently_reserved as u32),
			"Mismatch between `CurrentEraValidators` and `CommitteeSize`"
		);
