...
use selendra_primitives::{BanConfig as BanConfigStruct, CommitteeSeats};
...
fn storage_is_initialized_already_in_genesis() {
	...
	const COMMITTEE_SEATS: CommitteeSeats =
		CommitteeSeats { reserved_seats: 3, non_reserved_seats: 2 };
...
fn session_authorities_must_have_been_elected() {
	TestExtBuilder::new(vec![1, 2], vec![5, 6])
		.with_committee_seats(CommitteeSeats { reserved_seats: 2, non_reserved_seats: 2 })
		.build()
		.execute_with(|| {
			let next_era = 41;
