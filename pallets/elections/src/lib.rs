...
pub mod pallet {
	...
	impl<T: Config> Pallet<T> {
		fn ensure_validators_are_ok(
			...
			let member_set: BTreeSet<_> =
				reserved_validators.into_iter().chain(non_reserved_validators).collect();
	...
	impl<T: Config> ElectionProvider for Pallet<T> {
		...
		fn elect() -> Result<BoundedSupportsOf<Self>, Self::Error> {
			...
			let banned_validators =
				T::BannedValidators::banned().into_iter().collect::<BTreeSet<_>>();
			...
			let eligible_validators =
				staking_reserved_validators.into_iter().chain(new_non_reserved_validators);
