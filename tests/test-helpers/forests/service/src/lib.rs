...
//! Crate used for testing with Forests.
...
impl sc_executor::NativeExecutionDispatch for RuntimeExecutor {
	...
	fn dispatch(method: &str, data: &[u8]) -> Option<Vec<u8>> {
		forests_test_runtime::api::dispatch(method, data)
	}

	fn native_version() -> sc_executor::NativeVersion {
		forests_test_runtime::native_version()
	}
}
...
> {
	...
	let import_queue = forests_client_consensus_relay_chain::import_queue(
		client.clone(),
		client.clone(),
	...
	if let Some(collator_key) = collator_key {
		let parachain_consensus: Box<dyn ParachainConsensus<Block>> = match consensus {
			Consensus::RelayChain => {
				...
				Box::new(forests_client_consensus_relay_chain::RelayChainConsensus::new(
					para_id,
					proposer_factory,
...
/// A Forests test node instance used for testing.
pub struct TestNode {
...
/// Create a Forests `Configuration`.
///
...
pub fn node_config(
	...
	Ok(Configuration {
		impl_name: "forests-test-node".to_string(),
