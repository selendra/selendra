...
use indranet_pallets::pallet_registry::Attestation;
use indraxt::{
    dynamic::storage_key,
...
use indratory_api::blocks::{
    self, AuthoritySet, AuthoritySetChange, BlockHeader, BlockHeaderWithChanges, HeaderToSync,
    StorageProof,
};
use indratory_api::irpc::{self, InitRuntimeResponse, IndratoryInfo};
use indratory_api::iruntime_client;

use clap::Parser;
use headers_cache::Client as CacheClient;
use msg_sync::{Error as MsgSyncError, Receiver, Sender};
use notify_client::NotifyClient;
use indranet_types::AttestationProvider;

pub use indraxt::connect as subxt_connect;

#[derive(Parser, Debug)]
#[clap(
    about = "Sync messages between iruntime and the blockchain.",
    version,
    author
)]
pub struct Args {
    ...
    #[arg(short = 'n', long = "no-init", help = "Should init iruntime?")]
    no_init: bool,

    #[arg(
        long = "no-sync",
        help = "Don't sync iruntime. Quit right after initialization."
    )]
    no_sync: bool,

    #[arg(long, help = "Don't write iruntime egress data back to Substarte.")]
    no_msg_submit: bool,
    ...
    #[arg(
        long,
        help = "Inject dev key (0x1) to iruntime. Cannot be used with remote attestation enabled."
    )]
    use_dev_key: bool,
    ...
    #[arg(
        ...
        long,
        help = "iruntime http endpoint"
    )]
    iruntime_endpoint: String,

    #[arg(
        long,
        help = "iruntime http endpoint to handover the key. The handover will only happen when the old iruntime is synced."
    )]
    next_iruntime_endpoint: Option<String>,
    ...
    #[arg(
        default_value = "4",
        long = "sync-blocks",
        help = "The batch size to sync blocks to iruntime."
    )]
    ...
    /// Use IAS RA method, this is compatible with Indranet 1.x
    #[arg(
        short = 'r',
...
async fn get_header_hash(client: &indraxt::RpcClient, h: Option<u32>) -> Result<Hash> {
...
async fn get_block_at(client: &indraxt::RpcClient, h: Option<u32>) -> Result<(Block, Hash)> {
...
pub async fn get_header_at(client: &indraxt::RpcClient, h: Option<u32>) -> Result<(Header, Hash)> {
    let hash = get_header_hash(client, h).await?;
    let header = client
        .rpc()
        .header(Some(hash))
    ...
    let storage_changes = chain_client::fetch_storage_changes(client, &from_hash, &to_hash)
        ...
        .map(|(offset, storage_changes)| {
            BlockHeaderWithChanges {
                // Headers are synced separately. Only the `number` is used in iruntime while syncing blocks.
...
async fn get_authority_with_proof_at(
    ...
    // Storage
    let id_key = indraxt::dynamic::storage_key("Grandpa", "CurrentSetId");
    // Authority set
    let value = api
        .rpc()
...
async fn try_load_handover_proof(pr: &PrClient, api: &ParachainApi) -> Result<()> {
    ...
    let proof = chain_client::read_proofs(
        ...
        Some(hash),
        vec![
            &storage_key("IndranetRegistry", "IruntimeAddedAt")[..],
            &storage_key("IndranetRegistry", "IruntimeAllowList")[..],
    ...
    pr.load_storage_proof(irpc::StorageProof { proof }).await?;
    Ok(())
}
...
) -> Result<irpc::SyncedTo> {
    let resp = pr
        .sync_header(irpc::HeadersToSync::new(headers, authority_set_change))
        .await?;
    Ok(resp)
}
...
async fn req_sync_para_header(
    ...
    let resp = pr
        .sync_para_header(irpc::ParaHeadersToSync::new(headers, proof))
        .await?;
    Ok(resp)
}
...
async fn req_dispatch_block(
    ...
    let resp = pr.dispatch_blocks(irpc::Blocks::new(blocks)).await?;
    Ok(resp)
}
...
async fn init_runtime(
    ...
    let resp = pr
        .init_runtime(irpc::InitRuntimeRequest::new(
...
async fn register_worker(
    ...
    let tx = indraxt::dynamic::tx::register_worker(encoded_runtime_info, attestation, v2);
...
async fn try_register_worker(
    ...
    let info = pr
        .get_runtime_info(irpc::GetRuntimeInfoRequest::new(false, operator))
        .await?;
...
async fn try_load_chain_state(pr: &PrClient, para_api: &ParachainApi, args: &Args) -> Result<()> {
    ...
    let Ok(pubkey) = hex::decode(pubkey) else {
        return Err(anyhow!("iruntime returned an invalid pubkey"));
    };
    ...
    pr.load_chain_state(irpc::ChainState::new(block_number, state))
        .await?;
    Ok(())
}
...
async fn wait_until_synced(client: &indraxt::RpcClient) -> Result<()> {
    loop {
...
async fn bridge(
    ...
    let pr = iruntime_client::new_iruntime_client(args.iruntime_endpoint.clone());
    ...
    let mut iruntime_initialized = false;
    let mut iruntime_new_init = false;
    let mut initial_sync_finished = false;

    // Try to initialize iruntime and register on-chain
    let info = pr.get_info(()).await?;
    ...
    if !args.no_init {
        if !info.initialized {
            info!("iruntime not initialized. Requesting init...");
            let start_header =
            ...
            .await?;
            // STATUS: iruntime_initialized = true
            // STATUS: iruntime_new_init = true
            iruntime_initialized = true;
            iruntime_new_init = true;
            nc.notify(&NotifyReq {
                headernum: info.headernum,
                blocknum: info.blocknum,
                iruntime_initialized,
                iruntime_new_init,
            ...
            info!("iruntime already initialized.");
            // STATUS: iruntime_initialized = true
            // STATUS: iruntime_new_init = false
            iruntime_initialized = true;
            iruntime_new_init = false;
            nc.notify(&NotifyReq {
                headernum: info.headernum,
                blocknum: info.blocknum,
                iruntime_initialized,
    ...
    if args.no_sync {
        ...
        if !args.no_bind && info.public_key.is_some() {
            // Here the reason we dont directly report errors when `try_update_worker_endpoint` fails is that we want the endpoint can be registered anytime (e.g. days after the indranet initialization)
    ...
    for round in 0u64.. {
        // update the latest iruntime state
        let info = pr.get_info(()).await?;
        info!("iruntime get_info response: {:#?}", info);
        ...
        if args.parachain
            ...
            // `round == 0` is for old iruntimes which don't return `waiting_for_paraheaders`
        ...
        // send the blocks to iruntime in batch
        ...
        // check if iruntime has already reached the chain tip.
        if synced_blocks == 0 && !more_blocks {
            ...
            // Launch key handover if required only when the old iruntime is up-to-date
            if args.next_iruntime_endpoint.is_some() {
                let next_pr = iruntime_client::new_iruntime_client(
                    args.next_iruntime_endpoint.clone().unwrap(),
                );
...
async fn mk_params(
    ...
    longevity: u64,
    tip: u128,
) -> Result<indraxt::ExtrinsicParamsBuilder> {
    ...
    let params = if let Some((era, checkpoint)) = era {
        indraxt::ExtrinsicParamsBuilder::new()
            .tip(tip)
            .era(era, checkpoint)
    } else {
        indraxt::ExtrinsicParamsBuilder::new().tip(tip)
    };

    Ok(params)
}

pub async fn indranet_main() {
    env_logger::builder()
...
async fn handover_worker_key(server: &PrClient, client: &PrClient) -> Result<()> {
    ...
    panic!("Worker key handover done, the new iruntime is ready to go");
}
