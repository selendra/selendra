...
use codec::Decode;
use codec::Encode;
use indratory_api::blocks::StorageProof;
use indranet_node_rpc_ext::MakeInto as _;
use indranet_trie_storage::ser::StorageChanges;
use indranet_types::messaging::MessageOrigin;
use indraxt::{rpc::ExtraRpcExt as _, subxt, BlockNumber, RpcClient};
...
pub async fn mq_next_sequence(
    ...
    let seq: u64 = api
        .rpc()
        .request("indra_getMqNextSequence", rpc_params![to_value(sender_hex)?])
        .await?;
    Ok(seq)
}
