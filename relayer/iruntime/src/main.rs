...
use indranet_git_revision::git_revision_with_ts;
use indratory_api::ecall_args::InitArgs;

mod handover;
use indranet_sanitized_logger as logger;

#[derive(Parser, Debug, Clone)]
#[clap(about = "The Indranet TEE worker app.", version, author)]
struct Args {
	...
	/// Remove corrupted checkpoint so that iruntime can restart to continue to load others.
	#[arg(long)]
	...
	/// Handover key from another running iruntime instance
	#[arg(long)]
	...
	///     In this mode, it is needed to invoke irpc.LoadStorageProof to load the necessary values
	///     before accepting key handover request.
	#[arg(long)]
	#[arg(default_value_t = 0)]
	safe_mode_level: u8,

	/// Disable the RCU policy to update the Indratory state.
	#[arg(long)]
	no_rcu: bool,
}
...
async fn serve(sgx: bool) -> Result<(), rocket::Error> {
	...
	if let Some(from) = args.request_handover_from {
		...
		handover::handover_from(&from, init_args).await.expect("Handover failed");
		info!("Handover done");
		return Ok(())
	}
