...
use indranet_allocator::StatSizeAllocator;
use indratory_pal::{AppInfo, AppVersion, Machine, MemoryStats, MemoryUsage, Sealing, RA};
...
impl RA for GraminePlatform {
	...
	fn create_attestation_report(
		...
		match provider {
			Some(AttestationProvider::Ias) => {
				...
				let attestation_report = Some(indranet_types::AttestationReport::SgxIas {
	...
	fn measurement(&self) -> Option<Vec<u8>> {
		if is_gramine() {
			sgx_api_lite::target_info().map(|info| info.mr_enclave.m.to_vec()).ok()
		} else {
			None
		}
	}
...
impl AppInfo for GraminePlatform {
	fn app_version() -> AppVersion {
		...
		AppVersion { major: ver.major, minor: ver.minor, patch: ver.patch }
	}
}
...
pub(crate) fn print_target_info() {
	...
	println!("git revision: {}", indranet_git_revision::git_revision());
}
