...
use indratory::{benchmark, Indratory, RpcService};
use tracing::info;
...
pub fn ecall_handle(req_id: u64, action: u8, input: &[u8]) -> Result<Vec<u8>> {
	let allow_rcu = action == indratory_api::actions::ACTION_GET_INFO;
	let mut factory = APPLICATION.lock_indratory(allow_rcu, true)?;
	Ok(factory.handle_scale_api(req_id, action, input))
}

pub fn ecall_getinfo() -> String {
	let Ok(guard) = APPLICATION.lock_indratory(true, true) else {
        return r#"{"error": "Failed to lock Indratory""#.into();
    };
...
pub fn ecall_sign_http_response(data: &[u8]) -> Option<String> {
	APPLICATION.lock_indratory(true, true).ok()?.sign_http_response(data)
}

pub fn ecall_init(args: indratory_api::ecall_args::InitArgs) -> Result<()> {
	...
	if args.enable_checkpoint {
		match Indratory::restore_from_checkpoint(&GraminePlatform, &args) {
			Ok(Some(factory)) => {
				info!("Loaded checkpoint");
				**APPLICATION.lock_indratory(true, true).expect("Failed to lock Indratory") = factory;
				return Ok(())
			},
			Err(err) => {
	...
	APPLICATION.lock_indratory(true, true).unwrap().init(args);
...
pub async fn ecall_irpc_request(
	req_id: u64,
	path: String,
	data: &[u8],
	json: bool,
) -> (u16, Vec<u8>) {
	info!(%path, json, "Handling irpc request");
	let (code, data) = APPLICATION.dispatch_request(req_id, path, data, json).await;
	info!(code, size = data.len(), "irpc returned");
	(code, data)
}
